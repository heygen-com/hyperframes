...
### Animation Script Parsing

<Info>
  The anime.js and GSAP parsing layer now lives in its own standalone package, [`@hyperframes/parsers`](/packages/parsers). Core re-exports legacy GSAP APIs for back-compat and exposes parser subpaths for anime.js. Import from `@hyperframes/parsers` directly in new code.
</Info>
...
For anime.js parser and writer helpers, import from the anime.js subpaths:

```typescript
import { parseAnimeJsScript } from '@hyperframes/parsers/animejs-parser';
import { updateAnimeJsAnimationInScript } from '@hyperframes/parsers/animejs-writer-acorn';
```
...
// Generate just the legacy-named timeline script. New output uses anime.js.
...
- Missing animation registration (`hyperframesAnime.register(...)` for anime.js, or `window.__timelines` for the GSAP adapter path)
...
The core runtime includes the first-party anime.js adapter and the supported GSAP adapter. New authored compositions use `hyperframesAnime.register(...)`; existing GSAP compositions continue through the paused timeline on `window.__timelines` adapter path.

The core package also defines the v0 [Frame Adapter](/concepts/frame-adapters) interface and keeps the exported GSAP frame adapter factory for integrations that use that API directly:
...
// Create a GSAP frame adapter for the supported GSAP adapter path
...
<CardGroup cols={2}>
  <Card title="@hyperframes/parsers" icon="code" href="/packages/parsers">
    The standalone anime.js, GSAP, and HTML parsing layer extracted from core.
  </Card>
