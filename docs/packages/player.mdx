...
The composition runs inside a sandboxed `<iframe>` in the player's Shadow DOM. For most use cases you don't need direct access because the JavaScript API and events above are sufficient. But if you're building an editor, recorder, or custom timeline on top of the player, you may need to inspect the composition's DOM or read runtime objects such as `__player`, `hyperframesAnime`, or the GSAP adapter's `__timelines`. The `iframeElement` getter exposes the inner iframe for these consumers:
...
// Read the runtime (anime.js registry, GSAP adapter timelines, element registry, etc.)
iframe.contentWindow.hyperframesAnime?.entries();
iframe.contentWindow.__timelines; // GSAP adapter path
```
