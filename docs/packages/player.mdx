...
| `poster` | string | none | Image URL to show before first play |
| `playback-rate` | number | 1 | Playback speed multiplier |
| `variables` | JSON object string | none | Variables injected into the composition as `window.__hfVariables` before composition scripts run |

## Composition Variables

Use `variables` when a composition reads `window.__hfVariables` synchronously during startup.
...
player.variables = { title: "Hello" };
```

<Warning>
  **Limitation:** injecting `variables` into a cross-origin `src` is not possible. The player must fetch same-origin HTML so it can pre-inject the script before composition scripts run. With a cross-origin `src`, the player logs a warning, fires the `error` event, and loads the composition without applying variables.
</Warning>
...
player.variables;       // object | null, get/set composition variables
```
...
| `ended` | none | Playback reached end |
