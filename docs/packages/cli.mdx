...
<Tabs>
  <Tab title="Create">
    ...
    | `--model` | Whisper model for transcription (e.g. `small`, `medium`, `large-v3`; use `.en` only for English audio) |
    ...
    npx hyperframes transcribe video.mp4 --model medium --language ja
    ...
    | `--model, -m` | Whisper model (default: `small`). Options: `tiny`, `base`, `small`, `medium`, `large-v3`; `.en` variants are for English audio only |
    ...
    **Supported transcript formats:**
    ...
    <Tip>
      For music or noisy audio, use `--model medium` or `--model large-v3` for better accuracy. Use `.en` models only when the source audio is English. For the best results with production content, transcribe via the OpenAI or Groq Whisper API and import the JSON.
    </Tip>
