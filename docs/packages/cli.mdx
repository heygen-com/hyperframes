...
- Search and copy reusable local assets (`assets`)
...
<Tabs>
  <Tab title="Create">
    ...
    Search configured local asset libraries and copy selected files into the current project:

    ```bash
    # List or search reusable assets
    ...
    # Copy the best match into paths.assets
    ...
    | Flag | Description |
    |------|-------------|
    | `<query>` (positional) | File name, folder, library name, or tag-like search words |
    | `--dir` | Project directory (defaults to the current working directory) |
    | `--kind` | Filter by `image`, `video`, `audio`, `font`, `lottie`, or `data` |
    | `--limit` | Maximum number of matches to print |
    | `--copy` | Copy the best matching asset into the configured `paths.assets` directory |
    | `--to` | Copy target path relative to `paths.assets` |
    | `--force` | Allow `--copy` to overwrite an existing file |
    | `--json` | Print a machine-readable summary |

    Configure libraries with `hyperframes.json#assetLibraries`, `HYPERFRAMES_ASSET_LIBRARY`, or `~/.hyperframes/assets`. See [Pro Asset Libraries](/guides/pro-asset-libraries).
