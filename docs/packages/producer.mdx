...
<Steps>
  ...
  <Step title="Inject the Hyperframes runtime">
    Adds the runtime script that manages timeline seeking, clip lifecycle, and media playback. The runtime drives anime.js as the first-party default adapter and continues to drive GSAP through the supported adapter path.
  </Step>
