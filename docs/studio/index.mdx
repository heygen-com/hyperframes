...
## Compose preview guides in a host app

Host apps can compose the same rulers, safe margins, grid, and snap toolbar that
Studio uses over their own preview iframe. The provider owns the iframe geometry
and shared preferences, so the host only supplies its iframe ref and chooses the
parts to render:
...
function Preview({ iframeRef }: { iframeRef: RefObject<HTMLIFrameElement | null> }) {
  return (
