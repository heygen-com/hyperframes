...
<iframe src="/public/catalog/preview/components/command-menu.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Command Menu preview" />

## Install

<InstallCommand command="npx hyperframes add command-menu" />

That writes one file: `compositions/components/command-menu.html`.
...
| --- | --- | --- | --- |
| `query` | `Search commands...` | string | Line typed into the search field. |
| `active` | `1` | 1 to 8, step 1 | Which command row is highlighted, counting from 1. Out-of-range values clamp. |
| `accent` | `green` | `green`, `blue`, `violet` | Accent token behind the check icon and the inset bar on the active row. |
| `theme` | `dark` | `dark`, `light` | Token set for the palette: dark surfaces or light ones. |
...
```html wrap
<div
  data-composition-id="command-menu"
  data-composition-src="compositions/components/command-menu.html"
  data-variable-values='{"query":"Search commands...","active":1,"accent":"green","theme":"dark"}'
></div>
```

## Source

<Accordion title={`command-menu.html`}>

```html
<!--
  Command Menu - Operator Black command palette primitive.
  ...
  the bottom is untouched by them.
    ...
    - query (text, default "Search commands..."): the line typed into the
      search field.
    - active (number 1 to the number of rows, default 1): which command row is
      highlighted. The check icon and the accent bar move with it.
    - accent (green | blue | violet, default green): accent token behind the
      check icon and the inset bar on the active row.
    - theme (dark | light, default dark): swaps the hf-ui token set, so the
      palette renders on a light surface.

  The command rows themselves stay plain markup: add, remove or relabel them in
  the block below.
-->

<div
  data-hf-ui-root
  class="hf-ui-command-menu"
  data-composition-variables='[
    { "id": "query", "type": "string", "role": "content", "label": "Query", "description": "Line typed into the search field.", "default": "Search commands..." },
    { "id": "active", "type": "number", "role": "content", "label": "Active row", "description": "Which command row is highlighted, counting from 1. Out-of-range values clamp.", "default": 1, "min": 1, "max": 8, "step": 1 },
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Accent token behind the check icon and the inset bar on the active row.", "default": "green", "options": [{ "value": "green", "label": "Green" }, { "value": "blue", "label": "Blue" }, { "value": "violet", "label": "Violet" }] },
    { "id": "theme", "type": "enum", "role": "style", "label": "Theme", "description": "Token set for the palette: dark surfaces or light ones.", "default": "dark", "options": [{ "value": "dark", "label": "Dark" }, { "value": "light", "label": "Light" }] }
  ]'
>
  <div class="hf-ui-command-search">Search commands...</div>
  <div class="hf-ui-command-group">Create</div>
  <div class="hf-ui-command-item is-active">
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 12 4 4 8-9" /></svg>
    <span>New composition</span><kbd>N</kbd>
  </div>
  <div class="hf-ui-command-item"><span>Add text layer</span><kbd>T</kbd></div>
  <div class="hf-ui-command-item"><span>Render preview</span><kbd>R</kbd></div>
</div>
...
<style>
  ...
  .hf-ui-command-search {
    min-height: var(--_hf-ui-control-height);
    padding: 12px var(--_hf-ui-space-4);
...
<script>
  (function () {
    ...
    var query = text(vars.query, "Search commands...");

    var roots = document.querySelectorAll(".hf-ui-command-menu");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--hf-ui-accent", accent);
      root.setAttribute("data-hf-theme", theme);

      var search = root.querySelector(".hf-ui-command-search");
      if (search) search.textContent = query;

      var items = root.querySelectorAll(".hf-ui-command-item");
      if (items.length === 0) continue;

      var target = items[slot(vars.active, items.length, 1)];
      var check = root.querySelector(".hf-ui-command-item svg");
...
<!--
  ...
  tl.fromTo('.hf-ui-command-menu', { opacity: 0, y: -4, scale: 0.98 }, { opacity: 1, y: 0, scale: 1, duration: 0.21, ease: 'power3.out' }, startTime);
  tl.set('.hf-ui-command-item', { opacity: 1 }, startTime);
-->
```
...
Tagged `ui-primitive` `remocn-port` `command-menu` `overlay`.
