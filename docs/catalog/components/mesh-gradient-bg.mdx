...
<CatalogDetail
  previewSrc="/public/catalog/components/mesh-gradient-bg.json"
  compositionId="mesh-gradient-bg"
  compositionSrc="compositions/components/mesh-gradient-bg.html"
  title="Mesh Gradient Background"
  description="A render-safe mesh gradient using animated radial layers."
  variables={[]}
  meta={{"category":"Effects","badge":"Stable","codeLines":101}}
  attribution={{"path":"registry/components/mesh-gradient-bg","tags":["effect"]}}
  hasCode
  rawUrl="https://raw.githubusercontent.com/heygen-com/hyperframes/main/registry/components/mesh-gradient-bg/mesh-gradient-bg.html"
>

<CatalogSlot slot="code">

```html mesh-gradient-bg.html
<!--
  Mesh Gradient Background - catalog component for HyperFrames.
...
<style>
  ...
  .hf-catalog-mesh-gradient-bg .mesh {
    ...
    background:
      ...
      radial-gradient(circle at var(--hf-x3, 46%) var(--hf-y3, 78%), #34d399 0 18%, transparent 34%);
  }
