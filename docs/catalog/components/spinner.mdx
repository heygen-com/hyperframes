...
<Accordion title={`spinner.html`}>

```html
<!-- Spinner - finite Operator Black loading indicator for paused GSAP timelines. -->
<div data-hf-ui-root class="hf-ui-spinner" role="status" aria-label="Rendering">
  <span class="hf-ui-spinner-ring"></span><span>Rendering</span>
</div>
<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  ...
  .hf-ui-spinner-ring {
    width: 22px;
    height: 22px;
    border: 1.5px solid var(--_hf-ui-border-strong);
    border-top-color: var(--_hf-ui-accent);
    border-radius: 999px;
  }
  @media (prefers-reduced-motion: reduce) {
    .hf-ui-spinner-ring {
...
<!--
  ...
  tl.to('.hf-ui-spinner-ring', { rotation: 720, duration: 2.4, ease: 'none' }, startTime);
-->
```
