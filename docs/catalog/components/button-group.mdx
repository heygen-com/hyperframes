...
| --- | --- | --- | --- |
| `activeSegment` | `second` | `first`, `second`, `third` | Which segment carries the pressed state, the accent underline, and the timeline pop. |
| `accent` | `green` | `green`, `blue`, `violet` | Colour of the underline under the pressed segment. |
...
```html
<div
  data-composition-id="button-group"
  data-composition-src="compositions/components/button-group.html"
  data-variable-values='{"activeSegment":"second","accent":"green","theme":"dark"}'
></div>
```

## Source

<Accordion title={`button-group.html`}>

```html
<!--
  Button Group - Operator Black segmented action primitive for HyperFrames.
  ...
  anything missing or unrecognised, and writes an attribute or token the CSS
  above already keys off. The timeline recipe below is untouched by them: it
  targets whichever segment is pressed, so changing activeSegment moves the
  pop with it.

    - activeSegment (first | second | third, default second): which segment
      carries aria-pressed, the accent underline, and the entrance pop.
    - accent (green | blue | violet, default green): colour of that underline.
      Each option resolves to the matching data token for the active theme.
  ...
  On every default the result is identical to the original: Final pressed,
  green underline, dark tokens.
-->

<div
  data-hf-ui-root
  class="hf-ui-button-group"
  role="group"
  aria-label="Export mode"
  data-composition-variables='[
    { "id": "activeSegment", "type": "enum", "role": "content", "label": "Active segment", "description": "Which segment carries the pressed state, the accent underline, and the timeline pop.", "default": "second", "options": [{ "value": "first", "label": "First" }, { "value": "second", "label": "Second" }, { "value": "third", "label": "Third" }] },
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Colour of the underline under the pressed segment.", "default": "green", "options": [{ "value": "green", "label": "Green" }, { "value": "blue", "label": "Blue" }, { "value": "violet", "label": "Violet" }] },
...
>
  <button type="button" aria-pressed="false">Draft</button>
  <button type="button" aria-pressed="true">Final</button>
  <button type="button" aria-pressed="false">Archive</button>
</div>
...
<style>
  ...
  .hf-ui-button-group button:focus-visible {
    z-index: 1;
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-button-group button:active {
    transform: translateY(1px) scale(0.98);
  }
  .hf-ui-button-group button:disabled,
  .hf-ui-button-group button[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
    transform: none;
  }
  @media (hover: hover) and (pointer: fine) {
    .hf-ui-button-group button:hover {
      color: var(--_hf-ui-text-strong);
    }
  }
  @media (pointer: coarse) {
    ...
    .hf-ui-button-group button::before {
      position: absolute;
...
<script>
  (function () {
    ...
    var segments = { first: 0, second: 1, third: 2 };

    var index = segments[pick(segments, vars.activeSegment, "second")];
    var theme = pick(themes, vars.theme, "dark");
    var accent = accents[pick(accents, vars.accent, "green")][theme];

    var roots = document.querySelectorAll(".hf-ui-button-group");
    for (var i = 0; i < roots.length; i += 1) {
      ...
      for (var j = 0; j < buttons.length; j += 1) {
        buttons[j].setAttribute("aria-pressed", j === index ? "true" : "false");
      }
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.hf-ui-button-group button[aria-pressed="true"]', { scale: 0.98, y: 1 }, { scale: 1, y: 0, duration: 0.16, ease: 'power3.out' }, startTime);
-->
```
