...
| --- | --- | --- | --- |
| `width` | `standard` | `compact`, `standard`, `full` | Sheet width, from a narrow tray to full bleed. |
| `handle` | `visible` | `visible`, `hidden` | Whether the grab handle bar sits above the title. |
| `rows` | `inset` | `inset`, `outlined`, `plain` | Row surface treatment: filled inset, outlined, or bare. |
...
```html
<div
  data-composition-id="drawer"
  data-composition-src="compositions/components/drawer.html"
  data-variable-values='{"width":"standard","handle":"visible","rows":"inset"}'
></div>
```

## Source

<Accordion title={`drawer.html`}>

```html
<!--
  Drawer - Operator Black bottom-attached overlay primitive.
  ...
  untouched by them.
    ...
    - width (compact | standard | full, default standard): sheet width.
      compact is a 400px tray, standard the original 520px, full spans the
      whole frame. All three still cap at the available width.
    - handle (visible | hidden, default visible): whether the grab handle bar
      sits above the title. Hiding it pulls the content up by its 26px slot.
    - rows (inset | outlined | plain, default inset): row surface. inset is
      the original filled slab, outlined swaps the fill for a hairline border,
      plain leaves the rows bare on the sheet.

  On every default the result is identical to the original: a 520px sheet with
  a handle and two filled rows.
-->
...
<div
  ...
  data-composition-variables='[
    { "id": "width", "type": "enum", "role": "style", "label": "Width", "description": "Sheet width, from a narrow tray to full bleed.", "default": "standard", "options": [{ "value": "compact", "label": "Compact" }, { "value": "standard", "label": "Standard" }, { "value": "full", "label": "Full" }] },
    { "id": "handle", "type": "enum", "role": "style", "label": "Handle", "description": "Whether the grab handle bar sits above the title.", "default": "visible", "options": [{ "value": "visible", "label": "Visible" }, { "value": "hidden", "label": "Hidden" }] },
    { "id": "rows", "type": "enum", "role": "style", "label": "Rows", "description": "Row surface treatment: filled inset, outlined, or bare.", "default": "inset", "options": [{ "value": "inset", "label": "Inset" }, { "value": "outlined", "label": "Outlined" }, { "value": "plain", "label": "Plain" }] }
  ]'
  role="dialog"
  aria-modal="true"
  aria-labelledby="hf-ui-drawer-title"
>
  <div class="hf-ui-drawer-handle"></div>
  <h3 id="hf-ui-drawer-title">Export options</h3>
  <div class="hf-ui-drawer-row"><span>MP4 1080p</span><strong>Ready</strong></div>
  <div class="hf-ui-drawer-row"><span>Transparent WebM</span><strong>Beta</strong></div>
</div>
...
<style>
  ...
  /* hf-ui:tokens:end */
  .hf-ui-drawer {
    width: min(var(--hf-drawer-width, 520px), 100%);
    padding: var(--_hf-ui-space-3) var(--_hf-ui-space-6) var(--_hf-ui-space-6);
    border: 1px solid var(--_hf-ui-border-strong);
    border-block-end: 0;
    border-radius: var(--_hf-ui-radius-overlay) var(--_hf-ui-radius-overlay) 0 0;
    background: var(--_hf-ui-surface-raised);
    color: var(--_hf-ui-text);
    box-shadow: 0 -12px 32px rgba(0, 0, 0, 0.3);
  }
  .hf-ui-drawer-handle {
    display: var(--hf-drawer-handle-display, block);
    width: 56px;
    height: 4px;
    margin: 0 auto 22px;
    border-radius: 999px;
  ...
  .hf-ui-drawer-row {
    ...
    border: var(--hf-drawer-row-border, 0 solid currentColor);
    border-radius: var(--_hf-ui-radius-control);
    background: var(--hf-drawer-row-bg, var(--_hf-ui-surface-inset));
    font-size: 14px;
    font-weight: var(--_hf-ui-weight-medium);
  }
  .hf-ui-drawer-row strong {
...
<script>
  (function () {
    ...
    var widths = { compact: "400px", standard: "520px", full: "100%" };
    var handles = { visible: "block", hidden: "none" };
    var rowStyles = {
      inset: { bg: "var(--_hf-ui-surface-inset)", border: "0 solid currentColor" },
      outlined: { bg: "transparent", border: "1px solid var(--_hf-ui-border)" },
      plain: { bg: "transparent", border: "0 solid currentColor" },
    };
    ...
    var handle = handles[pick(handles, vars.handle, "visible")];
    var row = rowStyles[pick(rowStyles, vars.rows, "inset")];

    var roots = document.querySelectorAll(".hf-ui-drawer");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--hf-drawer-width", width);
      roots[i].style.setProperty("--hf-drawer-handle-display", handle);
      roots[i].style.setProperty("--hf-drawer-row-bg", row.bg);
      roots[i].style.setProperty("--hf-drawer-row-border", row.border);
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.hf-ui-drawer', { y: '100%', opacity: 0 }, { y: 0, opacity: 1, duration: 0.36, ease: 'power3.out' }, startTime);
  tl.fromTo('.hf-ui-drawer-row', { opacity: 0 }, { opacity: 1, duration: 0.16, ease: 'power2.out' }, startTime + 0.2);
-->
```
