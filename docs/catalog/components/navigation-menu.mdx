---
title: "Navigation Menu"
description: "An anchored navigation menu with a functional selection edge and opaque trigger-origin preview surface"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/navigation-menu.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Navigation Menu preview" />

## Install

<InstallCommand command="npx hyperframes add navigation-menu" />

That writes one file: `compositions/components/navigation-menu.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `accent` | `green` | `green`, `blue`, `violet` | Token family behind the underline under the current tab. green keeps the accent token, blue and violet borrow the data-2 and data-4 tokens, so all three follow the light and dark themes. |
| `active` | `second` | `first`, `second`, `third` | Which tab carries the current-page underline and the brighter label. |
| `featured` | `Featured: launch video kit` | string | Text printed in the featured resource panel under the tabs. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html wrap
<div
  data-composition-id="navigation-menu"
  data-composition-src="compositions/components/navigation-menu.html"
  data-variable-values='{"accent":"green","active":"second","featured":"Featured: launch video kit"}'
></div>
```

## Source

<Accordion title={`navigation-menu.html`}>

```html
<!--
  Navigation Menu - Operator Black anchored navigation primitive for HyperFrames.

  Paste the markup, CSS and script into a composition. Animate the CSS variables
  from a paused GSAP timeline for deterministic video renders.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and either rewrites the markup or writes a
  custom property the CSS above consumes. The timeline recipe stays untouched:
  it keeps revealing the featured panel.

    - accent (green | blue | violet, default green): the token family behind the
      underline under the current tab. green keeps the accent token, blue and
      violet borrow the data-2 and data-4 tokens, so all three follow the light
      and dark themes.
    - active (first | second | third, default second): which tab carries the
      current-page underline and the brighter label.
    - featured (any text, default Featured: launch video kit): the text printed
      in the featured resource panel under the tabs.

  On every default the result is identical to the original: three tabs with the
  second one underlined in accent, over the featured resource panel.
-->

<nav
  data-hf-ui-root
  class="hf-ui-navigation-menu"
  aria-label="Primary navigation"
  data-composition-variables='[
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Token family behind the underline under the current tab. green keeps the accent token, blue and violet borrow the data-2 and data-4 tokens, so all three follow the light and dark themes.", "default": "green", "options": [{ "value": "green", "label": "Green" }, { "value": "blue", "label": "Blue" }, { "value": "violet", "label": "Violet" }] },
    { "id": "active", "type": "enum", "role": "content", "label": "Current tab", "description": "Which tab carries the current-page underline and the brighter label.", "default": "second", "options": [{ "value": "first", "label": "First" }, { "value": "second", "label": "Second" }, { "value": "third", "label": "Third" }] },
    { "id": "featured", "type": "string", "role": "content", "label": "Featured text", "description": "Text printed in the featured resource panel under the tabs.", "default": "Featured: launch video kit" }
  ]'
>
  <a href="#product">Product</a><a href="#templates" aria-current="page">Templates</a
  ><a href="#docs">Docs</a>
  <div role="region" aria-label="Featured resource">Featured: launch video kit</div>
</nav>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-navigation-menu {
    position: relative;
    display: inline-grid;
    grid-template-columns: repeat(3, minmax(0, 1fr));
    gap: 4px;
    width: min(300px, 100%);
    padding: 0;
    border-block-end: 1px solid var(--_hf-ui-border);
    color: var(--_hf-ui-text);
    font-family: var(--_hf-ui-font-sans);
  }
  .hf-ui-navigation-menu a {
    position: relative;
    display: inline-flex;
    align-items: center;
    min-height: var(--_hf-ui-control-height);
    padding: 0 12px;
    border-radius: var(--_hf-ui-radius-control) var(--_hf-ui-radius-control) 0 0;
    color: var(--_hf-ui-text-muted);
    font: var(--_hf-ui-weight-medium) 13px/1 var(--_hf-ui-font-sans);
    text-decoration: none;
    overflow: hidden;
    text-align: center;
    text-overflow: ellipsis;
    white-space: nowrap;
    transform: translateY(0) scale(1);
    transition:
      color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response),
      transform var(--_hf-ui-duration-press) var(--_hf-ui-ease-response);
  }
  .hf-ui-navigation-menu [aria-current="page"] {
    color: var(--_hf-ui-text-strong);
    box-shadow: inset 0 -2px 0 var(--hf-nav-accent, var(--_hf-ui-accent));
  }
  .hf-ui-navigation-menu a:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-navigation-menu a:active {
    transform: translateY(1px) scale(0.98);
  }
  .hf-ui-navigation-menu div {
    position: absolute;
    inset-block-start: 48px;
    inset-inline-start: 0;
    width: 100%;
    padding: 16px;
    border: 1px solid var(--_hf-ui-border-strong);
    border-radius: var(--_hf-ui-radius-overlay);
    background: var(--_hf-ui-surface-raised);
    color: var(--_hf-ui-text);
    font-size: 13px;
    font-weight: var(--_hf-ui-weight-medium);
    box-shadow: var(--_hf-ui-shadow-contact);
    transform-origin: top center;
    overflow-wrap: anywhere;
  }
  @media (hover: hover) and (pointer: fine) {
    .hf-ui-navigation-menu a:hover {
      color: var(--_hf-ui-text);
    }
  }
  @media (pointer: coarse) {
    .hf-ui-navigation-menu a::before {
      position: absolute;
      inset-block: calc((var(--_hf-ui-control-height) - var(--_hf-ui-hit-target)) / 2);
      inset-inline: 0;
      content: "";
    }
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.
    // green stays empty so the underline keeps falling back to --_hf-ui-accent.
    var accents = { green: "", blue: "var(--_hf-ui-data-2)", violet: "var(--_hf-ui-data-4)" };
    var actives = { first: 0, second: 1, third: 2 };

    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    var accent = accents[pick(accents, vars.accent, "green")];
    var active = actives[pick(actives, vars.active, "second")];
    var featured =
      typeof vars.featured === "string" && vars.featured !== ""
        ? vars.featured
        : "Featured: launch video kit";

    var roots = document.querySelectorAll(".hf-ui-navigation-menu");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      if (accent !== "") root.style.setProperty("--hf-nav-accent", accent);

      var links = root.querySelectorAll("a");
      var index = Math.min(active, links.length - 1);
      for (var l = 0; l < links.length; l += 1) {
        if (l === index) links[l].setAttribute("aria-current", "page");
        else links[l].removeAttribute("aria-current");
      }

      var panel = root.querySelector("div");
      if (panel) panel.textContent = featured;
    }
  })();
</script>

<!--
  Timeline integration:
  tl.fromTo('.hf-ui-navigation-menu div', { opacity: 0, y: -4, scale: 0.98, filter: 'blur(1.5px)' }, { opacity: 1, y: 0, scale: 1, filter: 'blur(0px)', duration: 0.21, ease: 'power3.out' }, startTime);
-->
```

</Accordion>

## Usage

Open `compositions/components/navigation-menu.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `navigation-menu` `navigation`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
