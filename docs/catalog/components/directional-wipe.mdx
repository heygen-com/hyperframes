...
| --- | --- | --- | --- |
| `palette` | `slate` | `slate`, `teal`, `ember` | Colour family for both slabs, the outgoing panel and the incoming reveal. |
...
<!--
  ...
  untouched by them.
    ...
    - palette (slate | teal | ember, default slate): recolours both slabs as a
      pair, the outgoing panel and the card behind it plus the incoming panel
      the wipe reveals. slate is the original grey to indigo pairing, teal a
      cool ink to cyan, ember a warm charcoal to orange.
...
<div
  ...
  data-composition-variables='[
    { "id": "palette", "type": "enum", "role": "style", "label": "Palette", "description": "Colour family for both slabs, the outgoing panel and the incoming reveal.", "default": "slate", "options": [{ "value": "slate", "label": "Slate" }, { "value": "teal", "label": "Teal" }, { "value": "ember", "label": "Ember" }] },
...
<style>
  ...
  .hf-remocn-directional-wipe {
    ...
    background: var(--hf-from-a, #111827);
  ...
  .hf-remocn-directional-wipe .from {
    background: linear-gradient(135deg, var(--hf-from-a, #111827), var(--hf-from-b, #374151));
...
<script>
  (function () {
    ...
    var palettes = {
      slate: { fromA: "#111827", fromB: "#374151", toA: "#2563eb", toB: "#7c3aed" },
      teal: { fromA: "#0f172a", fromB: "#134e4a", toA: "#0d9488", toB: "#22d3ee" },
      ember: { fromA: "#1c1917", fromB: "#44403c", toA: "#ea580c", toB: "#db2777" },
    };
    ...
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--hf-from-a", palette.fromA);
      roots[i].style.setProperty("--hf-from-b", palette.fromB);
      roots[i].style.setProperty("--hf-to-a", palette.toA);
      roots[i].style.setProperty("--hf-to-b", palette.toB);
