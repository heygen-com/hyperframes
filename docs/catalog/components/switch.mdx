...
<Accordion title={`switch.html`}>

```html
<!--
  Switch - Operator Black toggle primitive for HyperFrames.

  Toggle data-state between off and on, or animate --hf-switch-progress from a
  paused GSAP timeline.
-->

<div data-hf-ui-root class="hf-ui-switch-row">
  <span
    class="hf-ui-switch"
    data-state="off"
    role="switch"
    aria-checked="false"
    aria-labelledby="hf-ui-switch-label"
    tabindex="0"
  >
    <span class="hf-ui-switch-thumb"></span>
  </span>
  <span id="hf-ui-switch-label" class="hf-ui-switch-label">Auto enhance</span>
</div>
...
<style>
  ...
  .hf-ui-switch {
    ...
    border: 1px solid
      color-mix(
        in srgb,
        var(--_hf-ui-accent) calc(var(--hf-switch-progress) * 100%),
  ...
  .hf-ui-switch[data-state="on"] {
    --hf-switch-progress: 1;
  }
  .hf-ui-switch-thumb {
    position: absolute;
    top: 3px;
    ...
    background: color-mix(
      in srgb,
      var(--_hf-ui-action-contrast) calc(var(--hf-switch-progress) * 100%),
      var(--_hf-ui-text-muted)
    );
    transform: translateX(calc(var(--hf-switch-progress) * 18px));
    box-shadow: 0 1px 2px rgba(0, 0, 0, 0.28);
    transition: transform var(--_hf-ui-duration-state) var(--_hf-ui-ease-response);
    will-change: transform;
  }
  .hf-ui-switch:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-switch:active {
  ...
  .hf-ui-switch[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
    transform: none;
  }
...
<!--
  ...
  tl.to(".hf-ui-switch", { "--hf-switch-progress": 1, duration: 0.16, ease: "power3.out" }, startTime);
-->
```
