...
<Accordion title={`spotlight-card.html`}>

```html
<!--
  Spotlight Card - remocn component ported to HyperFrames authoring format.
...
<style>
  ...
  .hf-remocn-spotlight-card .mesh {
    ...
    background:
      ...
      radial-gradient(circle at var(--hf-x3, 46%) var(--hf-y3, 78%), rgba(255, 255, 255, 0.08) 0 18%, transparent 34%);
  }
