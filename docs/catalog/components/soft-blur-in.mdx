...
<CatalogDetail
  ...
  compositionSrc="compositions/components/soft-blur-in.html"
  title="Soft Blur In"
  description="A soft opacity, blur, and lift reveal for headlines, captions, and product UI callouts"
  variables={[{"id":"distance","type":"enum","role":"motion","label":"Distance","description":"How far the element is offset before it slides home: 14px, 28px or 52px.","default":"standard","options":[{"value":"close","label":"Close"},{"value":"standard","label":"Standard"},{"value":"far","label":"Far"}]},{"id":"direction","type":"enum","role":"motion","label":"Direction","description":"up starts the element below its resting place so it rises in, down starts it above so it drops in.","default":"up","options":[{"value":"up","label":"Up"},{"value":"down","label":"Down"}]},{"id":"blur","type":"enum","role":"style","label":"Blur","description":"Starting blur radius, from a light haze to a full smear.","default":"standard","options":[{"value":"soft","label":"Soft"},{"value":"standard","label":"Standard"},{"value":"heavy","label":"Heavy"}]}]}
  meta={{"category":"Typography & Text","badge":"Stable","codeLines":83}}
  attribution={{"path":"registry/components/soft-blur-in","tags":["motion-primitive","text","reveal","blur"]}}
  hasCode
  rawUrl="https://raw.githubusercontent.com/heygen-com/hyperframes/main/registry/components/soft-blur-in/soft-blur-in.html"
>

<CatalogSlot slot="code">

```html soft-blur-in.html
<!--
  Soft Blur In - web-native motion primitive.

  Usage:
  1. Add class="hf-soft-blur-in" to the element you want to reveal.
  2. Paste this style block and script once.
  ...
  on the document root, which every element carrying the class inherits. These
  set where the reveal STARTS: the timeline snippet below still drives offset
  and blur to zero, so each knob decides how far and how hazy the element is
  before it settles, not how long it takes.

    - distance (close | standard | far, default standard): how far the element
      is offset before it slides home, 14px, 28px or 52px.
    - direction (up | down, default up): up starts the element below its
      resting place so it rises in, down starts it above so it drops in.
    - blur (soft | standard | heavy, default standard): the starting blur
      radius, 8px, 18px or 34px, from a light haze to a full smear.

  On every default the result is identical to the original: the element starts
  28px low behind an 18px blur.
-->
...
<div
  ...
  data-composition-src="compositions/components/soft-blur-in.html"
  data-variable-values='{"distance":"standard","direction":"up","blur":"standard"}'
></div>
```
