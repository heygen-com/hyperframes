---
title: "Pagination"
description: "A tactile pagination row with 40px controls, a recessed current page, and deterministic press feedback"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/pagination.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Pagination preview" />

## Install

<InstallCommand command="npx hyperframes add pagination" />

That writes one file: `compositions/components/pagination.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `size` | `standard` | `compact`, `standard`, `roomy` | Scales the control height, the side padding and the label size together. |
| `accent` | `green` | `green`, `blue`, `violet` | Colour of the seated underline on the current page. green keeps the themed accent, blue and violet borrow the data colours. |
| `labels` | `words` | `words`, `arrows` | Whether the step buttons read Prev and Next or show single angle marks. The aria labels never change. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html wrap
<div
  data-composition-id="pagination"
  data-composition-src="compositions/components/pagination.html"
  data-variable-values='{"size":"standard","accent":"green","labels":"words"}'
></div>
```

## Source

<Accordion title={`pagination.html`}>

```html
<!--
  Pagination - Operator Black navigation primitive for HyperFrames.

  Paste the markup, CSS and script into a composition. Animate the CSS
  variables from a paused GSAP timeline for deterministic video renders.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and writes the result as a custom property
  the CSS below consumes or as the label on the two step buttons. The timeline
  recipe at the end is untouched by them.

    - size (compact | standard | roomy, default standard): scales the control
      height, the side padding and the label size together, so the whole row
      grows or shrinks without touching the token theme.
    - accent (green | blue | violet, default green): colour of the seated
      underline on the current page. green leaves the themed accent alone,
      blue and violet borrow the data colours, so a light or dark theme still
      resolves the right shade.
    - labels (words | arrows, default words): whether the step buttons read
      Prev and Next or show single angle marks. The aria labels never change.

  On every default the result is identical to the original: 40px controls, the
  themed accent under page 2, and worded step buttons.
-->

<nav
  data-hf-ui-root
  class="hf-ui-pagination"
  aria-label="Pagination"
  data-composition-variables='[
    { "id": "size", "type": "enum", "role": "layout", "label": "Size", "description": "Scales the control height, the side padding and the label size together.", "default": "standard", "options": [{ "value": "compact", "label": "Compact" }, { "value": "standard", "label": "Standard" }, { "value": "roomy", "label": "Roomy" }] },
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Colour of the seated underline on the current page. green keeps the themed accent, blue and violet borrow the data colours.", "default": "green", "options": [{ "value": "green", "label": "Green" }, { "value": "blue", "label": "Blue" }, { "value": "violet", "label": "Violet" }] },
    { "id": "labels", "type": "enum", "role": "content", "label": "Step labels", "description": "Whether the step buttons read Prev and Next or show single angle marks. The aria labels never change.", "default": "words", "options": [{ "value": "words", "label": "Words" }, { "value": "arrows", "label": "Arrows" }] }
  ]'
>
  <button type="button" aria-label="Previous page">Prev</button><a href="#page-1">1</a
  ><a href="#page-2" aria-current="page">2</a><a href="#page-3">3</a
  ><button type="button" aria-label="Next page">Next</button>
</nav>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-pagination {
    display: inline-flex;
    flex-wrap: wrap;
    align-items: center;
    justify-content: center;
    gap: 4px;
    max-width: 100%;
    font-family: var(--_hf-ui-font-sans);
  }
  .hf-ui-pagination :is(a, button) {
    position: relative;
    display: grid;
    place-items: center;
    min-width: calc(var(--_hf-ui-control-height) * var(--hf-pagination-scale, 1));
    height: calc(var(--_hf-ui-control-height) * var(--hf-pagination-scale, 1));
    padding: 0 calc(12px * var(--hf-pagination-scale, 1));
    border: 1px solid var(--_hf-ui-control-border);
    border-radius: var(--_hf-ui-radius-control);
    background: var(--_hf-ui-surface);
    color: var(--_hf-ui-text);
    font: var(--_hf-ui-weight-medium) 13px/1 var(--_hf-ui-font-sans);
    font-size: calc(13px * var(--hf-pagination-scale, 1));
    text-decoration: none;
    overflow-wrap: anywhere;
    box-shadow: var(--_hf-ui-shadow-inset);
    cursor: pointer;
    transform: translateY(0) scale(1);
    transition:
      border-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response),
      background-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response),
      transform var(--_hf-ui-duration-press) var(--_hf-ui-ease-response);
  }
  .hf-ui-pagination [aria-current="page"] {
    border-color: var(--_hf-ui-border-strong);
    background: var(--_hf-ui-surface-inset);
    color: var(--_hf-ui-text-strong);
    box-shadow:
      inset 0 -2px 0 var(--hf-pagination-accent, var(--_hf-ui-accent)),
      var(--_hf-ui-shadow-inset);
  }
  .hf-ui-pagination :is(a, button):focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-pagination :is(a, button):active {
    transform: translateY(1px) scale(0.98);
  }
  @media (hover: hover) and (pointer: fine) {
    .hf-ui-pagination :is(a, button):hover {
      border-color: var(--_hf-ui-control-border-hover);
    }
  }
  @media (pointer: coarse) {
    .hf-ui-pagination :is(a, button)::before {
      position: absolute;
      inset: calc((var(--_hf-ui-control-height) - var(--_hf-ui-hit-target)) / 2);
      content: "";
    }
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.
    var scales = { compact: 0.8, standard: 1, roomy: 1.25 };
    // green stays unset, so an externally themed accent is never clobbered.
    var accents = {
      green: "",
      blue: "var(--_hf-ui-data-2)",
      violet: "var(--_hf-ui-data-4)",
    };
    var labelSets = {
      words: { prev: "Prev", next: "Next" },
      arrows: { prev: "\u2039", next: "\u203A" },
    };

    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    var scale = scales[pick(scales, vars.size, "standard")];
    var accent = accents[pick(accents, vars.accent, "green")];
    var labels = labelSets[pick(labelSets, vars.labels, "words")];

    var roots = document.querySelectorAll(".hf-ui-pagination");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--hf-pagination-scale", String(scale));
      if (accent) {
        root.style.setProperty("--hf-pagination-accent", accent);
      }
      var prev = root.querySelector('[aria-label="Previous page"]');
      var next = root.querySelector('[aria-label="Next page"]');
      if (prev) {
        prev.textContent = labels.prev;
      }
      if (next) {
        next.textContent = labels.next;
      }
    }
  })();
</script>

<!--
  Timeline integration:
  tl.fromTo('.hf-ui-pagination > *', { opacity: 0.55, y: 4 }, { opacity: 1, y: 0, duration: 0.21, stagger: 0.04, ease: 'power3.out' }, startTime);
-->
```

</Accordion>

## Usage

Open `compositions/components/pagination.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `pagination` `navigation`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
