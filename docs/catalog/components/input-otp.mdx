...
| --- | --- | --- | --- |
| `code` | `428` | string | Digits already entered. Non-digits are dropped and the next empty slot becomes active. |
| `length` | `4` | 4 to 8, step 1 | How many slots the code field shows. |
| `accent` | `green` | `green`, `blue`, `amber`, `violet` | Accent token family used for the active slot border, ring and caret. |
...
```html wrap
<div
  data-composition-id="input-otp"
  data-composition-src="compositions/components/input-otp.html"
  data-variable-values='{"code":"428","length":4,"accent":"green","theme":"dark"}'
></div>
```

## Source

<Accordion title={`input-otp.html`}>

```html
<!--
  Input OTP - Operator Black segmented code primitive for HyperFrames.
  ...
  anything missing or unrecognised, and rebuilds the slots the CSS below
  already styles. The timeline recipe at the bottom is untouched by them: it
  staggers whatever slots exist.

    - code (string, default "428"): the digits already entered. Anything that
      is not a digit is dropped, the code is trimmed to the slot count, and
      the first empty slot after it becomes the active one. A blank code
      leaves every slot empty with the first active.
    - length (number, 4 to 8, default 4): how many slots are shown.
    - accent (green | blue | amber | violet, default green): the data token
      family behind the active slot border, ring and caret. green is the theme
  ...
  On every default the result is identical to the original: four dark slots
  reading 4 2 8 with a green active caret in the fourth.
-->

<div
  data-hf-ui-root
  class="hf-ui-input-otp"
  role="group"
  aria-label="One time code"
  tabindex="0"
  data-composition-variables='[
    { "id": "code", "type": "string", "role": "content", "label": "Code", "description": "Digits already entered. Non-digits are dropped and the next empty slot becomes active.", "default": "428" },
    { "id": "length", "type": "number", "role": "layout", "label": "Length", "description": "How many slots the code field shows.", "default": 4, "min": 4, "max": 8, "step": 1 },
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Accent token family used for the active slot border, ring and caret.", "default": "green", "options": [{ "value": "green", "label": "Green" }, { "value": "blue", "label": "Blue" }, { "value": "amber", "label": "Amber" }, { "value": "violet", "label": "Violet" }] },
...
>
  <span aria-hidden="true">4</span><span aria-hidden="true">2</span><span aria-hidden="true">8</span
...
<style>
  ...
  .hf-ui-input-otp .active {
    border-color: var(--_hf-ui-accent);
    box-shadow:
      0 0 0 2px color-mix(in srgb, var(--_hf-ui-accent) 40%, transparent),
      var(--_hf-ui-shadow-inset);
  }
  .hf-ui-input-otp .active::after {
    width: 1px;
    height: 16px;
    background: var(--_hf-ui-accent);
    content: "";
  }
  .hf-ui-input-otp:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-input-otp[aria-invalid="true"] span {
    border-color: var(--_hf-ui-danger);
  }
  .hf-ui-input-otp[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
  }
...
<script>
  (function () {
    ...
    var length = clampNumber(vars.length, 4, 8, 4);
    var code = (typeof vars.code === "string" ? vars.code : "428")
      .replace(/[^0-9]/g, "")
      .slice(0, length);
    var accent = accents[pick(accents, vars.accent, "green")];
    var theme = vars.theme === "light" ? "light" : "dark";

    var roots = document.querySelectorAll(".hf-ui-input-otp");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.setAttribute("data-hf-theme", theme);
      root.style.setProperty("--hf-ui-accent", accent);
      while (root.firstChild) root.removeChild(root.firstChild);
      for (var slot = 0; slot < length; slot += 1) {
        var cell = document.createElement("span");
        if (slot === code.length) cell.setAttribute("class", "active");
        cell.setAttribute("aria-hidden", "true");
        cell.textContent = code.charAt(slot);
...
<!--
  ...
  tl.fromTo('.hf-ui-input-otp span', { opacity: 0, y: 4 }, { opacity: 1, y: 0, duration: 0.16, stagger: 0.04, ease: 'power3.out' }, startTime);
-->
```
