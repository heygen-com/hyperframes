...
| --- | --- | --- | --- |
| `separator` | `chevron` | `chevron`, `slash`, `arrow` | Glyph drawn between crumbs: a chevron, a slash, or a full arrow. |
| `size` | `standard` | `compact`, `standard`, `roomy` | Type size, spacing, and separator scale of the whole trail. |
...
```html
<div
  data-composition-id="breadcrumb"
  data-composition-src="compositions/components/breadcrumb.html"
  data-variable-values='{"separator":"chevron","size":"standard","theme":"dark"}'
></div>
```

## Source

<Accordion title={`breadcrumb.html`}>

```html
<!--
  Breadcrumb - Operator Black navigation primitive for HyperFrames.
  ...
  consumes or an attribute the token block already keys off. The timeline
  recipe below is untouched by them.

    - separator (chevron | slash | arrow, default chevron): the glyph drawn
      between crumbs. Each option swaps the path inside the inline SVG.
    - size (compact | standard | roomy, default standard): type size, gap, and
      separator scale, from 12px through 13px to 15px.
  ...
  On every default the result is identical to the original: 13px medium type,
  8px gaps, 14px chevrons, dark tokens.
-->

<nav
  data-hf-ui-root
  class="hf-ui-breadcrumb"
  aria-label="Breadcrumb"
  data-composition-variables='[
    { "id": "separator", "type": "enum", "role": "style", "label": "Separator", "description": "Glyph drawn between crumbs: a chevron, a slash, or a full arrow.", "default": "chevron", "options": [{ "value": "chevron", "label": "Chevron" }, { "value": "slash", "label": "Slash" }, { "value": "arrow", "label": "Arrow" }] },
    { "id": "size", "type": "enum", "role": "layout", "label": "Size", "description": "Type size, spacing, and separator scale of the whole trail.", "default": "standard", "options": [{ "value": "compact", "label": "Compact" }, { "value": "standard", "label": "Standard" }, { "value": "roomy", "label": "Roomy" }] },
...
>
  ...
  <i aria-hidden="true"
    ><svg viewBox="0 0 24 24"><path d="m9 6 6 6-6 6" /></svg
  ></i>
  <strong aria-current="page">Preview</strong>
</nav>
...
<style>
  ...
  .hf-ui-breadcrumb {
    ...
    align-items: center;
    max-width: 100%;
    gap: var(--hf-breadcrumb-gap, 8px);
    color: var(--_hf-ui-text-muted);
    font: var(--_hf-ui-weight-medium) var(--hf-breadcrumb-size, 13px) / 1 var(--_hf-ui-font-sans);
  }
  .hf-ui-breadcrumb a {
    position: relative;
    display: inline-flex;
    ...
    transition: color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response);
  }
  .hf-ui-breadcrumb a:focus-visible {
  ...
  .hf-ui-breadcrumb i svg {
    width: var(--hf-breadcrumb-icon, 14px);
    height: var(--hf-breadcrumb-icon, 14px);
    fill: none;
    stroke: currentColor;
    stroke-width: 1.5;
  ...
  @media (pointer: coarse) {
    .hf-ui-breadcrumb a {
      ...
      min-width: var(--_hf-ui-hit-target);
    }
    .hf-ui-breadcrumb a::before {
      position: absolute;
...
<script>
  (function () {
    ...
    var separators = {
      ...
      slash: "M15 5 9 19",
      arrow: "M5 12h11m-4-5 5 5-5 5",
    };
    var sizes = {
      compact: { size: "12px", gap: "6px", icon: "12px" },
      standard: { size: "13px", gap: "8px", icon: "14px" },
      roomy: { size: "15px", gap: "12px", icon: "17px" },
    };
    var themes = { dark: true, light: true };

    var separator = separators[pick(separators, vars.separator, "chevron")];
    var size = sizes[pick(sizes, vars.size, "standard")];
    var theme = pick(themes, vars.theme, "dark");

    var roots = document.querySelectorAll(".hf-ui-breadcrumb");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--hf-breadcrumb-size", size.size);
      root.style.setProperty("--hf-breadcrumb-gap", size.gap);
      root.style.setProperty("--hf-breadcrumb-icon", size.icon);
      if (theme === "light") root.setAttribute("data-hf-theme", "light");
      else root.removeAttribute("data-hf-theme");
      var paths = root.querySelectorAll("i svg path");
      for (var j = 0; j < paths.length; j += 1) paths[j].setAttribute("d", separator);
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.hf-ui-breadcrumb > *', { opacity: 0.55, x: -4 }, { opacity: 1, x: 0, duration: 0.21, stagger: 0.04, ease: 'power3.out' }, startTime);
-->
```
