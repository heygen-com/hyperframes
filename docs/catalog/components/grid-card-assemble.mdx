      ...
      <div id="root" data-composition-id="grid-card-assemble" data-duration="4.5" data-fps="30">
        ...
        <script>
          (function () {
            ...
            var columns =
              layout === "list" ? 1 : Math.max(1, Math.min(4, requested > 0 ? requested : autoColumns));
            ...
            root.style.setProperty("--gca-body-gap", (rowH * (isList ? 0.03 : 0.035)).toFixed(3) + "cqh");
            ...
            var gap =
              count > 1 ? Math.max(0, targetLastStart - firstStart) / (count - 1) : 0;
