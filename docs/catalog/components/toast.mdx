...
<Accordion title={`toast.html`}>

```html
<!-- Toast - Operator Black success notice anchored to the logical top end. -->
<div data-hf-ui-root class="hf-ui-toast" role="status" data-state="success">
  <span class="hf-ui-toast-dot" data-status="success">
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 12 4 4 8-9" /></svg>
  </span>
  <div><strong>Render complete</strong><span>Your video is ready to share.</span></div>
  <span class="hf-ui-toast-rail"></span>
</div>
<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  ...
  .hf-ui-toast {
    ...
    opacity: var(--hf-toast-progress);
    transform: translateY(calc((1 - var(--hf-toast-progress)) * -8px))
      scale(calc(0.98 + var(--hf-toast-progress) * 0.02));
  ...
  .hf-ui-toast-rail {
    position: absolute;
    left: var(--_hf-ui-space-4);
    right: var(--_hf-ui-space-4);
    bottom: var(--_hf-ui-space-2);
    height: 2px;
    border-radius: 999px;
    background: var(--_hf-ui-accent);
    transform: scaleX(var(--hf-toast-progress));
    transform-origin: left;
  }
  .hf-ui-toast[data-state="open"],
  .hf-ui-toast[data-state="success"] {
...
<!--
  ...
  tl.fromTo('.hf-ui-toast', { '--hf-toast-progress': 0 }, { '--hf-toast-progress': 1, duration: 0.21, ease: 'power3.out' }, startTime);
-->
```
