...
<Accordion title={`tooltip.html`}>

```html
<!--
  Tooltip - shadcn-style tooltip primitive for HyperFrames.

  Animate --hf-tooltip-open from 0 to 1 to reveal the floating label.
-->

<div data-hf-ui-root class="hf-ui-tooltip">
  <button
    class="hf-ui-tooltip-trigger"
    type="button"
    aria-label="Brand voice guidance"
    aria-describedby="hf-ui-tooltip-content"
  >
  ...
  <div class="hf-ui-tooltip-content" id="hf-ui-tooltip-content" role="tooltip" aria-hidden="true">
    Match the brand voice before exporting.
    <span class="hf-ui-tooltip-arrow" aria-hidden="true"></span>
  </div>
</div>
...
<style>
  ...
  .hf-ui-tooltip-trigger:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-tooltip-content {
    position: absolute;
    ...
    line-height: 1.35;
    opacity: var(--hf-tooltip-open);
    transform: translateX(-50%) translateY(calc((1 - var(--hf-tooltip-open)) * 4px))
      scale(calc(0.98 + var(--hf-tooltip-open) * 0.02));
  ...
  .hf-ui-tooltip-arrow {
    ...
    width: 10px;
    height: 10px;
    border-inline-end: 1px solid var(--_hf-ui-border-strong);
    border-block-end: 1px solid var(--_hf-ui-border-strong);
    background: var(--_hf-ui-surface-raised);
...
<!--
  ...
  tl.to(".hf-ui-tooltip", { "--hf-tooltip-open": 1, duration: 0.21, ease: "power3.out" }, startTime);
-->
```
