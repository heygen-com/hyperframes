...
<Accordion title={`table.html`}>

```html
<!--
  Table - Operator Black structured-data primitive for HyperFrames.
...
<table data-hf-ui-root class="hf-ui-table" aria-label="Render queue">
  <thead>
    <tr>
      <th>Scene</th>
      <th>Status</th>
  ...
  <tbody>
    <tr>
      <td>Intro</td>
      <td data-status="ready">Ready</td>
      <td>0:04</td>
    </tr>
    <tr>
      <td>Demo</td>
      <td data-status="rendering">Rendering</td>
      <td>0:12</td>
    </tr>
    <tr>
      <td>CTA</td>
      <td data-status="queued">Queued</td>
      <td>0:03</td>
    </tr>
  </tbody>
</table>
...
<style>
  ...
  .hf-ui-table tr:last-child td {
    border-block-end: 0;
  }
  .hf-ui-table td:last-child {
    font-family: var(--_hf-ui-font-mono);
    font-variant-numeric: tabular-nums;
  }
  .hf-ui-table td[data-status="ready"] {
    color: var(--_hf-ui-accent);
  }
  .hf-ui-table td[data-status="rendering"] {
    color: var(--_hf-ui-warning);
  }
  .hf-ui-table td[data-status="queued"] {
    color: var(--_hf-ui-text-muted);
  }
</style>
...
<!--
  ...
  tl.fromTo('.hf-ui-table tbody tr', { opacity: 0.55, y: 4 }, { opacity: 1, y: 0, duration: 0.21, stagger: 0.04, ease: 'power3.out' }, startTime);
-->
```
