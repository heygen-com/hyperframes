...
<Accordion title={`textarea.html`}>

```html
<!--
  Textarea - Operator Black multiline field primitive for HyperFrames.
...
<label data-hf-ui-root class="hf-ui-textarea-field">
  <span>Scene notes</span>
  <textarea readonly>Open with the hero card, then reveal the dashboard metrics.</textarea>
</label>
...
<style>
  ...
  .hf-ui-textarea-field textarea {
    ...
    color: var(--_hf-ui-text);
    font: var(--_hf-ui-weight-regular) 14px/1.5 var(--_hf-ui-font-sans);
    resize: none;
    box-shadow:
      0 0 0 var(--hf-textarea-ring, 0px) color-mix(in srgb, var(--_hf-ui-accent) 36%, transparent),
  ...
  .hf-ui-textarea-field textarea:focus-visible {
    border-color: var(--_hf-ui-accent);
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-textarea-field textarea:disabled {
    cursor: not-allowed;
    opacity: 0.48;
  }
  .hf-ui-textarea-field textarea[aria-invalid="true"] {
    border-color: var(--_hf-ui-danger);
  }
  @media (hover: hover) and (pointer: fine) {
    .hf-ui-textarea-field textarea:hover {
...
<!--
  ...
  tl.fromTo('.hf-ui-textarea-field textarea', { '--hf-textarea-ring': '0px', opacity: 0, y: 4 }, { '--hf-textarea-ring': '2px', opacity: 1, y: 0, duration: 0.21, ease: 'power3.out' }, startTime);
-->
```
