---
title: "Marquee Select Cursor"
description: "A cursor drags a selection rectangle across a photo grid; each tile rings and pops a check the frame the rectangle touches it, while a pill counts the selection up."
mode: "frame"
---
...
<CatalogDetail
  previewSrc="/public/catalog/components/cursor-marquee-select.json"
  compositionId="cursor-marquee-select"
  compositionSrc="compositions/components/cursor-marquee-select.html"
  title="Marquee Select Cursor"
  description="A cursor drags a selection rectangle across a photo grid; each tile rings and pops a check the frame the rectangle touches it, while a pill counts the selection up."
  variables={[{"id":"accent","type":"color","role":"style","label":"Accent","default":"#2f7bf6"},{"id":"start_x","type":"number","role":"layout","label":"Drag start X","default":20.5,"min":0,"max":100,"step":0.5,"unit":"%"},{"id":"start_y","type":"number","role":"layout","label":"Drag start Y","default":25.5,"min":0,"max":100,"step":0.5,"unit":"%"},{"id":"end_x","type":"number","role":"layout","label":"Drag end X","default":57,"min":0,"max":100,"step":0.5,"unit":"%"},{"id":"end_y","type":"number","role":"layout","label":"Drag end Y","default":52,"min":0,"max":100,"step":0.5,"unit":"%"},{"id":"pill_label","type":"string","role":"content","label":"Pill label","default":"selected"}]}
  meta={{"category":"Cursors","badge":"Stable","codeLines":460}}
  attribution={{"path":"registry/components/cursor-marquee-select","tags":["cursor","motion-primitive","select","marquee","grid","product-demo"]}}
  hasCode
  rawUrl="https://raw.githubusercontent.com/heygen-com/hyperframes/main/registry/components/cursor-marquee-select/cursor-marquee-select.html"
>

<CatalogSlot slot="code">

```html cursor-marquee-select.html
<!doctype html>
<!--
...
<InstallCommand command="npx hyperframes add cursor-marquee-select" item="cursor-marquee-select" />

That writes one file: `compositions/components/cursor-marquee-select.html`.
...
Open `compositions/components/cursor-marquee-select.html` and copy what is inside into your own composition.
...
| --- | --- | --- | --- |
| `accent` | `#2f7bf6` | color |  |
| `start_x` | `20.5` | 0% to 100%, step 0.5% |  |
| `start_y` | `25.5` | 0% to 100%, step 0.5% |  |
| `end_x` | `57` | 0% to 100%, step 0.5% |  |
| `end_y` | `52` | 0% to 100%, step 0.5% |  |
| `pill_label` | `selected` | string |  |
...
<div
  data-composition-id="cursor-marquee-select"
  data-composition-src="compositions/components/cursor-marquee-select.html"
  data-variable-values='{"accent":"#2f7bf6","start_x":20.5,"start_y":25.5,"end_x":57,"end_y":52,"pill_label":"selected"}'
></div>
```

{/* hf:generated-footer */}

Tagged `cursor` `motion-primitive` `select` `marquee` `grid` `product-demo`.
