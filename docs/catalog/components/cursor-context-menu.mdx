---
title: "Context Menu Cursor"
description: "A cursor right-clicks a file, a context menu blooms from its tip, the hover highlight tracks the item under it, and the chosen action plays out: duplicate, rename, move to trash, or a confirmation toast."
mode: "frame"
---
...
<CatalogDetail
  previewSrc="/public/catalog/components/cursor-context-menu.json"
  compositionId="cursor-context-menu"
  compositionSrc="compositions/components/cursor-context-menu.html"
  title="Context Menu Cursor"
  description="A cursor right-clicks a file, a context menu blooms from its tip, the hover highlight tracks the item under it, and the chosen action plays out: duplicate, rename, move to trash, or a confirmation toast."
  variables={[{"id":"items","type":"string","role":"content","label":"Menu items","default":"Open,Rename,Duplicate,Share,Move to Trash"},{"id":"pick","type":"number","role":"content","label":"Chosen item (0 = first)","default":2,"min":0,"max":9,"step":1},{"id":"tone","type":"enum","role":"style","label":"Tone","default":"light","options":[{"value":"light","label":"Light"},{"value":"dark","label":"Dark"}]}]}
  meta={{"category":"Cursors","badge":"Stable","codeLines":437}}
  attribution={{"path":"registry/components/cursor-context-menu","tags":["cursor","motion-primitive","menu","right-click","desktop","files"]}}
  hasCode
  rawUrl="https://raw.githubusercontent.com/heygen-com/hyperframes/main/registry/components/cursor-context-menu/cursor-context-menu.html"
>

<CatalogSlot slot="code">

```html cursor-context-menu.html
<!doctype html>
<!--
    ...
    <template>
      <style>
        ...
        .cm-window {
          ...
          box-shadow: 0 2.4cqw 6cqw rgba(8, 12, 28, 0.35), 0 0 0 1px var(--cm-line);
        }
        .cm-bar {
          display: flex;
          align-items: center;
        ...
        .cm-menu {
          ...
          box-shadow: 0 1.2cqw 3cqw rgba(8, 12, 28, 0.3), 0 0 0 1px var(--cm-line);
          opacity: 0;
      ...
      <div id="root" data-composition-id="cursor-context-menu" data-duration="5.2">
        <div class="cm-stage">
          <div class="cm-window">
            <div class="cm-bar"><i class="cm-dot"></i><i class="cm-dot"></i><i class="cm-dot"></i><span>Launch assets</span></div>
      ...
      <script>
        (function () {
          ...
          var duration = Math.max(
            0.001,
            parseFloat(root.dataset.duration || html.getAttribute("data-composition-duration") || "5.2"),
          );
          ...
          var FILES = [
            ...
            ["Launch film v4", "mp4", "2.4 GB &middot; Video", "linear-gradient(135deg,#8f7bff,#4b3bd6)"],
            ["Pricing notes", "md", "18 KB &middot; Text", "linear-gradient(135deg,#6fc3ff,#2f7de1)"],
            ["Team offsite", "jpg", "4.8 MB &middot; Image", "linear-gradient(135deg,#7fe0a6,#1f9d6b)"],
            ["Launch film v4 copy", "mp4", "2.4 GB &middot; Video", "linear-gradient(135deg,#8f7bff,#4b3bd6)"],
          ];
          var TARGET = 1;
          var cards = FILES.map(function (f) {
            ...
            card.innerHTML =
              '<div class="cm-thumb" style="background:' + f[3] + '"></div>' +
              '<div class="cm-name">' + f[0] + "." + f[1] + '</div><div class="cm-meta">' + f[2] + "</div>";
          ...
          var T = {
            glide: 0.85, rclick: 0.95, bloom: 1.02, travel0: 1.45, travel1: 2.1,
            click: 2.25, collapse: 2.43, act: 2.55, away0: 3.1, away1: 3.7,
          };
          ...
          function render(t) {
            ...
            rows.forEach(function (row, i) {
              var k = easeOut(seg(t, T.bloom + i * 0.025 * squeeze, T.bloom + (i * 0.025 + 0.16) * squeeze));
            ...
            var aside = { x: itemPoint.x + (left ? -1 : 1) * 160 * u, y: Math.min(H - 60 * u, itemPoint.y + 180 * u) };
            var p;
            if (t < T.glide) p = lerp(start, anchor, glide(seg(t, 0, T.glide)));
            else if (t < T.travel0) p = anchor;
            else if (t < T.travel1) p = lerp(anchor, itemPoint, easeInOut(seg(t, T.travel0, T.travel1)));
            ...
            cards[TARGET].classList.toggle("is-on", t >= T.rclick && !(done && action === "duplicate"));
            copy.classList.toggle("is-on", done && action === "duplicate");
            var showCopy = action === "duplicate" ? k : 0;
            copy.style.opacity = String(showCopy);
            copy.style.transform = "translateY(" + (1 - showCopy) * 18 * u + "px) scale(" + (0.94 + 0.06 * showCopy) + ")";
...
<InstallCommand command="npx hyperframes add cursor-context-menu" item="cursor-context-menu" />

That writes one file: `compositions/components/cursor-context-menu.html`.
...
Open `compositions/components/cursor-context-menu.html` and copy what is inside into your own composition.
...
| --- | --- | --- | --- |
| `items` | `Open,Rename,Duplicate,Share,Move to Trash` | string |  |
| `pick` | `2` | 0 to 9, step 1 |  |
...
<div
  data-composition-id="cursor-context-menu"
  data-composition-src="compositions/components/cursor-context-menu.html"
  data-variable-values='{"items":"Open,Rename,Duplicate,Share,Move to Trash","pick":2,"tone":"light"}'
></div>
```

{/* hf:generated-footer */}

Tagged `cursor` `motion-primitive` `menu` `right-click` `desktop` `files`.
