---
title: "Caret"
description: "A matte command-field caret with finite blink timing and a deterministic typed-value reveal"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/caret.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Caret preview" />

## Install

<InstallCommand command="npx hyperframes add caret" />

That writes one file: `compositions/components/caret.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `accent` | `green` | `green`, `blue`, `amber`, `violet` | Accent token family that colours the caret mark. |
| `shape` | `bar` | `bar`, `block`, `underline` | The caret mark itself: a thin bar, a filled block, or an underscore below the baseline. |
| `field` | `framed` | `framed`, `bare` | Whether the command field chrome is drawn: framed keeps the bordered input, bare drops it so the line can sit over footage. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html
<div
  data-composition-id="caret"
  data-composition-src="compositions/components/caret.html"
  data-variable-values='{"accent":"green","shape":"bar","field":"framed"}'
></div>
```

## Source

<Accordion title={`caret.html`}>

```html
<!--
  Caret - Operator Black typed-command primitive for HyperFrames.

  Paste the markup, CSS and script into a composition. Use the timeline
  integration note at the bottom as the starting point for deterministic GSAP
  animation.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and writes the result as a custom property
  the CSS above consumes. The timeline recipe is untouched by them: it keeps
  typing the same value and blinking on the same cadence. The typed width and
  the blink live in that recipe, which the user owns, so neither is exposed.

    - accent (green | blue | amber | violet, default green): the data token
      family that colours the caret mark. green is the theme accent, so the
      default is unchanged in either theme.
    - shape (bar | block | underline, default bar): the caret mark itself. bar
      is the original 2px rule, block is a filled cell, underline sits below
      the baseline like a terminal underscore.
    - field (framed | bare, default framed): whether the command field chrome
      is drawn. bare drops the border, fill and inset shadow so the typed line
      can sit directly over footage.

  On every default the result is identical to the original: a green 2px caret
  inside a bordered inset command field.
-->

<div
  data-hf-ui-root
  class="hf-ui-caret-line"
  data-composition-variables='[
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Accent token family that colours the caret mark.", "default": "green", "options": [{ "value": "green", "label": "Green" }, { "value": "blue", "label": "Blue" }, { "value": "amber", "label": "Amber" }, { "value": "violet", "label": "Violet" }] },
    { "id": "shape", "type": "enum", "role": "style", "label": "Caret shape", "description": "The caret mark itself: a thin bar, a filled block, or an underscore below the baseline.", "default": "bar", "options": [{ "value": "bar", "label": "Bar" }, { "value": "block", "label": "Block" }, { "value": "underline", "label": "Underline" }] },
    { "id": "field", "type": "enum", "role": "style", "label": "Field", "description": "Whether the command field chrome is drawn: framed keeps the bordered input, bare drops it so the line can sit over footage.", "default": "framed", "options": [{ "value": "framed", "label": "Framed" }, { "value": "bare", "label": "Bare" }] }
  ]'
>
  <span class="hf-ui-caret-value">Generate captions</span><span class="hf-ui-caret"></span>
</div>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-caret-line {
    --hf-caret-width: 167px;
    display: inline-flex;
    align-items: center;
    min-height: 48px;
    padding: var(--hf-caret-field-pad, 0 14px);
    border: 1px solid var(--hf-caret-field-border, var(--_hf-ui-control-border));
    border-radius: var(--_hf-ui-radius-control);
    background: var(--hf-caret-field-bg, var(--_hf-ui-surface-inset));
    color: var(--_hf-ui-text-strong);
    font: var(--_hf-ui-weight-medium) 18px/1 var(--_hf-ui-font-sans);
    box-shadow: var(--hf-caret-field-shadow, var(--_hf-ui-shadow-inset));
  }
  .hf-ui-caret-value {
    display: inline-block;
    width: var(--hf-caret-width);
    overflow: hidden;
    white-space: nowrap;
  }
  .hf-ui-caret {
    display: inline-block;
    width: var(--hf-caret-mark-w, 2px);
    height: var(--hf-caret-mark-h, 22px);
    margin-left: 4px;
    background: var(--_hf-ui-accent);
    opacity: 1;
    transform: translateY(var(--hf-caret-mark-shift, 0px));
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.
    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    // The data tokens are theme-aware, and data-1 is the theme accent itself,
    // so the default leaves both themes exactly as they shipped.
    var accents = {
      green: "var(--_hf-ui-data-1)",
      blue: "var(--_hf-ui-data-2)",
      amber: "var(--_hf-ui-data-3)",
      violet: "var(--_hf-ui-data-4)",
    };
    var shapes = {
      bar: { w: "2px", h: "22px", shift: "0px" },
      block: { w: "11px", h: "22px", shift: "0px" },
      underline: { w: "16px", h: "2px", shift: "9px" },
    };
    var fields = {
      framed: {
        pad: "0 14px",
        border: "var(--_hf-ui-control-border)",
        bg: "var(--_hf-ui-surface-inset)",
        shadow: "var(--_hf-ui-shadow-inset)",
      },
      bare: { pad: "0", border: "transparent", bg: "transparent", shadow: "none" },
    };

    var accent = accents[pick(accents, vars.accent, "green")];
    var shape = shapes[pick(shapes, vars.shape, "bar")];
    var field = fields[pick(fields, vars.field, "framed")];

    var roots = document.querySelectorAll(".hf-ui-caret-line");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--hf-ui-accent", accent);
      roots[i].style.setProperty("--hf-caret-mark-w", shape.w);
      roots[i].style.setProperty("--hf-caret-mark-h", shape.h);
      roots[i].style.setProperty("--hf-caret-mark-shift", shape.shift);
      roots[i].style.setProperty("--hf-caret-field-pad", field.pad);
      roots[i].style.setProperty("--hf-caret-field-border", field.border);
      roots[i].style.setProperty("--hf-caret-field-bg", field.bg);
      roots[i].style.setProperty("--hf-caret-field-shadow", field.shadow);
    }
  })();
</script>

<!--
  Timeline integration:
  tl.fromTo('.hf-ui-caret-value', { width: 0 }, { width: 167, duration: 1.1, ease: 'steps(17)' }, 0.35);
  tl.to('.hf-ui-caret', { opacity: 0.18, duration: 0.15, ease: 'none' }, 0.35);
  tl.to('.hf-ui-caret', { opacity: 1, duration: 0.15, ease: 'none' }, 0.51);
  tl.to('.hf-ui-caret', { opacity: 0.18, duration: 0.15, ease: 'none' }, 0.67);
  tl.to('.hf-ui-caret', { opacity: 1, duration: 0.15, ease: 'none' }, 0.83);
  tl.to('.hf-ui-caret', { opacity: 0.18, duration: 0.15, ease: 'none' }, 0.99);
  tl.to('.hf-ui-caret', { opacity: 1, duration: 0.15, ease: 'none' }, 1.15);
  tl.to('.hf-ui-caret', { opacity: 0.18, duration: 0.15, ease: 'none' }, 1.31);
  tl.to('.hf-ui-caret', { opacity: 1, duration: 0.15, ease: 'none' }, 1.47);
-->
```

</Accordion>

## Usage

Open `compositions/components/caret.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `caret` `typing`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
