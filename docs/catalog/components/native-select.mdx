---
title: "Native Select"
description: "A compact native select with a clear label, technical value, and focus state"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/native-select.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Native Select preview" />

## Install

<InstallCommand command="npx hyperframes add native-select" />

That writes one file: `compositions/components/native-select.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `label` | `Quality` | string | The field name above the control. |
| `options` | `1080p, 4K` | string | Comma separated choices. The first one is the value on show. |
| `width` | `standard` | `compact`, `standard`, `wide` | How wide the control sits, from 180px through 240px to 320px. |
| `accent` | `green` | `green`, `blue`, `violet` | Colour family of the focus ring the timeline pushes out. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html wrap
<div
  data-composition-id="native-select"
  data-composition-src="compositions/components/native-select.html"
  data-variable-values='{"label":"Quality","options":"1080p, 4K","width":"standard","accent":"green"}'
></div>
```

## Source

<Accordion title={`native-select.html`}>

```html
<!--
  Native Select - Operator Black native choice primitive for HyperFrames.

  Paste the markup and CSS into a composition. Animate the CSS variables
  from a paused GSAP timeline for deterministic video renders.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and writes either the control wording or a
  custom property the CSS below consumes. The focus ring recipe at the bottom
  is untouched by them.

    - label (any short text, default Quality): the field name above the
      control. Blank or non-text input falls back to the default.
    - options (comma separated list, default 1080p, 4K): the choices. The first
      entry is the value on show, and the list also decides how many options
      the menu carries. An empty list falls back to the default.
    - width (compact | standard | wide, default standard): how wide the control
      sits, from 180px through 240px to 320px, still capped by the space it is
      given.
    - accent (green | blue | violet, default green): colour family of the ring
      the timeline pushes out around the control. Each option rides a data
      token, so it tracks the dark and the light theme. The ring is 0px wide at
      rest, so this only shows once the ring recipe runs.

  On every default the result is identical to the original: a 240px control
  labelled Quality reading 1080p, with a green ring on focus.
-->

<label
  data-hf-ui-root
  class="hf-ui-native-select"
  data-composition-variables='[
    {"id": "label", "type": "string", "role": "content", "label": "Label", "description": "The field name above the control.", "default": "Quality"},
    {"id": "options", "type": "string", "role": "content", "label": "Options", "description": "Comma separated choices. The first one is the value on show.", "default": "1080p, 4K"},
    {"id": "width", "type": "enum", "role": "layout", "label": "Width", "description": "How wide the control sits, from 180px through 240px to 320px.", "default": "standard", "options": [{"value": "compact", "label": "Compact"}, {"value": "standard", "label": "Standard"}, {"value": "wide", "label": "Wide"}]},
    {"id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Colour family of the focus ring the timeline pushes out.", "default": "green", "options": [{"value": "green", "label": "Green"}, {"value": "blue", "label": "Blue"}, {"value": "violet", "label": "Violet"}]}
  ]'
  ><span>Quality</span
  ><select>
    <option>1080p</option>
    <option>4K</option>
  </select>
  <svg class="hf-ui-native-select-icon" viewBox="0 0 24 24" aria-hidden="true">
    <path d="m7 9.5 5 5 5-5" /></svg
></label>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-native-select {
    position: relative;
    display: grid;
    gap: 8px;
    width: min(var(--hf-select-width, 240px), 100%);
    color: var(--_hf-ui-text);
    font-family: var(--_hf-ui-font-sans);
  }
  .hf-ui-native-select span {
    font-size: 13px;
    font-weight: var(--_hf-ui-weight-semibold);
  }
  .hf-ui-native-select select {
    height: var(--_hf-ui-control-height);
    padding: 0 38px 0 13px;
    border: 1px solid var(--_hf-ui-control-border);
    border-radius: var(--_hf-ui-radius-control);
    background: var(--_hf-ui-surface);
    color: var(--_hf-ui-text);
    font: var(--_hf-ui-weight-medium) 13px/1 var(--_hf-ui-font-mono);
    appearance: none;
    box-shadow:
      0 0 0 var(--hf-select-ring, 0px) color-mix(in srgb, var(--_hf-ui-accent) 36%, transparent),
      var(--_hf-ui-shadow-inset);
    cursor: pointer;
    transition:
      border-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response),
      box-shadow var(--_hf-ui-duration-state) var(--_hf-ui-ease-response);
  }
  .hf-ui-native-select select:focus-visible {
    border-color: var(--_hf-ui-accent);
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-native-select select:disabled {
    cursor: not-allowed;
    opacity: 0.48;
  }
  .hf-ui-native-select select[aria-invalid="true"] {
    border-color: var(--_hf-ui-danger);
  }
  .hf-ui-native-select-icon {
    position: absolute;
    right: 12px;
    bottom: 11px;
    width: 18px;
    height: 18px;
    fill: none;
    stroke: var(--_hf-ui-text-muted);
    stroke-width: 1.5;
    stroke-linecap: square;
    stroke-linejoin: miter;
    pointer-events: none;
  }
  @media (pointer: coarse) {
    .hf-ui-native-select {
      min-block-size: var(--_hf-ui-hit-target);
    }
    .hf-ui-native-select::after {
      position: absolute;
      inset-inline: 0;
      bottom: calc((var(--_hf-ui-control-height) - var(--_hf-ui-hit-target)) / 2);
      height: var(--_hf-ui-hit-target);
      content: "";
    }
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.
    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    function text(value, fallback) {
      return typeof value === "string" && value.trim() !== "" ? value.trim() : fallback;
    }

    function list(value, fallback) {
      var raw = typeof value === "string" ? value.split(",") : [];
      var parts = [];
      for (var i = 0; i < raw.length; i += 1) {
        if (raw[i].trim() !== "") parts.push(raw[i].trim());
      }
      return parts.length > 0 ? parts : fallback;
    }

    var widths = { compact: "180px", standard: "240px", wide: "320px" };
    var accents = {
      green: "var(--_hf-ui-data-1)",
      blue: "var(--_hf-ui-data-2)",
      violet: "var(--_hf-ui-data-4)",
    };

    var width = widths[pick(widths, vars.width, "standard")];
    var accent = accents[pick(accents, vars.accent, "green")];
    var labelText = text(vars.label, "Quality");
    var choices = list(vars.options, ["1080p", "4K"]);

    var roots = document.querySelectorAll(".hf-ui-native-select");
    for (var r = 0; r < roots.length; r += 1) {
      var root = roots[r];
      root.style.setProperty("--hf-select-width", width);
      root.style.setProperty("--hf-ui-accent", accent);

      var caption = root.querySelector("span");
      if (caption) caption.textContent = labelText;

      var select = root.querySelector("select");
      if (!select) continue;
      var options = select.querySelectorAll("option");
      for (var i = 0; i < choices.length; i += 1) {
        var option = options[i];
        if (!option) {
          option = document.createElement("option");
          select.appendChild(option);
        }
        option.textContent = choices[i];
      }
      for (var j = options.length - 1; j >= choices.length; j -= 1) {
        options[j].remove();
      }
      select.selectedIndex = 0;
    }
  })();
</script>

<!--
  Timeline integration:
  tl.fromTo('.hf-ui-native-select select', { '--hf-select-ring': '0px' }, { '--hf-select-ring': '2px', duration: 0.16, ease: 'power3.out' }, startTime);
-->
```

</Accordion>

## Usage

Open `compositions/components/native-select.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `native-select` `control`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
