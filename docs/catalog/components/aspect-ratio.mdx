---
title: "Aspect Ratio"
description: "A matte aspect-ratio frame for media and previews with a compact technical dimension label"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/aspect-ratio.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Aspect Ratio preview" />

## Install

<InstallCommand command="npx hyperframes add aspect-ratio" />

That writes one file: `compositions/components/aspect-ratio.html`.
...
| --- | --- | --- | --- |
| `ratio` | `16:9` | `16:9`, `4:3`, `1:1`, `9:16`, `21:9` | Frame shape, mirrored into the dimension label. |
| `size` | `standard` | `small`, `standard`, `large` | Maximum frame width before it shrinks to the host box. |
| `fill` | `matte` | `matte`, `canvas`, `outline` | Frame interior: matte is the inset surface, canvas is the deeper page colour, outline is a dashed empty placeholder. |
...
```html
<div
  data-composition-id="aspect-ratio"
  data-composition-src="compositions/components/aspect-ratio.html"
  data-variable-values='{"ratio":"16:9","size":"standard","fill":"matte"}'
></div>
```

## Source

<Accordion title={`aspect-ratio.html`}>

```html
<!--
  Aspect Ratio - Operator Black media-frame primitive for HyperFrames.
  ...
  the CSS consumes. The timeline recipe below is untouched by them.
    ...
    - ratio (16:9 | 4:3 | 1:1 | 9:16 | 21:9, default 16:9): frame shape. The
      dimension label is rewritten to match, so the frame never lies.
    - size (small | standard | large, default standard): maximum frame width,
      300px, 420px or 560px, before it shrinks to the host box.
    - fill (matte | canvas | outline, default matte): frame interior. matte is
      the inset surface, canvas is the deeper page colour, outline drops the
      fill and shadow for a dashed empty placeholder.

  On every default the result is identical to the original: a 420px 16:9 matte
  frame labelled 16:9 Preview.
-->

<div
  data-hf-ui-root
  class="hf-ui-aspect-ratio"
  data-composition-variables='[
    { "id": "ratio", "type": "enum", "role": "style", "label": "Ratio", "description": "Frame shape, mirrored into the dimension label.", "default": "16:9", "options": [{ "value": "16:9", "label": "16:9" }, { "value": "4:3", "label": "4:3" }, { "value": "1:1", "label": "1:1" }, { "value": "9:16", "label": "9:16" }, { "value": "21:9", "label": "21:9" }] },
    { "id": "size", "type": "enum", "role": "style", "label": "Size", "description": "Maximum frame width before it shrinks to the host box.", "default": "standard", "options": [{ "value": "small", "label": "Small" }, { "value": "standard", "label": "Standard" }, { "value": "large", "label": "Large" }] },
    { "id": "fill", "type": "enum", "role": "style", "label": "Fill", "description": "Frame interior: matte is the inset surface, canvas is the deeper page colour, outline is a dashed empty placeholder.", "default": "matte", "options": [{ "value": "matte", "label": "Matte" }, { "value": "canvas", "label": "Canvas" }, { "value": "outline", "label": "Outline" }] }
  ]'
>
...
<style>
  ...
  .hf-ui-aspect-ratio {
    display: grid;
    place-items: center;
    width: min(var(--hf-aspect-width, 420px), 100%);
    aspect-ratio: var(--hf-aspect-ratio, 16 / 9);
    border: 1px var(--hf-aspect-border-style, solid) var(--_hf-ui-border-strong);
    border-radius: var(--_hf-ui-radius-surface);
    background: var(--hf-aspect-fill, var(--_hf-ui-surface-inset));
    color: var(--_hf-ui-text);
    font: var(--_hf-ui-weight-medium) 12px/1 var(--_hf-ui-font-mono);
    overflow: hidden;
    box-shadow: var(--hf-aspect-shadow, var(--_hf-ui-shadow-inset));
  }
  .hf-ui-aspect-ratio span {
...
<script>
  (function () {
    ...
    var sizes = { small: "300px", standard: "420px", large: "560px" };
    var fills = {
      matte: {
        fill: "var(--_hf-ui-surface-inset)",
        border: "solid",
        shadow: "var(--_hf-ui-shadow-inset)",
      },
      canvas: {
        fill: "var(--_hf-ui-canvas)",
        border: "solid",
        shadow: "var(--_hf-ui-shadow-inset)",
      },
      outline: { fill: "transparent", border: "dashed", shadow: "none" },
    };
    ...
    var ratioName = pick(ratios, vars.ratio, "16:9");
    var size = sizes[pick(sizes, vars.size, "standard")];
    var fill = fills[pick(fills, vars.fill, "matte")];

    var roots = document.querySelectorAll(".hf-ui-aspect-ratio");
    for (var i = 0; i < roots.length; i += 1) {
      var style = roots[i].style;
      style.setProperty("--hf-aspect-ratio", ratios[ratioName]);
      style.setProperty("--hf-aspect-width", size);
      style.setProperty("--hf-aspect-fill", fill.fill);
      style.setProperty("--hf-aspect-border-style", fill.border);
      style.setProperty("--hf-aspect-shadow", fill.shadow);
      var label = roots[i].querySelector("span");
      if (label) label.textContent = ratioName + " Preview";
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.hf-ui-aspect-ratio', { opacity: 0, scale: 0.98, filter: 'blur(1.5px)' }, { opacity: 1, scale: 1, filter: 'blur(0px)', duration: 0.21, ease: 'power3.out' }, startTime);
-->
```
...
Tagged `ui-primitive` `remocn-port` `aspect-ratio` `layout`.
