---
title: "Combobox"
description: "A searchable combobox with an anchored option list and explicit selection check"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/combobox.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Combobox preview" />

## Install

<InstallCommand command="npx hyperframes add combobox" />

That writes one file: `compositions/components/combobox.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `label` | `Framework` | string | Field name shown at the left of the trigger. |
| `value` | `HyperFrames` | string | Chosen value. Written to both the trigger and the checked option. |
| `accent` | `green` | `green`, `blue`, `violet` | Accent token behind the selected-option check and the focus ring. |
| `theme` | `dark` | `dark`, `light` | Token set for the whole control: dark surfaces or light ones. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html wrap
<div
  data-composition-id="combobox"
  data-composition-src="compositions/components/combobox.html"
  data-variable-values='{"label":"Framework","value":"HyperFrames","accent":"green","theme":"dark"}'
></div>
```

## Source

<Accordion title={`combobox.html`}>

```html
<!--
  Combobox - Operator Black searchable choice primitive for HyperFrames.

  Paste the markup and CSS into a composition. Use the timeline integration
  note at the bottom as the starting point for deterministic GSAP animation.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and writes the result into the markup or
  into the hf-ui token the CSS above already consumes. The timeline recipe at
  the bottom is untouched by them.

    - label (text, default "Framework"): the field name at the left of the
      trigger.
    - value (text, default "HyperFrames"): the chosen value. It writes both the
      trigger's value and the checked option's label, so the two stay in sync.
    - accent (green | blue | violet, default green): accent token behind the
      selected-option check and the focus ring. green is the family accent the
      primitive already shipped with.
    - theme (dark | light, default dark): swaps the hf-ui token set, so the
      whole control renders on a light surface.

  The option list itself stays plain markup: add or remove option rows in the
  block below.
-->

<div
  data-hf-ui-root
  class="hf-ui-combobox"
  data-state="open"
  data-composition-variables='[
    { "id": "label", "type": "string", "role": "content", "label": "Field label", "description": "Field name shown at the left of the trigger.", "default": "Framework" },
    { "id": "value", "type": "string", "role": "content", "label": "Selected value", "description": "Chosen value. Written to both the trigger and the checked option.", "default": "HyperFrames" },
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Accent token behind the selected-option check and the focus ring.", "default": "green", "options": [{ "value": "green", "label": "Green" }, { "value": "blue", "label": "Blue" }, { "value": "violet", "label": "Violet" }] },
    { "id": "theme", "type": "enum", "role": "style", "label": "Theme", "description": "Token set for the whole control: dark surfaces or light ones.", "default": "dark", "options": [{ "value": "dark", "label": "Dark" }, { "value": "light", "label": "Light" }] }
  ]'
>
  <button
    class="hf-ui-combobox-trigger"
    type="button"
    aria-haspopup="listbox"
    aria-expanded="true"
    aria-controls="hf-ui-combobox-menu"
  >
    <span>Framework</span><strong>HyperFrames</strong>
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 9.5 5 5 5-5" /></svg>
  </button>
  <div class="hf-ui-combobox-menu">
    <div
      class="hf-ui-combobox-search"
      role="searchbox"
      aria-label="Search frameworks"
      aria-readonly="true"
    >
      Search frameworks...
    </div>
    <div
      id="hf-ui-combobox-menu"
      class="hf-ui-combobox-options"
      role="listbox"
      aria-label="Framework options"
    >
      <div class="hf-ui-combobox-option is-muted" role="option" aria-selected="false">Remotion</div>
      <div class="hf-ui-combobox-option is-active" role="option" aria-selected="true">
        HyperFrames
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12.5 4.5 4.5L19 7" /></svg>
      </div>
      <div class="hf-ui-combobox-option" role="option" aria-selected="false">Motion Canvas</div>
    </div>
  </div>
</div>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-combobox {
    width: min(360px, 100%);
    color: var(--_hf-ui-text);
    font-family: var(--_hf-ui-font-sans);
  }
  .hf-ui-combobox-trigger,
  .hf-ui-combobox-menu {
    border: 1px solid var(--_hf-ui-border-strong);
    background: var(--_hf-ui-surface-raised);
  }
  .hf-ui-combobox-trigger {
    display: flex;
    align-items: center;
    justify-content: space-between;
    width: 100%;
    height: var(--_hf-ui-control-height);
    padding: 0 var(--_hf-ui-space-3);
    border-radius: var(--_hf-ui-radius-control);
    box-shadow: var(--_hf-ui-shadow-inset);
    color: var(--_hf-ui-text);
    cursor: pointer;
    transition:
      transform var(--_hf-ui-duration-press) var(--_hf-ui-ease-response),
      border-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response);
  }
  .hf-ui-combobox-trigger span {
    color: var(--_hf-ui-text-muted);
    font-size: 12px;
    font-weight: var(--_hf-ui-weight-medium);
  }
  .hf-ui-combobox-trigger strong {
    margin-inline-start: auto;
    font-size: 13px;
    font-weight: var(--_hf-ui-weight-semibold);
  }
  .hf-ui-combobox-trigger svg,
  .hf-ui-combobox-option svg {
    width: 16px;
    height: 16px;
    fill: none;
    stroke: currentColor;
    stroke-width: 1.5;
    stroke-linecap: square;
    stroke-linejoin: miter;
  }
  .hf-ui-combobox-trigger:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-combobox-trigger:active {
    transform: translateY(1px) scale(0.98);
  }
  .hf-ui-combobox-trigger:disabled,
  .hf-ui-combobox-trigger[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
    transform: none;
  }
  .hf-ui-combobox[aria-invalid="true"] .hf-ui-combobox-trigger,
  .hf-ui-combobox-trigger[aria-invalid="true"] {
    border-color: var(--_hf-ui-danger);
  }
  .hf-ui-combobox-menu {
    display: grid;
    gap: 2px;
    margin-top: var(--_hf-ui-space-2);
    padding: var(--_hf-ui-space-1);
    border-radius: var(--_hf-ui-radius-overlay);
    box-shadow: var(--_hf-ui-shadow-contact);
    transform-origin: top left;
    will-change: opacity, transform;
  }
  .hf-ui-combobox-search,
  .hf-ui-combobox-option {
    min-height: 36px;
    border-radius: var(--_hf-ui-radius-control);
    padding: 8px 10px;
    font-size: 13px;
    font-weight: var(--_hf-ui-weight-medium);
  }
  .hf-ui-combobox-search {
    margin-bottom: 2px;
    border: 1px solid var(--_hf-ui-border);
    background: var(--_hf-ui-surface-inset);
    color: var(--_hf-ui-text-muted);
  }
  .hf-ui-combobox-options {
    display: grid;
    gap: 2px;
  }
  .hf-ui-combobox-option {
    display: flex;
    align-items: center;
    min-height: 40px;
    color: var(--_hf-ui-text);
  }
  .hf-ui-combobox-option.is-active {
    background: var(--_hf-ui-surface-inset);
    color: var(--_hf-ui-text-strong);
  }
  .hf-ui-combobox-option.is-active svg {
    margin-inline-start: auto;
    color: var(--_hf-ui-accent);
  }
  .hf-ui-combobox-option.is-muted {
    color: var(--_hf-ui-text-muted);
  }
  .hf-ui-combobox-option[aria-disabled="true"] {
    color: var(--_hf-ui-text-faint);
    cursor: not-allowed;
    opacity: 0.58;
  }
  @media (pointer: coarse) {
    .hf-ui-combobox-trigger {
      position: relative;
      margin-block: calc((var(--_hf-ui-hit-target) - var(--_hf-ui-control-height)) / 2);
    }
    .hf-ui-combobox-trigger::before {
      position: absolute;
      inset-block: calc((var(--_hf-ui-control-height) - var(--_hf-ui-hit-target)) / 2);
      inset-inline: 0;
      content: "";
    }
    .hf-ui-combobox-option {
      position: relative;
      margin-block: 2px;
    }
    .hf-ui-combobox-option::before {
      position: absolute;
      inset-block: -2px;
      inset-inline: 0;
      content: "";
    }
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.

    var accents = {
      green: "var(--_hf-ui-data-1)",
      blue: "var(--_hf-ui-data-2)",
      violet: "var(--_hf-ui-data-4)",
    };

    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    function text(value, fallback) {
      return typeof value === "string" && value.trim() !== "" ? value.trim() : fallback;
    }

    // Rewrites the element's own text run, leaving child elements in place.
    function setOwnText(el, value) {
      for (var n = 0; n < el.childNodes.length; n += 1) {
        var node = el.childNodes[n];
        if (node.nodeType === 3 && node.nodeValue.trim() !== "") {
          node.nodeValue = value;
          return;
        }
      }
      el.insertBefore(document.createTextNode(value), el.firstChild);
    }

    var accent = accents[pick(accents, vars.accent, "green")];
    var theme = vars.theme === "light" ? "light" : "dark";
    var label = text(vars.label, "Framework");
    var value = text(vars.value, "HyperFrames");

    var roots = document.querySelectorAll(".hf-ui-combobox");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.style.setProperty("--hf-ui-accent", accent);
      root.setAttribute("data-hf-theme", theme);

      var field = root.querySelector(".hf-ui-combobox-trigger span");
      if (field) field.textContent = label;

      var current = root.querySelector(".hf-ui-combobox-trigger strong");
      if (current) current.textContent = value;

      var checked = root.querySelector(".hf-ui-combobox-option.is-active");
      if (checked) setOwnText(checked, value);
    }
  })();
</script>

<!--
  Timeline integration:
  tl.fromTo('.hf-ui-combobox-trigger', { y: 4, opacity: 0 }, { y: 0, opacity: 1, duration: 0.21, ease: 'power3.out' }, 0.3);
  tl.fromTo('.hf-ui-combobox-menu', { opacity: 0, y: -4, scale: 0.98 }, { opacity: 1, y: 0, scale: 1, duration: 0.21, ease: 'power3.out' }, 0.52);
  tl.fromTo('.hf-ui-combobox-option', { opacity: 0 }, { opacity: 1, duration: 0.16, ease: 'power3.out' }, 0.68);
-->
```

</Accordion>

## Usage

Open `compositions/components/combobox.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `combobox` `control`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
