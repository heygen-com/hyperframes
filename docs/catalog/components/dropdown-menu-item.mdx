...
| --- | --- | --- | --- |
| `accent` | `mint` | `mint`, `blue`, `violet` | Colour of the check mark on the selected row. |
| `density` | `comfortable` | `comfortable`, `compact` | Row height and horizontal padding. |
| `hint` | `shortcut` | `shortcut`, `hidden` | Whether the trailing shortcut column is shown. |
...
```html
<div
  data-composition-id="dropdown-menu-item"
  data-composition-src="compositions/components/dropdown-menu-item.html"
  data-variable-values='{"accent":"mint","density":"comfortable","hint":"shortcut"}'
></div>
```

## Source

<Accordion title={`dropdown-menu-item.html`}>

```html
<!--
  Dropdown Menu Item - Operator Black menu-row state specimen.
  ...
  untouched by them.
    ...
    - accent (mint | blue | violet, default mint): colour of the check mark on
      the selected row. Each option rides an existing theme token, so both the
      dark and light palettes stay correct.
    - density (comfortable | compact, default comfortable): row height and
      horizontal padding. comfortable is the 40px control height, compact
      tightens rows to 30px for long menus. Coarse pointers keep the 44px hit
      target either way.
    - hint (shortcut | hidden, default shortcut): whether the trailing
      shortcut column is shown, so the labels can run full width.
...
<div
  data-hf-ui-root
  class="hf-remocn-dropdown-menu-item"
  data-composition-variables='[
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Colour of the check mark on the selected row.", "default": "mint", "options": [{ "value": "mint", "label": "Mint" }, { "value": "blue", "label": "Blue" }, { "value": "violet", "label": "Violet" }] },
    { "id": "density", "type": "enum", "role": "style", "label": "Density", "description": "Row height and horizontal padding.", "default": "comfortable", "options": [{ "value": "comfortable", "label": "Comfortable" }, { "value": "compact", "label": "Compact" }] },
    { "id": "hint", "type": "enum", "role": "style", "label": "Hint", "description": "Whether the trailing shortcut column is shown.", "default": "shortcut", "options": [{ "value": "shortcut", "label": "Shortcut" }, { "value": "hidden", "label": "Hidden" }] }
  ]'
  role="menu"
>
  <div class="row" data-state="idle" role="menuitem" tabindex="0">
    <span class="label">Duplicate scene</span><span class="hint">D</span>
  </div>
  <div class="row" data-state="hover" role="menuitem" tabindex="0">
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 12 4 4 8-9" /></svg>
    <span class="label">Export selection</span><span class="hint">E</span>
  </div>
  <div class="row" role="menuitem" aria-disabled="true" tabindex="-1">
    <span class="label">Archive project</span><span class="hint">Unavailable</span>
  </div>
</div>
...
<style>
  ...
  .hf-remocn-dropdown-menu-item .row {
    min-height: var(--hf-row-height, var(--_hf-ui-control-height));
  ...
  .hf-remocn-dropdown-menu-item .row:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
...
<script>
  (function () {
    ...
    var densities = {
      comfortable: { height: "var(--_hf-ui-control-height)", pad: "var(--_hf-ui-space-3)" },
      compact: { height: "30px", pad: "var(--_hf-ui-space-2)" },
    };
    var hints = { shortcut: "block", hidden: "none" };
    ...
    var hint = hints[pick(hints, vars.hint, "shortcut")];

    var roots = document.querySelectorAll(".hf-remocn-dropdown-menu-item");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--hf-accent", accent);
      roots[i].style.setProperty("--hf-row-height", density.height);
      roots[i].style.setProperty("--hf-row-pad", density.pad);
      roots[i].style.setProperty("--hf-hint-display", hint);
    }
  })();
</script>
...
<!--
  ...
  tl.set('.hf-remocn-dropdown-menu-item', { opacity: 1 }, startTime);
  tl.fromTo('.hf-remocn-dropdown-menu-item .row', { '--hf-row-x': '-4px', '--hf-row-opacity': 0 }, { '--hf-row-x': '0px', '--hf-row-opacity': 1, duration: 0.16, stagger: 0.04, ease: 'power2.out' }, startTime);
-->
```
