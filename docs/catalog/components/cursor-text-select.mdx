---
title: "Text Select Cursor"
description: "An arrow turns into an I-beam over a document, drags a selection across a phrase character by character, raises a format toolbar, turns back into an arrow and clicks Bold."
mode: "frame"
---
...
<CatalogDetail
  previewSrc="/public/catalog/components/cursor-text-select.json"
  compositionId="cursor-text-select"
  compositionSrc="compositions/components/cursor-text-select.html"
  title="Text Select Cursor"
  description="An arrow turns into an I-beam over a document, drags a selection across a phrase character by character, raises a format toolbar, turns back into an arrow and clicks Bold."
  variables={[{"id":"title","type":"string","role":"content","label":"Heading","default":"Spring release notes"},{"id":"text","type":"string","role":"content","label":"Paragraph","default":"Our spring release ships on Thursday. Teams can now share a live review link, collect time-stamped comments, and approve the final cut without leaving the editor. Early customers cut their review rounds in half."},{"id":"phrase","type":"string","role":"content","label":"Phrase to select","default":"approve the final cut"},{"id":"accent","type":"color","role":"style","label":"Accent","default":"#3b6cf6"}]}
  meta={{"category":"Cursors","badge":"Stable","codeLines":401}}
  attribution={{"path":"registry/components/cursor-text-select","tags":["cursor","motion-primitive","text","selection","editor","toolbar"]}}
  hasCode
  rawUrl="https://raw.githubusercontent.com/heygen-com/hyperframes/main/registry/components/cursor-text-select/cursor-text-select.html"
>

<CatalogSlot slot="code">

```html cursor-text-select.html
<!doctype html>
<!--
...
<InstallCommand command="npx hyperframes add cursor-text-select" item="cursor-text-select" />

That writes one file: `compositions/components/cursor-text-select.html`.
...
Open `compositions/components/cursor-text-select.html` and copy what is inside into your own composition.
...
| --- | --- | --- | --- |
| `title` | `Spring release notes` | string |  |
| `text` | `Our spring release ships on Thursday. Teams can now share a live review link, collect time-stamped comments, and approve the final cut without leaving the editor. Early customers cut their review rounds in half.` | string |  |
| `phrase` | `approve the final cut` | string |  |
| `accent` | `#3b6cf6` | color |  |
...
<div
  data-composition-id="cursor-text-select"
  data-composition-src="compositions/components/cursor-text-select.html"
  data-variable-values='{"title":"Spring release notes","text":"Our spring release ships on Thursday. Teams can now share a live review link, collect time-stamped comments, and approve the final cut without leaving the editor. Early customers cut their review rounds in half.","phrase":"approve the final cut","accent":"#3b6cf6"}'
></div>
```

{/* hf:generated-footer */}

Tagged `cursor` `motion-primitive` `text` `selection` `editor` `toolbar`.
