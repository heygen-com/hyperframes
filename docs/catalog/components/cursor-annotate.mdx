...
<CatalogDetail
  ...
  meta={{"category":"Cursors","badge":"Stable","codeLines":404}}
      ...
      <script>
        (function () {
          ...
          function arrowStrokes() {
            ...
            var from = {
              x: tip.x - Math.max(160 * unit, box.w * 0.7),
              y: tip.y + Math.max(120 * unit, box.h * 0.9),
            };
            ...
            for (var i = 0; i <= 40; i++) {
              ...
              var v = 1 - u;
              pts.push([
                v * v * from.x + 2 * v * u * ctrl.x + u * u * tip.x,
                v * v * from.y + 2 * v * u * ctrl.y + u * u * tip.y,
              ]);
            }
          ...
          function penAt(t) {
            ...
            for (var i = 0; i < windows.length; i++) {
              ...
              if (t <= w.end)
                return {
                  p: pointOn(strokes[i], stroke((t - w.start) / (w.end - w.start))),
                  lift: 0,
                };
              ...
              if (next && t < next.start) {
                ...
                return {
                  p: mix(pointOn(strokes[i], 1), pointOn(strokes[i + 1], 0), hop(u)),
            ...
            if (t <= T_EXIT) {
              ...
              return {
                p: mix(endPoint, ASIDE, gsap.parseEase("power2.out")(a)),
                lift: Math.min(1, a * 2.5),
              };
            }
            return {
          ...
          function render(t) {
            strokes.forEach(function (s, i) {
              ...
              s.thin.setAttribute(
                "stroke-dasharray",
                len.toFixed(2) + " " + (s.length * 2).toFixed(2),
              );
              ...
              s.thick.setAttribute(
                "stroke-dasharray",
                Math.max(0, core).toFixed(2) + " " + (s.length * 3).toFixed(2),
              );
