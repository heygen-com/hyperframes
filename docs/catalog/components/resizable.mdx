---
title: "Resizable"
description: "An accessible split-pane primitive with structural edges, a keyboard-focusable handle, and deterministic resizing"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/resizable.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Resizable preview" />

## Install

<InstallCommand command="npx hyperframes add resizable" />

That writes one file: `compositions/components/resizable.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `split` | `standard` | `inspector`, `standard`, `preview` | Scales the driven pane divide, so the primary pane keeps a narrower or wider share of the frame at every point of the drag. |
| `size` | `standard` | `compact`, `standard`, `wide` | Overall footprint of the split pane on the frame. |
| `panes` | `Preview, Inspector` | string | Comma separated names for the primary and secondary pane, in order. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html wrap
<div
  data-composition-id="resizable"
  data-composition-src="compositions/components/resizable.html"
  data-variable-values='{"split":"standard","size":"standard","panes":"Preview, Inspector"}'
></div>
```

## Source

<Accordion title={`resizable.html`}>

```html
<!--
  Resizable - Operator Black split-pane primitive for HyperFrames.

  Paste the markup and CSS into a composition. Use the timeline integration
  note at the bottom as the starting point for deterministic GSAP animation.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and writes the result as a custom property
  the CSS consumes. The drag itself stays yours to drive: the timeline keeps
  animating --hf-resizable-left, and split only scales whatever it reports.

    - split (inspector | standard | preview, default standard): multiplies the
      driven divide, so the primary pane holds a narrower or wider share at
      every point of the drag, not only at rest. The handle reports the
      resting share through aria-valuenow to match.
    - size (compact | standard | wide, default standard): the footprint of the
      whole pane, 460x210 / 620x280 / 820x360.
    - panes (comma separated text, default "Preview, Inspector"): the name
      shown in the primary pane and in the secondary pane, in order.

  On every default the result is identical to the original: a 620x280 frame
  split 58 percent to Preview and the rest to Inspector.
-->

<div
  data-hf-ui-root
  class="hf-ui-resizable"
  data-composition-variables='[
    { "id": "split", "type": "enum", "role": "layout", "label": "Split", "description": "Scales the driven pane divide, so the primary pane keeps a narrower or wider share of the frame at every point of the drag.", "default": "standard", "options": [{ "value": "inspector", "label": "Inspector heavy" }, { "value": "standard", "label": "Standard" }, { "value": "preview", "label": "Preview heavy" }] },
    { "id": "size", "type": "enum", "role": "layout", "label": "Size", "description": "Overall footprint of the split pane on the frame.", "default": "standard", "options": [{ "value": "compact", "label": "Compact" }, { "value": "standard", "label": "Standard" }, { "value": "wide", "label": "Wide" }] },
    { "id": "panes", "type": "string", "role": "content", "label": "Pane labels", "description": "Comma separated names for the primary and secondary pane, in order.", "default": "Preview, Inspector", "maxLength": 80 }
  ]'
>
  <div class="hf-ui-resizable-pane primary">Preview</div>
  <div
    class="hf-ui-resizable-handle"
    role="separator"
    tabindex="0"
    aria-orientation="vertical"
    aria-valuemin="20"
    aria-valuemax="80"
    aria-valuenow="58"
    aria-label="Resize preview and inspector"
  ></div>
  <div class="hf-ui-resizable-pane secondary">Inspector</div>
</div>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-resizable {
    --hf-resizable-left: 58%;
    display: grid;
    grid-template-columns:
      calc(var(--hf-resizable-left) * var(--hf-resizable-bias, 1))
      12px
      1fr;
    width: min(var(--hf-resizable-width, 620px), 100%);
    height: var(--hf-resizable-height, 280px);
    border: 1px solid var(--_hf-ui-border-strong);
    border-radius: var(--_hf-ui-radius-surface);
    background: var(--_hf-ui-surface);
    color: var(--_hf-ui-text);
    font-family: var(--_hf-ui-font-sans);
    overflow: hidden;
    box-shadow: var(--_hf-ui-shadow-inset);
  }
  .hf-ui-resizable-pane {
    display: grid;
    place-items: center;
    min-width: 0;
    font-size: 13px;
    font-weight: var(--_hf-ui-weight-medium);
  }
  .hf-ui-resizable-pane.primary {
    background: var(--_hf-ui-surface-inset);
    color: var(--_hf-ui-text-strong);
  }
  .hf-ui-resizable-pane.secondary {
    background: var(--_hf-ui-surface);
    color: var(--_hf-ui-text-muted);
  }
  .hf-ui-resizable-handle {
    position: relative;
    align-self: stretch;
    justify-self: center;
    width: 12px;
    margin-block: 4px;
    border-inline: 1px solid var(--_hf-ui-border);
    background: var(--_hf-ui-surface);
    cursor: col-resize;
  }
  .hf-ui-resizable-handle::before {
    position: absolute;
    inset-block-start: 50%;
    inset-inline-start: 50%;
    width: 2px;
    height: 48px;
    background: var(--_hf-ui-control-border);
    content: "";
    transform: translate(-50%, -50%);
    pointer-events: none;
  }
  .hf-ui-resizable-handle:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-resizable-handle:focus-visible::before {
    background: var(--_hf-ui-accent);
  }
  @media (hover: hover) and (pointer: fine) {
    .hf-ui-resizable-handle:hover::before {
      background: var(--_hf-ui-control-border-hover);
    }
  }
  @media (pointer: coarse) {
    .hf-ui-resizable-handle::after {
      position: absolute;
      inset-block: 0;
      inset-inline-start: 50%;
      width: var(--_hf-ui-hit-target);
      content: "";
      transform: translateX(-50%);
    }
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.
    var splits = { inspector: 0.62, standard: 1, preview: 1.34 };
    var sizes = {
      compact: { width: "460px", height: "210px" },
      standard: { width: "620px", height: "280px" },
      wide: { width: "820px", height: "360px" },
    };

    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    var bias = splits[pick(splits, vars.split, "standard")];
    var size = sizes[pick(sizes, vars.size, "standard")];

    var names = [];
    if (typeof vars.panes === "string") {
      names = vars.panes.split(",").map(function (part) {
        return part.trim();
      });
    }
    if (names.join("") === "") {
      names = ["Preview", "Inspector"];
    }

    var roots = document.querySelectorAll(".hf-ui-resizable");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--hf-resizable-bias", String(bias));
      roots[i].style.setProperty("--hf-resizable-width", size.width);
      roots[i].style.setProperty("--hf-resizable-height", size.height);

      var panes = roots[i].querySelectorAll(".hf-ui-resizable-pane");
      for (var j = 0; j < panes.length; j += 1) {
        if (typeof names[j] === "string" && names[j] !== "") {
          panes[j].textContent = names[j];
        }
      }

      var handle = roots[i].querySelector(".hf-ui-resizable-handle");
      if (handle) {
        handle.setAttribute("aria-valuenow", String(Math.round(58 * bias)));
      }
    }
  })();
</script>

<!--
  Timeline integration:
  tl.fromTo('.hf-ui-resizable', { opacity: 0, y: 4, scale: 0.98 }, { opacity: 1, y: 0, scale: 1, duration: 0.21, ease: 'power3.out' }, 0.3);
  tl.to('.hf-ui-resizable', { '--hf-resizable-left': '68%', duration: 0.72, ease: 'power3.out' }, 0.85);
  tl.to('.hf-ui-resizable-handle', { scaleY: 0.98, duration: 0.12, ease: 'power3.out' }, 0.85);
  tl.to('.hf-ui-resizable-handle', { scaleY: 1, duration: 0.12, ease: 'power3.out' }, 0.97);
-->
```

</Accordion>

## Usage

Open `compositions/components/resizable.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `resizable` `layout`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
