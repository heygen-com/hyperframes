---
title: "Button"
description: "A video-safe action button with tactile press, loading, and success states"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/button.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Button preview" />

## Install

<InstallCommand command="npx hyperframes add button" />

That writes one file: `compositions/components/button.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `label` | `Continue` | string | Text shown on the button while it is idle. |
| `variant` | `default` | `default`, `secondary`, `outline` | Filled action, raised secondary surface, or a transparent outline. |
| `theme` | `dark` | `dark`, `light` | Operator Black dark surface or the light-frame token set. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html
<div
  data-composition-id="button"
  data-composition-src="compositions/components/button.html"
  data-variable-values='{"label":"Continue","variant":"default","theme":"dark"}'
></div>
```

## Source

<Accordion title={`button.html`}>

```html
<!--
  Button - Operator Black action primitive for HyperFrames.

  States:
    idle | hover | press | loading | success

  Add data-state and data-variant to the .hf-ui-button element, or animate the
  CSS variables from a paused GSAP timeline.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and writes the attribute or text the CSS
  above already keys off. The timeline recipe below is untouched by them: it
  still drives data-state through loading and success on top of whatever
  variant and theme are chosen.

    - label (string, default "Continue"): the text shown while idle. A blank
      or non-text override keeps the default.
    - variant (default | secondary | outline, default default): filled action,
      raised secondary surface, or transparent outline. Each maps to the
      data-variant rules already in the stylesheet.
    - theme (dark | light, default dark): dark keeps the Operator Black
      surface, light switches the shared token block to its light values.

  On every default the result is identical to the original: a filled dark-theme
  button reading Continue.
-->

<button
  data-hf-ui-root
  class="hf-ui-button"
  data-variant="default"
  data-state="idle"
  type="button"
  data-composition-variables='[
    { "id": "label", "type": "string", "role": "content", "label": "Label", "description": "Text shown on the button while it is idle.", "default": "Continue" },
    { "id": "variant", "type": "enum", "role": "style", "label": "Variant", "description": "Filled action, raised secondary surface, or a transparent outline.", "default": "default", "options": [{ "value": "default", "label": "Default" }, { "value": "secondary", "label": "Secondary" }, { "value": "outline", "label": "Outline" }] },
    { "id": "theme", "type": "enum", "role": "style", "label": "Theme", "description": "Operator Black dark surface or the light-frame token set.", "default": "dark", "options": [{ "value": "dark", "label": "Dark" }, { "value": "light", "label": "Light" }] }
  ]'
>
  <span class="hf-ui-button-label">Continue</span>
  <span class="hf-ui-button-spinner" aria-hidden="true"></span>
  <svg class="hf-ui-button-check" viewBox="0 0 24 24" aria-hidden="true">
    <path d="M5 12.5l4.5 4.5L19 7" />
  </svg>
</button>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-button {
    --hf-button-bg: var(--_hf-ui-action);
    --hf-button-fg: var(--_hf-ui-action-contrast);
    --hf-button-border: var(--_hf-ui-action);
    --hf-button-scale: 1;
    --hf-button-y: 0px;
    position: relative;
    display: inline-flex;
    align-items: center;
    justify-content: center;
    gap: 8px;
    min-width: 128px;
    height: var(--_hf-ui-control-height);
    padding: 0 var(--_hf-ui-space-4);
    border: 1px solid var(--hf-button-border);
    border-radius: var(--_hf-ui-radius-control);
    background: var(--hf-button-bg);
    color: var(--hf-button-fg);
    font: var(--_hf-ui-weight-semibold) 13px/1 var(--_hf-ui-font-sans);
    transform: translateY(var(--hf-button-y)) scale(var(--hf-button-scale));
    box-shadow: var(--_hf-ui-shadow-inset);
    cursor: pointer;
    transition:
      transform var(--_hf-ui-duration-press) var(--_hf-ui-ease-response),
      background-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response),
      border-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response),
      color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response);
    will-change: transform;
  }
  .hf-ui-button[data-variant="secondary"] {
    --hf-button-bg: var(--_hf-ui-surface);
    --hf-button-fg: var(--_hf-ui-text-strong);
    --hf-button-border: var(--_hf-ui-control-border);
  }
  .hf-ui-button[data-variant="outline"] {
    --hf-button-bg: transparent;
    --hf-button-fg: var(--_hf-ui-text);
    --hf-button-border: var(--_hf-ui-control-border);
    box-shadow: var(--_hf-ui-shadow-inset);
  }
  .hf-ui-button[data-state="hover"] {
    --hf-button-border: var(--_hf-ui-control-border-hover);
  }
  .hf-ui-button[data-state="press"] {
    --hf-button-y: 1px;
    --hf-button-scale: 0.98;
  }
  .hf-ui-button:disabled,
  .hf-ui-button[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
    transform: none;
  }
  @media (hover: hover) and (pointer: fine) {
    .hf-ui-button:not(:disabled, [aria-disabled="true"]):hover {
      --hf-button-border: var(--_hf-ui-control-border-hover);
    }
  }
  .hf-ui-button:not(:disabled, [aria-disabled="true"]):active {
    --hf-button-y: 1px;
    --hf-button-scale: 0.98;
  }
  .hf-ui-button-spinner,
  .hf-ui-button-check {
    position: absolute;
    opacity: 0;
  }
  .hf-ui-button-spinner {
    width: 18px;
    height: 18px;
    border: 1.5px solid color-mix(in srgb, currentColor 32%, transparent);
    border-top-color: currentColor;
    border-radius: 50%;
  }
  .hf-ui-button-check {
    width: 18px;
    height: 18px;
    fill: none;
    stroke: var(--_hf-ui-accent);
    stroke-width: 1.5;
    stroke-linecap: square;
    stroke-linejoin: miter;
  }
  .hf-ui-button[data-state="loading"] .hf-ui-button-label,
  .hf-ui-button[data-state="success"] .hf-ui-button-label {
    opacity: 0;
  }
  .hf-ui-button[data-state="loading"] .hf-ui-button-spinner,
  .hf-ui-button[data-state="success"] .hf-ui-button-check {
    opacity: 1;
  }
  @media (pointer: coarse) {
    .hf-ui-button {
      margin-block: calc((var(--_hf-ui-hit-target) - var(--_hf-ui-control-height)) / 2);
    }
    .hf-ui-button::before {
      position: absolute;
      inset-block: calc((var(--_hf-ui-control-height) - var(--_hf-ui-hit-target)) / 2);
      inset-inline: 0;
      content: "";
    }
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.
    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    var variants = { default: true, secondary: true, outline: true };
    var themes = { dark: true, light: true };

    var variant = pick(variants, vars.variant, "default");
    var theme = pick(themes, vars.theme, "dark");
    var label =
      typeof vars.label === "string" && vars.label.trim() !== "" ? vars.label : "Continue";

    var roots = document.querySelectorAll(".hf-ui-button");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.setAttribute("data-variant", variant);
      if (theme === "light") root.setAttribute("data-hf-theme", "light");
      else root.removeAttribute("data-hf-theme");
      var text = root.querySelector(".hf-ui-button-label");
      if (text) text.textContent = label;
    }
  })();
</script>

<!--
  Timeline integration:

  tl.to(".hf-ui-button", { "--hf-button-y": "1px", "--hf-button-scale": 0.98, duration: 0.12, ease: "power3.out" }, startTime);
  tl.set(".hf-ui-button", { attr: { "data-state": "loading" } }, startTime + 0.2);
  tl.to(".hf-ui-button-spinner", { rotate: 360, duration: 0.8, ease: "none" }, startTime + 0.2);
  tl.set(".hf-ui-button", { attr: { "data-state": "success" } }, startTime + 1.1);
-->
```

</Accordion>

## Usage

Open `compositions/components/button.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `button` `control`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
