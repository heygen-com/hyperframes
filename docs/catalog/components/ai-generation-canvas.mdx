---
title: "AI Generation Canvas"
description: "A prompt composer expands into a generated dashboard preview."
---

# AI Generation Canvas

A prompt composer expands into a generated dashboard preview.

`showcase` `remocn-port` `ai`

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/components/ai-generation-canvas.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/components/ai-generation-canvas.png" autoPlay muted loop playsInline />

## Install

<CodeGroup>

```bash Terminal
npx hyperframes add ai-generation-canvas
```

</CodeGroup>

## Details

| Property | Value |
| --- | --- |
| Type | Component |

## Files

| File | Target | Type |
| --- | --- | --- |
| `ai-generation-canvas.html` | `compositions/components/ai-generation-canvas.html` | hyperframes:snippet |

## Usage

Open `compositions/components/ai-generation-canvas.html` and paste its contents into your composition. See the comment header in the file for timeline integration guidance.
