...
| --- | --- | --- | --- |
| `align` | `center` | `center`, `start` | How the icon, copy and action sit: centred as a full-width empty state, or flush left inside a panel. |
| `density` | `comfortable` | `comfortable`, `compact` | Padding, gap and icon size. Compact tightens the block for a small panel. |
| `action` | `button` | `button`, `none` | Whether the call to action button is present. None leaves an informational empty state with no button. |
...
```html
<div
  data-composition-id="empty"
  data-composition-src="compositions/components/empty.html"
  data-variable-values='{"align":"center","density":"comfortable","action":"button"}'
></div>
```

## Source

<Accordion title={`empty.html`}>

```html
<!--
  Empty - Operator Black open-space empty state.
  ...
  the CSS consumes. The timeline recipe below is untouched by them.
    ...
    - align (center | start, default center): how the icon, copy and action
      sit. center is the full-width empty state, start flushes everything left
      for a panel or sidebar.
    - density (comfortable | compact, default comfortable): padding, gap and
      icon size. compact tightens 36px padding to 22px and the icon to 40px.
    - action (button | none, default button): whether the call to action
      button is present. none removes it, leaving an informational state.

  On every default the result is identical to the original: a centred 36px
  block with a 48px icon frame and an Upload media button.
-->
...
<section
  ...
  data-composition-variables='[
    { "id": "align", "type": "enum", "role": "style", "label": "Align", "description": "How the icon, copy and action sit: centred as a full-width empty state, or flush left inside a panel.", "default": "center", "options": [{ "value": "center", "label": "Center" }, { "value": "start", "label": "Start" }] },
    { "id": "density", "type": "enum", "role": "style", "label": "Density", "description": "Padding, gap and icon size. Compact tightens the block for a small panel.", "default": "comfortable", "options": [{ "value": "comfortable", "label": "Comfortable" }, { "value": "compact", "label": "Compact" }] },
    { "id": "action", "type": "enum", "role": "style", "label": "Action", "description": "Whether the call to action button is present. None leaves an informational empty state with no button.", "default": "button", "options": [{ "value": "button", "label": "Button" }, { "value": "none", "label": "None" }] }
  ]'
>
  <div class="hf-ui-empty-icon" aria-hidden="true">
    <svg viewBox="0 0 24 24"><path d="M5 7h14v11H5zM9 7l1.5-2h3L15 7M9 12h6" /></svg>
  </div>
  <h3>No clips yet</h3>
  <p>Add media to start building the timeline.</p>
...
<style>
  ...
  .hf-ui-empty {
    display: grid;
    justify-items: var(--hf-empty-align, center);
    gap: var(--hf-empty-gap, 12px);
    width: min(420px, 100%);
    padding: var(--hf-empty-pad, 36px);
    color: var(--_hf-ui-text);
    text-align: var(--hf-empty-text-align, center);
  }
  .hf-ui-empty-icon {
    display: grid;
    place-items: center;
    width: var(--hf-empty-icon, 48px);
    height: var(--hf-empty-icon, 48px);
  ...
  .hf-ui-empty button:active {
    transform: translateY(1px) scale(0.98);
  }
  .hf-ui-empty button:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
...
<script>
  (function () {
    ...
    var densities = {
      comfortable: { pad: "36px", gap: "12px", icon: "48px" },
      compact: { pad: "22px", gap: "8px", icon: "40px" },
    };

    var align = aligns[pick(aligns, vars.align, "center")];
    var density = densities[pick(densities, vars.density, "comfortable")];
    var hideAction = vars.action === "none";

    var roots = document.querySelectorAll(".hf-ui-empty");
    for (var i = 0; i < roots.length; i += 1) {
      var style = roots[i].style;
      style.setProperty("--hf-empty-align", align.items);
      style.setProperty("--hf-empty-text-align", align.text);
      style.setProperty("--hf-empty-pad", density.pad);
      style.setProperty("--hf-empty-gap", density.gap);
      style.setProperty("--hf-empty-icon", density.icon);
      if (hideAction) {
        var button = roots[i].querySelector("button");
        if (button) {
          button.remove();
        }
      }
...
<!--
  ...
  tl.fromTo('.hf-ui-empty', { opacity: 0, y: 4 }, { opacity: 1, y: 0, duration: 0.21, ease: 'power3.out' }, startTime);
-->
```
