...
| --- | --- | --- | --- |
| `title` | `Audio synced` | string | Row title. Its first letter also becomes the monogram. |
| `meta` | `Beat grid detected` | string | Secondary line under the title. |
| `density` | `comfortable` | `comfortable`, `compact` | Row height, padding and column gap. |
...
```html
<div
  data-composition-id="item"
  data-composition-src="compositions/components/item.html"
  data-variable-values='{"title":"Audio synced","meta":"Beat grid detected","density":"comfortable","theme":"dark"}'
></div>
```

## Source

<Accordion title={`item.html`}>

```html
<!--
  Item - Operator Black list-row primitive for HyperFrames.
  ...
  anything missing or unrecognised, and writes the text or custom property the
  CSS below already consumes. The timeline recipe at the bottom is untouched
  by them: it keeps sliding the same row in.

    - title (string, default "Audio synced"): the row title. Its first letter
      is also the monogram in the leading tile, so the two never drift apart.
      A blank or non-text override keeps the default.
    - meta (string, default "Beat grid detected"): the secondary line under
      the title.
    - density (comfortable | compact, default comfortable): row height,
      padding, column gap and the space above the meta line.
  ...
  On every default the result is identical to the original: a 64px dark row
  reading A / Audio synced / Beat grid detected / Now.
-->
...
<div
  ...
  data-composition-variables='[
    { "id": "title", "type": "string", "role": "content", "label": "Title", "description": "Row title. Its first letter also becomes the monogram.", "default": "Audio synced" },
    { "id": "meta", "type": "string", "role": "content", "label": "Meta", "description": "Secondary line under the title.", "default": "Beat grid detected" },
    { "id": "density", "type": "enum", "role": "layout", "label": "Density", "description": "Row height, padding and column gap.", "default": "comfortable", "options": [{ "value": "comfortable", "label": "Comfortable" }, { "value": "compact", "label": "Compact" }] },
...
>
  <span class="hf-ui-item-icon">A</span>
  <div>
    <strong>Audio synced</strong>
    <p>Beat grid detected</p>
  </div>
  <em>Now</em>
</div>
...
<style>
  ...
  .hf-ui-item {
    ...
    align-items: center;
    gap: var(--hf-ui-item-gap, 12px);
    width: min(420px, 100%);
    min-height: var(--hf-ui-item-min-h, 64px);
    padding: var(--hf-ui-item-pad, 8px);
  ...
  .hf-ui-item p {
    margin: var(--hf-ui-item-rule, 4px) 0 0;
...
<script>
  (function () {
    ...
    var densities = {
      comfortable: { gap: "12px", minHeight: "64px", pad: "8px", rule: "4px" },
      compact: { gap: "8px", minHeight: "48px", pad: "4px", rule: "2px" },
    };

    var title = text(vars.title, "Audio synced");
    var meta = text(vars.meta, "Beat grid detected");
    var density = densities[pick(densities, vars.density, "comfortable")];
    var theme = vars.theme === "light" ? "light" : "dark";

    var roots = document.querySelectorAll(".hf-ui-item");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.setAttribute("data-hf-theme", theme);
      root.style.setProperty("--hf-ui-item-gap", density.gap);
      root.style.setProperty("--hf-ui-item-min-h", density.minHeight);
      root.style.setProperty("--hf-ui-item-pad", density.pad);
      root.style.setProperty("--hf-ui-item-rule", density.rule);
      var titleEl = root.querySelector("strong");
      if (titleEl) titleEl.textContent = title;
      var metaEl = root.querySelector("p");
      if (metaEl) metaEl.textContent = meta;
      var icon = root.querySelector(".hf-ui-item-icon");
      if (icon) icon.textContent = title.trim().charAt(0).toUpperCase();
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.hf-ui-item', { opacity: 0.55, x: -4 }, { opacity: 1, x: 0, duration: 0.21, ease: 'power3.out' }, startTime);
-->
```
