---
title: "Input"
description: "A video-safe text field with label, helper copy, focus state, and typed-value reveal"
---

import { InstallCommand } from "/snippets/install-command.jsx";

<iframe src="/public/catalog/preview/components/input.html" className="w-full aspect-video rounded-xl border border-zinc-200 dark:border-zinc-800" loading="lazy" title="Input preview" />

## Install

<InstallCommand command="npx hyperframes add input" />

That writes one file: `compositions/components/input.html`.

## Variables

Every one of these has a default, so the piece works untouched. Set the ones you
want to change on the element:

| Variable | Default | Accepts | What it does |
| --- | --- | --- | --- |
| `label` | `Project name` | string | Field label, also used as the accessible name of the shell. |
| `value` | `Launch recap` | string | Typed value revealed as the progress variable runs. |
| `state` | `default` | `default`, `invalid`, `disabled` | Resting validation state: neutral, danger border, or dimmed. |
| `theme` | `dark` | `dark`, `light` | Operator Black dark surface or the light-frame token set. |

Set them with `data-variable-values` on the element that mounts it. These are the
defaults, so this behaves exactly like the preview above until you change one:

```html wrap
<div
  data-composition-id="input"
  data-composition-src="compositions/components/input.html"
  data-variable-values='{"label":"Project name","value":"Launch recap","state":"default","theme":"dark"}'
></div>
```

## Source

<Accordion title={`input.html`}>

```html
<!--
  Input - Operator Black text field primitive for HyperFrames.

  Animate --hf-input-focus and --hf-input-value-progress from a paused GSAP
  timeline to move from empty to focused to typed states.

  Variables. The script reads each one, falls back to the declared default on
  anything missing or unrecognised, and writes the text or attribute the CSS
  below already keys off. The timeline recipe at the bottom is untouched by
  them: it keeps driving the same focus ring and typed reveal.

    - label (string, default "Project name"): the label above the field, also
      used as the accessible name of the shell. A blank or non-text override
      keeps the default.
    - value (string, default "Launch recap"): the typed value the progress
      variable reveals.
    - state (default | invalid | disabled, default default): how the field
      reads at rest. invalid swaps the shell border to the danger token,
      disabled dims the shell.
    - theme (dark | light, default dark): dark keeps the Operator Black
      surface, light switches the shared token block to its light values.

  On every default the result is identical to the original: a dark-theme field
  labelled Project name holding Launch recap.
-->

<label
  data-hf-ui-root
  class="hf-ui-input-field"
  data-composition-variables='[
    { "id": "label", "type": "string", "role": "content", "label": "Label", "description": "Field label, also used as the accessible name of the shell.", "default": "Project name" },
    { "id": "value", "type": "string", "role": "content", "label": "Value", "description": "Typed value revealed as the progress variable runs.", "default": "Launch recap" },
    { "id": "state", "type": "enum", "role": "style", "label": "State", "description": "Resting validation state: neutral, danger border, or dimmed.", "default": "default", "options": [{ "value": "default", "label": "Default" }, { "value": "invalid", "label": "Invalid" }, { "value": "disabled", "label": "Disabled" }] },
    { "id": "theme", "type": "enum", "role": "style", "label": "Theme", "description": "Operator Black dark surface or the light-frame token set.", "default": "dark", "options": [{ "value": "dark", "label": "Dark" }, { "value": "light", "label": "Light" }] }
  ]'
>
  <span class="hf-ui-input-label">Project name</span>
  <span
    class="hf-ui-input-shell"
    role="textbox"
    aria-label="Project name"
    aria-readonly="true"
    tabindex="0"
  >
    <span class="hf-ui-input-value">Launch recap</span>
    <span class="hf-ui-input-caret" aria-hidden="true"></span>
  </span>
  <span class="hf-ui-input-helper">Visible in the workspace and exported renders.</span>
</label>

<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  [data-hf-ui-root] {
    --_hf-ui-accent: var(--hf-ui-accent, #3ce6ac);
    --_hf-ui-action: var(--hf-ui-action, #eeeeee);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #111111);
    --_hf-ui-border: var(--hf-ui-border, #2a2a2a);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #383838);
    --_hf-ui-canvas: var(--hf-ui-canvas, #0a0a0a);
    --_hf-ui-control-border: var(--hf-ui-control-border, #686868);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #777777);
    --_hf-ui-control-height: var(--hf-ui-control-height, 40px);
    --_hf-ui-danger: var(--hf-ui-danger, #ff7878);
    --_hf-ui-data-1: var(--hf-ui-data-1, #3ce6ac);
    --_hf-ui-data-2: var(--hf-ui-data-2, #63a5ff);
    --_hf-ui-data-3: var(--hf-ui-data-3, #e9ab55);
    --_hf-ui-data-4: var(--hf-ui-data-4, #a78bfa);
    --_hf-ui-data-5: var(--hf-ui-data-5, #ff8a75);
    --_hf-ui-duration-drawer-enter: var(--hf-ui-duration-drawer-enter, 360ms);
    --_hf-ui-duration-drawer-exit: var(--hf-ui-duration-drawer-exit, 240ms);
    --_hf-ui-duration-modal-enter: var(--hf-ui-duration-modal-enter, 280ms);
    --_hf-ui-duration-modal-exit: var(--hf-ui-duration-modal-exit, 180ms);
    --_hf-ui-duration-overlay-enter: var(--hf-ui-duration-overlay-enter, 210ms);
    --_hf-ui-duration-overlay-exit: var(--hf-ui-duration-overlay-exit, 140ms);
    --_hf-ui-duration-press: var(--hf-ui-duration-press, 120ms);
    --_hf-ui-duration-state: var(--hf-ui-duration-state, 160ms);
    --_hf-ui-ease-enter: var(--hf-ui-ease-enter, cubic-bezier(0.32, 0.72, 0, 1));
    --_hf-ui-ease-exit: var(--hf-ui-ease-exit, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-ease-response: var(--hf-ui-ease-response, cubic-bezier(0.23, 1, 0.32, 1));
    --_hf-ui-font-mono: var(--hf-ui-font-mono, ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace);
    --_hf-ui-font-sans: var(--hf-ui-font-sans, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif);
    --_hf-ui-hit-target: var(--hf-ui-hit-target, 44px);
    --_hf-ui-radius-control: var(--hf-ui-radius-control, 5px);
    --_hf-ui-radius-overlay: var(--hf-ui-radius-overlay, 10px);
    --_hf-ui-radius-surface: var(--hf-ui-radius-surface, 7px);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 8px 18px rgba(0, 0, 0, 0.38));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.035));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(0, 0, 0, 0.52));
    --_hf-ui-space-1: var(--hf-ui-space-1, 4px);
    --_hf-ui-space-12: var(--hf-ui-space-12, 48px);
    --_hf-ui-space-16: var(--hf-ui-space-16, 64px);
    --_hf-ui-space-2: var(--hf-ui-space-2, 8px);
    --_hf-ui-space-3: var(--hf-ui-space-3, 12px);
    --_hf-ui-space-4: var(--hf-ui-space-4, 16px);
    --_hf-ui-space-6: var(--hf-ui-space-6, 24px);
    --_hf-ui-space-8: var(--hf-ui-space-8, 32px);
    --_hf-ui-surface: var(--hf-ui-surface, #141414);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #101010);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #1a1a1a);
    --_hf-ui-text: var(--hf-ui-text, #e5e5e5);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #7e7e7e);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #929292);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #f3f3f3);
    --_hf-ui-warning: var(--hf-ui-warning, #e9ab55);
    --_hf-ui-weight-medium: var(--hf-ui-weight-medium, 500);
    --_hf-ui-weight-regular: var(--hf-ui-weight-regular, 400);
    --_hf-ui-weight-semibold: var(--hf-ui-weight-semibold, 600);
    box-sizing: border-box;
    color-scheme: dark;
    font-family: var(--_hf-ui-font-sans);
  }
  /* prettier-ignore */
  [data-hf-ui-root] *,
  [data-hf-ui-root] *::before,
  [data-hf-ui-root] *::after {
    box-sizing: border-box;
  }
  /* prettier-ignore */
  [data-hf-theme="light"] [data-hf-ui-root],
  [data-hf-ui-root][data-hf-theme="light"] {
    --_hf-ui-accent: var(--hf-ui-accent, #087a57);
    --_hf-ui-action: var(--hf-ui-action, #171717);
    --_hf-ui-action-contrast: var(--hf-ui-action-contrast, #ffffff);
    --_hf-ui-border: var(--hf-ui-border, #e0dfdb);
    --_hf-ui-border-strong: var(--hf-ui-border-strong, #cbc9c3);
    --_hf-ui-canvas: var(--hf-ui-canvas, #f6f5f1);
    --_hf-ui-control-border: var(--hf-ui-control-border, #888888);
    --_hf-ui-control-border-hover: var(--hf-ui-control-border-hover, #6f6e68);
    --_hf-ui-danger: var(--hf-ui-danger, #a61b1b);
    --_hf-ui-data-1: var(--hf-ui-data-1, #087a57);
    --_hf-ui-data-2: var(--hf-ui-data-2, #245ea8);
    --_hf-ui-data-3: var(--hf-ui-data-3, #8a4b00);
    --_hf-ui-data-4: var(--hf-ui-data-4, #6d3fc0);
    --_hf-ui-data-5: var(--hf-ui-data-5, #a83d2d);
    --_hf-ui-shadow-contact: var(--hf-ui-shadow-contact, 0 7px 16px rgba(26, 24, 20, 0.13));
    --_hf-ui-shadow-inset: var(--hf-ui-shadow-inset, inset 0 1px 0 rgba(255, 255, 255, 0.78));
    --_hf-ui-shadow-modal: var(--hf-ui-shadow-modal, 0 18px 46px rgba(26, 24, 20, 0.18));
    --_hf-ui-surface: var(--hf-ui-surface, #ffffff);
    --_hf-ui-surface-inset: var(--hf-ui-surface-inset, #f0efeb);
    --_hf-ui-surface-raised: var(--hf-ui-surface-raised, #ffffff);
    --_hf-ui-text: var(--hf-ui-text, #171717);
    --_hf-ui-text-faint: var(--hf-ui-text-faint, #6f6e68);
    --_hf-ui-text-muted: var(--hf-ui-text-muted, #5f5f5a);
    --_hf-ui-text-strong: var(--hf-ui-text-strong, #0a0a0a);
    --_hf-ui-warning: var(--hf-ui-warning, #8a4b00);
    color-scheme: light;
  }
  /* prettier-ignore */
  [data-hf-ui-root]:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  /* prettier-ignore */
  [data-hf-rendering="true"][data-hf-ui-root],
  [data-hf-rendering="true"] [data-hf-ui-root],
  [data-hf-rendering="true"][data-hf-ui-root] *,
  [data-hf-rendering="true"] [data-hf-ui-root] *,
  [data-hf-rendering="true"][data-hf-ui-root]::before,
  [data-hf-rendering="true"][data-hf-ui-root]::after,
  [data-hf-rendering="true"] [data-hf-ui-root]::before,
  [data-hf-rendering="true"] [data-hf-ui-root]::after,
  [data-hf-rendering="true"][data-hf-ui-root] *::before,
  [data-hf-rendering="true"][data-hf-ui-root] *::after,
  [data-hf-rendering="true"] [data-hf-ui-root] *::before,
  [data-hf-rendering="true"] [data-hf-ui-root] *::after {
    animation-duration: 0s !important;
    animation-delay: 0s !important;
    transition-duration: 0s !important;
    transition-delay: 0s !important;
  }
  /* prettier-ignore */
  @media (prefers-reduced-motion: reduce) {
    [data-hf-ui-root],
    [data-hf-ui-root] *,
    [data-hf-ui-root] *::before,
    [data-hf-ui-root] *::after {
      animation-duration: 0s !important;
      animation-delay: 0s !important;
      transition-duration: 0s !important;
      transition-delay: 0s !important;
      scroll-behavior: auto !important;
    }
  }
  /* hf-ui:tokens:end */
  .hf-ui-input-field {
    --hf-input-focus: 0;
    --hf-input-value-progress: 1;
    display: grid;
    gap: 8px;
    width: min(360px, 100%);
    color: var(--_hf-ui-text);
    font-family: var(--_hf-ui-font-sans);
  }
  .hf-ui-input-label {
    color: var(--_hf-ui-text-strong);
    font-size: 13px;
    font-weight: var(--_hf-ui-weight-semibold);
    line-height: 1;
  }
  .hf-ui-input-shell {
    position: relative;
    display: flex;
    align-items: center;
    height: var(--_hf-ui-control-height);
    padding: 0 var(--_hf-ui-space-3);
    border: 1px solid
      color-mix(
        in srgb,
        var(--_hf-ui-accent) calc(var(--hf-input-focus) * 100%),
        var(--_hf-ui-control-border)
      );
    border-radius: var(--_hf-ui-radius-control);
    background: var(--_hf-ui-surface);
    box-shadow:
      0 0 0 calc(var(--hf-input-focus) * 2px)
        color-mix(in srgb, var(--_hf-ui-accent) 42%, transparent),
      var(--_hf-ui-shadow-inset);
    overflow: hidden;
    transition:
      border-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response),
      box-shadow var(--_hf-ui-duration-state) var(--_hf-ui-ease-response);
  }
  .hf-ui-input-shell:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-input-shell[aria-invalid="true"] {
    border-color: var(--_hf-ui-danger);
  }
  .hf-ui-input-shell[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
  }
  .hf-ui-input-value {
    display: inline-block;
    max-width: calc(var(--hf-input-value-progress) * 100%);
    overflow: hidden;
    white-space: nowrap;
    color: var(--_hf-ui-text);
    font-size: 14px;
    font-weight: var(--_hf-ui-weight-regular);
    line-height: 1;
  }
  .hf-ui-input-caret {
    width: 1px;
    height: 16px;
    margin-inline-start: 2px;
    background: var(--_hf-ui-accent);
    opacity: var(--hf-input-focus);
  }
  .hf-ui-input-helper {
    color: var(--_hf-ui-text-muted);
    font-size: 12px;
    line-height: 1.35;
  }
  @media (pointer: coarse) {
    .hf-ui-input-shell::before {
      position: absolute;
      inset-block: calc((var(--_hf-ui-control-height) - var(--_hf-ui-hit-target)) / 2);
      inset-inline: 0;
      content: "";
    }
  }
</style>

<script>
  (function () {
    "use strict";

    var vars =
      window.__hyperframes && window.__hyperframes.getVariables
        ? window.__hyperframes.getVariables()
        : {};

    // Unrecognised overrides return to their declared defaults.
    function pick(table, value, fallback) {
      return Object.prototype.hasOwnProperty.call(table, value) ? value : fallback;
    }

    function text(value, fallback) {
      return typeof value === "string" && value.trim() !== "" ? value : fallback;
    }

    var states = { default: 1, invalid: 1, disabled: 1 };

    var labelText = text(vars.label, "Project name");
    var valueText = text(vars.value, "Launch recap");
    var state = pick(states, vars.state, "default");
    var theme = vars.theme === "light" ? "light" : "dark";

    var roots = document.querySelectorAll(".hf-ui-input-field");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.setAttribute("data-hf-theme", theme);
      var labelEl = root.querySelector(".hf-ui-input-label");
      if (labelEl) labelEl.textContent = labelText;
      var valueEl = root.querySelector(".hf-ui-input-value");
      if (valueEl) valueEl.textContent = valueText;
      var shell = root.querySelector(".hf-ui-input-shell");
      if (!shell) continue;
      shell.setAttribute("aria-label", labelText);
      if (state === "invalid") shell.setAttribute("aria-invalid", "true");
      else shell.removeAttribute("aria-invalid");
      if (state === "disabled") shell.setAttribute("aria-disabled", "true");
      else shell.removeAttribute("aria-disabled");
    }
  })();
</script>

<!--
  Timeline integration:

  tl.set(".hf-ui-input-field", { "--hf-input-value-progress": 0 });
  tl.to(".hf-ui-input-field", { "--hf-input-focus": 1, duration: 0.16, ease: "power3.out" }, startTime);
  tl.to(".hf-ui-input-field", { "--hf-input-value-progress": 1, duration: 0.8, ease: "steps(12)" }, startTime + 0.2);
-->
```

</Accordion>

## Usage

Open `compositions/components/input.html` and paste its contents into your composition. See the comment header in the file for detailed instructions.

{/* hf:generated-footer */}

Tagged `ui-primitive` `remocn-port` `input` `field`.

## Related topics

- [Browse the complete Catalog](/catalog)
- [Add assets and Catalog items in Studio](/studio/assets-and-blocks)
- [Build a richer composition](/go-further)
