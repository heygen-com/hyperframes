...
<Accordion title={`toggle-group.html`}>

```html
<!-- Toggle Group - animate --hf-toggle-index between segment indices. -->
<div
  data-hf-ui-root
  class="hf-ui-toggle-group"
  style="--hf-toggle-index: 0"
  role="group"
  aria-label="Output mode"
>
  <span class="hf-ui-toggle-indicator" aria-hidden="true"></span>
  <button type="button" aria-pressed="true">Draft</button
  ><button type="button" aria-pressed="false">Preview</button
  ><button type="button" aria-pressed="false">Export</button>
</div>
<style>
  /* hf-ui:tokens:start */
  /* prettier-ignore */
  ...
  .hf-ui-toggle-group button[aria-pressed="true"] {
    color: var(--_hf-ui-text-strong);
  }
  .hf-ui-toggle-group button:focus-visible {
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-toggle-group button:active {
    transform: translateY(1px) scale(0.98);
  }
  .hf-ui-toggle-group button:disabled,
  .hf-ui-toggle-group button[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
    transform: none;
  }
  @media (hover: hover) and (pointer: fine) {
    .hf-ui-toggle-group button:hover {
      color: var(--_hf-ui-text-strong);
    }
  }
  @media (pointer: coarse) {
    ...
    .hf-ui-toggle-group button::before {
      position: absolute;
