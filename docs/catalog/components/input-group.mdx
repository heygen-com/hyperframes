...
| --- | --- | --- | --- |
| `prefix` | `https://` | string | Static technical prefix shown before the value. |
| `value` | `hyperframes.dev` | string | Text held by the field itself. |
| `action` | `Copy` | string | Label on the attached trailing button. |
...
```html wrap
<div
  data-composition-id="input-group"
  data-composition-src="compositions/components/input-group.html"
  data-variable-values='{"prefix":"https://","value":"hyperframes.dev","action":"Copy","theme":"dark"}'
></div>
```

## Source

<Accordion title={`input-group.html`}>

```html
<!--
  Input Group - Operator Black compound field primitive for HyperFrames.
  ...
  anything missing or unrecognised, and writes it into the markup the CSS
  below already styles. The timeline recipe at the bottom is untouched by
  them: it keeps revealing the same row.

    - prefix (string, default "https://"): the static segment in the leading
      slot. A blank or non-text override keeps the default.
    - value (string, default "hyperframes.dev"): the text held by the field.
    - action (string, default "Copy"): the label on the attached button.
  ...
  On every default the result is identical to the original: a dark-theme row
  reading https:// hyperframes.dev with a Copy button.
-->

<div
  data-hf-ui-root
  class="hf-ui-input-group"
  data-composition-variables='[
    { "id": "prefix", "type": "string", "role": "content", "label": "Prefix", "description": "Static technical prefix shown before the value.", "default": "https://" },
    { "id": "value", "type": "string", "role": "content", "label": "Value", "description": "Text held by the field itself.", "default": "hyperframes.dev" },
    { "id": "action", "type": "string", "role": "content", "label": "Action", "description": "Label on the attached trailing button.", "default": "Copy" },
...
>
  <span>https://</span><input aria-label="Project URL" value="hyperframes.dev" readonly /><button
    type="button"
  >
    Copy
  </button>
...
<style>
  ...
  .hf-ui-input-group {
    ...
    font-family: var(--_hf-ui-font-sans);
    transition: border-color var(--_hf-ui-duration-state) var(--_hf-ui-ease-response);
  }
  .hf-ui-input-group span {
    padding: 0 var(--_hf-ui-space-3);
    color: var(--_hf-ui-text-muted);
    font: var(--_hf-ui-weight-medium) 12px/1 var(--_hf-ui-font-mono);
  }
  .hf-ui-input-group input {
    min-width: 0;
    height: 100%;
    border: 0;
    outline: 0;
    ...
    font: var(--_hf-ui-weight-regular) 13px/1 var(--_hf-ui-font-mono);
  }
  .hf-ui-input-group button {
    align-self: stretch;
    border: 0;
    border-inline-start: 1px solid var(--_hf-ui-border);
    background: var(--_hf-ui-surface-inset);
    padding: 0 var(--_hf-ui-space-3);
    color: var(--_hf-ui-text-strong);
    font: var(--_hf-ui-weight-semibold) 12px/1 var(--_hf-ui-font-sans);
    cursor: pointer;
    transition:
  ...
  .hf-ui-input-group:focus-within {
    border-color: var(--_hf-ui-accent);
    outline: 2px solid var(--_hf-ui-accent);
    outline-offset: 3px;
  }
  .hf-ui-input-group button:active {
    transform: translateY(1px) scale(0.98);
  }
  .hf-ui-input-group[aria-invalid="true"] {
    border-color: var(--_hf-ui-danger);
  }
  .hf-ui-input-group[aria-disabled="true"],
  .hf-ui-input-group:has(input:disabled) {
    cursor: not-allowed;
    opacity: 0.48;
  }
  .hf-ui-input-group input:disabled,
  .hf-ui-input-group button:disabled,
  .hf-ui-input-group button[aria-disabled="true"] {
    cursor: not-allowed;
  }
  @media (pointer: coarse) {
    ...
    .hf-ui-input-group input {
      ...
      margin-block: calc((var(--_hf-ui-control-height) - var(--_hf-ui-hit-target)) / 2);
    }
    .hf-ui-input-group button {
      position: relative;
    }
    .hf-ui-input-group button::before {
      position: absolute;
...
<script>
  (function () {
    ...
    var prefix = text(vars.prefix, "https://");
    var value = text(vars.value, "hyperframes.dev");
    var action = text(vars.action, "Copy");
    var theme = vars.theme === "light" ? "light" : "dark";

    var roots = document.querySelectorAll(".hf-ui-input-group");
    for (var i = 0; i < roots.length; i += 1) {
      var root = roots[i];
      root.setAttribute("data-hf-theme", theme);
      var prefixEl = root.querySelector("span");
      ...
      if (button) button.textContent = action;
    }
  })();
</script>
...
<!--
  ...
  tl.fromTo('.hf-ui-input-group', { opacity: 0, y: 4 }, { opacity: 1, y: 0, duration: 0.21, ease: 'power3.out' }, startTime);
-->
```
