...
| --- | --- | --- | --- |
| `accent` | `mint` | `mint`, `blue`, `violet` | Token family that fills the box and its border once the check lands. |
| `size` | `standard` | `compact`, `standard`, `large` | Box edge length; the tick and the label text scale with it. |
| `label` | `Include captions` | string | Text shown beside the box. |
...
```html wrap
<div
  data-composition-id="checkbox"
  data-composition-src="compositions/components/checkbox.html"
  data-variable-values='{"accent":"mint","size":"standard","label":"Include captions"}'
></div>
```

## Source

<Accordion title={`checkbox.html`}>

```html
<!--
  Checkbox - Operator Black binary-choice primitive for HyperFrames.

  Toggle data-state between unchecked and checked, or animate
  --hf-checkbox-progress from a paused GSAP timeline.
  ...
  the CSS consumes. The check progress stays entirely yours to drive.

    - accent (mint | blue | violet, default mint): the token family the box
      fills with once the check lands. mint rides the shared accent token,
      blue and violet ride data tokens 2 and 4, so all three follow the theme.
    - size (compact | standard | large, default standard): the box edge, 16px
      / 20px / 28px. The tick and the label text scale with it.
    - label (any text, default "Include captions"): the text beside the box.

  On every default the result is identical to the original: a 20px mint box
  with a 15px "Include captions" label.
-->

<div
  data-hf-ui-root
  class="hf-ui-checkbox"
  data-state="unchecked"
  role="checkbox"
  aria-checked="false"
  tabindex="0"
  data-composition-variables='[
    { "id": "accent", "type": "enum", "role": "style", "label": "Accent", "description": "Token family that fills the box and its border once the check lands.", "default": "mint", "options": [{ "value": "mint", "label": "Mint" }, { "value": "blue", "label": "Blue" }, { "value": "violet", "label": "Violet" }] },
    { "id": "size", "type": "enum", "role": "style", "label": "Size", "description": "Box edge length; the tick and the label text scale with it.", "default": "standard", "options": [{ "value": "compact", "label": "Compact" }, { "value": "standard", "label": "Standard" }, { "value": "large", "label": "Large" }] },
    { "id": "label", "type": "string", "role": "content", "label": "Label", "description": "Text shown beside the box.", "default": "Include captions", "maxLength": 60 }
  ]'
>
  <span class="hf-ui-checkbox-box" aria-hidden="true">
    <svg viewBox="0 0 24 24">
      <path d="M5 12.5l4.5 4.5L19 7" />
    </svg>
  </span>
  <span class="hf-ui-checkbox-label">Include captions</span>
</div>
...
<style>
  ...
  /* hf-ui:tokens:end */
  .hf-ui-checkbox {
    --hf-checkbox-progress: 0;
    display: inline-flex;
    align-items: center;
    gap: calc(var(--hf-checkbox-size, 20px) * 0.6);
    color: var(--_hf-ui-text);
    font-family: var(--_hf-ui-font-sans);
    cursor: pointer;
    transition: transform var(--_hf-ui-duration-press) var(--_hf-ui-ease-response);
  }
  .hf-ui-checkbox[data-state="checked"] {
    --hf-checkbox-progress: 1;
  }
  .hf-ui-checkbox-box {
    display: grid;
    place-items: center;
    width: var(--hf-checkbox-size, 20px);
    height: var(--hf-checkbox-size, 20px);
    border: 1.5px solid
      color-mix(
        in srgb,
        var(--hf-checkbox-accent, var(--_hf-ui-accent)) calc(var(--hf-checkbox-progress) * 100%),
    ...
    transform: scale(calc(0.98 + var(--hf-checkbox-progress) * 0.02));
    box-shadow: var(--_hf-ui-shadow-inset);
  }
  .hf-ui-checkbox svg {
    width: calc(var(--hf-checkbox-size, 20px) * 0.75);
    height: calc(var(--hf-checkbox-size, 20px) * 0.75);
    fill: none;
    stroke: var(--_hf-ui-action-contrast);
    stroke-width: 1.5;
    stroke-linecap: square;
    stroke-linejoin: miter;
    opacity: var(--hf-checkbox-progress);
    stroke-dasharray: 24;
    stroke-dashoffset: calc((1 - var(--hf-checkbox-progress)) * 24);
  }
  .hf-ui-checkbox-label {
    font-size: calc(var(--hf-checkbox-size, 20px) * 0.75);
    font-weight: var(--_hf-ui-weight-medium);
    line-height: 1;
  }
  .hf-ui-checkbox:active {
    transform: translateY(1px) scale(0.98);
  }
  .hf-ui-checkbox[aria-disabled="true"] {
    cursor: not-allowed;
    opacity: 0.48;
    transform: none;
  }
  .hf-ui-checkbox[aria-invalid="true"] .hf-ui-checkbox-box {
...
<script>
  (function () {
    ...
    var sizes = { compact: "16px", standard: "20px", large: "28px" };
    ...
    var roots = document.querySelectorAll(".hf-ui-checkbox");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--hf-checkbox-accent", accent);
      roots[i].style.setProperty("--hf-checkbox-size", size);
      var text = roots[i].querySelector(".hf-ui-checkbox-label");
      if (text) {
...
<!--
  ...
  tl.to(".hf-ui-checkbox", { "--hf-checkbox-progress": 1, duration: 0.16, ease: "power3.out" }, startTime);
-->
```
