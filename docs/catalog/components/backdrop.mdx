...
| --- | --- | --- | --- |
| `dim` | `standard` | `sheer`, `standard`, `blackout` | How far the backdrop darkens the scene underneath once its fade lands. |
| `tint` | `black` | `black`, `ink`, `plum` | Colour the dimming layer is mixed from: neutral black, a cool ink, or a warm plum. |
| `frost` | `none` | `none`, `soft`, `heavy` | Optional blur applied to whatever the backdrop covers, for a frosted glass sheet. |
...
```html wrap
<div
  data-composition-id="backdrop"
  data-composition-src="compositions/components/backdrop.html"
  data-variable-values='{"dim":"standard","tint":"black","frost":"none"}'
></div>
```

## Source

<Accordion title={`backdrop.html`}>

```html
<!--
  Backdrop - Operator Black flat dimming layer specimen.
  ...
  them: it keeps fading one opacity in, and these decide what that opacity
  looks like when it lands.

    - dim (sheer | standard | blackout, default standard): scales the settled
      darkness. sheer leaves the scene readable underneath, blackout takes it
      almost to solid.
    - tint (black | ink | plum, default black): the colour the dimming layer
      is mixed from, for a neutral, a cool, or a warm scrim.
    - frost (none | soft | heavy, default none): an optional blur of whatever
      the backdrop covers, turning the scrim into frosted glass.

  On every default the result is identical to the original: a flat black scrim
  fading to 62% with no blur.
-->

<div
  data-hf-ui-root
  class="hf-remocn-backdrop"
  data-composition-variables='[
    { "id": "dim", "type": "enum", "role": "style", "label": "Dim", "description": "How far the backdrop darkens the scene underneath once its fade lands.", "default": "standard", "options": [{ "value": "sheer", "label": "Sheer" }, { "value": "standard", "label": "Standard" }, { "value": "blackout", "label": "Blackout" }] },
    { "id": "tint", "type": "enum", "role": "style", "label": "Tint", "description": "Colour the dimming layer is mixed from: neutral black, a cool ink, or a warm plum.", "default": "black", "options": [{ "value": "black", "label": "Black" }, { "value": "ink", "label": "Ink" }, { "value": "plum", "label": "Plum" }] },
    { "id": "frost", "type": "enum", "role": "style", "label": "Frost", "description": "Optional blur applied to whatever the backdrop covers, for a frosted glass sheet.", "default": "none", "options": [{ "value": "none", "label": "None" }, { "value": "soft", "label": "Soft" }, { "value": "heavy", "label": "Heavy" }] }
  ]'
>
  <div class="row"><span class="label">Timeline</span><span class="hint">00:04.00</span></div>
  <div class="row"><span class="label">Layers</span><span class="hint">12 visible</span></div>
  <div class="row"><span class="label">Export</span><span class="hint">1080p</span></div>
</div>
...
<style>
  ...
  .hf-remocn-backdrop::after {
    content: "";
    position: absolute;
    inset: 0;
    background: rgb(
      var(--hf-backdrop-tint, 0 0 0) / calc(var(--hf-backdrop-open) * var(--hf-backdrop-dim, 1))
    );
    backdrop-filter: var(--hf-backdrop-frost, none);
    pointer-events: none;
  }
  .hf-remocn-backdrop .row {
  ...
  .hf-remocn-backdrop .label {
    font-size: 13px;
    font-weight: var(--_hf-ui-weight-medium);
  }
  .hf-remocn-backdrop .hint {
    color: var(--hf-muted);
...
<script>
  (function () {
    ...
    var dims = { sheer: 0.55, standard: 1, blackout: 1.45 };
    var tints = { black: "0 0 0", ink: "8 12 26", plum: "28 10 26" };
    var frosts = { none: "none", soft: "blur(4px)", heavy: "blur(12px)" };

    var dim = dims[pick(dims, vars.dim, "standard")];
    var tint = tints[pick(tints, vars.tint, "black")];
    var frost = frosts[pick(frosts, vars.frost, "none")];

    var roots = document.querySelectorAll(".hf-remocn-backdrop");
    for (var i = 0; i < roots.length; i += 1) {
      roots[i].style.setProperty("--hf-backdrop-dim", String(dim));
      roots[i].style.setProperty("--hf-backdrop-tint", tint);
      roots[i].style.setProperty("--hf-backdrop-frost", frost);
    }
  })();
</script>
...
<!--
  ...
  tl.set('.hf-remocn-backdrop .row', { '--hf-row-x': '0px', '--hf-row-opacity': 1 }, startTime);
  tl.fromTo('.hf-remocn-backdrop', { '--hf-backdrop-open': 0 }, { '--hf-backdrop-open': 0.62, duration: 0.16, ease: 'power2.out' }, startTime);
-->
```
