...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-flip-5.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-flip-5.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-flip-5" item="loop-flip-5" />

That writes one file: `compositions/loop-flip-5.html`.
...
```html index.html
<div
  data-composition-id="loop-flip-5"
  data-composition-src="compositions/loop-flip-5.html"
  data-start="0"
  data-duration="16"
  data-track-index="1"
  data-width="1080"
...
| `borderRadius` | `38` | 0px to 200px, step 1px | Card corner radius in px at 1080. |
...
| `planeSize` | `520` | 0 to 1340, step 10 | Card size in scene units. |
| `gap` | `0` | 0px to 48px, step 1px | Spacing between cards. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-flip-5-bg"></div>
        <div id="loop-flip-5-stage"><div id="loop-flip-5-col"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-flip-5-loop").dataset.duration = DUR;
        background(document.getElementById("loop-flip-5-bg"));
        document.getElementById("loop-flip-5-stage").style.perspective = f + "px";
        const col = document.getElementById("loop-flip-5-col");
        ...
        window.__timelines["loop-flip-5"] = tl;
      })();
    </script>
  </body>
</html>
