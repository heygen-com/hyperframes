---
title: "Loop Iso Focus"
description: "A queue of isometric-skewed image cards on a rising line, the focused block brought forward. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-isofocus.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-isofocus.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-isofocus" item="loop-isofocus" />

That writes one file: `compositions/loop-isofocus.html`.
...
```html index.html
<div
  data-composition-id="loop-isofocus"
  data-composition-src="compositions/loop-isofocus.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `skew` | `20` | 0 to 40, step 1 | Skew (engine parameter; see the block header comment). |
| `spacing` | `20` | 0 to 40, step 1 | Spacing (engine parameter; see the block header comment). |
| `focusGap` | `85` | 0 to 170, step 1 | Focus Gap (engine parameter; see the block header comment). |
| `centerScale` | `100` | 0 to 200, step 1 | Center Scale (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-isofocus-bg"></div>
        <div id="loop-isofocus-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Blocks loop family — a queue of skewed cards on a 35deg up-right line through the
        // frame centre; the centre slot is isolated by a focus gap on both sides, the rest bunch up into stacks.
        // Measured on the 512 preview (2026-09-08):
        //   card: square side = scaleIntensity*H (0.42 -> 213px), skewY with edges falling right at slope skew/100
        //         (0.19-0.21 fitted on 2 cards), corners 32@1080; centre card exactly centred.
        //   queue: slot pitch dx = spacing*0.35%*H (20 -> 36px, tops 25px apart -> slope -0.70), focus gap adds
        //          focusGap*0.35%*H (85 -> 152; measured centre->next = 186 = 36+150) on each side of the centre;
        //          the line slope is -0.70 (independent of skew). Down-left card is drawn ON TOP.
        //   step: one slot per DUR/N (45 frames); motion occupies frames 7..32 of 45 -> window [0.17, 0.71] of the step,
        //         symmetric energy peak at 0.44; (.65,0,.35,1) over [0.15,0.75] was too broad (round 1), (.7,0,.3,1) fits the peak. direction down = queue advances down-left.
        //   centerScale 100 -> centre card scale 1.0 (unverified: only default seen).
        // Knobs: centerScale unit (% -> scale), shadow (off by default), z-order when direction=up (mirrored).
        ...
        const SKEW = (Math.atan(num("skew", 20) / 100) * 180) / Math.PI;
        const PITCH = num("spacing", 20) * 0.0035 * REF;
        const FGAP = num("focusGap", 85) * 0.0035 * REF;
        const CSCALE = num("centerScale", 100) / 100;
        const DOWN = str("direction", "down") === "down";
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const LINE = -0.7; // screen slope of the queue line (measured)
        ...
        document.getElementById("loop-isofocus-loop").dataset.duration = DUR;
        background(document.getElementById("loop-isofocus-bg"));
        const stage = document.getElementById("loop-isofocus-stage");
        ...
        // slot j (integer, 0 = focused) -> x offset along the line; y = LINE * x
        const slotX = (j) => j * PITCH + Math.sign(j) * FGAP;
        // slots that can ever be on screen: |x| up to half the diagonal plus the gap
        const JMAX = Math.ceil((Math.hypot(W, H) / 2 + FGAP) / PITCH) + 1;
        const cards = [];
        for (let j = -JMAX - 1; j <= JMAX + 1; j++) {
          const el = document.createElement("div");
          el.style.cssText = `position:absolute;left:0;top:0;width:0;height:0;`;
          el.style.zIndex = JMAX + 2 - j; // down-left (negative j) on top
          stage.appendChild(el);
          cards.push({ j, el, img: null, m: null });
        }
        function setImage(slot, m) {
          if (slot.m === m) return;
          slot.m = m;
          slot.el.innerHTML = "";
        ...
        function renderAt(t) {
          const s = t * N,
            k = Math.floor(s),
            u = Math.min(1, Math.max(0, (s - k - 0.17) / 0.54));
          const e = ease(u);
          for (const slot of cards) {
            // the card in slot j during step k is image (j + k) (down: queue advances toward negative j)
            const idx = DOWN ? slot.j + k : slot.j - k;
            setImage(slot, IMAGES[((idx % N) + N) % N]);
            // it travels from slot j to slot j-1 (down) / j+1 (up)
            const j0 = slot.j,
              j1 = DOWN ? slot.j - 1 : slot.j + 1;
            const x = slotX(j0) + (slotX(j1) - slotX(j0)) * e;
            const y = LINE * x;
            // scale: centre slot is CSCALE, others 1 (blend during travel)
            const sc0 = j0 === 0 ? CSCALE : 1,
              sc1 = j1 === 0 ? CSCALE : 1;
            slot.el.style.transform = `translate3d(${x}px,${y}px,0) scale(${sc0 + (sc1 - sc0) * e})`;
          }
        }
        ...
        window.__timelines["loop-isofocus"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `isofocus`.
