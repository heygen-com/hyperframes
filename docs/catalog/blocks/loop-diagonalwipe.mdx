---
title: "Loop Diagonal Wipe"
description: "One rounded image card; each new image wipes in along a diagonal. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-diagonalwipe.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-diagonalwipe.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-diagonalwipe" item="loop-diagonalwipe" />

That writes one file: `compositions/loop-diagonalwipe.html`.
...
```html index.html
<div
  data-composition-id="loop-diagonalwipe"
  data-composition-src="compositions/loop-diagonalwipe.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `angle` | `-20` | -180° to 180°, step 1° | Angle (engine parameter; see the block header comment). |
| `edgeGlow` | `60` | 0 to 120, step 1 | Edge Glow (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-diagonalwipe-bg {
        position: absolute;
        inset: 0;
      }
      #loop-diagonalwipe-card {
        position: absolute;
        overflow: hidden;
      }
      .layer {
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-diagonalwipe-bg"></div>
        <div id="loop-diagonalwipe-card"><div id="loop-diagonalwipe-glow"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // DiagonalWipe loop family — one rounded card (frame inset by `padding`),
        // images hold, then the next image is revealed by a tilted straight edge sweeping right -> left.
        // Measured on the 512 preview (2026-09-08): card x 31..480 = frame inset 6% of H (padding 6);
        //   period = DUR/N = 30f; the sweep is FAST: centre-row edge at 0.95 of the card width on f24, 0.33 on f25,
        //   complete on f26, identical in every period (f54/55, f84/85). Edge tilt 20.9 deg from vertical, top of
        //   the edge to the left (angle -20). With cubic-bezier(.5,0,.5,1) the sweep is 4.4 frames (0.147 of the
        //   period) starting at 0.751 of the period; the edge travels from +tan(20)/2 beyond the right side to the
        //   same beyond the left (1.36 card widths). edgeGlow 60: a faint bright line at the edge (+6 luma).
        // Knobs (guesses): sweep start/duration expressed as fractions of the period (only DUR 8 seen);
        //   easing names other than smooth/custom/linear mapped to GSAP eases; glow line = 0.4% of H wide,
        //   white at edgeGlow/100 * 0.25 opacity.
        ...
        const ANGLE = num("angle", -20);
        const GLOW = num("edgeGlow", 60) / 100;
        ...
        const SWEEP_START = 0.751,
          SWEEP_LEN = 0.147; // fractions of the per-image period

        document.getElementById("loop-diagonalwipe-loop").dataset.duration = DUR;
        background(document.getElementById("loop-diagonalwipe-bg"));
        const cardEl = document.getElementById("loop-diagonalwipe-card");
        const CW = W - 2 * PAD,
          CH = H - 2 * PAD;
        cardEl.style.cssText += `left:${PAD}px;top:${PAD}px;width:${CW}px;height:${CH}px;border-radius:${RADIUS}px;`;

        // cubic bezier (CSS semantics) + named eases
        ...
        const layers = IMAGES.map((m) => {
          ...
          c.style.left = (CW - w) / 2 + "px";
          c.style.top = (CH - h) / 2 + "px";
          L.appendChild(c);
          cardEl.insertBefore(L, document.getElementById("loop-diagonalwipe-glow"));
          return L;
        });
        const glow = document.getElementById("loop-diagonalwipe-glow");
        const TAN = Math.tan((Math.abs(ANGLE) * Math.PI) / 180) * (ANGLE < 0 ? 1 : -1); // dx/dy of the edge (top-left lean for negative angles)
        const HALF = (Math.abs(TAN) * CH) / 2; // horizontal half-extent of the tilted edge
        glow.style.cssText += `width:${0.004 * REF}px;height:${CH * 2}px;background:#fff;opacity:${0.25 * GLOW};box-shadow:0 0 ${0.01 * REF}px #fff;`;
        ...
        function renderAt(t) {
          ...
          shown = [outI, inI];
          const q = EASE((u - SWEEP_START) / SWEEP_LEN);
          layers[outI].style.display = "block";
          layers[outI].style.zIndex = "1";
          layers[outI].style.clipPath = "";
          const inn = layers[inI];
          inn.style.zIndex = "2";
          if (q <= 0) {
          ...
          // edge x at the card's vertical centre travels from CW+HALF to -HALF; the incoming image is to its right
          const xc = CW + HALF - q * (CW + 2 * HALF);
          const xTop = xc - (TAN * CH) / 2,
            xBot = xc + (TAN * CH) / 2;
          inn.style.clipPath = `polygon(${xTop}px 0px, ${CW + HALF + 10}px 0px, ${CW + HALF + 10}px ${CH}px, ${xBot}px ${CH}px)`;
          if (GLOW > 0 && q < 1) {
            glow.style.display = "block";
            glow.style.transform = `translate(${xc}px, ${-CH / 2}px) rotate(${ANGLE}deg)`;
            glow.style.transformOrigin = "0 50%";
        ...
        window.__timelines["loop-diagonalwipe"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `diagonalwipe`.
