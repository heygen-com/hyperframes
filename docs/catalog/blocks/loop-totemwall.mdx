---
title: "Loop Totem Wall"
description: "A concave drum of image cards, horizontal axis, rolls past the camera. 8 image slots, 20s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-totemwall.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-totemwall.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-totemwall" item="loop-totemwall" />

That writes one file: `compositions/loop-totemwall.html`.
...
```html index.html
<div
  data-composition-id="loop-totemwall"
  data-composition-src="compositions/loop-totemwall.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `columns` | `3` | 0 to 10, step 0.1 | Columns (engine parameter; see the block header comment). |
...
| `gap` | `4.5` | 0px to 9px, step 0.1px | Spacing between cards. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-totemwall-bg"></div>
        <div id="loop-totemwall-stage"><div id="loop-totemwall-wall"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // TotemWall loop family (Drum, LoopTotemWallTemplate) — a concave DRUM (cylinder, horizontal axis) of
        // 16:9 cards around the viewer, columns rolling vertically. Measured on the 512 preview (2026-09-08):
        //   centre card 112x64, pitch 134 x 86 (cell_x = 0.74*512/3 = 126.3 world => magnification 1.064),
        //   top/bottom rows 7% wider and nearer (x-pitch 143 at y~12), side columns same size as the centre
        //   => bent in y only (z = R - sqrt(R^2 - y^2)). Fit (rms 0.46px): f=1.712H, drum axis 0.319H in front
        //   of the camera, R=1.291H. gap 4.5 -> 22px => 0.94% H per unit. One cell per 75 frames => N cells
        //   per loop, direction=up: cards move up (1.15px/frame at centre). images: index = (row - 3*column) mod N.
        // Shared notes (from spherecascade.html; SphereCascade's own numbers are quoted there):
        ...
        const LANE_OFF = 3; // image index shift per column (measured)

        document.getElementById("loop-totemwall-loop").dataset.duration = DUR;
        background(document.getElementById("loop-totemwall-bg"));

        const f = 1.712 * REF,
          R = (1.291 * REF * 100) / CURV,
          MAG = 1.064;
        const DEPTH = f / MAG; // camera -> wall centre (= a + R at curvature -100)
        const stage = document.getElementById("loop-totemwall-stage");
        stage.style.perspective = f + "px";
        stage.style.transform = `rotate(${TILT}deg)`;
        const wall = document.getElementById("loop-totemwall-wall");
        ...
        function place(c, y) {
          ...
          c.el.style.display = "";
          const z = R - Math.sqrt(R * R - y * y); // drum: top/bottom come toward the viewer
          const beta = Math.asin(y / R);
          c.el.style.transform = `translate3d(${c.x}px,${y}px,${z}px) rotateX(${beta * RAD}deg)`;
        }
        function renderAt(t) {
        ...
        window.__timelines["loop-totemwall"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `totemwall` `3d`.
