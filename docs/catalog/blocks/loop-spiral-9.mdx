---
title: "Loop Spiral 9"
description: "Image cards evenly spaced along a helix that advances toward the camera. 8 image slots, 24s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-9.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-9.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spiral-9" item="loop-spiral-9" />

That writes one file: `compositions/loop-spiral-9.html`.
...
```html index.html
<div
  data-composition-id="loop-spiral-9"
  data-composition-src="compositions/loop-spiral-9.html"
  data-start="0"
  data-duration="24"
  data-track-index="1"
  data-width="1080"
...
| `flipImage` | `on` | `off`, `on` | Flip Image (engine parameter; see the block header comment). |
| `fade` | `0` | 0 to 100, step 1 | Fade (engine parameter; see the block header comment). |
| `autoFaceCamera` | `off` | `on`, `off` | Auto Face Camera (engine parameter; see the block header comment). |
| `cycles` | `12` | 0 to 24, step 1 | Number of motion cycles per loop. |
| `cycleDeg` | `60` | -180° to 180°, step 1° | Degrees turned per cycle. |
| `delay` | `0` | 0s to 2s, step 0.01s | Hold time between steps, seconds. |
| `direction` | `reverse` | `forward`, `reverse` | Direction of travel. |
| `count` | `53` | 0 to 240, step 10 | Count (engine parameter; see the block header comment). |
| `radius` | `3000` | 0 to 6000, step 10 | Radius (engine parameter; see the block header comment). |
| `turns` | `10` | 0 to 20, step 0.1 | Turns (engine parameter; see the block header comment). |
| `height` | `5000` | 0 to 10000, step 10 | Height (engine parameter; see the block header comment). |
| `wobble` | `0` | 0 to 100, step 1 | Wobble (engine parameter; see the block header comment). |
| `planeSize` | `810` | 0 to 1620, step 10 | Card size in scene units. |
| `distance` | `4650` | 0 to 9300, step 10 | Camera distance from the formation. |
...
| `cameraView` | `down` | `side`, `down` | Camera View (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spiral-9-bg"></div>
        <div id="loop-spiral-9-stage"><div id="loop-spiral-9-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-spiral-9-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spiral-9-bg"));
        ...
        const stage = document.getElementById("loop-spiral-9-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-spiral-9-world");
        ...
        window.__timelines["loop-spiral-9"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `spiral` `3d`.
