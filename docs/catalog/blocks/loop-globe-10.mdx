---
title: "Loop Globe 10"
description: "Image cards scattered evenly over a sphere that rotates under a 3D camera. 8 image slots, 9s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-10.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-10.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-globe-10" item="loop-globe-10" />

That writes one file: `compositions/loop-globe-10.html`.
...
```html index.html
<div
  data-composition-id="loop-globe-10"
  data-composition-src="compositions/loop-globe-10.html"
  data-start="0"
  data-duration="9"
  data-track-index="1"
  data-width="1080"
...
| `cycles` | `6` | 0 to 12, step 0.1 | Number of motion cycles per loop. |
...
| `count` | `10` | 0 to 120, step 1 | Count (engine parameter; see the block header comment). |
| `radius` | `480` | 0 to 1780, step 10 | Radius (engine parameter; see the block header comment). |
| `minScale` | `5` | 0 to 28, step 1 | Min Scale (engine parameter; see the block header comment). |
| `maxScale` | `20` | 0 to 103, step 1 | Max Scale (engine parameter; see the block header comment). |
| `distance` | `650` | 0 to 8900, step 10 | Camera distance from the formation. |
| `perspective` | `150` | 0 to 300, step 10 | Camera focal length: smaller = stronger perspective. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-globe-10-bg"></div>
        <div id="loop-globe-10-stage"><div id="loop-globe-10-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-globe-10-loop").dataset.duration = DUR;
        background(document.getElementById("loop-globe-10-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R); // world -> stage px (ratios are all that matter)
        const stage = document.getElementById("loop-globe-10-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-globe-10-world");
        ...
        window.__timelines["loop-globe-10"] = tl;
      })();
    </script>
  </body>
</html>
