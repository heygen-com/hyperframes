...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-6.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-6.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-globe-6" item="loop-globe-6" />

That writes one file: `compositions/loop-globe-6.html`.
...
```html index.html
<div
  data-composition-id="loop-globe-6"
  data-composition-src="compositions/loop-globe-6.html"
  data-start="0"
  data-duration="15"
  data-track-index="1"
  data-width="1080"
...
| `radius` | `505` | 0 to 1780, step 10 | Radius (engine parameter; see the block header comment). |
| `minScale` | `7` | 0 to 28, step 1 | Min Scale (engine parameter; see the block header comment). |
| `maxScale` | `12` | 0 to 103, step 1 | Max Scale (engine parameter; see the block header comment). |
| `distance` | `0` | 0 to 8900, step 10 | Camera distance from the formation. |
| `perspective` | `145` | 0 to 300, step 10 | Camera focal length: smaller = stronger perspective. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-globe-6-bg"></div>
        <div id="loop-globe-6-stage"><div id="loop-globe-6-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-globe-6-loop").dataset.duration = DUR;
        background(document.getElementById("loop-globe-6-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R); // world -> stage px (ratios are all that matter)
        const stage = document.getElementById("loop-globe-6-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-globe-6-world");
        ...
        window.__timelines["loop-globe-6"] = tl;
      })();
    </script>
  </body>
</html>
