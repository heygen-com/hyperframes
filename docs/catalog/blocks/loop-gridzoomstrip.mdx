---
title: "Loop Grid Zoom Strip"
description: "A 3x3 grid of image cards flies into a single strip and back. 9 image slots, 6s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-gridzoomstrip.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-gridzoomstrip.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-gridzoomstrip" item="loop-gridzoomstrip" />

That writes one file: `compositions/loop-gridzoomstrip.html`.

## Add it to your video

It runs for 6 seconds at 1080×1080. Paste this into your composition:

```html index.html
<div
  data-composition-id="loop-gridzoomstrip"
  data-composition-src="compositions/loop-gridzoomstrip.html"
  data-start="0"
  data-duration="6"
  data-track-index="1"
  data-width="1080"
...
| `direction` | `horizontal` | `horizontal`, `vertical` | Direction of travel. |
| `movement` | `smooth` | `smooth`, `stepped` | Movement (engine parameter; see the block header comment). |
| `fade` | `false` | `true`, `false` | Fade (engine parameter; see the block header comment). |
| `padding` | `6` | 0px to 100px, step 1px | Padding (engine parameter; see the block header comment). |
| `zoom` | `2` | 0 to 10, step 0.1 | Zoom (engine parameter; see the block header comment). |
| `gridGap` | `2` | 0 to 10, step 0.1 | Grid Gap (engine parameter; see the block header comment). |
| `focusMode` | `start` | `start`, `center` | Focus Mode (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-gridzoomstrip-bg {
        position: absolute;
        inset: 0;
      }
      #loop-gridzoomstrip-stage {
        position: absolute;
        inset: 0;
      }
      .card {
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-gridzoomstrip-bg"></div>
        <div id="loop-gridzoomstrip-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Panel loop family — 3x3 grid of 16:9 cards; the cards fly into a single strip at
        // `zoom`x, the strip slides through every card, then they fly back. Measured on the 512 preview (2026-09-08):
        //   layout: padding 6% (31px) horizontally, gridGap 2 -> 10.5px = 2%; cards 143x80.5 (16:9), the 3-row block is
        //           vertically centred (rows at 125/216/307).
        //   zoomIn  [0.55,1.35]s: card 0 width 144,151,165,191,232,261,277,284,286 @0.6..1.4 -> cubic-bezier(.5,0,.5,1);
        //           strip = cards in one row at 2x with 2x gap (pitch 307 = 2*153.5), vertically centred, card 0 centred (focusMode start).
        //   slide   [1.5,4.62]s: offset/2456px = .021@2.0 .14@2.5 .447@3.0 .807@3.5 .963@4.0 -> cubic inOut (8 pitches).
        //   zoomOut [4.8,5.9]s: widths 284,278,268,251,227,197,174,159,151,146,144 @4.9..5.9 -> same bezier, 1.1s.
        //   fractions of DUR: hold .092 | zoomIn .133 | hold .025 | slide .52 | hold .03 | zoomOut .183 | hold .017.
        // Knobs: direction=vertical (strip along y) and movement=stepped (n-1 equal eased steps), focusMode=center (start on
        // the middle card, slide to the last) and fade (non-focused cards to 30%) are unmeasured interpretations.
        ...
        const __V = Object.assign(
          ...
          (window.__hyperframes && window.__hyperframes.getVariables
            ...
            : null) || {},
          { loopDuration: 6, loopRepeat: 2, imageCount: 9 },
        );
        const __PH = [
        ...
        const GAP = (num("gridGap", 2) / 100) * REF;
        const Z = Math.max(1, num("zoom", 2));
        const VERT = str("direction", "horizontal") === "vertical";
        const CENTER = str("focusMode", "start") === "center";
        const STEPPED = str("movement", "smooth") === "stepped";
        const FADE = bool("fade", false);
        ...
        document.getElementById("loop-gridzoomstrip-loop").dataset.duration = DUR;
        background(document.getElementById("loop-gridzoomstrip-bg"));
        const stage = document.getElementById("loop-gridzoomstrip-stage");
        const shadow = bool("shadowEnabled", false)
          ? `${(num("shadowX", 0) * REF) / 100}px ${(num("shadowY", 1) * REF) / 100}px ${(num("shadowBlur", 1.5) * REF) / 100}px ${(num("shadowSpread", -0.25) * REF) / 100}px ${str("shadowColor", "#000000")}${Math.round(
              ...
              .padStart(2, "0")}`
          : "";
        const AR = aspect({ w: 16, h: 9 }, "crop", CROP_AR);
        const CW = (W - 2 * PAD - (COLS - 1) * GAP) / COLS,
          CH = CW / AR;
        const TOP = (H - (ROWS * CH + (ROWS - 1) * GAP)) / 2;
        const PITCH = (VERT ? CH : CW) + GAP;
        const FOCUS0 = CENTER ? Math.floor(N / 2) : 0,
          TRAVEL = N - 1 - FOCUS0;
        const cards = [];
        for (let i = 0; i < N; i++) {
          const c = i % COLS,
            r = Math.floor(i / COLS);
          const m = IMAGES[i % IMAGES.length];
          const el = card(m, CW, CH, RADIUS);
          if (shadow) el.style.boxShadow = shadow;
          stage.appendChild(el);
          // grid top-left and strip top-left (at zoom Z, card FOCUS0 centred, slide offset 0)
          const gx = PAD + c * (CW + GAP),
            gy = TOP + r * (CH + GAP);
          const sx = VERT ? (W - CW * Z) / 2 : W / 2 - (CW * Z) / 2 + (i - FOCUS0) * PITCH * Z;
          const sy = VERT ? H / 2 - (CH * Z) / 2 + (i - FOCUS0) * PITCH * Z : (H - CH * Z) / 2;
          cards.push({ el, gx, gy, sx, sy, i });
        }
        // phase table (fractions of DUR)
        const PH = [
          ["hold", 0.092],
          ["in", 0.133],
          ["hold", 0.025],
        ...
        function renderAt(t) {
          let f = frac(t),
            zoom = 0,
            slide = 0,
            acc = 0; // zoom: 0 grid .. 1 strip ; slide: 0 .. 1 (TRAVEL pitches)
          for (const [kind, len] of PH) {
            if (f < acc + len || (kind === PH[PH.length - 1][0] && acc + len >= 1)) {
              const u = Math.max(0, Math.min(1, (f - acc) / len));
              if (kind === "in") zoom = ease(u);
              else if (kind === "slide") {
                zoom = 1;
                slide = STEPPED
                  ? (Math.floor(u * TRAVEL) + cubicInOut(frac(u * TRAVEL))) / TRAVEL
                  : cubicInOut(u);
                if (u >= 1) slide = 1;
              } else if (kind === "out") {
                zoom = 1 - ease(u);
          ...
          const off = slide * TRAVEL * PITCH * Z;
          for (const c of cards) {
            const s = 1 + (Z - 1) * zoom;
            const x = c.gx + (c.sx - (VERT ? 0 : off) - c.gx) * zoom,
              y = c.gy + (c.sy - (VERT ? off : 0) - c.gy) * zoom;
            c.el.style.transform = `translate3d(${x}px,${y}px,0) scale(${s})`;
            c.el.style.left = "0px";
            c.el.style.top = "0px";
            if (FADE) {
              const focus = FOCUS0 + slide * TRAVEL;
              c.el.style.opacity = 1 - 0.7 * zoom * Math.min(1, Math.abs(c.i - focus));
            }
          }
        }
        ...
        window.__timelines["loop-gridzoomstrip"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `gridzoomstrip`.
