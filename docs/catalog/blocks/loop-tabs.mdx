---
title: "Loop Tabs"
description: "A hero image above an arc of round tab buttons; the arc rotates to select the next image. 8 image slots, 14s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-tabs.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-tabs.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-tabs" item="loop-tabs" />

That writes one file: `compositions/loop-tabs.html`.
...
```html index.html
<div
  data-composition-id="loop-tabs"
  data-composition-src="compositions/loop-tabs.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.4` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `buttonScale` | `0.065` | 0 to 2, step 0.01 | Button Scale (engine parameter; see the block header comment). |
| `curve` | `5` | 0 to 10, step 0.1 | Curve (engine parameter; see the block header comment). |
| `gap` | `65` | 0px to 130px, step 1px | Spacing between cards. |
| `visible` | `7` | 0 to 14, step 0.1 | Visible (engine parameter; see the block header comment). |
| `highlight` | `100` | 0 to 200, step 1 | Highlight (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-tabs-bg"></div>
        <div id="loop-tabs-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Tabs loop family — a hero image (height scaleIntensity*H) above an arc of round buttons; the arc
        // holds 2N slots (pitch 22.5deg for 8 images), only `visible` show; per step the arc slides one slot (forward: buttons
        // move left, the new active arrives from the right) and the hero swaps by a two-panel 3D turn: the old panel rotates
        // away about its bottom-LEFT edge while shrinking, the new one rotates in about its bottom-RIGHT edge while growing;
        // panel brightness = cos(angle) (Lambert), ease-out.
        // Measured (512 preview, 8 images): hero 208x206 at (256,156) -> 0.4*H tall, bottom at 0.506 H; button centres on a circle
        // (255.6,449.1) R 140.8 at 0, +-22.5, +-45, +-67.5deg (7 visible); active button 48 px (~1.4 x 33.3 = buttonScale*H) with a
        // light ring, others 31/27/24 px (x0.9 per slot away); active centre y 308 = 0.602 H; arc spacing 55.3 px @ gap 65.
        // Step 52.5 f: buttons slide f3.5..8.5; old panel f4.5..9 (w/h ratio -> 13/37/53/68deg at f5..8, height 197/163/134/111);
        // new panel f5..15 (64/46/34/26/20/13/10/5deg at f6..13, left edge 306,256,220,196,180,169,163,158,155,154 -> cubic-out),
        // luma 50->100 as cos(angle) (44/69/83/90/94 predicted vs 50/66/78/85/91).
        // Guesses/knobs: slot count 2N (only the 22.5deg pitch is measured); arc spacing = gap * 0.166% of H, pitch = 22.5deg * curve/5
        // (curve and gap cannot be separated on one preview); highlight = ring opacity %; button size falloff 10%/slot;
        // panel scale 0.5..1 and the 3D perspective (1000px*H/1080) are eyeballed; easing enum: smooth=cubic-out, snappy, elastic
        // (overshooting bezier), linear.
        ...
        const HERO = num("scaleIntensity", 0.4) * REF; // hero height
        const BTN = num("buttonScale", 0.065) * REF; // button diameter
        const SPACING = num("gap", 65) * 0.00166 * REF; // arc length between buttons
        const PITCH = (22.5 * num("curve", 5)) / 5; // deg per slot
        const R = SPACING / ((PITCH * Math.PI) / 180); // arc radius
        const VISIBLE = Math.max(1, num("visible", 7) | 0),
          HALF = Math.floor(VISIBLE / 2);
        const HIGHL = Math.max(0, Math.min(1, num("highlight", 100) / 100));
        const DIR = str("direction", "forward") === "backward" ? -1 : 1;
        const RADIUS = (num("borderRadius", 0) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const M = 2 * N;
        const HERO_BOTTOM = 0.506 * REF - H / 2; // stage coords (stage origin = frame centre)
        const ARC_TOP = 0.602 * REF - H / 2; // active button centre
        const ARC_CY = ARC_TOP + R;
        ...
        document.getElementById("loop-tabs-loop").dataset.duration = DUR;
        background(document.getElementById("loop-tabs-bg"));
        const stage = document.getElementById("loop-tabs-stage");
        // hero panels: one per image, shown only when active/incoming/outgoing
        const panels = IMAGES.map((m) => {
          const a = aspect(m, FIT, CROP_AR),
            w = HERO * a;
          const c = card(m, w, HERO, RADIUS);
          c.style.left = -w / 2 + "px";
          c.style.top = HERO_BOTTOM - HERO + "px";
          c.style.display = "none";
          stage.appendChild(c);
          return { el: c, w };
        });
        // buttons: one per slot (2N), image j mod N
        const btns = [];
        for (let j = 0; j < M; j++) {
          const b = document.createElement("div");
          b.className = "btn";
          const im = document.createElement("img");
          im.src = IMAGES[j % N].src;
          b.appendChild(im);
          stage.appendChild(b);
          btns.push(b);
        }
        ...
        ring.className = "ring";
        ring.style.opacity = HIGHL;
        stage.appendChild(ring);

        const PERSP = (1000 * REF) / 1080;
        // step timing (fractions of one step, measured on 52.5 f)
        const SLIDE0 = 0.067,
          SLIDE1 = 0.162,
          OUT0 = 0.086,
          OUT1 = 0.171,
          IN0 = 0.095,
          IN1 = 0.286;
        function placePanel(p, angDeg, scale, hinge /* -1 left, +1 right */) {
          const c = Math.max(0, Math.cos((angDeg * Math.PI) / 180));
          p.el.style.display = "";
          p.el.style.transformOrigin = (hinge < 0 ? "0" : "100%") + " 100%";
          p.el.style.transform = `perspective(${PERSP}px) rotateY(${-hinge * angDeg}deg) scale(${scale})`; // +x side recedes for rotateY(+)
          p.el.style.filter = `brightness(${c})`;
        }
        function renderAt(t) {
          const s = frac(t) * N,
            k = Math.floor(s),
            u = s - k;
          const slide = ease(clamp01((u - SLIDE0) / (SLIDE1 - SLIDE0))); // 0..1 slot shift
          const active = k + slide; // continuous active slot (forward: increasing)
          // buttons
          for (let j = 0; j < M; j++) {
            // signed slot distance from the active slot, wrapped to -M/2..M/2
            let d = (((j - active) % M) + M) % M;
            if (d > M / 2) d -= M;
            const b = btns[j];
            if (Math.abs(d) > HALF + 0.5) {
              b.style.display = "none";
              continue;
            }
            b.style.display = "";
            const ang = DIR * d * PITCH,
              r = (ang * Math.PI) / 180;
            const size =
              BTN * (Math.abs(d) < 1 ? 1.4 - 0.4 * Math.abs(d) : 1) * (1 - 0.1 * Math.abs(d));
            const x = R * Math.sin(r),
              y = ARC_CY - R * Math.cos(r);
            b.style.width = b.style.height = size + "px";
            b.style.left = x - size / 2 + "px";
            b.style.top = y - size / 2 + "px";
            b.style.opacity = Math.abs(d) > HALF - 0.5 ? clamp01(HALF + 0.5 - Math.abs(d)) : 1;
            b.style.zIndex = Math.round(100 - Math.abs(d) * 10);
          }
          // highlight ring follows the active slot position
          const ringSize = BTN * 1.4 + (6 * REF) / 512;
          ring.style.width = ring.style.height = ringSize + "px";
          ring.style.left = -ringSize / 2 + "px";
          ring.style.top = ARC_TOP - ringSize / 2 + "px";
          // hero panels: current = image k (mod N), incoming = k+1
          panels.forEach((p) => (p.el.style.display = "none"));
          const cur = panels[k % N],
            nxt = panels[(k + 1) % N];
          const vo = clamp01((u - OUT0) / (OUT1 - OUT0)),
            vi = clamp01((u - IN0) / (IN1 - IN0));
          if (vo <= 0) placePanel(cur, 0, 1, -DIR);
          else if (vo < 1) placePanel(cur, 90 * vo, 1 - 0.5 * vo, -DIR); // rotates away about its outer-bottom corner
          if (vi > 0) {
            const e = ease(vi);
            placePanel(nxt, 90 * (1 - e), 0.5 + 0.5 * e, DIR);
            nxt.el.style.zIndex = 5;
          }
          cur.el.style.zIndex = 4;
        }
        ...
        window.__timelines["loop-tabs"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `tabs` `3d`.
