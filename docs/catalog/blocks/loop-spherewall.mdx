---
title: "Loop Sphere Wall"
description: "A concave bowed wall of image cards scrolls past the camera. 8 image slots, 20s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spherewall.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spherewall.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spherewall" item="loop-spherewall" />

That writes one file: `compositions/loop-spherewall.html`.
...
```html index.html
<div
  data-composition-id="loop-spherewall"
  data-composition-src="compositions/loop-spherewall.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `rows` | `5` | 0 to 10, step 0.1 | Rows (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spherewall-bg"></div>
        <div id="loop-spherewall-stage"><div id="loop-spherewall-wall"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // SphereWall loop family (Bow, LoopSphereWallTemplate) — the SphereCascade bowl (same camera/curvature:
        // rows bow up/down at the edges, side columns 2% wider at +-123px, edge rows 5% taller) panning SIDEWAYS.
        // Measured on the 512 preview (2026-09-08): centre card 101x57 (16:9), pitch 124 x 82.5, gap ~23-25px;
        // cell_y = (1-2*padding%)*H/rows = 75.8 world px * 1.069 = 81 (measured 82.5); card = cell - gap*0.9%H,
        // card w = h*16/9, cell_x = w + gap. One cell per 75 frames => N cells per loop; direction=left: cards
        // move LEFT (1.75px/frame at centre; unlike the SphereGlobe family, no near-side inversion here).
        // images: index = (column - 2*row) mod N. A card is centred on the frame centre at t=0.
        // Shared measurement notes (from spherecascade.html):
        ...
        const ROWS = Math.max(1, num("rows", 5) | 0);
        ...
        const SPAN = REF; // rows fill the height
        const LANE_OFF = 2; // image index shift per row (measured)

        document.getElementById("loop-spherewall-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spherewall-bg"));

        const f = 1.63 * REF,
          R = (0.834 * REF * 100) / CURV,
          MAG = 1.069;
        const DEPTH = f / MAG; // camera -> wall centre (= a + R at curvature -100)
        const stage = document.getElementById("loop-spherewall-stage");
        stage.style.perspective = f + "px";
        stage.style.transform = `rotate(${TILT}deg)`;
        const wall = document.getElementById("loop-spherewall-wall");
        wall.style.transform = `translate3d(0,0,${f - DEPTH}px)`;

        const cellY = ((1 - 2 * PAD) * SPAN) / ROWS;
        const ch = Math.max(1, cellY - GAP);
        const aspects = IMAGES.map((m) => aspect(m, FIT, CROP_AR));
        const cellX = ch * aspects.slice().sort((a, b) => a - b)[aspects.length >> 1] + GAP; // median aspect sets the column pitch
        const rowsHalf = Math.ceil(H / 2 / (cellY * MAG)) + 1;
        const need = Math.ceil(W / (cellX * MAG)) + 2;
        const K = N * Math.ceil(need / N); // slots per row: multiple of N so images stay fixed per slot

        const cards = [];
        for (let j = -rowsHalf; j <= rowsHalf; j++) {
          for (let k = 0; k < K; k++) {
            const idx = (((k - LANE_OFF * j) % N) + N) % N,
              m = IMAGES[idx];
            const w = ch * aspects[idx];
            const el = card(m, w, ch, RADIUS);
            el.style.left = -w / 2 + "px";
            el.style.top = -ch / 2 + "px";
            wall.appendChild(el);
            cards.push({ el, y: j * cellY, k });
          }
        }
        const RAD = 180 / Math.PI;
        function place(c, x) {
          const r2 = x * x + c.y * c.y;
          if (r2 >= 0.9 * R * R) {
          ...
          const beta = Math.asin(c.y / R),
            alpha = Math.asin(-x / (R * Math.cos(beta)));
          c.el.style.transform = `translate3d(${x}px,${c.y}px,${z}px) rotateY(${alpha * RAD}deg) rotateX(${beta * RAD}deg)`;
        }
        function renderAt(t) {
          const s = DIR * N * t; // cells travelled; left = slots move to smaller x
        ...
        window.__timelines["loop-spherewall"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `spherewall` `3d`.
