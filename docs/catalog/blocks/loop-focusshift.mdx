---
title: "Loop Focus Shift"
description: "One large image slot on the left and a rail of thumbnails on the right; each step the top thumbnail relays into the slot. 6 image slots, 10s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-focusshift.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-focusshift.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-focusshift" item="loop-focusshift" />

That writes one file: `compositions/loop-focusshift.html`.
...
```html index.html
<div
  data-composition-id="loop-focusshift"
  data-composition-src="compositions/loop-focusshift.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `railSize` | `26` | 0 to 52, step 1 | Rail Size (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-focusshift-bg"></div>
        <div id="loop-focusshift-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // FocusShift loop family (Relay) — one big slot on the left, a rail of N-1 thumbs on the right. Each step the TOP
        // thumb grows into the big slot (on top), the big card shrinks into the BOTTOM rail slot, the rail shifts up one.
        // Measured @512 (2026-09-08, 6 images, step 1.667s): padding 6% (31px), rail 117 = railSize% of the inner
        // width, gap 12.5 = gap% of H (big->rail 13px, thumb pitch 92.5 = 80 + 12.5), thumbs fill the inner height
        // (5 x 80). Every rect lerps linearly (x, y, w, h) with one eased progress; at 0.3s the incoming is at
        // p ~0.42 -> transition = 0.4·step (~0.67s; motion detected on frames 1-18 of 50). `smooth` = easeInOutCubic.
        // Slots are fixed rects -> media is always cover-fitted (mediaFit natural would need letterboxing: not done).
        // Knobs: shadow* units 1% of H; elastic/bounce/overshoot/impulse/swing -> smooth.
        ...
        const __V = Object.assign(
          ...
          (window.__hyperframes && window.__hyperframes.getVariables
            ...
            : null) || {},
          { loopDuration: 10, loopRepeat: 2, imageCount: 6 },
        );
        const __PH = [
        ...
        RF.use(__V, __MEDIA);
        const { W, H, REF, num, str, pick, card, background, frac } = RF;
        const bool = (k, d) => {
        ...
        const RAIL = num("railSize", 26) / 100;
        const GAP = (num("gap", 2.5) / 100) * REF;
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(Math.min(6, Math.max(3, num("imageCount", 16))));
        ...
        document.getElementById("loop-focusshift-loop").dataset.duration = DUR;
        background(document.getElementById("loop-focusshift-bg"));
        const shadow = bool("shadowEnabled", false)
          ? `${(num("shadowX", 0) * REF) / 100}px ${(num("shadowY", 1) * REF) / 100}px ${(num("shadowBlur", 1.5) * REF) / 100}px ${(num("shadowSpread", -0.25) * REF) / 100}px ${str("shadowColor", "#000000")}${Math.round(
              ...
              .padStart(2, "0")}`
          : "";
        // slots: 0 = big, 1..N-1 = rail top to bottom
        const IW = W - 2 * PAD,
          IH = H - 2 * PAD,
          RW = RAIL * IW,
          TH = (IH - (N - 2) * GAP) / (N - 1);
        const SLOT = [{ x: PAD, y: PAD, w: IW - RW - GAP, h: IH }];
        for (let j = 0; j < N - 1; j++)
          SLOT.push({ x: PAD + IW - RW, y: PAD + j * (TH + GAP), w: RW, h: TH });
        const stage = document.getElementById("loop-focusshift-stage");
        const cards = IMAGES.map((m) => {
          const c = card(m, SLOT[0].w, SLOT[0].h, RADIUS);
          c.style.left = "0px";
          c.style.top = "0px";
        ...
        // slot index of image i when image k is big: rail order k+1 .. k+N-1
        const slotOf = (i, k) => (((i - k) % N) + N) % N;
        function renderAt(t) {
          const u = frac(t) * N,
            k = Math.floor(u),
            e = ease(Math.min(1, (u - k) / TRANS));
          for (let i = 0; i < N; i++) {
            const A = SLOT[slotOf(i, k)],
              B = SLOT[slotOf(i, (k + 1) % N)],
              c = cards[i];
            ...
            // incoming (was slot 1, becomes big) on top, outgoing big next, rail below
            c.style.zIndex = slotOf(i, k) === 1 ? "3" : slotOf(i, k) === 0 ? "2" : "1";
          }
        }
        ...
        window.__timelines["loop-focusshift"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `focusshift`.
