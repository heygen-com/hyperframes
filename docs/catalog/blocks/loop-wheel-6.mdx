---
title: "Loop Wheel 6"
description: "A 2D ring of image thumbnails turns around a centre that may sit off frame. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheel-6.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheel-6.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-wheel-6" item="loop-wheel-6" />

That writes one file: `compositions/loop-wheel-6.html`.
...
```html index.html
<div
  data-composition-id="loop-wheel-6"
  data-composition-src="compositions/loop-wheel-6.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `ringRadius` | `467` | 0 to 1980, step 10 | Ring Radius (engine parameter; see the block header comment). |
| `thumbSize` | `71` | 0 to 450, step 10 | Thumb Size (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-wheel-6-bg"></div>
        <div id="loop-wheel-6-big"></div>
        <div id="loop-wheel-6-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-wheel-6-loop").dataset.duration = DUR;
        background(document.getElementById("loop-wheel-6-bg"));
        const stage = document.getElementById("loop-wheel-6-stage");
        const big = document.getElementById("loop-wheel-6-big");
        const bigImgs = [];
        if (BIG)
        ...
        window.__timelines["loop-wheel-6"] = tl;
      })();
    </script>
  </body>
</html>
