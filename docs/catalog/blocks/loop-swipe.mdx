---
title: "Loop Swipe"
description: "A fanned deck; the top image card is swiped to the back each beat. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-swipe.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-swipe.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-swipe" item="loop-swipe" />

That writes one file: `compositions/loop-swipe.html`.
...
```html index.html
<div
  data-composition-id="loop-swipe"
  data-composition-src="compositions/loop-swipe.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.645` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `tiltAngle` | `-45` | -90 to 90, step 1 | Tilt Angle (engine parameter; see the block header comment). |
| `xOffset` | `10` | 0 to 20, step 0.1 | X Offset (engine parameter; see the block header comment). |
| `stackOffset` | `8` | 0 to 16, step 0.1 | Stack Offset (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-swipe-bg"></div>
        <div id="loop-swipe-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Swipe loop family — a fanned deck; every DUR/N the top card is sent to the back and the deck
        // advances one rank. Measured on the 512x512/12s/8-card preview (2026-09-08):
        //   top card 264x329 = 4:5 crop, height = scaleIntensity*H (0.645*512 = 330). Deck centred.
        //   rank k pose (silhouette-envelope fit, IoU 0.69 outside the top card, all four edges within ~5px):
        //     scale 0.95^k, rotate -5deg*k, centre offset (+1px, -6px)*k  (fan peeks out top-right, slightly left).
        //   cycle = 45 frames; the cut (hard z-order swap, interior grey flips 101->128 between f59 and f60, no fade) is at
        //     frame 15 of the cycle. Before the cut the top card eases INTO the rank-1 pose over 11 frames (angle -0.2->-5.5,
        //     scale ->0.946, centre up 6px, right 1px); at the cut it goes to the back and the new top eases rank1->rank0 with
        //     a smooth bezier over 16 frames (progress .39@6f .72@10f .86@12f). Deeper ranks ease k->k-1 in step (assumed);
        //     the thrown card eases rank8->rank7 behind everything. t=0 and t=1 are both "all at rest" -> exact seam.
        // Guesses/knobs: tiltAngle(-45)->per-rank rotation = tiltAngle/9 deg; stackOffset(8)->per-rank rise 0.75px/unit @512
        //   (0.00146H); xOffset(10)->per-rank x shift 0.1px/unit @512; direction=left mirrors the fan; easing enum: smooth/
        //   snappy/linear are beziers, elastic falls back to smooth; rest tilt of the top card is +0.6deg in the preview (ignored).
        ...
        const CROP_AR = str("cropAspectRatio", "4:5");
        const SCALE = num("scaleIntensity", 0.645);
        const TILT = num("tiltAngle", -45);
        const XOFF = num("xOffset", 10);
        const SOFF = num("stackOffset", 8);
        const DIRS = str("direction", "right") === "left" ? -1 : 1;
        ...
        const EASES = { smooth: [0.5, 0, 0.5, 1], linear: [0, 0, 1, 1], snappy: [0.4, 0, 0.2, 1] };
        const ease = bezier(...(EASES[str("easing", "smooth")] || EASES.smooth));

        document.getElementById("loop-swipe-loop").dataset.duration = DUR;
        background(document.getElementById("loop-swipe-bg"));
        const stage = document.getElementById("loop-swipe-stage");

        const CH = SCALE * REF;
        const ROT = TILT / 9,
          DX = ((XOFF * 0.1 * REF) / 512) * DIRS,
          DY = (-SOFF * 0.75 * REF) / 512;
        const pose = (k) => ({ s: Math.pow(0.95, k), r: ROT * k * DIRS, x: DX * k, y: DY * k });
        const mix = (a, b, u) => ({
          s: a.s + (b.s - a.s) * u,
          r: a.r + (b.r - a.r) * u,
          x: a.x + (b.x - a.x) * u,
        ...
        const CUT = 15 / 45,
          WIND = 11 / 45,
          SETTLE = 16 / 45; // fractions of one cycle

        const cards = IMAGES.map((m) => {
          const a = aspect(m, FIT, CROP_AR),
            w = CH * a;
          const c = card(m, w, CH, RADIUS);
          c.style.left = -w / 2 + "px";
          c.style.top = -CH / 2 + "px";
          stage.appendChild(c);
          return c;
        ...
        function renderAt(t) {
          const cyc = t * N,
            ci = Math.floor(cyc),
            u = cyc - ci; // cycle index / progress; card ci is on top at the cycle start
          const afterCut = u >= CUT;
          for (let i = 0; i < N; i++) {
            // rank at the start of the cycle
            const k = (((i - ci) % N) + N) % N;
            let p, z;
            if (!afterCut) {
              p = k === 0 ? mix(pose(0), pose(1), ease((u - (CUT - WIND)) / WIND)) : pose(k);
              z = N - k;
            } else {
              const e = ease((u - CUT) / SETTLE);
              p = k === 0 ? mix(pose(N), pose(N - 1), e) : mix(pose(k), pose(k - 1), e);
              z = k === 0 ? 1 : N - k + 1;
            }
        ...
        window.__timelines["loop-swipe"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `swipe`.
