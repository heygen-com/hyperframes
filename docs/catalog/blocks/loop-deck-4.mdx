...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-4.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-4.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-deck-4" item="loop-deck-4" />

That writes one file: `compositions/loop-deck-4.html`.
...
```html index.html
<div
  data-composition-id="loop-deck-4"
  data-composition-src="compositions/loop-deck-4.html"
  data-start="0"
  data-duration="7"
  data-track-index="1"
  data-width="1080"
...
| `gap` | `0.37` | 0px to 2px, step 0.01px | Spacing between cards. |
| `maxScale` | `1.15` | 0 to 3, step 0.1 | Max Scale (engine parameter; see the block header comment). |
| `tilt` | `4.5` | -180° to 180°, step 1° | Tilt (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-deck-4-bg"></div>
        <div id="loop-deck-4-stage"><div id="loop-deck-4-belt"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-deck-4-loop").dataset.duration = DUR;
        background(document.getElementById("loop-deck-4-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / D; // world -> px at the belt plane (z = 0)
        const S = PLANE * s;
        const P = S * (1 + GAP) * PITCH_K;
        const stage = document.getElementById("loop-deck-4-stage");
        stage.style.perspective = f + "px";
        const belt = document.getElementById("loop-deck-4-belt");
        ...
        window.__timelines["loop-deck-4"] = tl;
      })();
    </script>
  </body>
</html>
