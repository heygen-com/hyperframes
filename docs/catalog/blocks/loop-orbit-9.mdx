...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbit-9.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbit-9.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-orbit-9" item="loop-orbit-9" />

That writes one file: `compositions/loop-orbit-9.html`.
...
```html index.html
<div
  data-composition-id="loop-orbit-9"
  data-composition-src="compositions/loop-orbit-9.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `512` | 0 to 1024, step 10 | Card size in scene units. |
| `orbitRadius` | `216` | 0 to 1036, step 10 | Ring radius in scene units. |
...
| `rotationZ` | `270` | -180° to 180°, step 1° | Formation roll about the view axis, degrees. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-orbit-9-bg"></div>
        <div id="loop-orbit-9-stage"><div id="loop-orbit-9-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-orbit-9-loop").dataset.duration = DUR;
        background(document.getElementById("loop-orbit-9-bg"));
        const f = 1200 * (100 / PERSP) * s;
        document.getElementById("loop-orbit-9-stage").style.perspective = f + "px";
        const world = document.getElementById("loop-orbit-9-world");
        ...
        window.__timelines["loop-orbit-9"] = tl;
      })();
    </script>
  </body>
</html>
