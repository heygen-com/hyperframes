...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-field-2.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-field-2.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-field-2" item="loop-field-2" />

That writes one file: `compositions/loop-field-2.html`.
...
```html index.html
<div
  data-composition-id="loop-field-2"
  data-composition-src="compositions/loop-field-2.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `cycleDeg` | `120` | -180° to 180°, step 1° | Degrees turned per cycle. |
| `delay` | `0` | 0s to 2s, step 0.01s | Hold time between steps, seconds. |
| `direction` | `forward` | `forward`, `reverse` | Direction of travel. |
| `count` | `18` | 0 to 100, step 1 | Count (engine parameter; see the block header comment). |
| `spread` | `1900` | 0 to 6300, step 10 | Spread (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-field-2-bg"></div>
        <div id="loop-field-2-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-field-2-loop").dataset.duration = DUR;
        background(document.getElementById("loop-field-2-bg"));
        const stage = document.getElementById("loop-field-2-stage");
        ...
        window.__timelines["loop-field-2"] = tl;
      })();
    </script>
  </body>
</html>
