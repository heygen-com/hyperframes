---
title: "Loop Spiral 6"
description: "Image cards evenly spaced along a helix that advances toward the camera. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-6.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-6.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spiral-6" item="loop-spiral-6" />

That writes one file: `compositions/loop-spiral-6.html`.
...
```html index.html
<div
  data-composition-id="loop-spiral-6"
  data-composition-src="compositions/loop-spiral-6.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `count` | `120` | 0 to 240, step 10 | Count (engine parameter; see the block header comment). |
| `radius` | `450` | 0 to 6000, step 10 | Radius (engine parameter; see the block header comment). |
| `turns` | `2` | 0 to 20, step 0.1 | Turns (engine parameter; see the block header comment). |
...
| `planeSize` | `50` | 0 to 1620, step 10 | Card size in scene units. |
| `distance` | `700` | 0 to 9300, step 10 | Camera distance from the formation. |
| `perspective` | `260` | 0 to 600, step 10 | Camera focal length: smaller = stronger perspective. |
...
| `cameraView` | `side` | `side`, `down` | Camera View (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spiral-6-bg"></div>
        <div id="loop-spiral-6-stage"><div id="loop-spiral-6-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-spiral-6-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spiral-6-bg"));
        ...
        const stage = document.getElementById("loop-spiral-6-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-spiral-6-world");
        ...
        window.__timelines["loop-spiral-6"] = tl;
      })();
    </script>
  </body>
</html>
