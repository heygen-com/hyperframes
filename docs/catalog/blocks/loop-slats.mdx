---
title: "Loop Slats"
description: "The focused image card is full size; every other card is a thin slat beside it. 8 image slots, 14s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-slats.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-slats.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-slats" item="loop-slats" />

That writes one file: `compositions/loop-slats.html`.
...
```html index.html
<div
  data-composition-id="loop-slats"
  data-composition-src="compositions/loop-slats.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `motion` | `step` | `glide`, `step` | Motion (engine parameter; see the block header comment). |
| `easing` | `smooth` | `smooth`, `snappy`, `elastic`, `linear` | Easing family for stepped motion. |
| `slatWidth` | `33` | 0 to 66, step 1 | Slat Width (engine parameter; see the block header comment). |
| `slatHeight` | `68` | 0 to 136, step 1 | Slat Height (engine parameter; see the block header comment). |
| `spacing` | `5` | 0 to 10, step 0.1 | Spacing (engine parameter; see the block header comment). |
| `visible` | `5` | 0 to 10, step 0.1 | Visible (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-slats-bg"></div>
        <div id="loop-slats-row"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Slats loop family — the focused card is full size; every other card is a thin slat
        // (slatWidth % of the card width, slatHeight % of its height). Stepping right-to-left: the focused card
        // slides left shrinking into a slat while the next slat slides in growing; every rect (x, w, h) lerps
        // linearly with one eased progress (measured mid-frame: heights 258/270 vs 314<->217 lerp 260/271).
        // Measured @512 (2026-09-08): card 472x314 (3:2 crop) -> height = min(scaleIntensity·H, 0.92·W/aspect)
        // (0.645·512 = 330 would be 495 wide > 92% of W, so width-clamped; 0.92 from one data point), gap between
        // card and slat 23px = spacing% of the card width, slat 217 tall = 68% (slatHeight), off-screen at rest.
        // Timing: step = DUR/N; incoming edge crosses mid at frame 14.5 of 52.5 with easeInOutCubic over 29 frames
        // -> transition = 0.55·step (same as CoverFlow), then rest. `glide` = continuous linear travel (guess).
        // Knobs: `visible` slats per side; elastic -> smooth; snappy guess (0.4,0,0.2,1).
        ...
        const SW = num("slatWidth", 33) / 100;
        const SH = num("slatHeight", 68) / 100;
        const SPACING = num("spacing", 5) / 100;
        const VISIBLE = Math.max(1, num("visible", 5) | 0);
        const MOTION = str("motion", "step");
        ...
        document.getElementById("loop-slats-loop").dataset.duration = DUR;
        background(document.getElementById("loop-slats-bg"));
        const row = document.getElementById("loop-slats-row");
        const cards = [],
          full = []; // full-size rects per card {w,h}
        for (let i = 0; i < N; i++) {
          const m = IMAGES[i],
            a = aspect(m, FIT, CROP_AR);
          const h = Math.min(SI * REF, (0.92 * W) / a),
            w = h * a;
        ...
        // rect of card i when card k is focused: {x (centre), w, h}
        function rect(i, k) {
          const r = wrap(i - k),
            ar = Math.abs(r),
            f = full[i];
          if (r === 0) return { x: 0, w: f.w, h: f.h, r };
          const fk = full[k],
            gap = SPACING * fk.w;
          let x = fk.w / 2 + gap; // inner edge of the first slat
          for (let j = 1; j < ar; j++) x += SW * full[(k + Math.sign(r) * j + N) % N].w + gap;
          return { x: Math.sign(r) * (x + (SW * f.w) / 2), w: SW * f.w, h: SH * f.h, r };
        }
        function renderAt(t) {
          const u = frac(t) * N,
            k = Math.floor(u),
            g = u - k;
          const e = MOTION === "glide" ? g : ease(Math.min(1, g / TRANS));
          for (let i = 0; i < N; i++) {
            const A = rect(i, k),
              B = rect(i, (k + 1) % N),
              c = cards[i];
            if (Math.abs(A.r) > VISIBLE && Math.abs(B.r) > VISIBLE) {
            ...
            c.style.width = w + "px";
            c.style.height = h + "px";
            c.style.transform = `translate(${x - w / 2}px,${-h / 2}px)`;
            c.style.zIndex = String(100 - Math.min(Math.abs(A.r), Math.abs(B.r)));
          }
        }
        ...
        window.__timelines["loop-slats"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `slats`.
