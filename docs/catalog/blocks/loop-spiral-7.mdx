---
title: "Loop Spiral 7"
description: "Image cards evenly spaced along a helix that advances toward the camera. 8 image slots, 20s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-7.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-7.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spiral-7" item="loop-spiral-7" />

That writes one file: `compositions/loop-spiral-7.html`.
...
```html index.html
<div
  data-composition-id="loop-spiral-7"
  data-composition-src="compositions/loop-spiral-7.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `count` | `73` | 0 to 240, step 10 | Count (engine parameter; see the block header comment). |
| `radius` | `1595` | 0 to 6000, step 10 | Radius (engine parameter; see the block header comment). |
| `turns` | `7.5` | 0 to 20, step 0.1 | Turns (engine parameter; see the block header comment). |
| `height` | `5000` | 0 to 10000, step 10 | Height (engine parameter; see the block header comment). |
| `wobble` | `50` | 0 to 100, step 1 | Wobble (engine parameter; see the block header comment). |
| `planeSize` | `235` | 0 to 1620, step 10 | Card size in scene units. |
| `distance` | `3450` | 0 to 9300, step 10 | Camera distance from the formation. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spiral-7-bg"></div>
        <div id="loop-spiral-7-stage"><div id="loop-spiral-7-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-spiral-7-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spiral-7-bg"));
        ...
        const stage = document.getElementById("loop-spiral-7-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-spiral-7-world");
        ...
        window.__timelines["loop-spiral-7"] = tl;
      })();
    </script>
  </body>
</html>
