...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbit-7.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbit-7.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-orbit-7" item="loop-orbit-7" />

That writes one file: `compositions/loop-orbit-7.html`.
...
```html index.html
<div
  data-composition-id="loop-orbit-7"
  data-composition-src="compositions/loop-orbit-7.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `257` | 0 to 1024, step 10 | Card size in scene units. |
| `orbitRadius` | `518` | 0 to 1036, step 10 | Ring radius in scene units. |
...
| `rotationZ` | `234` | -180° to 180°, step 1° | Formation roll about the view axis, degrees. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-orbit-7-bg"></div>
        <div id="loop-orbit-7-stage"><div id="loop-orbit-7-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-orbit-7-loop").dataset.duration = DUR;
        background(document.getElementById("loop-orbit-7-bg"));
        const f = 1200 * (100 / PERSP) * s;
        document.getElementById("loop-orbit-7-stage").style.perspective = f + "px";
        const world = document.getElementById("loop-orbit-7-world");
        ...
        window.__timelines["loop-orbit-7"] = tl;
      })();
    </script>
  </body>
</html>
