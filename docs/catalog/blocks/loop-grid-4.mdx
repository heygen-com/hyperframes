---
title: "Loop Grid 4"
description: "A camera pans over an infinite lattice of image cards. 8 image slots, 24s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-grid-4.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-grid-4.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-grid-4" item="loop-grid-4" />

That writes one file: `compositions/loop-grid-4.html`.
...
```html index.html
<div
  data-composition-id="loop-grid-4"
  data-composition-src="compositions/loop-grid-4.html"
  data-start="0"
  data-duration="24"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `637` | 0 to 1400, step 10 | Card size in scene units. |
| `gap` | `500` | 0px to 1000px, step 10px | Spacing between cards. |
| `zoom` | `on` | `off`, `on` | Zoom (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-grid-4-bg"></div>
        <div id="loop-grid-4-stage"><div id="loop-grid-4-plane"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-grid-4-loop").dataset.duration = DUR;
        background(document.getElementById("loop-grid-4-bg"));
        ...
        const plane = document.getElementById("loop-grid-4-plane");
        ...
        const stage = document.getElementById("loop-grid-4-stage");
        function renderAt(t) {
        ...
        window.__timelines["loop-grid-4"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `grid`.
