---
title: "Loop Globe 9"
description: "Image cards scattered evenly over a sphere that rotates under a 3D camera. 8 image slots, 10s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-9.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-9.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-globe-9" item="loop-globe-9" />

That writes one file: `compositions/loop-globe-9.html`.
...
```html index.html
<div
  data-composition-id="loop-globe-9"
  data-composition-src="compositions/loop-globe-9.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `backface` | `hide` | `show`, `hide` | Backface (engine parameter; see the block header comment). |
...
| `autoFaceCamera` | `on` | `on`, `off` | Auto Face Camera (engine parameter; see the block header comment). |
| `cycles` | `2` | 0 to 12, step 0.1 | Number of motion cycles per loop. |
...
| `count` | `40` | 0 to 120, step 1 | Count (engine parameter; see the block header comment). |
| `radius` | `580` | 0 to 1780, step 10 | Radius (engine parameter; see the block header comment). |
| `minScale` | `14` | 0 to 28, step 1 | Min Scale (engine parameter; see the block header comment). |
| `maxScale` | `51.5` | 0 to 103, step 1 | Max Scale (engine parameter; see the block header comment). |
| `distance` | `2000` | 0 to 8900, step 10 | Camera distance from the formation. |
| `perspective` | `135` | 0 to 300, step 10 | Camera focal length: smaller = stronger perspective. |
...
| `axis` | `y` | `y`, `x`, `z` | Axis (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-globe-9-bg"></div>
        <div id="loop-globe-9-stage"><div id="loop-globe-9-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-globe-9-loop").dataset.duration = DUR;
        background(document.getElementById("loop-globe-9-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R); // world -> stage px (ratios are all that matter)
        const stage = document.getElementById("loop-globe-9-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-globe-9-world");
        ...
        window.__timelines["loop-globe-9"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `globe` `3d`.
