...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-5.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-5.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-deck-5" item="loop-deck-5" />

That writes one file: `compositions/loop-deck-5.html`.
...
```html index.html
<div
  data-composition-id="loop-deck-5"
  data-composition-src="compositions/loop-deck-5.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `320` | 0 to 1000, step 10 | Card size in scene units. |
| `distance` | `1000` | 0 to 8260, step 10 | Camera distance from the formation. |
| `perspective` | `85` | 0 to 200, step 1 | Camera focal length: smaller = stronger perspective. |
| `gap` | `0.32` | 0px to 2px, step 0.01px | Spacing between cards. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-deck-5-bg"></div>
        <div id="loop-deck-5-stage"><div id="loop-deck-5-belt"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-deck-5-loop").dataset.duration = DUR;
        background(document.getElementById("loop-deck-5-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / D; // world -> px at the belt plane (z = 0)
        const S = PLANE * s;
        const P = S * (1 + GAP) * PITCH_K;
        const stage = document.getElementById("loop-deck-5-stage");
        stage.style.perspective = f + "px";
        const belt = document.getElementById("loop-deck-5-belt");
        ...
        window.__timelines["loop-deck-5"] = tl;
      })();
    </script>
  </body>
</html>
