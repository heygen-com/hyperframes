---
title: "Loop Gravity 3"
description: "Image cards fall from above, tip flat onto the floor and pile up. 8 image slots, 11s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-gravity-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-gravity-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-gravity-3" item="loop-gravity-3" />

That writes one file: `compositions/loop-gravity-3.html`.
...
```html index.html
<div
  data-composition-id="loop-gravity-3"
  data-composition-src="compositions/loop-gravity-3.html"
  data-start="0"
  data-duration="11"
  data-track-index="1"
  data-width="1080"
...
| `stagger` | `0.22` | 0 to 2, step 0.01 | Stagger (engine parameter; see the block header comment). |
| `cardSize` | `410` | 0 to 820, step 10 | Card Size (engine parameter; see the block header comment). |
| `gravity` | `1` | 0 to 10, step 0.1 | Gravity (engine parameter; see the block header comment). |
| `bounce` | `0.5` | 0 to 2, step 0.01 | Bounce (engine parameter; see the block header comment). |
| `spin` | `0.02` | 0 to 2, step 0.01 | Spin (engine parameter; see the block header comment). |
| `tilt` | `12` | -180° to 180°, step 1° | Tilt (engine parameter; see the block header comment). |
| `seed` | `1` | 0 to 10, step 0.1 | Seed (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-gravity-3-bg"></div>
        <div id="loop-gravity-3-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-gravity-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-gravity-3-bg"));
        const stage = document.getElementById("loop-gravity-3-stage");
        ...
        window.__timelines["loop-gravity-3"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `gravity`.
