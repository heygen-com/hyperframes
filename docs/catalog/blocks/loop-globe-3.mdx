...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-globe-3" item="loop-globe-3" />

That writes one file: `compositions/loop-globe-3.html`.
...
```html index.html
<div
  data-composition-id="loop-globe-3"
  data-composition-src="compositions/loop-globe-3.html"
  data-start="0"
  data-duration="15"
  data-track-index="1"
  data-width="1080"
...
| `radius` | `890` | 0 to 1780, step 10 | Radius (engine parameter; see the block header comment). |
| `minScale` | `3` | 0 to 28, step 1 | Min Scale (engine parameter; see the block header comment). |
| `maxScale` | `22` | 0 to 103, step 1 | Max Scale (engine parameter; see the block header comment). |
| `distance` | `900` | 0 to 8900, step 10 | Camera distance from the formation. |
...
| `axis` | `z` | `y`, `x`, `z` | Axis (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-globe-3-bg"></div>
        <div id="loop-globe-3-stage"><div id="loop-globe-3-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-globe-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-globe-3-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R); // world -> stage px (ratios are all that matter)
        const stage = document.getElementById("loop-globe-3-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-globe-3-world");
        ...
        window.__timelines["loop-globe-3"] = tl;
      })();
    </script>
  </body>
</html>
