---
title: "Loop Circle"
description: "Concentric rings of upright image cards rotate in the frame. 8 image slots, 18s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-circle.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-circle.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-circle" item="loop-circle" />

That writes one file: `compositions/loop-circle.html`.
...
```html index.html
<div
  data-composition-id="loop-circle"
  data-composition-src="compositions/loop-circle.html"
  data-start="0"
  data-duration="18"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.16` | 0 to 1, step 0.05 | Strength of the size change. |
| `borderRadius` | `20` | 0px to 200px, step 1px | Card corner radius in px at 1080. |
| `direction` | `cw` | `cw`, `ccw`, `alternate` | Direction of travel. |
| `cards` | `24` | 0 to 48, step 1 | Cards (engine parameter; see the block header comment). |
| `rings` | `2` | 0 to 10, step 0.1 | Rings (engine parameter; see the block header comment). |
| `innerRadius` | `14` | 0 to 28, step 1 | Inner Radius (engine parameter; see the block header comment). |
| `ringGap` | `23` | 0 to 46, step 1 | Ring Gap (engine parameter; see the block header comment). |
| `tilt` | `0` | -180° to 180°, step 1° | Tilt (engine parameter; see the block header comment). |
| `turns` | `1` | 0 to 10, step 0.1 | Turns (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-circle-bg"></div>
        <div id="loop-circle-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Circle loop family — `rings` concentric rings of upright cards, `cards` cards in total
        // split between the rings in proportion to their radius, all rings spinning continuously.
        // Measured on the 512 preview (2026-09-08): ring radii 71.7px / 190.5px = innerRadius% * H and
        // (innerRadius + ringGap)% * H (14% -> 71.7, 37% -> 189.4; bbox centres of unoccluded cards);
        // card 81px square = scaleIntensity * H (0.16 -> 81.9); counts 7 inner + 17 outer = 24 = round(24 * R_k / sum R);
        // both rings 10deg per 15 frames = 360deg per 18 s = `turns` revolutions per loop, clockwise (direction=cw);
        // cards stay upright (bbox 81x81 everywhere, tilt=0); each ring's cards stack in ring order.
        // Guesses: tilt = per-card rotation in degrees alternating +/- ("each set at its own small angle"); ring k is
        // phase-shifted by half its pitch (outer ring measured at ~10deg = pitch/2); direction=alternate flips odd rings.
        ...
        const SIZE = num("scaleIntensity", 0.16) * REF;
        const CARDS = Math.max(1, Math.round(num("cards", 24)));
        const RINGS = Math.max(1, Math.round(num("rings", 2)));
        const R0 = (num("innerRadius", 14) / 100) * REF;
        const GAP = (num("ringGap", 23) / 100) * REF;
        const TILT = num("tilt", 0);
        const TURNS = Math.max(1, Math.round(num("turns", 1)));
        const DIRMODE = str("direction", "cw");
        const RADIUS = (num("borderRadius", 20) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;

        document.getElementById("loop-circle-loop").dataset.duration = DUR;
        background(document.getElementById("loop-circle-bg"));
        const stage = document.getElementById("loop-circle-stage");

        // cards per ring: proportional to radius, rounded, remainder fixed on the largest ring
        const radii = Array.from({ length: RINGS }, (_, k) => R0 + k * GAP);
        const sumR = radii.reduce((s, r) => s + r, 0);
        const counts = radii.map((r) => Math.max(1, Math.round((CARDS * r) / sumR)));
        counts[RINGS - 1] += CARDS - counts.reduce((s, c) => s + c, 0);
        if (counts[RINGS - 1] < 1) counts[RINGS - 1] = 1;
        ...
        radii.forEach((R, k) => {
          const n = counts[k],
            dir = DIRMODE === "ccw" ? -1 : DIRMODE === "alternate" && k % 2 ? -1 : 1;
          const items = [];
          for (let j = 0; j < n; j++) {
            const m = IMAGES[img++ % N],
            ...
            c.style.zIndex = z++;
            stage.appendChild(c);
            items.push({ el: c, theta: ((j + 0.5 * k) * 360) / n, tilt: TILT * (j % 2 ? -1 : 1) });
          }
          rings.push({ R, dir, items });
        });
        function renderAt(t) {
          const spin = TURNS * 360 * frac(t);
          for (const g of rings)
            for (const c of g.items) {
              const phi = c.theta + g.dir * spin,
                r = (phi * Math.PI) / 180;
              c.el.style.transform = `translate(${g.R * Math.cos(r)}px,${g.R * Math.sin(r)}px) rotate(${c.tilt}deg)`;
            }
        }
        ...
        window.__timelines["loop-circle"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `circle`.
