---
title: "Loop Cascade Deck"
description: "A centre stack fans into a diagonal deck, flips, and restacks in a loop. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-cascadedeck.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-cascadedeck.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-cascadedeck" item="loop-cascadedeck" />

That writes one file: `compositions/loop-cascadedeck.html`.
...
```html index.html
<div
  data-composition-id="loop-cascadedeck"
  data-composition-src="compositions/loop-cascadedeck.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.8` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `motion` | `staggered` | `staggered`, `together` | Motion (engine parameter; see the block header comment). |
| `padding` | `6` | 0px to 100px, step 1px | Padding (engine parameter; see the block header comment). |
| `overlap` | `70` | 0 to 140, step 1 | Overlap (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-cascadedeck-bg"></div>
        <div id="loop-cascadedeck-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Shuffle loop family — a centre stack fans into a diagonal deck, flips diagonal twice, restacks.
        // Measured on the 512x512/8s/8-card preview (2026-09-08, per-card grey tracking + synthetic-fit, mean err 0.64 levels):
        //   card 115px square; deck pitch 34.7px = card*(1-overlap/100) on BOTH axes; deck span 358px centred in frame.
        //   span = (H - 2*padding%H) * scaleIntensity = 360px (1% over measured) -> card = span / (1 + (N-1)*(1-overlap)).
        //   z: the top card of the stack takes slot 0 and stays on top; slot k sits k pitches down-right of slot 0.
        //   4 phases, frame schedule of 240 (start / per-card stagger / per-card move, all smooth bezier(.5,0,.5,1)):
        //     A  1/4/6  centre -> TL-BR deck (slot k at +k,+k), order z-top first
        //     B 53/5/8  y mirrored (slot k -> +k, N-1-k)  order z-top first
        //     C 113/5/8 x mirrored (-> N-1-k, N-1-k)      order REVERSED (z-bottom first)
        //     D 179/7/10 back to centre                    order z-top first
        //   Rests between phases are the leftovers; frame 239 == frame 0 (one card at centre), seam is exact.
        // Guesses/knobs: `motion: together` = stagger 0 (unseen); easing enum elastic/bounce/overshoot/impulse/swing -> smooth;
        //   padding assumed % of H; shadow* units 1% of H (same as flipgrid); natural fit: card HEIGHT is the measured size.
        ...
        const SCALE = num("scaleIntensity", 0.8);
        const OVERLAP = num("overlap", 70) / 100;
        const TOGETHER = str("motion", "staggered") === "together";
        ...
        document.getElementById("loop-cascadedeck-loop").dataset.duration = DUR;
        background(document.getElementById("loop-cascadedeck-bg"));
        const stage = document.getElementById("loop-cascadedeck-stage");
        ...
        const SPAN = (REF - 2 * PAD) * SCALE;
        const SIZE = SPAN / (1 + (N - 1) * (1 - OVERLAP)); // card height
        const PITCH = SIZE * (1 - OVERLAP);
        const O = -SPAN / 2; // slot 0 top-left, deck centred on the stage origin
        const slot = (i, j) => [O + i * PITCH, O + j * PITCH];
        const CTR = [-SIZE / 2, -SIZE / 2];
        // phases: [startFrame, stagger, move, reversedOrder, from(k), to(k)] in 240ths of the loop
        const PH = [
          [1, 4, 6, false, (k) => CTR, (k) => slot(k, k)],
          [53, 5, 8, false, (k) => slot(k, k), (k) => slot(k, N - 1 - k)],
          [113, 5, 8, true, (k) => slot(k, N - 1 - k), (k) => slot(N - 1 - k, N - 1 - k)],
          [179, 7, 10, false, (k) => slot(N - 1 - k, N - 1 - k), (k) => CTR],
        ];
        const cards = [];
        for (let k = N - 1; k >= 0; k--) {
          // append bottom first: k=0 ends on top
          const m = IMAGES[k],
            a = aspect(m, FIT, CROP_AR);
          const c = card(m, SIZE * a, SIZE, RADIUS);
          if (shadow) c.style.boxShadow = shadow;
          c.style.left = "0px";
          c.style.top = "0px";
          stage.appendChild(c);
          cards[k] = { el: c, w: SIZE * a };
        }
        function renderAt(t) {
          const f = t * 240;
          for (let k = 0; k < N; k++) {
            let p = CTR;
            for (const [st, sg, du, rev, from, to] of PH) {
              const d = TOGETHER ? 0 : (rev ? N - 1 - k : k) * sg;
              const u = (f - st - d) / du;
              if (u <= 0) continue;
            ...
            // natural fit: keep the card's centre where the square slot's centre would be
            const dx = (cards[k].w - SIZE) / 2;
            cards[k].el.style.transform = `translate3d(${p[0] - dx}px,${p[1]}px,0)`;
          }
        }
        ...
        window.__timelines["loop-cascadedeck"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `cascadedeck`.
