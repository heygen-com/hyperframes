...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-deck-3" item="loop-deck-3" />

That writes one file: `compositions/loop-deck-3.html`.
...
```html index.html
<div
  data-composition-id="loop-deck-3"
  data-composition-src="compositions/loop-deck-3.html"
  data-start="0"
  data-duration="7"
  data-track-index="1"
  data-width="1080"
...
| `direction` | `up` | `up`, `down`, `left`, `right` | Direction of travel. |
| `flipAxis` | `vertical` | `vertical`, `horizontal` | Flip Axis (engine parameter; see the block header comment). |
| `planeSize` | `445` | 0 to 1000, step 10 | Card size in scene units. |
| `distance` | `4130` | 0 to 8260, step 10 | Camera distance from the formation. |
| `perspective` | `40` | 0 to 200, step 1 | Camera focal length: smaller = stronger perspective. |
| `gap` | `0.44` | 0px to 2px, step 0.01px | Spacing between cards. |
| `maxScale` | `1` | 0 to 10, step 0.1 | Max Scale (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-deck-3-bg"></div>
        <div id="loop-deck-3-stage"><div id="loop-deck-3-belt"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-deck-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-deck-3-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / D; // world -> px at the belt plane (z = 0)
        const S = PLANE * s;
        const P = S * (1 + GAP) * PITCH_K;
        const stage = document.getElementById("loop-deck-3-stage");
        stage.style.perspective = f + "px";
        const belt = document.getElementById("loop-deck-3-belt");
        ...
        window.__timelines["loop-deck-3"] = tl;
      })();
    </script>
  </body>
</html>
