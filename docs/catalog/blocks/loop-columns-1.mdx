---
title: "Loop Columns 1"
description: "Four columns of square image cards climb through a padded mask at alternating speeds. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-columns-1.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-columns-1.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-columns-1" item="loop-columns-1" />

That writes one file: `compositions/loop-columns-1.html`.
...
```html index.html
<div
  data-composition-id="loop-columns-1"
  data-composition-src="compositions/loop-columns-1.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `stagger` | `100` | 0 to 200, step 1 | Stagger (engine parameter; see the block header comment). |
| `direction` | `forward` | `forward`, `reverse` | Direction of travel. |
| `planeSize` | `100` | 0 to 200, step 1 | Card size in scene units. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-columns-1-bg"></div>
        <div id="loop-columns-1-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Columns loop family — four columns of square cards climbing (content moves UP) in
        // eased page-steps that never fully stop. Measured on the 512x512 preview (2026-09-08):
        //   4 columns (spec says lanes=3; description and preview say four) of 102px squares, gap 16px,
        //   block centred (margins 28px): planeSize 100 -> 0.2% H per unit, gap 15 -> 0.2% H per unit.
        //   row pitch 155px = 1.52 x card (NOT card+gap; unexplained, kept as a constant).
        //   motion: cumulative displacement of column 0 = 636px per step (~4 row pitches), velocity 0.5..32 px/f,
        //   peaks at f58 and f176.5 => step 3.95s => 3 steps per 12s loop; profile fits 0.15*t + 0.85*expoInOut(t)
        //   over the whole step (predicted peak 33 px/f vs 32.4 measured), no hold.
        //   column 3 lags column 0 by ~5.5 frames => ~1.8 frames (0.5% of the loop) per column at stagger=100.
        //   lane content: column c shows all images in order starting at image c*N/2 (cols 0,2 identical, 1,3 identical).
        // Reference caveat: the preview's belt RUNS OUT at 7.0s (empty 7.1-7.8s, a lone row jumps in at 7.8s,
        //   fragments until 10.1s, empty to the end) — a non-seamless/broken preview. Ours keeps the belt
        //   infinite and seamless: lane period = steps*rows cards, so one loop shifts exactly one period.
        // Guesses: 4 rows per page (= ceil(H/pitch) at 512); lanes ignored (COLS=4); hold/offsetN/alternate
        //   unused; direction=reverse moves down; axis=horizontal swaps axes (unverified); stepped == continuous.
        ...
        const COLS = 4; // "Four columns" — preview shows 4 with lanes=3
        const CARD = num("planeSize", 100) * 0.002 * REF;
        const GAP = num("gap", 15) * 0.002 * REF;
        const PITCHK = 1.52; // row pitch / card height (measured 155/102)
        const CYCLES = Math.max(1, num("cycles", 1) | 0);
        const STAGGER = num("stagger", 100);
        const FWD = str("direction", "forward") === "forward" ? 1 : -1;
        const VERT = str("axis", "vertical") === "vertical";
        const RADIUS = num("borderRadius", 0) * 0.00009 * REF; // Bento calibration (50 -> 4.5px @512); 0 here
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const STEPS_PER_TOUR = 3; // measured: 3 page-steps per loop
        const ease = (x) => {
          x = Math.min(1, Math.max(0, x));
          return 0.15 * x + 0.85 * expoInOut(x);
        };

        document.getElementById("loop-columns-1-loop").dataset.duration = DUR;
        background(document.getElementById("loop-columns-1-bg"));
        const stage = document.getElementById("loop-columns-1-stage");

        const along = VERT ? H : W; // axis the belt travels along
        const a0 = aspect(IMAGES[0], FIT, CROP_AR);
        const cardAlong = VERT ? CARD / a0 : CARD * a0; // crop 1:1 -> square; natural follows the first image
        const PITCH = cardAlong * PITCHK;
        const ROWS = Math.ceil(along / PITCH); // rows per page (4 @512)
        const STEPS = STEPS_PER_TOUR * CYCLES;
        const PERIOD_CARDS = STEPS * ROWS; // lane period in cards => loop shift == one period
        const P = PERIOD_CARDS * PITCH;
        const blockCross = COLS * CARD + (COLS - 1) * GAP;
        const DIAG = Math.hypot(W, H);
        const lanes = [];
        for (let c = 0; c < COLS; c++) {
          const el = document.createElement("div");
          el.className = "lane";
          const cross = -blockCross / 2 + c * (CARD + GAP);
          const reps = Math.ceil(DIAG / P) + 2;
          const start0 = -P * Math.ceil(reps / 2);
          el.style.cssText = VERT
            ? `left:${cross}px;top:${start0}px;width:${CARD}px;height:${reps * P}px;`
            : `top:${cross}px;left:${start0}px;height:${CARD}px;width:${reps * P}px;`;
          const first = (c * Math.floor(N / 2)) % N;
          for (let r = 0; r < reps; r++)
            for (let k = 0; k < PERIOD_CARDS; k++) {
              const m = IMAGES[(first + k) % N];
              const cd = VERT ? card(m, CARD, cardAlong, RADIUS) : card(m, cardAlong, CARD, RADIUS);
              const pos = (r * PERIOD_CARDS + k) * PITCH;
              if (VERT) {
                cd.style.left = "0px";
                cd.style.top = pos + "px";
              } else {
          ...
          lanes.push({ el, lag: ((c * STAGGER) / 100) * 0.005 }); // fraction of the loop
        }
        function travel(t) {
          // px moved along the belt, one eased step per STEPS
          const u = frac(t) * STEPS,
            s = Math.floor(u);
          return (s + ease(u - s)) * ROWS * PITCH;
        }
        function renderAt(t) {
          for (const L of lanes) {
            const sh = frac((-FWD * travel(t - L.lag)) / P) * P; // forward = content moves up/left
        ...
        window.__timelines["loop-columns-1"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `columns`.
