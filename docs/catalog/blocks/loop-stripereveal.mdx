---
title: "Loop Stripe Reveal"
description: "Images reassemble from sliding strips. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-stripereveal.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-stripereveal.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-stripereveal" item="loop-stripereveal" />

That writes one file: `compositions/loop-stripereveal.html`.
...
```html index.html
<div
  data-composition-id="loop-stripereveal"
  data-composition-src="compositions/loop-stripereveal.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `strips` | `7` | 0 to 14, step 0.1 | Strips (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-stripereveal-bg {
        position: absolute;
        inset: 0;
      }
      #loop-stripereveal-card {
        position: absolute;
        overflow: hidden;
      }
      .layer {
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-stripereveal-bg"></div>
        <div id="loop-stripereveal-card"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // StripeReveal loop family — "Images reassemble from sliding strips": the card
        // is cut into `strips` vertical strips; the next image's strips slide in over the current image, strip 0
        // from the top, strip 1 from the bottom, alternating, left to right.
        // Measured on the 512 preview (2026-09-08): card = frame inset 6% of H (padding 6), 7 strips of 64.3px;
        //   period = DUR/N = 30f. Per strip coverage (frame: fraction): s0 92:.28 94:.98 | s1 94:.43 96:1 | s2 94:.005
        //   96:.61 98:1 | s3 96:.02 98:.75 | s4 98:.06 100:.85 | s5 100:.12 102:.92 | s6 102:.22 104:.96 =>
        //   with cubic-bezier(.5,0,.5,1): slide 3.6f per strip, stagger 1.68f, first strip starts 0.7f after
        //   the period boundary (transition 91..105 of 90..120, then hold).
        // Knobs (guesses): timings expressed as fractions of the period (slide 0.12, stagger 0.056, start 0.023;
        //   only DUR 8 seen); the strip carries the new image's own slice and translates (a mask reveal looks the
        //   same on uniform placeholders); easing names other than smooth/custom/linear mapped to GSAP eases.
        ...
        const STRIPS = Math.max(1, num("strips", 7) | 0);
        ...
        const START = 0.023,
          SLIDE = 0.12,
          STAGGER = 0.056; // fractions of the per-image period

        document.getElementById("loop-stripereveal-loop").dataset.duration = DUR;
        background(document.getElementById("loop-stripereveal-bg"));
        const cardEl = document.getElementById("loop-stripereveal-card");
        const CW = W - 2 * PAD,
          CH = H - 2 * PAD;
        ...
        // each layer: a whole image (used when it is the resting image) + `strips` slices of it (used while sliding in)
        const layers = IMAGES.map((m) => {
          ...
          for (let i = 0; i < STRIPS; i++) {
            ...
            s.className = "strip";
            s.style.cssText += `left:${i * SW}px;width:${SW + 0.5}px;height:${CH}px;`;
            const c = card(m, g.w, g.h, 0);
            c.style.left = g.x - i * SW + "px";
            c.style.top = g.y + "px";
            s.appendChild(c);
            L.appendChild(s);
            strips.push(s);
        ...
        function renderAt(t) {
          ...
          for (const st of out.strips) st.style.display = "none";
          inn.el.style.display = "block";
          inn.el.style.zIndex = "2";
          inn.whole.style.display = "none";
          for (let i = 0; i < STRIPS; i++) {
            const q = EASE((u - START - i * STAGGER) / SLIDE);
            const st = inn.strips[i];
            if (q <= 0) {
              st.style.display = "none";
              continue;
            }
            st.style.display = "block";
            const off = (1 - q) * CH * (i % 2 === 0 ? -1 : 1); // even strips from the top, odd from the bottom
            st.style.transform = `translate3d(0,${off}px,0)`;
          }
        }
        ...
        window.__timelines["loop-stripereveal"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `stripereveal`.
