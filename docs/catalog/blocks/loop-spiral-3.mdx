...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spiral-3" item="loop-spiral-3" />

That writes one file: `compositions/loop-spiral-3.html`.
...
```html index.html
<div
  data-composition-id="loop-spiral-3"
  data-composition-src="compositions/loop-spiral-3.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `count` | `33` | 0 to 240, step 10 | Count (engine parameter; see the block header comment). |
| `radius` | `1195` | 0 to 6000, step 10 | Radius (engine parameter; see the block header comment). |
...
| `planeSize` | `320` | 0 to 1620, step 10 | Card size in scene units. |
| `distance` | `3100` | 0 to 9300, step 10 | Camera distance from the formation. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spiral-3-bg"></div>
        <div id="loop-spiral-3-stage"><div id="loop-spiral-3-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-spiral-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spiral-3-bg"));
        ...
        const stage = document.getElementById("loop-spiral-3-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-spiral-3-world");
        ...
        window.__timelines["loop-spiral-3"] = tl;
      })();
    </script>
  </body>
</html>
