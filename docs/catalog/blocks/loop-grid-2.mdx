---
title: "Loop Grid 2"
description: "A camera pans over an infinite lattice of image cards. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-grid-2.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-grid-2.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-grid-2" item="loop-grid-2" />

That writes one file: `compositions/loop-grid-2.html`.
...
```html index.html
<div
  data-composition-id="loop-grid-2"
  data-composition-src="compositions/loop-grid-2.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `537` | 0 to 1400, step 10 | Card size in scene units. |
| `gap` | `79` | 0px to 1000px, step 10px | Spacing between cards. |
| `zoom` | `off` | `off`, `on` | Zoom (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-grid-2-bg"></div>
        <div id="loop-grid-2-stage"><div id="loop-grid-2-plane"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-grid-2-loop").dataset.duration = DUR;
        background(document.getElementById("loop-grid-2-bg"));
        ...
        const plane = document.getElementById("loop-grid-2-plane");
        ...
        const stage = document.getElementById("loop-grid-2-stage");
        function renderAt(t) {
        ...
        window.__timelines["loop-grid-2"] = tl;
      })();
    </script>
  </body>
</html>
