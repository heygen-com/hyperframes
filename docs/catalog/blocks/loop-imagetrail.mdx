---
title: "Loop Image Trail"
description: "A head point sweeps along an arc and image cards pop in behind it as a trail. 8 image slots, 10s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-imagetrail.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-imagetrail.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-imagetrail" item="loop-imagetrail" />

That writes one file: `compositions/loop-imagetrail.html`.
...
```html index.html
<div
  data-composition-id="loop-imagetrail"
  data-composition-src="compositions/loop-imagetrail.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.19` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `trailLength` | `18` | 0 to 36, step 1 | Trail Length (engine parameter; see the block header comment). |
| `popFrom` | `50` | 0 to 100, step 1 | Pop From (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-imagetrail-bg"></div>
        <div id="loop-imagetrail-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // ImageTrail loop family — a head point sweeps along an arc; `trailLength` cards pop in
        // behind it (one per DUR/4/trailLength), each living DUR/3 then melting away. Two passes per loop, the second
        // the exact point-mirror of the first through the frame centre. Measured on the 512 preview (2026-09-08):
        //   card:    0.19*H = 97px full size (scaleIntensity); pops from popFrom% (50 -> 50px) about its centre, growth
        //            50/56/73/90/96/97 over frames +4..+24 (slow start -> the `smooth` ease over ~0.67s = DUR/15);
        //            melt 97/90/62/50/gone over ~12 frames (DUR/25) back to popFrom%, then removed; no opacity change.
        //   timing:  18 cards per pass born every 4.17 frames (2.5s = DUR/4 of emission), each alive 100-102 frames
        //            (= DUR/3); pass period DUR/2, so the previous pass's last cards still melt when the next starts.
        //   path:    head speed ~constant ALONG the curve (6.5-7.3 px/frame; x-speed 4.9 steep / 7.0 flat); the pass-2 (smile, left->right) path fits the quadratic
        //            Bezier P0=(107,313) P1=(243,576) P2=(554,320) @512 within ~5px (extreme y(0.5)=446) (samples (107,313) (163,377) (287,446)
        //            (396,413) (487,369)); pass 1 = -path (frown, right->left), (403,198)<->(109,314), (224,65)<->(288,447).
        //            Stored as offsets from the centre in H units; 16:9 keeps the H-relative path (unverified).
        //   z-order: newest card on top (DOM order = birth order).
        // Knobs: easing enum applies to grow/melt; shadow ignored (disabled by default); mediaFit natural -> cover into a
        // square card (the preview shows squares).
        ...
        const SCALE = num("scaleIntensity", 0.19);
        const TRAIL = Math.max(1, num("trailLength", 18) | 0);
        const POP = Math.min(1, Math.max(0.05, num("popFrom", 50) / 100));
        const RADIUS = (num("borderRadius", 0) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const NI = IMAGES.length;
        const SIZE = SCALE * REF;

        document.getElementById("loop-imagetrail-loop").dataset.duration = DUR;
        background(document.getElementById("loop-imagetrail-bg"));
        const stage = document.getElementById("loop-imagetrail-stage");
        ...
        // head path (pass 1 = smile, left->right), offsets from the centre in units of H
        const P0 = [-0.291, 0.111],
          P1 = [-0.024, 0.624],
          P2 = [0.582, 0.125];
        const PASSES = 2,
          PASS = DUR / PASSES,
          ...
          GROW = DUR / 15,
          MELT = DUR / 25;
        // the head moves at constant speed ALONG the curve (ref: 6.5 px/frame on the steep part, 7.3 on the flat part,
        // vs 4.9/7.0 in x) -> arc-length table, u in [0,1] over the emission window
        const bz = (s) => [
        ...
        const ARC = [0];
        {
          ...
          for (let i = 1; i <= 200; i++) {
            const q = bz(i / 200);
            ARC.push(ARC[i - 1] + Math.hypot(q[0] - prev[0], q[1] - prev[1]));
            prev = q;
          }
        }
        function headAt(u, sign) {
          const target = u * ARC[200];
          let i = 1;
          while (i < 200 && ARC[i] < target) i++;
          const s = (i - 1 + (target - ARC[i - 1]) / Math.max(1e-9, ARC[i] - ARC[i - 1])) / 200;
          const [x, y] = bz(s);
          return [sign * x * REF, sign * y * REF];
        }

        const cards = [];
        for (let p = 0; p < PASSES; p++)
          for (let i = 0; i < TRAIL; i++) {
            const m = IMAGES[(p * TRAIL + i) % NI];
            const el = document.createElement("div");
            el.className = "card";
            ...
            const birth = p * PASS + (i / TRAIL) * EMIT;
            const [x, y] = headAt(i / TRAIL, p === 0 ? -1 : 1); // pass 0 = frown right->left (mirror), pass 1 = smile
            cards.push({ el, birth, x, y });
          }
        // birth order = DOM order = z-order (newest on top); the previous loop's tail is handled by the modulo age
        function renderAt(t) {
          const now = t * DUR;
          for (const c of cards) {
            let age = now - c.birth;
            if (age < 0) age += DUR; // cards born late in the loop are still melting at t=0
            if (age < 0 || age >= LIFE) {
              c.el.style.visibility = "hidden";
              continue;
            }
            let k;
            if (age < GROW) k = POP + (1 - POP) * ease(age / GROW);
            else if (age > LIFE - MELT)
              k = POP + (1 - POP) * (1 - ease((age - (LIFE - MELT)) / MELT));
            else k = 1;
            c.el.style.visibility = "visible";
            c.el.style.transform = `translate3d(${c.x - SIZE / 2}px,${c.y - SIZE / 2}px,0) scale(${k})`;
          }
        }
        ...
        window.__timelines["loop-imagetrail"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `imagetrail`.
