---
title: "Loop Parallax Totem"
description: "Scattered image cards on a concave drum drift with mirage-like parallax. 8 image slots, 20s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-parallaxtotem.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-parallaxtotem.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-parallaxtotem" item="loop-parallaxtotem" />

That writes one file: `compositions/loop-parallaxtotem.html`.
...
```html index.html
<div
  data-composition-id="loop-parallaxtotem"
  data-composition-src="compositions/loop-parallaxtotem.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `direction` | `up` | `up`, `down` | Direction of travel. |
| `columns` | `2` | 0 to 10, step 0.1 | Columns (engine parameter; see the block header comment). |
| `scatter` | `70` | 0 to 140, step 1 | Scatter (engine parameter; see the block header comment). |
| `sizeVar` | `30` | 0 to 60, step 1 | Size Var (engine parameter; see the block header comment). |
| `parallax` | `50` | 0 to 100, step 1 | Parallax (engine parameter; see the block header comment). |
...
| `gap` | `6.75` | 0px to 14px, step 0.1px | Spacing between cards. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-parallaxtotem-bg"></div>
        <div id="loop-parallaxtotem-stage"><div id="loop-parallaxtotem-drum"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // ParallaxTotem loop family (Mirage, LoopParallaxTotemTemplate) — scattered 16:9 cards on the TotemWall drum
        // (concave cylinder, camera inside: f=1.712H, axis 0.319H in front, R=1.291H), 2 columns, drifting up in depth
        // layers. Measured on the 512 preview (2026-09-08): card heights bimodal 61–69 and 173–264 px (base card =
        // (0.74·W/2 − gap)·9/16 = 89 ⇒ size factors 0.7–2.9); screen speeds 1.4–1.8 px/frame (±13% ⇒ depth spread
        // ±12.5% = parallax/400); column x-centres 58–141 and 285–470 (scatter ±0.7·cell); 5–6 cards cross the
        // centre line per column per loop (x-scatter makes some miss the probe) ⇒ each lane holds all N images with random extra spacing.
        // Loop: the drum turns by each lane's angular period (cards keep their own radius ⇒ own speed/size) — seamless
        // by construction. Everything random comes from a fixed-seed LCG (sizes, x scatter, depth, spacing).
        // Knobs (no way to measure exactly): size-factor law (0.6 + u²·1.8 scaled by sizeVar/30; round 1 used 0.7+u²·2.2 and 4 images/lane — too sparse/big), spacing (u·0.1H),
        // depth = ±parallax/400, scatter = ±scatter/100·cell. direction=down mirrors. edgeFade not implemented.
        ...
        const COLS = Math.max(1, num("columns", 2) | 0);
        const SCATTER = num("scatter", 70) / 100;
        const SIZEVAR = num("sizeVar", 30) / 30;
        const PARA = num("parallax", 50) / 400;
        const PAD = num("padding", 13) / 100;
        const CURV = Math.max(1, Math.abs(num("curvature", -100)));
        const GAP = num("gap", 6.75) * 0.0094 * REF;
        ...
        document.getElementById("loop-parallaxtotem-loop").dataset.duration = DUR;
        background(document.getElementById("loop-parallaxtotem-bg"));

        let seed = 12345; // fixed-seed LCG: deterministic per frame and per render
        const rnd = () => (seed = (seed * 1664525 + 1013904223) >>> 0) / 4294967296;

        const f = 1.712 * REF,
          R = (1.291 * REF * 100) / CURV,
          MAG = 1.064;
        const DEPTH = f / MAG; // camera -> wall centre
        const stage = document.getElementById("loop-parallaxtotem-stage");
        stage.style.perspective = f + "px";
        const drum = document.getElementById("loop-parallaxtotem-drum");
        drum.style.transform = `translate3d(0,0,${f - (DEPTH - R)}px)`; // origin = drum axis

        const cellX = ((1 - 2 * PAD) * SPAN) / COLS;
        const baseW = Math.max(1, cellX - GAP);
        const RAD = 180 / Math.PI;
        const lanes = [];
        for (let i = 0; i < COLS; i++) {
          const x0 = (i - (COLS - 1) / 2) * cellX;
          const items = [];
          let ang = 0;
          for (let k = 0; k < N; k++) {
            const m = IMAGES[(k + i * Math.floor(N / COLS)) % N]; // every lane shows all N images, rotated start
            const sz = 0.6 + Math.pow(rnd(), 2) * 1.8 * SIZEVAR;
            const w = baseW * sz,
              h = w / aspect(m, FIT, CROP_AR);
            const Ri = R * (1 + (rnd() - 0.5) * 2 * PARA); // depth layer: bigger radius = farther = smaller & slower
            const x = x0 + ((rnd() - 0.5) * 2 * SCATTER * cellX) / 2;
            const pitch = h + GAP + rnd() * 0.1 * REF; // world px along the drum
            const el = card(m, w, h, RADIUS);
            el.style.left = -w / 2 + "px";
            el.style.top = -h / 2 + "px";
            drum.appendChild(el);
            items.push({ el, x, Ri, ang: ang + pitch / R / 2 });
            ang += pitch / R;
          }
          const period = ang; // radians per lane loop
          for (const it of items) it.ang -= period / 2; // centre the lane
          lanes.push({ items, period });
        }
        function renderAt(t) {
          for (const L of lanes) {
            const shift = DIR * L.period * t; // up = angle decreasing
            for (const it of L.items) {
              const a = frac((it.ang - shift) / L.period + 0.5) * L.period - L.period / 2;
              if (Math.abs(a) > 1.7) {
                it.el.style.display = "none";
                continue;
              }
              it.el.style.display = "";
              it.el.style.transform = `translateX(${it.x}px) rotateX(${a * RAD}deg) translateZ(${-it.Ri}px)`;
            }
          }
        }
        ...
        window.__timelines["loop-parallaxtotem"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `parallaxtotem` `3d`.
