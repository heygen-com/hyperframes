---
title: "Loop Orbit Showcase"
description: "Upright rounded image cards on a screen-space ellipse, the front card featured. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbitshowcase.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbitshowcase.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-orbitshowcase" item="loop-orbitshowcase" />

That writes one file: `compositions/loop-orbitshowcase.html`.
...
```html index.html
<div
  data-composition-id="loop-orbitshowcase"
  data-composition-src="compositions/loop-orbitshowcase.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.22` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `ringWidth` | `64` | 0 to 128, step 1 | Ring Width (engine parameter; see the block header comment). |
| `ringHeight` | `26` | 0 to 52, step 1 | Ring Height (engine parameter; see the block header comment). |
| `ringTilt` | `0` | 0 to 2, step 0.01 | Ring Tilt (engine parameter; see the block header comment). |
| `spread` | `100` | 0 to 200, step 1 | Spread (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-orbitshowcase-bg {
        position: absolute;
        inset: 0;
      }
      #loop-orbitshowcase-stage {
        position: absolute;
        left: 50%;
        top: 50%;
        width: 0;
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-orbitshowcase-bg"></div>
        <div id="loop-orbitshowcase-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Ring loop family — N upright rounded cards on a screen-space ellipse, front
        // (bottom) card biggest, back (top) card smallest and faded. Continuous linear spin, 1 revolution per loop,
        // front card moves right (direction=right).
        // Measured on the 512 preview, frame 0, 8 cards (phi=0 front): centre y = 255 + 59cos(phi) (symmetric to 1px),
        // centre x = 144 sin(phi) (1 + 0.095 cos(phi)) (45deg: 109, 90deg: 144, 135deg: 95), size = 106 (1 + 0.264 cos(phi))
        // (134/126/106/87/78 px). 144 = ringWidth% * (1 - 2*padding%) * H/2, 59 = ringHeight% * 0.88 * H/2 — same padding
        // law as PhotoOrbit's radius. Front card crosses one pitch in 45 f = DUR/N, linear. Back card luma ~0.5 of its
        // front value -> opacity 1 - backFade% * (1-cos)/2.
        // Guesses: base card size 0.207*H*(spread/100); the x-only foreshortening 0.095 scaled by 55/perspective; size
        // factor = 1.2*scaleIntensity (0.264 measured vs 0.22 setting); motion=pulse treated as linear; ringTilt rotates
        // the ellipse (cards stay upright); shadow units = % of card.
        ...
        const A = ((num("ringWidth", 64) / 100) * (1 - 2 * PAD) * REF) / 2;
        const B = ((num("ringHeight", 26) / 100) * (1 - 2 * PAD) * REF) / 2;
        const TILT = num("ringTilt", 0);
        const SIZE = (0.207 * REF * num("spread", 100)) / 100;
        const KS = 1.2 * num("scaleIntensity", 0.22);
        const KX = (0.095 * 55) / Math.max(1, num("perspective", 55));
        const FADE = Math.max(0, Math.min(1, num("backFade", 45) / 100));
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const SHADOW = str("shadowEnabled", "false") === "true";
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;

        document.getElementById("loop-orbitshowcase-loop").dataset.duration = DUR;
        background(document.getElementById("loop-orbitshowcase-bg"));
        const stage = document.getElementById("loop-orbitshowcase-stage");
        ...
        for (let i = 0; i < N; i++) {
          ...
          c.style.top = -h / 2 + "px";
          if (SHADOW)
            c.style.boxShadow = `${(num("shadowX", 0) / 100) * SIZE}px ${(num("shadowY", 1) / 100) * SIZE}px ${(num("shadowBlur", 1.5) / 100) * SIZE}px ${(num("shadowSpread", -0.25) / 100) * SIZE}px ${str("shadowColor", "#000")}${Math.round(
              (num("shadowOpacity", 40) / 100) * 255,
            )
              .toString(16)
              .padStart(2, "0")}`;
          stage.appendChild(c);
          cards.push({ el: c, theta: (i * 2 * Math.PI) / N });
        }
        function renderAt(t) {
          const spin = DIR * frac(t) * 2 * Math.PI;
          for (const c of cards) {
            const phi = c.theta + spin,
              cs = Math.cos(phi),
              sn = Math.sin(phi);
            const x = A * sn * (1 + KX * cs),
              y = B * cs,
              s = 1 + KS * cs;
            c.el.style.transform = `translate(${x}px,${y}px) scale(${s})`;
            c.el.style.zIndex = Math.round(1000 + 500 * cs);
            c.el.style.opacity = 1 - (FADE * (1 - cs)) / 2;
          }
        }
        ...
        window.__timelines["loop-orbitshowcase"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `orbitshowcase` `3d`.
