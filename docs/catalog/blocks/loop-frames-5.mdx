...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-5.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-5.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-frames-5" item="loop-frames-5" />

That writes one file: `compositions/loop-frames-5.html`.
...
```html index.html
<div
  data-composition-id="loop-frames-5"
  data-composition-src="compositions/loop-frames-5.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `tilt` | `-15` | -180° to 180°, step 1° | Tilt (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-frames-5-bg"></div>
        <div id="loop-frames-5-sheet"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-frames-5-loop").dataset.duration = DUR;
        background(document.getElementById("loop-frames-5-bg"));
        const sheet = document.getElementById("loop-frames-5-sheet");
        ...
        window.__timelines["loop-frames-5"] = tl;
      })();
    </script>
  </body>
</html>
