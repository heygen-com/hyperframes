---
title: "Loop Spin 3"
description: "One image card at the frame centre turns on the spot, showing the next image each half turn. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spin-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spin-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spin-3" item="loop-spin-3" />

That writes one file: `compositions/loop-spin-3.html`.
...
```html index.html
<div
  data-composition-id="loop-spin-3"
  data-composition-src="compositions/loop-spin-3.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `imageScale` | `60` | 0 to 130, step 1 | Image Scale (engine parameter; see the block header comment). |
...
| `axis` | `y` | `y`, `x` | Axis (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spin-3-bg"></div>
        <div id="loop-spin-3-stage"><div id="loop-spin-3-pivot"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-spin-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spin-3-bg"));
        const f = 1.33 * REF * (50 / PERSP);
        document.getElementById("loop-spin-3-stage").style.perspective = f + "px";
        const pivot = document.getElementById("loop-spin-3-pivot");
        ...
        window.__timelines["loop-spin-3"] = tl;
      })();
    </script>
  </body>
</html>
