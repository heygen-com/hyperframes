---
title: "Loop Book 3"
description: "A book-page spread whose pages turn one after another to reveal each image. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-book-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-book-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-book-3" item="loop-book-3" />

That writes one file: `compositions/loop-book-3.html`.
...
```html index.html
<div
  data-composition-id="loop-book-3"
  data-composition-src="compositions/loop-book-3.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `lightIntensity` | `100` | 0 to 250, step 10 | Light Intensity (engine parameter; see the block header comment). |
| `lightAngle` | `30` | 0 to 452, step 10 | Light Angle (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-book-3-bg"></div>
        <div id="loop-book-3-stage">
          <div id="loop-book-3-book"><div id="loop-book-3-spin"></div></div>
        </div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-book-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-book-3-bg"));
        const f = 1000 * (100 / PERSP) * (REF / 1080),
          s = f / D;
        document.getElementById("loop-book-3-stage").style.perspective = f + "px";
        const book = document.getElementById("loop-book-3-book"),
          spin = document.getElementById("loop-book-3-spin");
        ...
        window.__timelines["loop-book-3"] = tl;
      })();
    </script>
  </body>
</html>
