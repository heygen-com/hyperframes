---
title: "Loop Wheel Spin"
description: "Image cards on a 2D wheel centred in the frame, each rotated to its spoke, spin. 8 image slots, 14s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheelspin.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheelspin.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-wheelspin" item="loop-wheelspin" />

That writes one file: `compositions/loop-wheelspin.html`.
...
```html index.html
<div
  data-composition-id="loop-wheelspin"
  data-composition-src="compositions/loop-wheelspin.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.26` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `wheelSize` | `92` | 0% to 100%, step 1% | Wheel Size (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-wheelspin-bg"></div>
        <div id="loop-wheelspin-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Dial loop family — N cards on a 2D wheel centred in the frame, each rotated to its
        // ring angle (top faces outward), the wheel spinning continuously and linearly, `rotations` turns per loop.
        // Measured (512 preview, 8 cards): card 133 px = scaleIntensity*H (0.26 -> 133.1); wheel outer extent 188 px and
        // hole radius 55 px -> ring radius 121.5 px; spin 25.5deg per 30 frames = 357deg/420 f = 1 rev per loop, clockwise
        // for direction=right. Cards overlap in a pinwheel (z-order = slot index).
        // Radius law: 121.5 px = 0.92 * 133 -> R = wheelSize% * cardSize (fits 0.8%); the sibling families (WheelCarousel,
        // WheelSpinBottom) use R = wheelSize% * H instead — per-family law, flagged.
        ...
        const SIZE = num("scaleIntensity", 0.26) * REF; // card height
        const R = (num("wheelSize", 92) / 100) * SIZE; // ring radius (card-relative, see header)
        ...
        document.getElementById("loop-wheelspin-loop").dataset.duration = DUR;
        background(document.getElementById("loop-wheelspin-bg"));
        const stage = document.getElementById("loop-wheelspin-stage");
        ...
        function renderAt(t) {
          ...
          for (const c of cards) {
            const phi = c.theta + spin,
              r = (phi * Math.PI) / 180;
            let tr = `translate(${R * Math.sin(r)}px,${-R * Math.cos(r)}px) rotate(${phi}deg)`;
        ...
        window.__timelines["loop-wheelspin"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `wheelspin` `3d`.
