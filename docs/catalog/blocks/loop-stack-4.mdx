---
title: "Loop Stack 4"
description: "Image cards deal onto a receding stack. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-stack-4.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-stack-4.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-stack-4" item="loop-stack-4" />

That writes one file: `compositions/loop-stack-4.html`.
...
```html index.html
<div
  data-composition-id="loop-stack-4"
  data-composition-src="compositions/loop-stack-4.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `stagger` | `0` | 0 to 2, step 0.01 | Stagger (engine parameter; see the block header comment). |
| `delay` | `0` | 0s to 2s, step 0.01s | Hold time between steps, seconds. |
| `direction` | `up` | `down`, `up` | Direction of travel. |
| `planeSize` | `24` | 0 to 76, step 1 | Card size in scene units. |
| `visible` | `6` | 0 to 12, step 0.1 | Visible (engine parameter; see the block header comment). |
| `perspective` | `123` | 0 to 246, step 10 | Camera focal length: smaller = stronger perspective. |
| `zoom` | `267` | 0 to 534, step 10 | Zoom (engine parameter; see the block header comment). |
| `offsetX` | `0` | -100% to 100%, step 1% | Horizontal offset of the formation, % of width. |
| `offsetY` | `7.5` | -100% to 100%, step 1% | Vertical offset of the formation, % of height. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-stack-4-bg"></div>
        <div id="loop-stack-4-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-stack-4-loop").dataset.duration = DUR;
        background(document.getElementById("loop-stack-4-bg"));
        const stage = document.getElementById("loop-stack-4-stage");
        ...
        window.__timelines["loop-stack-4"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `stack` `3d`.
