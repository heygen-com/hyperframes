---
title: "Loop Carousel Flow"
description: "A stepped belt of cards; the centred card is full size and its neighbours shrink away. 8 image slots, 10s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-carouselflow.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-carouselflow.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-carouselflow" item="loop-carouselflow" />

That writes one file: `compositions/loop-carouselflow.html`.
...
```html index.html
<div
  data-composition-id="loop-carouselflow"
  data-composition-src="compositions/loop-carouselflow.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `sideScale` | `0.82` | 0 to 2, step 0.01 | Side Scale (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-carouselflow-bg {
        position: absolute;
        inset: 0;
      }
      #loop-carouselflow-stage {
        position: absolute;
        left: 50%;
        top: 50%;
        width: 0;
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-carouselflow-bg"></div>
        <div id="loop-carouselflow-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // CarouselFlow loop family — a stepped belt; the centred card is full size, neighbours shrink.
        // Read off the preview (2026-09-08, 10s, 8 placeholders => 8 steps of 37.5f):
        //   centre card cross size = H − 2·padding% (padding 6 -> 31px margins, card 450px @512 ✓); gap 5 = 5% of H (26px ✓).
        //   layout UNSCALED (pitch = card + gap: f8 centres 476px apart), scale about the centre, linear 1 -> sideScale
        //     at one pitch (f8: d=-203 -> .924 measured / .923 model; d=273 -> .898 / .897).
        //   step: move over frames 2..15 of 37.5 = window [0.053, 0.40] of the step, ease cubic-bezier(.5,0,.5,1)
        //     ('smooth', maxerr 0.017), then hold. Belt moves toward negative x (new card enters from the right).
        //   borderRadius 32 -> 15px @512 ✓ (px at 1080).
        // Guesses: easing names other than smooth/custom/linear (all mapped to smooth), shadow units (% of H),
        //   vertical direction (axes swapped), move window as a fraction of the step (could be fixed 0.43s).
        ...
        const HORIZ = str("direction", "horizontal") === "horizontal";
        const SIDE = num("sideScale", 0.82);
        const GAP = (num("gap", 5) / 100) * REF;
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const CROSS = REF - 2 * PAD;

        // cubic-bezier easing (CSS semantics); 'custom' reads easeBezier "x1,y1,x2,y2"
        ...
        document.getElementById("loop-carouselflow-loop").dataset.duration = DUR;
        background(document.getElementById("loop-carouselflow-bg"));
        const stage = document.getElementById("loop-carouselflow-stage");
        const items = IMAGES.map((m) => {
          const a = aspect(m, FIT, CROP_AR);
          const along = HORIZ ? CROSS * a : CROSS / a;
          const w = HORIZ ? along : CROSS,
            h = HORIZ ? CROSS : along;
        ...
        const MOVE0 = 0.053,
          MOVE = 0.347; // move window, fraction of the step

        function renderAt(t) {
          const s = Math.min(N - 1e-9, t * N),
            si = Math.floor(s),
            tl = s - si;
          const u = ease((tl - MOVE0) / MOVE);
          const travel = cum[si] + u * items[si].pitch;
          for (let k = 0; k < N; k++) {
            const d = wrap(cum[k] - travel);
            const sc = 1 - (1 - SIDE) * Math.min(1, Math.abs(d) / PITCH);
            items[k].el.style.transform =
              (HORIZ ? `translate3d(${d}px,0,0)` : `translate3d(0,${d}px,0)`) + ` scale(${sc})`;
          }
        }
        ...
        window.__timelines["loop-carouselflow"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `carouselflow` `carousel`.
