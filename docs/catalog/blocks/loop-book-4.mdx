...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-book-4.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-book-4.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-book-4" item="loop-book-4" />

That writes one file: `compositions/loop-book-4.html`.
...
```html index.html
<div
  data-composition-id="loop-book-4"
  data-composition-src="compositions/loop-book-4.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `finish` | `glossy` | `flat`, `matte`, `glossy` | Finish (engine parameter; see the block header comment). |
...
| `rotationX` | `-17` | -180° to 180°, step 1° | Formation tilt about the horizontal axis, degrees. |
| `rotationY` | `-27` | -180° to 180°, step 1° | Formation turn about the vertical axis, degrees. |
| `rotationZ` | `-11` | -180° to 180°, step 1° | Formation roll about the view axis, degrees. |
...
| `lightIntensity` | `120` | 0 to 250, step 10 | Light Intensity (engine parameter; see the block header comment). |
| `lightAngle` | `226` | 0 to 452, step 10 | Light Angle (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-book-4-bg"></div>
        <div id="loop-book-4-stage">
          <div id="loop-book-4-book"><div id="loop-book-4-spin"></div></div>
        </div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-book-4-loop").dataset.duration = DUR;
        background(document.getElementById("loop-book-4-bg"));
        const f = 1000 * (100 / PERSP) * (REF / 1080),
          s = f / D;
        document.getElementById("loop-book-4-stage").style.perspective = f + "px";
        const book = document.getElementById("loop-book-4-book"),
          spin = document.getElementById("loop-book-4-spin");
        ...
        window.__timelines["loop-book-4"] = tl;
      })();
    </script>
  </body>
</html>
