---
title: "Loop Tour Room 1"
description: "A tour through a room of hanging image frames. 8 image slots, 23s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-tour01-1.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-tour01-1.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-tour01-1" item="loop-tour01-1" />

That writes one file: `compositions/loop-tour01-1.html`.

## Add it to your video

It runs for 23 seconds at 1080×1080. Paste this into your composition:

```html index.html
<div
  data-composition-id="loop-tour01-1"
  data-composition-src="compositions/loop-tour01-1.html"
  data-start="0"
  data-duration="23"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `55` | 0 to 110, step 1 | Card size in scene units. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-tour01-1-bg"></div>
        <div id="loop-tour01-1-stage"><div id="loop-tour01-1-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Tour loop family.
        // tour02.html is generated from this file by flipping BILLBOARD — keep the two in sync.
        // Mechanism read off the previews (2026-09-08): the camera visits the N images in turn; each image is
        // centred while it grows to its peak and dwells there, then the camera swings to the next one (Tour01 dense
        // sheet: 8 hero episodes in 23s = one per image; hero centred cx=cy=256±1 for 25-30 frames per episode).
        // Tour01 cards stay axis-aligned squares -> billboards; Tour02 cards keep a fixed orientation (they
        // foreshorten / go edge-on as the camera swings, aspect w/h 1.0 -> 0.14 on one track) -> fixed yaw ±35° per card.
        // Measured: hero peak width 195..242px @512 (median 215 = 0.42·H; two episodes pass through the card, 512px);
        // other cards 20..80px while a hero dwells => the next cards sit ≈ 3.5 / 7 / 10 dwell-distances ahead.
        // Model (guess): cards along a forward path, one every 3.5 units (±0.3 jitter), alternating sides ±0.75 / ±0.5, the field repeating every N cards so the forward tour is seamless; camera keyframe i = 1 unit
        // in front of card i; between keyframes the LATERAL move eases first (done at 72% of the segment) and the depth
        // move lags (starts at 28%): the old hero slides out sideways instead of being flown through, and the next hero is
        // already centred while it grows (ref: cx = 256 for the last ~1/3 of every episode); the camera yaws toward its
        // travel direction mid-segment and back to straight at each dwell (Tour02's oblique panels).
        // Failed rounds: (1) cards on a ring, camera inside looking outward — neighbours sat beside/behind the camera;
        // (2) lateral scatter ±2 — neighbours landed outside the frame and the camera flew through each hero (flash).
        // Knobs: `fade` = per-distance opacity (0 in both previews), `delay` = extra hold fraction (0), `cycles` = tours
        // per loop, planeSize 55 -> 0.42·H peak (linear), direction reverse = walk the path backwards.
        const BILLBOARD = true; // tour01: cards face the camera; tour02: cards face down the path (fixed)
        const __V = Object.assign(
          {},
          (window.__hyperframes && window.__hyperframes.getVariables
            ? window.__hyperframes.getVariables()
            : null) || {},
          { loopDuration: 23, loopRepeat: 2, imageCount: 8 },
        );
        const __PH = [
        ...
        document.getElementById("loop-tour01-1-loop").dataset.duration = DUR;
        background(document.getElementById("loop-tour01-1-bg"));
        const f = 1000 * (REF / 1080),
          U = f; // px per world unit: a unit-size card at distance 1 spans f px
        document.getElementById("loop-tour01-1-stage").style.perspective = f + "px";
        const world = document.getElementById("loop-tour01-1-world");

        const S = 0.42 * (REF / f) * (PS / 55); // card size, world units (=> 0.42·H at the dwell)
        const PITCH = 3.5,
          SPAN = N * PITCH; // one card every 3.5 dwell distances; field repeats every SPAN
        let seed = 7;
        const rnd = () => (seed = (seed * 1664525 + 1013904223) >>> 0) / 4294967296;
        const P = []; // card positions (one period)
        // consecutive cards alternate sides (Δlateral ≈ 1.8 units): the previous hero slides out instead of being flown through,
        // and the next card sits ~200px off-centre at 3.5 units (ref: neighbours 40-80px inside the frame while a hero dwells)
        for (let i = 0; i < N; i++)
          P.push({
            x: 0.75 * (i % 2 ? 1 : -1) + (rnd() - 0.5) * 0.4,
            y: 0.5 * ((i >> 1) % 2 ? 1 : -1) + (rnd() - 0.5) * 0.4,
            z: i * PITCH + (rnd() - 0.5) * 0.6,
          });
        const slots = []; // 3 periods of cards so the field is continuous ahead and behind
        for (let k = -1; k <= 1; k++)
          for (let i = 0; i < N; i++) {
            const m = IMAGES[i],
              a = aspect(m, FIT, CROP_AR);
            const cw = S * U * Math.min(1, a),
              ch = S * U * Math.min(1, 1 / a);
            const slot = document.createElement("div");
            slot.className = "slot";
            const c = card(m, cw, ch, RADIUS);
            ...
            slots.push({
              ...
              y: P[i].y,
              z: P[i].z + k * SPAN,
              yaw: (frac(i * 0.618) - 0.5) * 70,
            }); // fixed yaw (tour02): ±35°
          }
        const key = (i) => {
          const q = P[((i % N) + N) % N];
          return { x: q.x, y: q.y, z: q.z + Math.floor(i / N) * SPAN - 1 };
        };
        const lerp = (a, b, e) => a + (b - a) * e;
        function renderAt(t) {
          const u = DIR * frac(t) * CYCLES * N,
            i = Math.floor(u),
            g = u - i;
          const gg = Math.min(1, Math.max(0, (g - HOLD / 2) / (1 - HOLD)));
          const e = expoInOut(gg); // yaw progress
          const eL = expoInOut(Math.min(1, gg / 0.72)),
            eZ = expoInOut(Math.max(0, (gg - 0.28) / 0.72)); // lateral leads, depth lags
          const A = key(i),
            B = key(i + 1);
          const C = { x: lerp(A.x, B.x, eL), y: lerp(A.y, B.y, eL), z: lerp(A.z, B.z, eZ) };
          const psi = Math.atan2(B.x - A.x, B.z - A.z) * 0.8 * Math.sin(Math.PI * e); // look where you go, straight at the dwell
          world.style.transform = `translateZ(${f}px) rotateY(${(psi * 180) / Math.PI}deg) translate3d(${-C.x * U}px,${-C.y * U}px,${C.z * U}px)`;
          const sn = Math.sin(psi),
            cs = Math.cos(psi);
          for (const k of slots) {
            const rx = k.x - C.x,
              rz = k.z - C.z,
              zc = rx * sn + rz * cs; // camera-space depth
            if (zc < 0.25) {
              k.slot.style.display = "none";
              continue;
            }
            k.slot.style.display = "";
            const theta = BILLBOARD ? (-psi * 180) / Math.PI : k.yaw;
            k.slot.style.transform = `translate3d(${k.x * U}px,${k.y * U}px,${-k.z * U}px) rotateY(${theta}deg)`;
            if (FADE > 0) k.c.style.opacity = Math.max(0, 1 - (FADE * (zc - 1)) / SPAN).toFixed(3);
          }
        }
        ...
        window.__timelines["loop-tour01-1"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `tour` `3d`.
