...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheel-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheel-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-wheel-3" item="loop-wheel-3" />

That writes one file: `compositions/loop-wheel-3.html`.
...
```html index.html
<div
  data-composition-id="loop-wheel-3"
  data-composition-src="compositions/loop-wheel-3.html"
  data-start="0"
  data-duration="13"
  data-track-index="1"
  data-width="1080"
...
| `ringRadius` | `784` | 0 to 1980, step 10 | Ring Radius (engine parameter; see the block header comment). |
| `thumbSize` | `158` | 0 to 450, step 10 | Thumb Size (engine parameter; see the block header comment). |
| `offsetX` | `0` | -100% to 100%, step 1% | Horizontal offset of the formation, % of width. |
| `offsetY` | `48` | -100% to 100%, step 1% | Vertical offset of the formation, % of height. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-wheel-3-bg"></div>
        <div id="loop-wheel-3-big"></div>
        <div id="loop-wheel-3-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-wheel-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-wheel-3-bg"));
        const stage = document.getElementById("loop-wheel-3-stage");
        const big = document.getElementById("loop-wheel-3-big");
        const bigImgs = [];
        if (BIG)
        ...
        window.__timelines["loop-wheel-3"] = tl;
      })();
    </script>
  </body>
</html>
