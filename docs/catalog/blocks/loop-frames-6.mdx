...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-6.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-6.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-frames-6" item="loop-frames-6" />

That writes one file: `compositions/loop-frames-6.html`.
...
```html index.html
<div
  data-composition-id="loop-frames-6"
  data-composition-src="compositions/loop-frames-6.html"
  data-start="0"
  data-duration="15"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `20` | 0 to 200, step 1 | Card size in scene units. |
| `gap` | `107` | 0px to 214px, step 10px | Spacing between cards. |
...
| `rowsSkipped` | `1` | 0 to 10, step 0.1 | Rows Skipped (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-frames-6-bg"></div>
        <div id="loop-frames-6-sheet"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-frames-6-loop").dataset.duration = DUR;
        background(document.getElementById("loop-frames-6-bg"));
        const sheet = document.getElementById("loop-frames-6-sheet");
        ...
        window.__timelines["loop-frames-6"] = tl;
      })();
    </script>
  </body>
</html>
