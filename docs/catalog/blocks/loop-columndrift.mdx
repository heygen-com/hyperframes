---
title: "Loop Column Drift"
description: "Three columns of square image cards drift vertically inside a padded mask. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-columndrift.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-columndrift.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-columndrift" item="loop-columndrift" />

That writes one file: `compositions/loop-columndrift.html`.
...
```html index.html
<div
  data-composition-id="loop-columndrift"
  data-composition-src="compositions/loop-columndrift.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-columndrift-bg"></div>
        <div id="loop-columndrift-mask"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Drift loop family — three columns of square cards inside a padded mask, drifting at
        // constant speed in counter-flow (outer columns DOWN, middle UP). Measured on the 512x512 preview (2026-09-08):
        //   padding 32px, gap 16px (padding 6 / gap 3 -> H/96 per unit), cards 140px squares (3 columns fill the width,
        //   3 rows fill the height exactly), cards clipped at the padding edge;
        //   speed 1.29 px/frame constant (l74 card y 32 @f0 -> 148 @f90) = 3 cards (3 x 156 = 468px) per 12s loop;
        //   column c shows images c*3.. c*3+2 repeating (period 3 cards; image 9 wraps to image 1 with 8 images).
        // Guesses: card side from the HEIGHT (3 rows) at 16:9, 3 columns centred; natural fit follows the image aspect
        //   along the column (period = sum of item pitches).
        ...
        const PAD = (num("padding", 6) * REF) / 96;
        ...
        document.getElementById("loop-columndrift-loop").dataset.duration = DUR;
        background(document.getElementById("loop-columndrift-bg"));
        const mask = document.getElementById("loop-columndrift-mask");
        const MW = W - 2 * PAD,
          MH = H - 2 * PAD;
        mask.style.cssText = `left:${PAD}px;top:${PAD}px;width:${MW}px;height:${MH}px;`;
        const SIDE = (MH - (ROWS - 1) * GAP) / ROWS; // square side; 3 rows fill the mask height
        const x0 = (MW - (COLS * SIDE + (COLS - 1) * GAP)) / 2;

        const lanes = [];
        for (let c = 0; c < COLS; c++) {
          const items = [];
          for (let k = 0; k < ROWS; k++) {
            const m = IMAGES[(c * ROWS + k) % N];
            items.push({ m, h: SIDE / aspect(m, FIT, CROP_AR) });
          }
          const P = items.reduce((s, it) => s + it.h + GAP, 0);
          const reps = Math.ceil(MH / P) + 2;
          const el = document.createElement("div");
          el.className = "lane";
          el.style.cssText = `left:${x0 + c * (SIDE + GAP)}px;width:${SIDE}px;height:${reps * P}px;`;
          let y = 0;
          for (let r = 0; r < reps; r++)
            for (const it of items) {
              const cd = card(it.m, SIDE, it.h, RADIUS);
              cd.style.left = "0px";
              cd.style.top = y + "px";
              el.appendChild(cd);
              y += it.h + GAP;
          ...
          mask.appendChild(el);
          lanes.push({ el, P, dir: c % 2 === 0 ? 1 : -1 }); // even columns down, middle up
        }
        function renderAt(t) {
          for (const L of lanes) {
            const sh = frac(L.dir * frac(t)) * L.P; // one period per loop
            L.el.style.transform = `translate3d(0,${sh - L.P}px,0)`;
          }
        }
        ...
        window.__timelines["loop-columndrift"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `columndrift`.
