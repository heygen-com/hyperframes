---
title: "Loop Grid Reveal"
description: "A padded mosaic grid assembles tile by tile, holds, then disperses. 8 image slots, 6s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-gridreveal.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-gridreveal.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-gridreveal" item="loop-gridreveal" />

That writes one file: `compositions/loop-gridreveal.html`.
...
```html index.html
<div
  data-composition-id="loop-gridreveal"
  data-composition-src="compositions/loop-gridreveal.html"
  data-start="0"
  data-duration="6"
  data-track-index="1"
  data-width="1080"
...
| `order` | `row` | `row`, `column`, `diagonal` | Order (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-gridreveal-bg"></div>
        <div id="loop-gridreveal-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Mosaic loop family — a padded grid assembles tile by tile, holds, then disperses.
        // Measured on the 512 preview (2026-09-08, per-slot bright-bbox + luma per frame):
        //   layout:  padding 6 -> 31px = 6% of frame, gap 3 -> 15.4px = 3%; 8 images -> 3x3 cells of 140px (last cell empty).
        //   fill:    tile i (row-major) first visible at f6 + 6.5*i (0.217s stagger); each tile grows 0.75->1.0
        //            (110->140px over 8 frames, ease-out) while fading in (alpha .34,.5,.69,.82,.93,.97,.99 = cubicOut).
        //            Last tile complete at 2.0s = DUR/3. Hold to 4.0s. Clear is the exact time mirror: state(t) = state(DUR-t).
        //   timing model: d = DUR/20 (0.3s), t0 = DUR/36 (0.167s), stagger = (DUR/3 - d - t0)/(n-1) = 0.219s.
        // Knobs: easing/easeBezier NOT wired (measured curve is cubic-out, not the "smooth" in-out); shadow units = 1% of H
        // (guess); order=column/diagonal orderings are guesses (only row measured).
        ...
        document.getElementById("loop-gridreveal-loop").dataset.duration = DUR;
        background(document.getElementById("loop-gridreveal-bg"));
        const stage = document.getElementById("loop-gridreveal-stage");
        ...
        for (let i = 0; i < N; i++) {
          ...
          const m = IMAGES[i],
            a = aspect(m, FIT, "1:1");
          // crop fills the cell; natural fits inside it
          let w = CW,
            h = CH;
          if (FIT === "natural") {
            if (CW / CH > a) w = CH * a;
        ...
        const key =
          ORDER === "column"
            ...
            : ORDER === "diagonal"
              ? (t) => (t.c + t.r) * 100 + t.r
              : (t) => t.r * 100 + t.c;
        const order = tiles.slice().sort((a, b) => key(a) - key(b));
        const d = DUR / 20,
          t0 = DUR / 36,
          stagger = N > 1 ? (DUR / 3 - d - t0) / (N - 1) : 0;
        const cubicOut = (u) => 1 - Math.pow(1 - u, 3);
        function renderAt(t) {
          let T = frac(t) * DUR;
          if (T > DUR / 2) T = DUR - T; // clear = mirrored fill
          order.forEach((tile, i) => {
            const u = Math.max(0, Math.min(1, (T - t0 - i * stagger) / d)),
              e = cubicOut(u);
            tile.el.style.opacity = e;
            tile.el.style.transform = `scale(${0.75 + 0.25 * e})`;
          });
        }
        ...
        window.__timelines["loop-gridreveal"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `gridreveal`.
