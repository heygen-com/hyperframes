...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-2.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-2.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spiral-2" item="loop-spiral-2" />

That writes one file: `compositions/loop-spiral-2.html`.
...
```html index.html
<div
  data-composition-id="loop-spiral-2"
  data-composition-src="compositions/loop-spiral-2.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `fade` | `43` | 0 to 100, step 1 | Fade (engine parameter; see the block header comment). |
...
| `count` | `72` | 0 to 240, step 10 | Count (engine parameter; see the block header comment). |
| `radius` | `1435` | 0 to 6000, step 10 | Radius (engine parameter; see the block header comment). |
| `turns` | `3.5` | 0 to 20, step 0.1 | Turns (engine parameter; see the block header comment). |
...
| `planeSize` | `265` | 0 to 1620, step 10 | Card size in scene units. |
| `distance` | `3150` | 0 to 9300, step 10 | Camera distance from the formation. |
| `perspective` | `100` | 0 to 600, step 10 | Camera focal length: smaller = stronger perspective. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spiral-2-bg"></div>
        <div id="loop-spiral-2-stage"><div id="loop-spiral-2-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-spiral-2-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spiral-2-bg"));
        ...
        const stage = document.getElementById("loop-spiral-2-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-spiral-2-world");
        ...
        window.__timelines["loop-spiral-2"] = tl;
      })();
    </script>
  </body>
</html>
