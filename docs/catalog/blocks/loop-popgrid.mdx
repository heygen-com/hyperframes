---
title: "Loop Pop Grid"
description: "A padded grid of image tiles; each tile pops out and back on its own phase. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-popgrid.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-popgrid.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-popgrid" item="loop-popgrid" />

That writes one file: `compositions/loop-popgrid.html`.
...
```html index.html
<div
  data-composition-id="loop-popgrid"
  data-composition-src="compositions/loop-popgrid.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
| `visible` | `62` | 0 to 124, step 1 | Visible (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-popgrid-bg"></div>
        <div id="loop-popgrid-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Pop loop family — padded grid; each tile pops out and back in once per loop on its own phase.
        // Measured on the 512 preview (2026-09-08, per-slot bright-bbox width + luma every 6 frames):
        //   layout: padding 6% (31px), gap 3% (15px); 8 images -> 3x3 cells of 140px, last empty.
        //   cycle per tile: full -> shrink 1.0->0 LINEAR over ~1.5s (slot3: .95@1.2s .82 .69 .56 .42 .29 .15 0@2.6s), alpha 1
        //           until scale ~0.4 then linear to 0 -> ~0.15s gap -> pop-in ~1.4s: 0->1.06 in 0.4s, overshoot 1.09 held
        //           to ~0.55, settles to 1.0 by 1.4s; alpha 0->1 linear over the first 0.8s.
        //   `visible` 62 = % of the loop at full scale: (1-0.62)*8 = 3.04s = 1.52 shrink + 0.15 gap + 1.37 pop (matches).
        //   phases (shrink start / DUR) per slot 0..7: .225 .375 .95 .15 .675 .30 .55 .20 (their RNG; hard-coded for 8 slots,
        //   golden-ratio spread for other counts).
        // Knobs: pop-in curve shape is a hand fit; luma-based alpha has ±0.1 uncertainty; shadow not in this family.
        ...
        const VIS = Math.max(0, Math.min(99, num("visible", 62))) / 100;
        ...
        const PHASES8 = [0.225, 0.375, 0.95, 0.15, 0.675, 0.3, 0.55, 0.2];
        const phase = (i) => (N === 8 ? PHASES8[i] : frac(0.15 + i * 0.6180339887));

        document.getElementById("loop-popgrid-loop").dataset.duration = DUR;
        background(document.getElementById("loop-popgrid-bg"));
        const stage = document.getElementById("loop-popgrid-stage");
        const tiles = [];
        for (let i = 0; i < N; i++) {
          const c = i % COLS,
            r = Math.floor(i / COLS);
          ...
          el.style.left = PAD + c * (CW + GAP) + (CW - w) / 2 + "px";
          el.style.top = PAD + r * (CH + GAP) + (CH - h) / 2 + "px";
          stage.appendChild(el);
          tiles.push({ el, ph: phase(i) });
        }
        const A = (1 - VIS) * DUR,
          SHRINK = 0.5 * A,
          GAPT = 0.05 * A,
        ...
        function pop(u) {
          // 0..1 -> scale, overshoot 1.09
          if (u < 0.3) return 1.09 * Math.sin(((Math.PI / 2) * u) / 0.3);
          if (u < 0.55) return 1.09;
          return 1.09 - 0.09 * smooth((u - 0.55) / 0.45);
        }
        function renderAt(t) {
          const T = frac(t) * DUR;
          for (const tile of tiles) {
            const x = frac((T - tile.ph * DUR) / DUR) * DUR; // time since this tile's shrink started
            let s = 1,
              al = 1;
            if (x < SHRINK) {
              s = 1 - x / SHRINK;
              al = Math.min(1, s / 0.4);
            } else if (x < SHRINK + GAPT) {
              s = 0;
              al = 0;
            } else if (x < A) {
              const u = (x - SHRINK - GAPT) / POP;
              s = pop(u);
              al = Math.min(1, u / 0.58);
            }
            tile.el.style.transform = `scale(${s})`;
            tile.el.style.opacity = al;
          }
        }
        ...
        window.__timelines["loop-popgrid"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `popgrid`.
