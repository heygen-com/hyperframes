---
title: "Loop Scale 4"
description: "A new image swells up out of nothing at the centre each beat, over the previous ones. 8 image slots, 5s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-scale-4.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-scale-4.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-scale-4" item="loop-scale-4" />

That writes one file: `compositions/loop-scale-4.html`.
...
```html index.html
<div
  data-composition-id="loop-scale-4"
  data-composition-src="compositions/loop-scale-4.html"
  data-start="0"
  data-duration="5"
  data-track-index="1"
  data-width="1080"
...
| `stagger` | `0.6` | 0 to 2, step 0.01 | Stagger (engine parameter; see the block header comment). |
| `spin` | `0` | -90 to 90, step 1 | Spin (engine parameter; see the block header comment). |
...
| `scaleStyle` | `bloom` | `bloom`, `recede` | Scale Style (engine parameter; see the block header comment). |
| `growFrom` | `bottom` | `center`, `top`, `bottom`, `left`, `right` | Grow From (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-scale-4-bg"></div>
        <div id="loop-scale-4-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-scale-4-loop").dataset.duration = DUR;
        background(document.getElementById("loop-scale-4-bg"));
        const stage = document.getElementById("loop-scale-4-stage");
        ...
        window.__timelines["loop-scale-4"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `scale`.
