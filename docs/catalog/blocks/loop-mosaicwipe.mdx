---
title: "Loop Mosaic Wipe"
description: "Images reassemble through a tiled iris or venetian-blind wipe. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-mosaicwipe.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-mosaicwipe.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-mosaicwipe" item="loop-mosaicwipe" />

That writes one file: `compositions/loop-mosaicwipe.html`.
...
```html index.html
<div
  data-composition-id="loop-mosaicwipe"
  data-composition-src="compositions/loop-mosaicwipe.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-mosaicwipe-bg"></div>
        <div id="loop-mosaicwipe-card"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // MosaicWipe loop family — "Images reassemble through a tiled iris or venetian-blind
        // wipe": the card is a rows x columns grid; each tile of the NEXT image grows from 0 to its cell (iris open),
        // tiles staggered in `pattern` order, over the resting current image.
        // Measured on the 512 preview (2026-09-08): card = frame inset 6% of H (padding 6), 4x4 cells of 112.5px;
        //   period = DUR/N = 30f. Tile sizes (fraction of cell) by frame: t0 94:.67 96:.95 | t1 94:.39 96:.93 |
        //   t3 96:.60 98:.96 | t7 98:.27 100:.87 | t11 100:.07 102:.68 | t15 104:.36 106:.92 => grow ~5f per tile
        //   with cubic-bezier(.5,0,.5,1), consecutive tiles (row-major) 0.74f apart, first tile starts ~1f after the
        //   period boundary; tiles are centred in their cell and close the seams exactly at 1.0 (4px seam visible at .96).
        // Knobs (guesses): timings as fractions of the period (grow 0.167, per-tile stagger 0.0247 = stagger/60 * 0.0247,
        //   start 0.033); stagger unit unknown -> linear in `stagger` around the measured 60; speedVariation ignored (0);
        //   style blinds = `slats` bars growing from `openFrom`; patterns diagonal/radial/spiral/random unverified (random = LCG seed 7).
        ...
        const COLS = Math.max(1, num("columns", 4) | 0),
          ROWS = Math.max(1, num("rows", 4) | 0);
        const SLATS = Math.max(1, num("slats", 8) | 0);
        const HORIZ = str("orientation", "horizontal") === "horizontal";
        const OPEN = str("openFrom", "start");
        const PATTERN = str("pattern", "normal");
        const STAG = num("stagger", 60);
        ...
        const START = 0.033,
          GROW = 0.167,
          STEP = (0.0247 * STAG) / 60; // fractions of the per-image period

        document.getElementById("loop-mosaicwipe-loop").dataset.duration = DUR;
        background(document.getElementById("loop-mosaicwipe-bg"));
        const cardEl = document.getElementById("loop-mosaicwipe-card");
        const CW = W - 2 * PAD,
          CH = H - 2 * PAD;
        ...
        // cells: grid tiles or blind slats (x, y, w, h, order)
        const cells = [];
        if (STYLE === "blinds") {
          for (let i = 0; i < SLATS; i++)
            cells.push(
              HORIZ
                ? { x: 0, y: (i * CH) / SLATS, w: CW, h: CH / SLATS }
                : { x: (i * CW) / SLATS, y: 0, w: CW / SLATS, h: CH },
            );
        } else {
        ...
        // order index per cell
        let seed = 7;
        const rnd = () => (seed = (seed * 1103515245 + 12345) % 2147483648) / 2147483648;
        const keys = cells.map((k, i) => {
          if (STYLE === "blinds") return i;
          if (PATTERN === "diagonal") return k.r + k.c;
          if (PATTERN === "radial") return Math.hypot(k.c - (COLS - 1) / 2, k.r - (ROWS - 1) / 2);
          if (PATTERN === "spiral")
            return (
              Math.atan2(k.r - (ROWS - 1) / 2, k.c - (COLS - 1) / 2) +
              10 * Math.hypot(k.c - (COLS - 1) / 2, k.r - (ROWS - 1) / 2)
            );
          if (PATTERN === "random") return rnd();
          return i;
        });
        const ranked = keys.map((v, i) => i).sort((a, b) => keys[a] - keys[b]);
        const order = new Array(M);
        ranked.forEach((ci, rank) => (order[ci] = rank));
        const delayOf = (i) => {
          const o = order[i],
            last = M - 1;
          if (OPEN === "end") return (last - o) * STEP;
          if (OPEN === "center") return Math.abs(o - last / 2) * STEP;
          if (OPEN === "alternate") return ((o % 2 ? last - o : o) * STEP) / 2;
          return o * STEP;
        };
        // grid: scale about the cell centre; blinds: grow from the openFrom edge of the card
        const originOf = (k) => (STYLE !== "blinds" ? "50% 50%" : HORIZ ? "50% 0%" : "0% 50%");
        ...
        const layers = IMAGES.map((m) => {
          ...
          L.className = "layer";
          const whole = card(m, g.w, g.h, 0);
          whole.style.left = g.x + "px";
          whole.style.top = g.y + "px";
          L.appendChild(whole);
          const tiles = cells.map((k) => {
            const tdiv = document.createElement("div");
            tdiv.className = "tile";
            tdiv.style.cssText += `left:${k.x}px;top:${k.y}px;width:${k.w + 0.5}px;height:${k.h + 0.5}px;transform-origin:${originOf(k)};`;
            const c = card(m, g.w, g.h, 0);
            c.style.left = g.x - k.x + "px";
            c.style.top = g.y - k.y + "px";
            tdiv.appendChild(c);
            L.appendChild(tdiv);
            return tdiv;
          });
          cardEl.appendChild(L);
          return { el: L, whole, tiles };
        });
        ...
        function renderAt(t) {
          ...
          const outI = k % N,
            inI = (k + 1) % N;
          for (const i of shown) if (i !== outI && i !== inI) layers[i].el.style.display = "none";
          shown = [outI, inI];
          const out = layers[outI],
            inn = layers[inI];
          out.el.style.display = "block";
          out.el.style.zIndex = "1";
          out.whole.style.display = "block";
          for (const td of out.tiles) td.style.display = "none";
          inn.el.style.display = "block";
          inn.el.style.zIndex = "2";
          inn.whole.style.display = "none";
          for (let i = 0; i < M; i++) {
            const q = EASE((u - START - delayOf(i)) / GROW),
              td = inn.tiles[i];
            if (q <= 0) {
            ...
            td.style.transform =
              STYLE !== "blinds" ? `scale(${q})` : HORIZ ? `scaleY(${q})` : `scaleX(${q})`;
          }
        }
        ...
        window.__timelines["loop-mosaicwipe"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `mosaicwipe`.
