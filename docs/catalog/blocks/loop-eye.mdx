---
title: "Loop Eye"
description: "Two rows of upright image cards ride opposing parabolic bows, forming an eye shape. 8 image slots, 14s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-eye.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-eye.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-eye" item="loop-eye" />

That writes one file: `compositions/loop-eye.html`.
...
```html index.html
<div
  data-composition-id="loop-eye"
  data-composition-src="compositions/loop-eye.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `cropAspectRatio` | `2:3` | `16:9`, `9:16`, `1:1`, `4:5`, `3:2`, `3:4`, `2:3` | Card aspect used when Fit is Crop. |
...
| `scaleIntensity` | `0.23` | 0 to 1, step 0.05 | Strength of the size change. |
| `borderRadius` | `12` | 0px to 200px, step 1px | Card corner radius in px at 1080. |
| `direction` | `right` | `right`, `left` | Direction of travel. |
| `edgeFade` | `40` | 0 to 80, step 1 | Edge Fade (engine parameter; see the block header comment). |
| `spacing` | `32` | 0 to 64, step 1 | Spacing (engine parameter; see the block header comment). |
| `arc` | `63` | -180 to 180, step 1 | Arc (engine parameter; see the block header comment). |
| `rowGap` | `0` | 0 to 2, step 0.01 | Row Gap (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-eye-bg"></div>
        <div id="loop-eye-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Eye loop family — two rows of upright cards ride opposing parabolic bows (top row bows up,
        // bottom row bows down) that meet at the frame's left/right edges; the top row drifts one way, the bottom row
        // the other, at constant speed, one row period per loop. Cards fade linearly over the outer edgeFade% of W.
        // Measured on the 512 preview (2026-09-08, 8 images 2:3 crop): card 79.5x118 px -> HEIGHT = scaleIntensity*H
        // (0.23 -> 117.8), width from cropAspectRatio; x pitch 103.5 px (gap 24 = spacing% * card width 25.4 to 1.4 px);
        // top-row centre y = 131.5 + 0.0019*x'^2 (x' from frame centre; 30 samples f0..f108, err <= 2 px), i.e. apex
        // 124.5 px above centre = arc% * (H - cardH)/2 (0.63*197 = 124.1) and back to centre at x' = +-W/2; bottom row
        // mirrored (apex 379.5). Speed 1.971 px/frame = 8 pitches (= N) per 14 s: f210 layout == f0 layout.
        // Opacity of a 128-grey card vs x': 1.0 (|x'|<50), 0.88 (73), 0.77 (97), 0.65 (121), 0.54 (145) = linear ramp
        // from 1 at |x'| = (0.5 - 0.4) W to 0 at the frame edge.
        // Guesses: rowGap = extra apex separation in % of H; bottom row starts at image N/2; gap for natural fit = spacing% of mean width.
        ...
        const CROP_AR = str("cropAspectRatio", "2:3");
        const CARD_H = num("scaleIntensity", 0.23) * REF;
        const EDGE = Math.max(0.001, num("edgeFade", 40) / 100);
        const SPACING = num("spacing", 32) / 100;
        const ARC = num("arc", 63) / 100;
        const ROWGAP = (num("rowGap", 0) / 100) * REF;
        const DIR = str("direction", "right") === "right" ? 1 : -1;
        const RADIUS = (num("borderRadius", 12) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;

        document.getElementById("loop-eye-loop").dataset.duration = DUR;
        background(document.getElementById("loop-eye-bg"));
        const stage = document.getElementById("loop-eye-stage");

        const widths = IMAGES.map((m) => CARD_H * aspect(m, FIT, CROP_AR));
        const meanW = widths.reduce((s, w) => s + w, 0) / N;
        const GAP = SPACING * meanW;
        const P = widths.reduce((s, w) => s + w + GAP, 0); // row period
        const COPIES = Math.ceil((W + meanW) / P) + 1;
        const APEX = (ARC * (REF - CARD_H)) / 2 + ROWGAP / 2; // apex offset from the frame centre line
        const A = APEX / ((W / 2) * (W / 2)); // bow returns to centre at x' = +-W/2
        ...
        for (const sign of [-1, 1]) {
          ...
          let b = 0;
          for (let j = 0; j < N; j++) {
            const idx = (j + (sign > 0 ? N >> 1 : 0)) % N,
              m = IMAGES[idx],
              w = widths[idx];
            for (let c = 0; c < COPIES; c++) {
              const el = card(m, w, CARD_H, RADIUS);
              el.style.left = -w / 2 + "px";
              el.style.top = -CARD_H / 2 + "px";
              stage.appendChild(el);
              items.push({ el, base: b + w / 2 + c * P });
            }
            b += w + GAP;
          }
          rows.push({ sign, dir: -sign * DIR, items }); // direction=right: top row moves right
        }
        const span = COPIES * P;
        function renderAt(t) {
          const shift = frac(t) * P;
          for (const r of rows)
            for (const it of r.items) {
              // wrap into [-span/2, span/2) around the centre
              let x = it.base + r.dir * shift;
              x = ((((x + span / 2) % span) + span) % span) - span / 2;
              if (Math.abs(x) > W / 2 + meanW) {
                it.el.style.display = "none";
                continue;
              }
              it.el.style.display = "";
              const y = r.sign * (APEX - A * x * x);
              it.el.style.transform = `translate(${x}px,${y}px)`;
              it.el.style.opacity = Math.max(
                0,
                Math.min(1, 1 - (Math.abs(x) - (0.5 - EDGE) * W) / (EDGE * W)),
              );
            }
        }
        ...
        window.__timelines["loop-eye"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `eye`.
