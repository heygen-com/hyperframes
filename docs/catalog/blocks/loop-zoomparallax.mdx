---
title: "Loop Zoom Parallax"
description: "One padded rounded frame; each image holds with a slow zoom, then the next parallaxes in. 8 image slots, 9s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-zoomparallax.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-zoomparallax.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-zoomparallax" item="loop-zoomparallax" />

That writes one file: `compositions/loop-zoomparallax.html`.
...
```html index.html
<div
  data-composition-id="loop-zoomparallax"
  data-composition-src="compositions/loop-zoomparallax.html"
  data-start="0"
  data-duration="9"
  data-track-index="1"
  data-width="1080"
...
| `zoomAmount` | `12` | 0 to 24, step 1 | Zoom Amount (engine parameter; see the block header comment). |
| `panDir` | `alternate` | `alternate`, `left`, `right` | Pan Dir (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-zoomparallax-bg {
        position: absolute;
        inset: 0;
      }
      #loop-zoomparallax-frame {
        position: absolute;
        overflow: hidden;
      }
      .layer {
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-zoomparallax-bg"></div>
        <div id="loop-zoomparallax-frame"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Glide loop family — one padded, rounded frame; each image holds for DUR/N with a
        // Ken Burns drift and hands over with a crossfade. Measured on the 512 preview (2026-09-08):
        //   frame: 450x450 @512 = padding 6% of H on every side (x 31..480, y 31..480), corners 32@1080; static all loop.
        //   crossfade: luma 74->101 over frames 28.5..33.5 (~6 frames = 0.2s = 0.18 of the 1.125s slot), symmetric S-curve,
        //              ending exactly at the slot boundary (33.75, 67.5, 101.25 ... measured 33/67/101); spatially uniform
        //              (row/col samples identical) -> a plain opacity crossfade, no wipe.
        //   drift: invisible on flat placeholders -> zoomAmount 12 taken as scale 1.00 -> 1.12 over the slot, linear,
        //          pan = alternate left/right by (scale-1)*W/2 per panDir (all knobs); the outgoing image keeps drifting
        //          under the fade ("parallax": two layers moving at once).
        // Knobs: drift law, fade window (0.18 slot), easing applies to the fade (smooth = (.65,0,.35,1)).
        ...
        const ZOOM = num("zoomAmount", 12) / 100;
        const PANDIR = str("panDir", "alternate");
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(Math.min(8, num("imageCount", 16)));
        const N = IMAGES.length;
        const FADE = 0.18; // fraction of a slot, at the END of the slot
        ...
        document.getElementById("loop-zoomparallax-loop").dataset.duration = DUR;
        background(document.getElementById("loop-zoomparallax-bg"));
        const frame = document.getElementById("loop-zoomparallax-frame");
        const FW = W - 2 * PAD,
          FH = H - 2 * PAD;
        frame.style.cssText = `left:${PAD}px;top:${PAD}px;width:${FW}px;height:${FH}px;border-radius:${RADIUS}px;`;
        const layers = IMAGES.map((m, i) => {
          const L = document.createElement("div");
          L.className = "layer";
          const img = document.createElement("img");
          img.src = m.src;
          img.width = Math.round(FW);
          img.height = Math.round(FH);
          L.appendChild(img);
          L.style.opacity = 0;
          frame.appendChild(L);
          return L;
        });
        const panSign = (i) => (PANDIR === "left" ? -1 : PANDIR === "right" ? 1 : i % 2 ? -1 : 1);
        // drift of image i at slot-progress u (may run past 1 while fading out)
        const drift = (i, u) => {
          const s = 1 + ZOOM * u,
            dx = ((panSign(i) * (s - 1) * FW) / 2) * (u - 0.5);
          return `translate3d(${dx}px,0,0) scale(${s})`;
        };
        function renderAt(t) {
          const p = frac(t) * N,
            k = Math.floor(p),
          ...
          const cur = k % N,
            nxt = (k + 1) % N;
          const f = u < 1 - FADE ? 0 : ease((u - (1 - FADE)) / FADE);
          for (let i = 0; i < N; i++) {
            const L = layers[i];
            if (i === cur) {
              L.style.opacity = 1 - f;
              L.style.transform = drift(i, u);
              L.style.zIndex = 2;
            } else if (i === nxt && f > 0) {
              L.style.opacity = f;
              L.style.transform = drift(i, u - 1);
              L.style.zIndex = 3;
        ...
        window.__timelines["loop-zoomparallax"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `zoomparallax`.
