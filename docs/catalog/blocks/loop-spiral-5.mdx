...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-5.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-spiral-5.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-spiral-5" item="loop-spiral-5" />

That writes one file: `compositions/loop-spiral-5.html`.
...
```html index.html
<div
  data-composition-id="loop-spiral-5"
  data-composition-src="compositions/loop-spiral-5.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `fade` | `29` | 0 to 100, step 1 | Fade (engine parameter; see the block header comment). |
| `autoFaceCamera` | `on` | `on`, `off` | Auto Face Camera (engine parameter; see the block header comment). |
| `cycles` | `1` | 0 to 24, step 1 | Number of motion cycles per loop. |
...
| `radius` | `955` | 0 to 6000, step 10 | Radius (engine parameter; see the block header comment). |
| `turns` | `3` | 0 to 20, step 0.1 | Turns (engine parameter; see the block header comment). |
...
| `planeSize` | `145` | 0 to 1620, step 10 | Card size in scene units. |
| `distance` | `1200` | 0 to 9300, step 10 | Camera distance from the formation. |
| `perspective` | `300` | 0 to 600, step 10 | Camera focal length: smaller = stronger perspective. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-spiral-5-bg"></div>
        <div id="loop-spiral-5-stage"><div id="loop-spiral-5-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-spiral-5-loop").dataset.duration = DUR;
        background(document.getElementById("loop-spiral-5-bg"));
        ...
        const stage = document.getElementById("loop-spiral-5-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-spiral-5-world");
        ...
        window.__timelines["loop-spiral-5"] = tl;
      })();
    </script>
  </body>
</html>
