---
title: "Loop Wheel Spin Bottom"
description: "Doubled-up image cards on a 2D wheel whose hub sits below the frame spin past. 8 image slots, 14s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheelspinbottom.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wheelspinbottom.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-wheelspinbottom" item="loop-wheelspinbottom" />

That writes one file: `compositions/loop-wheelspinbottom.html`.
...
```html index.html
<div
  data-composition-id="loop-wheelspinbottom"
  data-composition-src="compositions/loop-wheelspinbottom.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.2` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `rotations` | `1` | 0 to 10, step 0.1 | Rotations (engine parameter; see the block header comment). |
| `movement` | `continuous` | `continuous`, `stepped` | Movement (engine parameter; see the block header comment). |
...
| `spinStyle` | `none` | `none`, `self`, `flip` | Spin Style (engine parameter; see the block header comment). |
| `spinRate` | `2` | 0 to 10, step 0.1 | Spin Rate (engine parameter; see the block header comment). |
| `flipAxis` | `y` | `y`, `x` | Flip Axis (engine parameter; see the block header comment). |
| `wheelSize` | `50` | 0% to 100%, step 1% | Wheel Size (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-wheelspinbottom-bg {
        position: absolute;
        inset: 0;
      }
      #loop-wheelspinbottom-stage {
        position: absolute;
        left: 50%;
        top: 50%;
        width: 0;
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-wheelspinbottom-bg"></div>
        <div id="loop-wheelspinbottom-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Cog loop family — 2N cards ("doubled up") on a 2D wheel whose hub sits R below the frame
        // centre (so the top card is centred in the frame), each rotated to its ring angle, spinning continuously, `rotations` turns/loop.
        // Measured (512 preview, 16 slots / 8 images): card 102 px = scaleIntensity*H (0.2 -> 102.4); top card centre (254.5,255.3);
        // neighbours at +-22.5deg (pitch 360/16) and r 257 -> R = 256 = wheelSize% * H (50); spin 2.57deg per 3 f = 360deg/420 f,
        // 1 rev per loop, clockwise for direction=right. Cards touch/overlap slightly (z-order = slot index).
        // Same engine as wheelspin.html except: R = wheelSize% * H, hub at (W/2, H/2 + R), 2N slots, off-frame cards culled.
        // stagger (40) has no visible effect on the continuous preview and is ignored (knob).
        // Knobs (unmeasured): movement=stepped uses the WheelCarousel tick law (anticipation/overshoot % of pitch, hold %
        // of step); spinStyle self/flip = spinRate extra turns of each card per loop about its centre (flip: 3D about flipAxis);
        // shadow units 1% of H (flipgrid convention).
        ...
        const DIR = str("direction", "right") === "left" ? -1 : 1; // + = clockwise on screen
        const SIZE = num("scaleIntensity", 0.2) * REF; // card height
        const R = (num("wheelSize", 50) / 100) * REF; // ring radius, % of H
        const ROT = Math.max(1, Math.round(num("rotations", 1)));
        const STEPPED = str("movement", "continuous") === "stepped";
        const ANT = num("anticipation", 20) / 100,
          OVER = num("overshoot", 10) / 100,
          HOLD = Math.min(0.9, num("hold", 33) / 100);
        const SPINST = str("spinStyle", "none"),
          SPINRATE = num("spinRate", 2),
          FLIPAX = str("flipAxis", "y") === "x" ? "rotateX" : "rotateY";
        const RADIUS = (num("borderRadius", 54) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const M = 2 * N,
          PITCH = 360 / M; // doubled ring
        const STEPS = N * ROT;
        ...
        // one tick: anticipate back by ANT, swing to 1+OVER, settle; then hold (measured on WheelCarousel)
        function tick(u) {
        ...
        document.getElementById("loop-wheelspinbottom-loop").dataset.duration = DUR;
        background(document.getElementById("loop-wheelspinbottom-bg"));
        const stage = document.getElementById("loop-wheelspinbottom-stage");
        ...
        function renderAt(t) {
          ...
          if (STEPPED) {
            const s = frac(t) * STEPS,
              k = Math.floor(s);
            spin = DIR * PITCH * (k + tick(s - k));
          } else spin = DIR * 360 * ROT * frac(t);
          const self = SPINST === "none" ? 0 : DIR * 360 * SPINRATE * frac(t);
          for (const c of cards) {
            const phi = c.theta + spin,
              r = (phi * Math.PI) / 180;
            const x = R * Math.sin(r),
              y = R - R * Math.cos(r); // hub at H/2 + R
            if (Math.abs(x) > W / 2 + SIZE || y > H / 2 + SIZE) {
              c.el.style.display = "none";
              continue;
            }
            c.el.style.display = "";
            let tr = `translate(${x}px,${y}px) rotate(${phi}deg)`;
            if (SPINST === "self") tr += ` rotate(${self}deg)`;
            else if (SPINST === "flip")
              tr += ` perspective(${(1000 * REF) / 1080}px) ${FLIPAX}(${self}deg)`;
        ...
        window.__timelines["loop-wheelspinbottom"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `wheelspinbottom` `3d`.
