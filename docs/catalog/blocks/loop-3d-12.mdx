...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-3d-12.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-3d-12.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-3d-12" item="loop-3d-12" />

That writes one file: `compositions/loop-3d-12.html`.
...
```html index.html
<div
  data-composition-id="loop-3d-12"
  data-composition-src="compositions/loop-3d-12.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `distance` | `4214` | 0 to 48424, step 10 | Camera distance from the formation. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-3d-12-bg"></div>
        <div id="loop-3d-12-stage">
          <div id="loop-3d-12-ring"><div id="loop-3d-12-spin"></div></div>
        </div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-3d-12-loop").dataset.duration = DUR;
        background(document.getElementById("loop-3d-12-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R, 1); // world -> px (ring centre or ring at z=0)
        const Rpx = R * s,
          Dpx = D * s;
        const stage = document.getElementById("loop-3d-12-stage");
        stage.style.perspective = f + "px";
        const ring = document.getElementById("loop-3d-12-ring");
        ...
        const spin = document.getElementById("loop-3d-12-spin");
        ...
        window.__timelines["loop-3d-12"] = tl;
      })();
    </script>
  </body>
</html>
