---
title: "Loop Deck 12"
description: "Image cards flip through a stacked deck, the front card leaving to reveal the next. 8 image slots, 10s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-12.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deck-12.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-deck-12" item="loop-deck-12" />

That writes one file: `compositions/loop-deck-12.html`.
...
```html index.html
<div
  data-composition-id="loop-deck-12"
  data-composition-src="compositions/loop-deck-12.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `500` | 0 to 1000, step 10 | Card size in scene units. |
| `distance` | `1090` | 0 to 8260, step 10 | Camera distance from the formation. |
| `perspective` | `100` | 0 to 200, step 1 | Camera focal length: smaller = stronger perspective. |
| `gap` | `0.5` | 0px to 2px, step 0.01px | Spacing between cards. |
...
| `twoSided` | `off` | `off`, `on` | Two Sided (engine parameter; see the block header comment). |
| `solo` | `on` | `off`, `on` | Solo (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-deck-12-bg"></div>
        <div id="loop-deck-12-stage"><div id="loop-deck-12-belt"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-deck-12-loop").dataset.duration = DUR;
        background(document.getElementById("loop-deck-12-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / D; // world -> px at the belt plane (z = 0)
        const S = PLANE * s;
        const P = S * (1 + GAP) * PITCH_K;
        const stage = document.getElementById("loop-deck-12-stage");
        stage.style.perspective = f + "px";
        const belt = document.getElementById("loop-deck-12-belt");
        ...
        window.__timelines["loop-deck-12"] = tl;
      })();
    </script>
  </body>
</html>
