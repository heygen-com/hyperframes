---
title: "Loop Split Reveal"
description: "Paired image panels slide in from opposite edges and meet. 4 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-splitreveal.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-splitreveal.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-splitreveal" item="loop-splitreveal" />

That writes one file: `compositions/loop-splitreveal.html`.
...
```html index.html
<div
  data-composition-id="loop-splitreveal"
  data-composition-src="compositions/loop-splitreveal.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-splitreveal-bg"></div>
        <div id="loop-splitreveal-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // SplitReveal loop family — "Paired panels slide in from opposite edges": two
        // rounded panels side by side (split by `splitRatio`, `gap` between), each showing an image; per period the
        // next pair slides in — the left panel's new image from the LEFT edge, the right panel's from the RIGHT edge,
        // the right panel lagging the left.
        // Measured on the 512 preview (2026-09-08): card = frame inset 6% of H (padding 6); panels x 31..250 and
        //   261..480 (gap 10px = 2% of H for gap 2 => gap unit % of H, not px); the preview cycles only TWO pairs
        //   (placeholder greys of images 3,4 -> 1,2 -> 3,4), i.e. it was rendered with 4 images: period 120f = DUR/2.
        //   Left panel coverage f12:.03 f18:.10 f24:.25 f30:.50 f36:.74 f42:.89 f48:.96 f54:.995 => cubic-bezier
        //   (.5,0,.5,1) over 48f starting at f6; right panel identical but starting f15 (9f later).
        // Knobs (guesses): with N images the loop shows N/2 pairs, period = DUR/(N/2); timings as fractions of the
        //   period (start 0.05, slide 0.40, right lag 0.075); the new image covers the resting one (a push looks the
        //   same on uniform placeholders); shadow* (shadowEnabled off in the spec) implemented as a box-shadow on the
        //   sliding image when enabled; easing names other than smooth/custom/linear mapped to GSAP eases.
        ...
        const __V = Object.assign(
          ...
          (window.__hyperframes && window.__hyperframes.getVariables
            ...
            : null) || {},
          { loopDuration: 8, loopRepeat: 2, imageCount: 4 },
        );
        const __PH = [
        ...
        const __MEDIA = await Promise.all(
          ["image1", "image2", "image3", "image4"].map(async (id, i) => {
            const v = __V[id];
        ...
        const SPLIT = Math.min(95, Math.max(5, num("splitRatio", 50))) / 100;
        ...
        const EASING = str("easing", "smooth"),
          BEZ = str("easeBezier", "0.50,0.00,0.50,1.00");
        const SHADOW = on("shadowEnabled", false);
        const SH = {
          x: num("shadowX", 0),
          y: num("shadowY", 1),
          blur: num("shadowBlur", 1.5),
          spread: num("shadowSpread", -0.25),
          op: num("shadowOpacity", 40) / 100,
          col: str("shadowColor", "#000000"),
        };
        ...
        const START = 0.05,
          SLIDE = 0.4,
          LAG = 0.075; // fractions of the per-pair period

        document.getElementById("loop-splitreveal-loop").dataset.duration = DUR;
        background(document.getElementById("loop-splitreveal-bg"));
        const stage = document.getElementById("loop-splitreveal-stage");
        const CW = W - 2 * PAD,
          CH = H - 2 * PAD;
        const LW = (CW - GAP) * SPLIT,
          RW = CW - GAP - LW;
        ...
        function easeFn(name, bez) {
          const b = (bez || "").split(",").map(Number);
          const table = {
            smooth: [0.5, 0, 0.5, 1],
          ...
          if (name === "linear") return (x) => Math.min(1, Math.max(0, x));
          if (name === "custom" && b.length === 4 && b.every((v) => !isNaN(v))) return bezier(...b);
          if (table[name]) return bezier(...table[name]);
          const g = {
          ...
          return g ? gsap.parseEase(g) : bezier(0.5, 0, 0.5, 1);
        }
        const EASE = easeFn(EASING, BEZ);

        function makePanel(x, w, imgs) {
          const p = document.createElement("div");
          p.className = "panel";
          p.style.cssText += `left:${x}px;top:${PAD}px;width:${w}px;height:${CH}px;border-radius:${RADIUS}px;`;
          const layers = imgs.map((m) => {
            ...
            L.className = "layer";
            const c = card(m, cw, ch, 0);
            c.style.left = (w - cw) / 2 + "px";
            c.style.top = (CH - ch) / 2 + "px";
            if (SHADOW)
              L.style.boxShadow = `${(SH.x / 100) * REF}px ${(SH.y / 100) * REF}px ${(SH.blur / 100) * REF}px ${(SH.spread / 100) * REF}px ${SH.col}${Math.round(
                SH.op * 255,
              )
                .toString(16)
                .padStart(2, "0")}`;
          ...
          stage.appendChild(p);
          return { el: p, layers, w };
        }
        const left = makePanel(PAD, LW, IMAGES.filter((m, i) => i % 2 === 0).slice(0, PAIRS));
        const right = makePanel(
          PAD + LW + GAP,
          RW,
          IMAGES.filter((m, i) => i % 2 === 1).slice(0, PAIRS),
        );

        function drive(panel, k, q, fromLeft) {
          const outI = k % PAIRS,
            inI = (k + 1) % PAIRS;
          panel.layers.forEach((L, i) => {
            if (i !== outI && i !== inI) L.style.display = "none";
          });
          const out = panel.layers[outI],
            inn = panel.layers[inI];
          out.style.display = "block";
          out.style.zIndex = "1";
          out.style.transform = "";
          if (q <= 0 || PAIRS === 1) {
            inn.style.display = inI === outI ? "block" : "none";
            return;
          }
          inn.style.display = "block";
          inn.style.zIndex = "2";
          inn.style.transform = `translate3d(${(fromLeft ? -1 : 1) * (1 - q) * panel.w}px,0,0)`;
        }
        function renderAt(t) {
          const s = frac(t) * PAIRS,
            k = Math.floor(s),
            u = s - k;
          drive(left, k, EASE((u - START) / SLIDE), true);
          drive(right, k, EASE((u - START - LAG) / SLIDE), false);
        }
        ...
        window.__timelines["loop-splitreveal"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `splitreveal`.
