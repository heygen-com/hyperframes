---
title: "Loop Orbit Carousel"
description: "Flat rounded image cards orbit a crown ring under a 3D camera, sized by depth. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbitcarousel.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbitcarousel.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-orbitcarousel" item="loop-orbitcarousel" />

That writes one file: `compositions/loop-orbitcarousel.html`.
...
```html index.html
<div
  data-composition-id="loop-orbitcarousel"
  data-composition-src="compositions/loop-orbitcarousel.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `spread` | `70` | 0 to 140, step 1 | Spread (engine parameter; see the block header comment). |
| `depthFade` | `60` | 0 to 120, step 1 | Depth Fade (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-orbitcarousel-bg {
        position: absolute;
        inset: 0;
      }
      #loop-orbitcarousel-stage {
        position: absolute;
        inset: 0;
        perspective-origin: 50% 50%;
      }
      #loop-orbitcarousel-world {
        position: absolute;
        left: 50%;
        top: 50%;
        width: 0;
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-orbitcarousel-bg"></div>
        <div id="loop-orbitcarousel-stage"><div id="loop-orbitcarousel-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Crown loop family — N flat cards (always parallel to the screen, rounded)
        // on a ring seen from the front: front card at z=0, x = R sin(phi), z = R(cos(phi) - 1); CSS perspective
        // gives scale P/(P - z) = 1/(1 + (R/P)(1 - cos phi)). Continuous linear spin, 1 revolution per loop,
        // front card moves right. Back cards fade (opacity) with depth.
        // Measured (2026-09-08, 512 preview, 8 cards): front card 234px (=0.457 H); 45deg neighbour 220px, 90deg
        // card 191px -> both fit R/P = 0.22; front-card centre 255->312px over frames 0..21 (linear, 2.75px/f)
        // => x amplitude R = 162px = 0.69 * card, i.e. spread 70 -> R = spread% * card. Card period 45 f = DUR/N.
        // Fade: opacity ~0.72 at 90deg, ~0.98 at 45deg (luma of the same card front vs side) -> 1 - depthFade%*(1-cos)/2.
        // Guesses: card size fixed 0.457*H (padding 6 not decoded — no-op knob); shadow units = % of card size.
        ...
        const FADE = Math.max(0, Math.min(1, num("depthFade", 60) / 100));
        ...
        document.getElementById("loop-orbitcarousel-loop").dataset.duration = DUR;
        background(document.getElementById("loop-orbitcarousel-bg"));
        const SIZE = 0.457 * REF; // front card height
        const R = SPREAD * SIZE; // ring radius (px)
        const P = R / 0.22; // focal length: measured R/P = 0.22
        document.getElementById("loop-orbitcarousel-stage").style.perspective = P + "px";
        const world = document.getElementById("loop-orbitcarousel-world");
        const cards = [];
        for (let i = 0; i < N; i++) {
          const m = IMAGES[i],
          ...
          if (SHADOW)
            c.style.boxShadow = `${(num("shadowX", 0) / 100) * SIZE}px ${(num("shadowY", 1) / 100) * SIZE}px ${(num("shadowBlur", 1.5) / 100) * SIZE}px ${(num("shadowSpread", -0.25) / 100) * SIZE}px ${str("shadowColor", "#000")}${Math.round(
              ...
              .padStart(2, "0")}`;
          world.appendChild(c);
          cards.push({ el: c, theta: (i * 2 * Math.PI) / N, w, h });
        }
        function renderAt(t) {
          const spin = frac(t) * 2 * Math.PI;
          for (const c of cards) {
            const phi = c.theta + spin,
              cs = Math.cos(phi);
            c.el.style.transform = `translate3d(${R * Math.sin(phi) - c.w / 2}px,${-c.h / 2}px,${R * (cs - 1)}px)`;
        ...
        window.__timelines["loop-orbitcarousel"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `orbitcarousel` `3d` `carousel`.
