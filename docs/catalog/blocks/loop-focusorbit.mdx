---
title: "Loop Focus Orbit"
description: "Upright rounded image cards on a screen-space circle pivot so the focused card comes forward. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-focusorbit.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-focusorbit.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-focusorbit" item="loop-focusorbit" />

That writes one file: `compositions/loop-focusorbit.html`.
...
```html index.html
<div
  data-composition-id="loop-focusorbit"
  data-composition-src="compositions/loop-focusorbit.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `zoom` | `2.4` | 0 to 5, step 0.1 | Zoom (engine parameter; see the block header comment). |
| `spinStops` | `5` | 0 to 10, step 0.1 | Spin Stops (engine parameter; see the block header comment). |
| `ringSize` | `72` | 0 to 144, step 1 | Ring Size (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-focusorbit-bg"></div>
        <div id="loop-focusorbit-cam"><div id="loop-focusorbit-ring"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Pivot loop family — N upright rounded cards on a screen-space circle
        // (r = ringSize% * (1-2*padding%) * H/2 = 162px @512, measured 166; card 91px = 0.178 H). The ring turns in
        // eased STEPS of one pitch (motion=smooth: hold + move); the camera zooms `zoom`x onto the top-of-ring point,
        // the ring keeps stepping (spinStops stops while zoomed), then zooms back out. 8 steps per loop = 1 rev = seamless.
        // Measured (512 preview, 360 f): ring phase f0-105 with 3 steps (8-fold angular phase 0 -> 135deg), zoom-in
        // f105-145 (card-mask bbox 415 -> full frame), zoomed holds f146-167 / 182-206 / 221-243 / 257-281 / 296-320
        // (moves 12-15 f between), zoom-out f320-355, ring phase again at f354 (phase 0). Zoomed pitch 280px
        // (= 2.4 * 166 * 2 sin 22.5). Direction: top card moves right (clockwise).
        // Guesses: hold/move split inside the ring phase (used 0.5 s hold / 0.667 s move); the zoomed framing — the
        // preview's hold positions drift by +-100px between stops (134/34/194/93/255 mod pitch), ours keeps the focus
        // fixed at the top-of-ring point with a card centred at every hold; zoomStyle/rotateY/cardFacing/scaleIntensity
        // are no-op knobs; easing enum -> cubic in-out.
        ...
        const RING = ((num("ringSize", 72) / 100) * (1 - 2 * PAD) * REF) / 2;
        const SIZE = 0.178 * REF;
        const ZOOM = Math.max(1, num("zoom", 2.4));
        const STOPS = Math.max(1, num("spinStops", 5) | 0);
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const PITCH = 360 / N;

        document.getElementById("loop-focusorbit-loop").dataset.duration = DUR;
        background(document.getElementById("loop-focusorbit-bg"));
        const cam = document.getElementById("loop-focusorbit-cam"),
          ring = document.getElementById("loop-focusorbit-ring");
        const cards = [];
        for (let i = 0; i < N; i++) {
          const m = IMAGES[i],
          ...
          c.style.top = -h / 2 + "px";
          ring.appendChild(c);
          cards.push({ el: c, theta: -90 + i * PITCH }); // card 0 at 12 o'clock
        }
        // schedule (fractions of DUR), measured on the 12 s preview: ring steps, zoom-in (with a step), zoomed stops, zoom-out
        const ease = (x) =>
        ...
        const RING_T = 3.5 / 12,
          ...
          TAIL = 0.15 / 12,
          HOLD = 0.8 / 12;
        const zoomedSteps = STOPS - 1; // stops while zoomed; one step happens during zoom-in
        const ringSteps = Math.max(0, N - STOPS); // remaining steps in the ring phase -> exactly 1 rev per loop
        const moveT = Math.max(
          0.05 / 12,
          (1 - RING_T - ZIN - ZOUT - TAIL - STOPS * HOLD) / Math.max(1, zoomedSteps),
        );
        const ZO_START = 1 - ZOUT - TAIL;
        const steps = []; // [start, end] of each move
        for (let k = 0; k < ringSteps; k++) {
          const s = RING_T / ringSteps;
          steps.push([k * s + s * 0.4, (k + 1) * s]);
        }
        steps.push([RING_T + ZIN * 0.1, RING_T + ZIN * 0.9]); // step during zoom-in
        let t0 = RING_T + ZIN;
        for (let k = 0; k < zoomedSteps; k++) {
          t0 += HOLD;
          steps.push([t0, t0 + moveT]);
          t0 += moveT;
        }
        function angle(t) {
          let a = 0;
          for (const [s, e] of steps) a += ease((t - s) / (e - s));
          return a * PITCH;
        }
        function zoomAt(t) {
          if (t < RING_T) return 0;
          if (t < RING_T + ZIN) return ease((t - RING_T) / ZIN);
          if (t < ZO_START) return 1;
          return 1 - ease((t - ZO_START) / ZOUT);
        }
        function renderAt(t) {
          t = frac(t);
          const spin = DIR * angle(t),
            z = zoomAt(t);
          const scale = 1 + (ZOOM - 1) * z,
            fy = -RING * z; // focus slides from ring centre to the 12 o'clock point
          cam.style.transform = `scale(${scale}) translate(0px,${-fy}px)`;
          for (const c of cards) {
            const phi = ((c.theta + spin) * Math.PI) / 180;
            c.el.style.transform = `translate(${RING * Math.cos(phi)}px,${RING * Math.sin(phi)}px)`;
          }
        }
        ...
        window.__timelines["loop-focusorbit"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `focusorbit`.
