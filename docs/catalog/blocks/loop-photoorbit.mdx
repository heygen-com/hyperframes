---
title: "Loop Photo Orbit"
description: "Upright rounded image cards cluster on a screen-space ellipse and orbit. 8 image slots, 18s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-photoorbit.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-photoorbit.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-photoorbit" item="loop-photoorbit" />

That writes one file: `compositions/loop-photoorbit.html`.
...
```html index.html
<div
  data-composition-id="loop-photoorbit"
  data-composition-src="compositions/loop-photoorbit.html"
  data-start="0"
  data-duration="18"
  data-track-index="1"
  data-width="1080"
...
| `ringWidth` | `56` | 0 to 112, step 1 | Ring Width (engine parameter; see the block header comment). |
| `ringHeight` | `56` | 0 to 112, step 1 | Ring Height (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-photoorbit-bg"></div>
        <div id="loop-photoorbit-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Cluster loop family — N upright rounded cards on a screen-space ellipse
        // (ringWidth x ringHeight, % of (1-2*padding%)*H), rotating continuously; card 0 at 12 o'clock at t=0,
        // clockwise (top card moves right), 1 revolution per loop.
        // Measured (512 preview, 8 cards): top card centre (255,128), right card (376,255) -> r ~ 120-126 px
        // = 0.56 * 0.88 * 256 = 126. Top-card x edge 188 -> 204 -> 219 over 20 f = 1.5 px/f = 20.5 deg/s = 369 deg
        // per 18 s loop (=1 rev). Sizes of the 8 cards passing 12 o'clock: 134,109,131,117,140,104,126,113 px —
        // per-card jitter about 122 px (0.238 H) of +-13% = scaleIntensity/2; card centres also wobble +-10 px radially.
        // Guesses: size/radius jitter from a fixed-seed LCG; z-order = card index; motion pulse/steps and the easing
        // enum are not implemented (default linear); shadow units = % of card.
        ...
        const A = ((num("ringWidth", 56) / 100) * (1 - 2 * PAD) * REF) / 2;
        const B = ((num("ringHeight", 56) / 100) * (1 - 2 * PAD) * REF) / 2;
        const SIZE = 0.238 * REF;
        const SI = num("scaleIntensity", 0.26);
        ...
        document.getElementById("loop-photoorbit-loop").dataset.duration = DUR;
        background(document.getElementById("loop-photoorbit-bg"));
        const stage = document.getElementById("loop-photoorbit-stage");
        let seed = 12345;
        const rnd = () => (seed = (seed * 1103515245 + 12345) % 2147483648) / 2147483648; // fixed-seed LCG
        const cards = [];
        for (let i = 0; i < N; i++) {
          const m = IMAGES[i],
            a = aspect(m, FIT, CROP_AR);
          const s = SIZE * (1 + SI * (rnd() - 0.5)),
            rj = 1 + 0.08 * (rnd() - 0.5);
          const h = s,
            w = s * a;
          ...
          c.style.zIndex = i + 1;
          if (SHADOW)
            c.style.boxShadow = `${(num("shadowX", 0) / 100) * s}px ${(num("shadowY", 1) / 100) * s}px ${(num("shadowBlur", 1.5) / 100) * s}px ${(num("shadowSpread", -0.25) / 100) * s}px ${str("shadowColor", "#000")}${Math.round(
              (num("shadowOpacity", 40) / 100) * 255,
            )
              .toString(16)
              .padStart(2, "0")}`;
          stage.appendChild(c);
          cards.push({ el: c, theta: -Math.PI / 2 + (i * 2 * Math.PI) / N, rj });
        }
        function renderAt(t) {
          const spin = DIR * frac(t) * 2 * Math.PI;
          for (const c of cards) {
            const phi = c.theta + spin;
            c.el.style.transform = `translate(${A * c.rj * Math.cos(phi)}px,${B * c.rj * Math.sin(phi)}px)`;
          }
        }
        ...
        window.__timelines["loop-photoorbit"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `photoorbit`.
