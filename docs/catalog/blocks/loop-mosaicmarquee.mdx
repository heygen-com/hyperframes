---
title: "Loop Mosaic Marquee"
description: "One horizontal masonry belt of mixed-size image cards scrolls across the frame. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-mosaicmarquee.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-mosaicmarquee.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-mosaicmarquee" item="loop-mosaicmarquee" />

That writes one file: `compositions/loop-mosaicmarquee.html`.
...
```html index.html
<div
  data-composition-id="loop-mosaicmarquee"
  data-composition-src="compositions/loop-mosaicmarquee.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.55` | 0 to 1, step 0.05 | Strength of the size change. |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-mosaicmarquee-bg {
        position: absolute;
        inset: 0;
      }
      #loop-mosaicmarquee-belt {
        position: absolute;
        left: 0;
      }
      .card img {
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-mosaicmarquee-bg"></div>
        <div id="loop-mosaicmarquee-belt"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Marquee loop family mosaic — one horizontal masonry belt, vertically centred, scrolling
        // at constant speed; exactly one belt period per loop. Measured on the 512x512 preview (2026-09-08):
        //   belt height 284px = 0.55 H (scaleIntensity 0.55 -> band height fraction); two rows of 133px squares + 16px gap;
        //   "big" cards are squares spanning both rows (280px); gap 3 -> 16px (H/96 per unit, same as ColumnDrift);
        //   speed 3.33 px/frame (big card right edge 312 @f0 -> 16 @f90) = 1200px per 12s = one period;
        //   6-column rhythm with 8 images: [big][s/s][big][s/-][s/s][-/s] (image order 0..7 in that reading order,
        //   the two '-' slots stay EMPTY), period 280+16+136+16+280+16+3*(136+16) = 1200 ✓.
        // Guesses: variant 2/3 rhythms unknown (same rhythm used); padding has no visible effect (ignored); shadow settings
        //   ignored (shadowEnabled=false by default); direction=right mirrors the motion; natural fit -> object-fit contain.
        ...
        const BAND = num("scaleIntensity", 0.55) * REF;
        const GAP = (num("gap", 3) * REF) / 96;
        ...
        document.getElementById("loop-mosaicmarquee-loop").dataset.duration = DUR;
        background(document.getElementById("loop-mosaicmarquee-bg"));
        const belt = document.getElementById("loop-mosaicmarquee-belt");
        const SMALL = (BAND - GAP) / 2,
          BIG = BAND;
        belt.style.top = (H - BAND) / 2 + "px";
        // rhythm: each column = list of slots; 'B' big, 't' small top, 'b' small bottom, '-' empty
        const RHYTHM = [["B"], ["t", "b"], ["B"], ["t", "-"], ["t", "b"], ["-", "b"]];
        const cols = []; // {w, items:[{slot, m}]}
        let i = 0,
          c = 0;
        while (i < N) {
          const col = RHYTHM[c % RHYTHM.length],
            items = [];
          for (const s of col) {
            if (s === "-") continue;
            if (i >= N) break;
            items.push({ slot: s, m: IMAGES[i++] });
          }
          cols.push({ w: col[0] === "B" ? BIG : SMALL, items });
          c++;
        }
        const P = cols.reduce((s, col) => s + col.w + GAP, 0);
        const reps = Math.ceil(W / P) + 2;
        belt.style.width = reps * P + "px";
        belt.style.height = BAND + "px";
        let x = 0;
        for (let r = 0; r < reps; r++)
          for (const col of cols) {
            for (const { slot, m } of col.items) {
              const side = slot === "B" ? BIG : SMALL;
              const cd = card(m, side, side, RADIUS);
              cd.style.left = x + "px";
              cd.style.top = (slot === "b" ? SMALL + GAP : 0) + "px";
              if (FIT === "natural") cd.firstChild.style.objectFit = "contain";
              belt.appendChild(cd);
            }
            x += col.w + GAP;
          }
        const X0 = (32 * REF) / 512; // first big card starts 32px in at t=0 (measured)
        function renderAt(t) {
          const sh = frac(DIR * frac(t)) * P; // one period per loop
          belt.style.transform = `translate3d(${X0 - P + sh}px,0,0)`;
        }
        ...
        window.__timelines["loop-mosaicmarquee"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `mosaicmarquee` `carousel`.
