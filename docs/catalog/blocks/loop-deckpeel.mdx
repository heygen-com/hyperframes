---
title: "Loop Deck Peel"
description: "A centred deck where the front card peels away in turn. 8 image slots, 9s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deckpeel.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-deckpeel.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-deckpeel" item="loop-deckpeel" />

That writes one file: `compositions/loop-deckpeel.html`.
...
```html index.html
<div
  data-composition-id="loop-deckpeel"
  data-composition-src="compositions/loop-deckpeel.html"
  data-start="0"
  data-duration="9"
  data-track-index="1"
  data-width="1080"
...
| `peek` | `4` | 0 to 10, step 0.1 | Peek (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-deckpeel-bg"></div>
        <div id="loop-deckpeel-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Peel loop family — "A centered deck where the front card drops away in turn."
        // Measured on the 512 preview (9s, 8 images -> 33.75f steps), 2026-09-08:
        //   front card 246px square (0.48 H) at y133-378 (centred), cards behind peek out ABOVE it: level L
        //   card top = frontTop - L*20.5px (peek 4 -> 4% of H per level), width 246 - 7.5*L (238/232/224/216
        //   measured), horizontally centred, drawn back-to-front.
        //   Step: the front card drops straight down, bottom edge offset 0.5*f^3 px @30fps (f2..f6 = 4, 13,
        //   32, 61, 108; exponent 3.0 +-0.1) with a slight rotateZ (~4deg by f7, guessed 20deg*tau); the
        //   rest of the stack advances one level over f3..f13 with the smooth ease (0,1,3,5,8,12,15,17,19,20,21);
        //   a new back card POPS in at the step boundary (region luma 27 -> 99 in one frame, no fade).
        //   Visible levels are {0,1,2,4} right after the boundary and {0,1,3} after the shift (the level-3 card
        //   snaps to level 2 at the boundary) — reproduced literally via LEVELS below.
        // Guesses/knobs: front size 0.48 H (single variant; scaleIntensity is 0.48 — may be the same number);
        //   per-level shrink 7.5px@512 = 1.46% of H per level; drop in tau units d = 37.5*H*tau^3 (only one
        //   duration seen); easing enum -> bezier map (custom via easeBezier); shadow units % of H (off).
        ...
        const PEEK = (num("peek", 4) / 100) * REF;
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const FRONT = 0.48 * REF;
        const SHRINK = 0.0146 * REF; // per level
        const LEVELS = [1, 2, 4]; // stack levels behind the front at p = 0
        ...
        document.getElementById("loop-deckpeel-loop").dataset.duration = DUR;
        background(document.getElementById("loop-deckpeel-bg"));
        const stage = document.getElementById("loop-deckpeel-stage");
        const shadow = bool("shadowEnabled", false)
          ? `${(num("shadowX", 0) * REF) / 100}px ${(num("shadowY", 1) * REF) / 100}px ${(num("shadowBlur", 1.5) * REF) / 100}px ${(num("shadowSpread", -0.25) * REF) / 100}px ${str("shadowColor", "#000000")}${Math.round(
              ...
              .padStart(2, "0")}`
          : "";
        // 4 card elements: index 0 = dropping front, 1..3 = stack (back to front drawn by DOM order)
        const cards = [];
        for (let i = 0; i < 4; i++) {
          const el = document.createElement("div");
          el.className = "card";
        ...
        const frontTop = -FRONT / 2;
        function place(c, m, level, dy, rot) {
          const size = FRONT - SHRINK * level;
          ...
          // top-anchored: the level's top edge sits PEEK*level above the front's top edge
          c.el.style.transform = `translate(${-w / 2}px, ${frontTop - PEEK * level + dy}px) rotate(${rot}deg)`;
        }
        function renderAt(t) {
          const g = frac(t) * N,
            k = Math.floor(g),
            tau = g - k;
          const shift = ease((tau - 0.09) / 0.295); // f3..f13 of 33.75
          // stack: image k+1+m at level LEVELS[m] - shift  (m = 0..2, back to front => reverse DOM order)
          for (let m = 2; m >= 0; m--)
            place(cards[3 - m], IMAGES[(k + 1 + m) % N], LEVELS[m] - shift, 0, 0);
          stage.appendChild(cards[0].el); // front card on top
          const drop = 37.5 * REF * tau * tau * tau;
          if (drop < H) place(cards[0], IMAGES[k % N], 0, drop, 20 * tau);
          else cards[0].el.style.display = "none";
        }
        ...
        window.__timelines["loop-deckpeel"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `deckpeel`.
