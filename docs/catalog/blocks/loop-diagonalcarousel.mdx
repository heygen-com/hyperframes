---
title: "Loop Diagonal Carousel"
description: "A fanned deck of overlapping image cards slides along a diagonal. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-diagonalcarousel.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-diagonalcarousel.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-diagonalcarousel" item="loop-diagonalcarousel" />

That writes one file: `compositions/loop-diagonalcarousel.html`.
...
```html index.html
<div
  data-composition-id="loop-diagonalcarousel"
  data-composition-src="compositions/loop-diagonalcarousel.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `direction` | `downRight` | `downRight`, `downLeft`, `upRight`, `upLeft` | Direction of travel. |
| `movement` | `continuous` | `continuous`, `stepped` | Movement (engine parameter; see the block header comment). |
| `overlap` | `55` | 0 to 110, step 1 | Overlap (engine parameter; see the block header comment). |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-diagonalcarousel-bg {
        position: absolute;
        inset: 0;
      }
      #loop-diagonalcarousel-stage {
        position: absolute;
        left: 50%;
        top: 50%;
        width: 0;
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-diagonalcarousel-bg"></div>
        <div id="loop-diagonalcarousel-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // DiagonalCarousel loop family — a fanned deck of overlapping cards sliding along a
        // 45° diagonal through the frame centre. Read off the preview (2026-09-08, 12s, 8 placeholders):
        //   card = 180px @512 (35.2% of H; no planeSize setting), corners 15px = borderRadius 32 at 1080 ✓.
        //   pitch = card·(1 − overlap%) = 81px ✓ in x AND y (45° line); z-order: the card nearest the frame centre is on
        //     top, z falls off both ways (deck fan); swaps when two cards are equidistant (f22).
        //   continuous: uniform 1.8px/frame per axis = one pitch per DUR/N (45f); 8 pitches per loop => seamless.
        //   motion for 'downRight' is toward the TOP-LEFT (grey-72 card left edge 166 -> 93 over 42f) — the enum names
        //     the diagonal, cards flow against it (mirrored for the other three values — unverified).
        //   sizes uniform within 2% => scaleIntensity has no visible size effect (unused). stagger (40) not visible in
        //     continuous mode (unused; stepped mode uses the Flow step ease, window [0.053,0.40] — guess).
        ...
        const DIRN = str("direction", "downRight");
        const STEPPED = str("movement", "continuous") === "stepped";
        const OVER = num("overlap", 55) / 100;
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const S = 0.352 * REF; // measured card size (see header)
        const DX = /Right/.test(DIRN) ? 1 : -1,
          DY = /^down/.test(DIRN) ? 1 : -1; // diagonal direction; cards flow against it
        ...
        const ease =
          EASING === "linear"
            ...
            : EASING === "custom" && BEZ.length === 4 && BEZ.every(isFinite)
              ? bezier(...BEZ)
              : bezier(0.5, 0, 0.5, 1);
        const shadow = () => {
          if (str("shadowEnabled", "False").toLowerCase() !== "true") return "";
          const p = REF / 100,
            c = str("shadowColor", "#000000"),
            op = num("shadowOpacity", 40) / 100;
          const [r, g, b] = [1, 3, 5].map((i) => parseInt(c.slice(i, i + 2), 16) || 0);
          return `${num("shadowX", 0) * p}px ${num("shadowY", 1) * p}px ${num("shadowBlur", 1.5) * p}px ${num("shadowSpread", -0.25) * p}px rgba(${r},${g},${b},${op})`;
        };

        document.getElementById("loop-diagonalcarousel-loop").dataset.duration = DUR;
        background(document.getElementById("loop-diagonalcarousel-bg"));
        const stage = document.getElementById("loop-diagonalcarousel-stage");
        const PITCH = S * (1 - OVER),
          P = N * PITCH;
        const REPS = Math.ceil((Math.hypot(W, H) + 2 * S) / P) + 1; // belt copies needed to cover the diagonal
        const els = [];
        for (let r = 0; r < REPS; r++)
          for (let k = 0; k < N; k++) {
            ...
            els.push({ el, k, r: r - Math.floor(REPS / 2) });
          }
        const wrap = (x) => frac((x + P / 2) / P) * P - P / 2;
        function renderAt(t) {
          let travel;
          if (STEPPED) {
            const s = Math.min(N - 1e-9, t * N),
              si = Math.floor(s);
            travel = (si + ease((s - si - 0.053) / 0.347)) * PITCH;
          } else travel = t * P;
          for (const e of els) {
            const d = wrap(e.k * PITCH - travel) + e.r * P; // along the diagonal (px per axis), centre = 0
            e.el.style.transform = `translate3d(${DX * d}px,${DY * d}px,0)`;
            e.el.style.zIndex = String(Math.round(10000 - Math.abs(d)));
          }
        }
        ...
        window.__timelines["loop-diagonalcarousel"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `diagonalcarousel` `carousel`.
