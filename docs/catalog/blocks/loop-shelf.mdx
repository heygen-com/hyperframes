...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-shelf.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-shelf.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-shelf" item="loop-shelf" />

That writes one file: `compositions/loop-shelf.html`.
...
<div
  ...
  data-composition-src="compositions/loop-shelf.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `borderRadius` | `18` | 0px to 200px, step 1px | Card corner radius in px at 1080. |
| `direction` | `rightToLeft` | `rightToLeft`, `leftToRight` | Direction of travel. |
| `easing` | `smooth` | `smooth`, `snappy`, `elastic`, `linear` | Easing family for stepped motion. |
| `spacing` | `8` | 0 to 16, step 0.1 | Spacing (engine parameter; see the block header comment). |
| `sideTilt` | `8` | 0 to 16, step 0.1 | Side Tilt (engine parameter; see the block header comment). |
| `dim` | `40` | 0 to 80, step 1 | Dim (engine parameter; see the block header comment). |
...
Tagged `loop` `images` `showcase` `gallery` `shelf` `3d`.
