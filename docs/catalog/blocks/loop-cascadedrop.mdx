---
title: "Loop Cascade Drop"
description: "Image cards drop in from above one by one onto a loose centre stack. 8 image slots, 7s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-cascadedrop.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-cascadedrop.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-cascadedrop" item="loop-cascadedrop" />

That writes one file: `compositions/loop-cascadedrop.html`.
...
```html index.html
<div
  data-composition-id="loop-cascadedrop"
  data-composition-src="compositions/loop-cascadedrop.html"
  data-start="0"
  data-duration="7"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.78` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `rotation` | `100` | 0 to 200, step 1 | Rotation (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-cascadedrop-bg"></div>
        <div id="loop-cascadedrop-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Tumble loop family — cards drop in from above one by one onto a loose centre stack
        // (each new card on top), the stack then falls off the bottom. Measured on the 512x512/7s/8-card preview (2026-09-08,
        // per-card grey tracking, bottom-edge + top-edge line fits):
        //   card 349px square = scaleIntensity*(H - 2*padding%H) (0.7% over); rest centre y = H/2, x = H/2 + (-1)^(i+1)*0.0056H*i.
        //   card i drops at frame -1+15i of 210 (stagger 0.5s): bottom edge -228 -> 513 (centre -0.79H absolute = -1.29H from the frame centre -> +0.158H
        //   overshoot) with a power-2.5 ease-out over 14.25 frames (maxerr 1px), then returns to H/2 with smooth bezier over
        //   12.25 frames starting 0.5 frame later. No undershoot.
        //   tilt (top/bottom edge fits, deg): rest = [-3.0, 2.1, -1.5, 2.3] repeating (cards 4-7: -3.2, 2.0, -1.5, 2.3);
        //   in flight the tilt eases from 2x rest to rest with the drop (card0 -6.3@f1 -> -3.0@f8).
        //   sweep: whole stack moves down with an exponential (x1.2/frame, i.e. expoIn) from f173 to the loop end, ~2H.
        //   frames 0 and 209 are both empty -> exact seam.
        // Guesses/knobs: `rotation` (100) scales the tilt table linearly; overshoot as % of H (could be % of travel);
        //   padding as % of H; shadow* units 1% of H; natural fit keeps the measured size as card HEIGHT.
        ...
        const SCALE = num("scaleIntensity", 0.78);
        const ROT = num("rotation", 100) / 100;
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(Math.min(8, num("imageCount", 16)));
        ...
        const outP = (u) => (u <= 0 ? 0 : u >= 1 ? 1 : 1 - Math.pow(1 - u, 2.5));
        const expoIn = (u) => (u <= 0 ? 0 : u >= 1 ? 1 : Math.pow(2, 10 * u - 10));

        document.getElementById("loop-cascadedrop-loop").dataset.duration = DUR;
        background(document.getElementById("loop-cascadedrop-bg"));
        const stage = document.getElementById("loop-cascadedrop-stage");
        ...
        const SIZE = (REF - 2 * PAD) * SCALE; // card height
        const TILTS = [-3.0, 2.1, -1.5, 2.3];
        // schedule in fractions of the loop (measured in 210ths on the 7 s preview)
        const F = 1 / 210;
        const DROP0 = -1 * F,
          STAG = 15 * F,
          DROP_D = 14.25 * F,
          ...
          SWEEP0 = 173 * F,
          SWEEP_D = 37 * F;
        const Y_START = -1.29 * REF,
          Y_OVER = 0.158 * REF,
          SWEEP = 2 * REF;

        const cards = [];
        for (let i = 0; i < N; i++) {
          // append in drop order: later cards land on top
          const m = IMAGES[i],
            a = aspect(m, FIT, CROP_AR);
          const c = card(m, SIZE * a, SIZE, RADIUS);
          if (shadow) c.style.boxShadow = shadow;
          c.style.left = (-SIZE * a) / 2 + "px";
          c.style.top = -SIZE / 2 + "px";
          stage.appendChild(c);
          cards.push({ el: c, x: (i % 2 ? 1 : -1) * 0.0056 * REF * i, tilt: TILTS[i % 4] * ROT });
        }
        function renderAt(t) {
          const sweep = SWEEP * expoIn((t - SWEEP0) / SWEEP_D);
          for (let i = 0; i < N; i++) {
            const c = cards[i],
              s = DROP0 + i * STAG;
            let y, rot;
            if (t < s + RET0) {
              // dropping: start -> overshoot
              const e = outP((t - s) / DROP_D);
              y = Y_START + (Y_OVER - Y_START) * e;
              rot = c.tilt * (2 - e);
            } else {
              // returning to rest, then resting
              y = Y_OVER * (1 - smooth((t - s - RET0) / RET_D));
              rot = c.tilt;
            }
            c.el.style.transform = `translate3d(${c.x}px,${y + sweep}px,0) rotate(${rot}deg)`;
          }
        }
        ...
        window.__timelines["loop-cascadedrop"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `cascadedrop`.
