...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-3d-9.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-3d-9.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-3d-9" item="loop-3d-9" />

That writes one file: `compositions/loop-3d-9.html`.
...
```html index.html
<div
  data-composition-id="loop-3d-9"
  data-composition-src="compositions/loop-3d-9.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `2478` | 0 to 8120, step 10 | Card size in scene units. |
| `orbitRadius` | `9047` | 0 to 25848, step 10 | Ring radius in scene units. |
| `rotationX` | `20` | -180° to 180°, step 1° | Formation tilt about the horizontal axis, degrees. |
| `rotationY` | `230` | -180° to 180°, step 1° | Formation turn about the vertical axis, degrees. |
| `rotationZ` | `48` | -180° to 180°, step 1° | Formation roll about the view axis, degrees. |
| `distance` | `20302` | 0 to 48424, step 10 | Camera distance from the formation. |
...
| `offsetY` | `5` | -100% to 100%, step 1% | Vertical offset of the formation, % of height. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-3d-9-bg"></div>
        <div id="loop-3d-9-stage">
          <div id="loop-3d-9-ring"><div id="loop-3d-9-spin"></div></div>
        </div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-3d-9-loop").dataset.duration = DUR;
        background(document.getElementById("loop-3d-9-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R, 1); // world -> px (ring centre or ring at z=0)
        const Rpx = R * s,
          Dpx = D * s;
        const stage = document.getElementById("loop-3d-9-stage");
        stage.style.perspective = f + "px";
        const ring = document.getElementById("loop-3d-9-ring");
        ...
        const spin = document.getElementById("loop-3d-9-spin");
        ...
        window.__timelines["loop-3d-9"] = tl;
      })();
    </script>
  </body>
</html>
