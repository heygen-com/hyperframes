---
title: "Loop Iso"
description: "One diagonal ribbon of isometric-skewed image cards flows through the frame centre. 8 image slots, 10s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-iso.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-iso.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-iso" item="loop-iso" />

That writes one file: `compositions/loop-iso.html`.
...
```html index.html
<div
  data-composition-id="loop-iso"
  data-composition-src="compositions/loop-iso.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `backgroundColor` | `#0e0e12` | color | Flat colour when Type is Color; also the ground under an Image. |
...
| `scaleIntensity` | `0.42` | 0 to 1, step 0.05 | Strength of the size change. |
| `shadowIntensity` | `0.4` | 0 to 1, step 0.05 | Shadow Intensity (engine parameter; see the block header comment). |
| `borderRadius` | `24` | 0px to 200px, step 1px | Card corner radius in px at 1080. |
| `direction` | `up-right` | `up-right`, `down-left`, `down-right`, `up-left` | Direction of travel. |
| `motion` | `glide` | `glide`, `step` | Motion (engine parameter; see the block header comment). |
| `easing` | `smooth` | `smooth`, `snappy`, `elastic`, `linear` | Easing family for stepped motion. |
| `tilt` | `70` | -180° to 180°, step 1° | Tilt (engine parameter; see the block header comment). |
| `spacing` | `40` | 0 to 80, step 1 | Spacing (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-iso-bg"></div>
        <div id="loop-iso-stage"><div id="loop-iso-ribbon"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Iso Cascade loop family — ONE diagonal ribbon of skewed cards through the frame centre,
        // streaming up-right. Measured on the 512 preview (2026-09-08):
        //   card: square, side = scaleIntensity*H (0.42 -> 215px; vertical edge 207-210px, corners r=24@1080), 2D skewY:
        //         top/bottom edges FALL to the right with slope tilt/100 (fits 0.684/0.700/0.687 on three cards).
        //   ribbon: consecutive cards offset (+71, -48.5)px = (0.14H, -0.14H*tilt/100) at spacing 40 -> step_x = spacing*0.35%*H;
        //           the ribbon slope is the mirror of the edge slope (-tilt/100). Up-right card is drawn ON TOP.
        //   motion: glide = linear, N pitches per loop (8 cards -> 1.93px/frame measured vs 1.89 predicted);
        //           seamless because the ribbon repeats every N cards. step = N eased steps (smooth ease, 60% of the step; guess from IsoFocus).
        //   shadow: ~8px dark band at the upper card's edge on the card below, ~30% at shadowIntensity 0.4 -> drop-shadow.
        //   ONE ribbon only (top-left / bottom-right corners empty in the preview) — at 16:9 corners stay empty (not invented).
        // Knobs: direction enum (mirrors), step-motion ease/window, shadow blur.
        ...
        const SIDE = num("scaleIntensity", 0.42) * REF;
        const SLOPE = num("tilt", 70) / 100;
        const STEPX = num("spacing", 40) * 0.0035 * REF;
        const DIRN = str("direction", "up-right");
        const MOTION = str("motion", "glide");
        const EASING = str("easing", "smooth");
        const SHADOW = num("shadowIntensity", 0.4);
        const RADIUS = (num("borderRadius", 24) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        // direction: up-right is the measured default; others mirror the whole stage
        const MX = /left/.test(DIRN) ? -1 : 1,
          MY = /down/.test(DIRN) ? -1 : 1;

        document.getElementById("loop-iso-loop").dataset.duration = DUR;
        background(document.getElementById("loop-iso-bg"));
        document.getElementById("loop-iso-stage").style.transform = `scale(${MX},${MY})`;
        const ribbon = document.getElementById("loop-iso-ribbon");
        ...
        const EASES = {
          ...
          linear: [0, 0, 1, 1],
          snappy: [0.4, 0, 0.2, 1],
          elastic: [0.65, 0, 0.35, 1],
        };
        const ease = bezier(...(EASES[EASING] || EASES.smooth));

        // ribbon step (screen): +x, -x*slope. Enough cards to cover the diagonal both ways, plus one period of slack.
        const STEP = { x: STEPX, y: -STEPX * SLOPE };
        const span = Math.hypot(W, H) / Math.hypot(STEP.x, STEP.y);
        const K0 = -Math.ceil(span / 2) - N,
          K1 = Math.ceil(span / 2) + N;
        const skew = (Math.atan(SLOPE) * 180) / Math.PI;
        for (let k = K0; k <= K1; k++) {
          const m = IMAGES[((k % N) + N) % N],
            a = aspect(m, FIT, CROP_AR);
          const h = SIDE,
            w = SIDE * a;
          const c = card(m, w, h, RADIUS);
          // card centre at k*STEP; skew about the centre so the ribbon axis passes through card centres
          c.style.left = k * STEP.x - w / 2 + "px";
          c.style.top = k * STEP.y - h / 2 + "px";
          c.style.transform = `skewY(${skew}deg)`;
          c.style.zIndex = k - K0 + 1;
          c.style.filter = `drop-shadow(0 0 ${0.015 * REF}px rgba(0,0,0,${SHADOW}))`;
          ribbon.appendChild(c);
        }
        function travel(t) {
          if (MOTION !== "step") return t * N;
          const s = t * N,
            k = Math.floor(s),
            u = s - k;
          return k + ease(Math.min(1, u / 0.6)); // ponytail: 60% window copied from IsoFocus, unmeasured here
        }
        function renderAt(t) {
          const p = travel(t); // pitches travelled, up-right
          ribbon.style.transform = `translate3d(${p * STEP.x}px,${p * STEP.y}px,0)`;
        }
        ...
        window.__timelines["loop-iso"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `iso`.
