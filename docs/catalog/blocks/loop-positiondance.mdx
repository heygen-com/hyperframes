---
title: "Loop Position Dance"
description: "Six image cards cycle through arrangements, changing position and size each beat. 6 image slots, 4s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-positiondance.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-positiondance.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-positiondance" item="loop-positiondance" />

That writes one file: `compositions/loop-positiondance.html`.

## Add it to your video

It runs for 4 seconds at 1080×1080. Paste this into your composition:

```html index.html
<div
  data-composition-id="loop-positiondance"
  data-composition-src="compositions/loop-positiondance.html"
  data-start="0"
  data-duration="4"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.28` | 0 to 1, step 0.05 | Strength of the size change. |
| `borderRadius` | `43` | 0px to 200px, step 1px | Card corner radius in px at 1080. |
...
>
  <head>
    ...
    <style>
      * {
      ...
      #loop-positiondance-bg {
        position: absolute;
        inset: 0;
      }
      #loop-positiondance-stage {
        position: absolute;
        inset: 0;
      }
      .card img {
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-positiondance-bg"></div>
        <div id="loop-positiondance-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Jumble loop family — 6 cards cycle through three random arrangements (position AND size)
        // and return to the first. Measured on the 512x512/4s preview (2026-09-08, per-card grey tracking):
        //   3 rests (f0-16, f38-56, f78-96 of 120) + 3 moves of 22 frames at frames 16-38 of each 40-frame third; every card
        //   moves at once; move profile smooth (card 101 width 92,95,95,100,107,116,125,130,134,136,139,140 at f16..38).
        //   card sizes at the rests 76..202 px (sqrt of area), i.e. 0.28H*[0.55,1.45] (scaleIntensity 0.28 -> mean 143 px);
        //   rest sets are NOT permutations of one slot set (A 196/158/153/138/101/91, B 202/156/140/110/95/76,
        //   C 196/185/155/137/123/95) -> seeded random layout, statistics matched, positions differ from the preview.
        //   cards touch the frame edge in the preview (rest C card 74 at x1,y510): padding used only as a soft margin.
        // Guesses/knobs: spacing (0) -> minimum gap between cards in the rejection sampler; easing enum as in flipgrid;
        //   shadow* 1% of H; natural fit keeps the size as card HEIGHT.
        ...
        const __V = Object.assign(
          ...
          (window.__hyperframes && window.__hyperframes.getVariables
            ...
            : null) || {},
          { loopDuration: 4, loopRepeat: 2, imageCount: 6 },
        );
        const __PH = [
        ...
        const __MEDIA = await Promise.all(
          ["image1", "image2", "image3", "image4", "image5", "image6"].map(async (id, i) => {
            const v = __V[id];
        ...
        const SCALE = num("scaleIntensity", 0.28);
        const SPACING = (num("spacing", 0) / 100) * REF;
        const RADIUS = (num("borderRadius", 43) * REF) / 1080;
        const IMAGES = pick(Math.min(6, num("imageCount", 16)));
        ...
        document.getElementById("loop-positiondance-loop").dataset.duration = DUR;
        background(document.getElementById("loop-positiondance-bg"));
        const stage = document.getElementById("loop-positiondance-stage");
        ...
        const MEAN = SCALE * REF;
        const aspects = IMAGES.map((m) => aspect(m, FIT, CROP_AR));
        // three arrangements: size + centre per card, rejection-sampled so cards overlap little (spacing = required gap)
        const arr = [];
        for (let p = 0; p < 3; p++) {
          const poses = [];
          for (let i = 0; i < N; i++) {
            const s = MEAN * (0.55 + 0.9 * rnd()),
              w = s * aspects[i],
              h = s;
            let best = null,
              bestPen = Infinity;
            for (let k = 0; k < 40; k++) {
              const x = PAD + w / 2 + rnd() * Math.max(1, W - 2 * PAD - w),
                y = PAD + h / 2 + rnd() * Math.max(1, H - 2 * PAD - h);
              let pen = 0;
              for (const q of poses) {
                const ox = Math.max(
                  0,
                  Math.min(x + w / 2, q.x + q.w / 2) - Math.max(x - w / 2, q.x - q.w / 2) + SPACING,
                );
                const oy = Math.max(
                  0,
                  Math.min(y + h / 2, q.y + q.h / 2) - Math.max(y - h / 2, q.y - q.h / 2) + SPACING,
                );
                pen += ox * oy;
              }
              if (pen < bestPen) {
        ...
        const cards = IMAGES.map((m, i) => {
          const c = card(m, MEAN * aspects[i], MEAN, RADIUS); // base size; scaled per frame
          if (shadow) c.style.boxShadow = shadow;
          c.style.left = (-MEAN * aspects[i]) / 2 + "px";
          c.style.top = -MEAN / 2 + "px";
          c.style.zIndex = i + 1;
          stage.appendChild(c);
          return c;
        });
        const MOVE0 = 16 / 40,
          MOVE1 = 38 / 40; // move window inside each third
        function renderAt(t) {
          const ph = t * 3,
            p = Math.min(2, Math.floor(ph)),
            u = ph - p;
          const e = ease((u - MOVE0) / (MOVE1 - MOVE0));
          const A = arr[p],
            B = arr[(p + 1) % 3];
          for (let i = 0; i < N; i++) {
            const a = A[i],
            ...
            const x = a.x + (b.x - a.x) * e,
              y = a.y + (b.y - a.y) * e,
              s = (a.s + (b.s - a.s) * e) / MEAN;
            cards[i].style.transform = `translate3d(${x}px,${y}px,0) scale(${s})`;
          }
        }
        ...
        window.__timelines["loop-positiondance"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `positiondance`.
