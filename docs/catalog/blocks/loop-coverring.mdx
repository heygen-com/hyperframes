---
title: "Loop Cover Ring"
description: "Image cards on a horizontal 3D band seen from outside, spinning past the camera. 8 image slots, 14s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-coverring.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-coverring.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-coverring" item="loop-coverring" />

That writes one file: `compositions/loop-coverring.html`.
...
```html index.html
<div
  data-composition-id="loop-coverring"
  data-composition-src="compositions/loop-coverring.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.37` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `motion` | `steps` | `steps`, `flow` | Motion (engine parameter; see the block header comment). |
| `ringSize` | `131` | 0 to 262, step 10 | Ring Size (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-coverring-bg"></div>
        <div id="loop-coverring-stage"><div id="loop-coverring-ring"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Band loop family — N cards on a horizontal 3D ring seen from outside (camera on the
        // ring's plane), the front card centred and largest, neighbours turned away, the back half faded. STEPPED spin:
        // one card pitch per DUR/N with a hold, front card moving right (direction=right).
        // Measured on the 512 preview (2026-09-08, 8 cards): rest pose front card 240 px tall (163..403 -> centre x 283,
        // i.e. shifted +27 px = 0.053 H from the frame centre — a constant offset, both 45deg neighbours have equal
        // inner-edge heights 233/232 so it is not a rotation); neighbours: inner edges at x 106 / 457 with heights
        // 233 / 232, heights 220.5 / 223.5 at x 0 / 504; back (135deg) cards peek out at x 110..165 with heights 160..147,
        // luma 27 + 0.455*(grey-27) -> opacity 1 - backFade% behind, 1.0 at 45deg. Least-squares fit of a ring with
        // cards yawed kappa*theta: kappa 0.73, R/D 0.282, s/R 0.60, f free 1200..1500 @512 (rms 1.1..2.7 px); with the
        // orbit-group focal law f = 1200*(100/perspective) @1080 (1034 @512) rms 2.7 vs 4.2 for the 1000 law.
        // Timing: rest f27..54 and f81..108, move f55..80 (25 f of a 52.5 f step = 0.476), incoming card's edge
        // progress 0.06 @ u=0.2 and 0.50 @ u=0.5 -> cubic-bezier(0.5,0,0.5,1) ("smooth"); loop starts with a 2.5 f hold.
        // Guesses: yaw = theta - 17.2deg*sin(theta) (equivalent to kappa 0.73 at 45deg but periodic; a raw kappa*theta broke the seam);
        // cards turned less than the ring angle ( fully tangent cards cannot place the neighbour
        // edge 57 px away from the front card at equal height); R = 0.618 H at ringSize 131 (fitted, no clean law in
        // padding); D = R/0.282; card s = scaleIntensity*H (0.37 -> 189 vs 187 fitted); +0.053 H x-offset kept to
        // match the preview; tilt = rotateX of the ring; easings other than smooth/linear/custom fall back to smooth.
        ...
        const S = num("scaleIntensity", 0.37) * REF; // card height
        const R = (0.618 * REF * num("ringSize", 131)) / 131;
        const D = R / 0.282;
        const YAWC = 63.6 * (1 - 0.73); // yaw = theta - YAWC*sin(theta): 32.85deg at 45deg (kappa 0.73), periodic, 180 at the back
        const PERSP = Math.max(1, num("perspective", 55));
        const f = (1200 * (100 / PERSP) * REF) / 1080;
        const TILT = num("tilt", 0);
        const FADE = Math.max(0, Math.min(1, num("backFade", 55) / 100));
        const DIR = str("direction", "right") === "right" ? 1 : -1;
        const STEPS = str("motion", "steps") === "steps";
        const EASING = str("easing", "smooth");
        const BEZ = (EASING === "custom" ? str("easeBezier", "0.5,0,0.5,1") : "0.5,0,0.5,1")
          .split(",")
          .map(Number);
        const OFFX = 0.053 * REF;
        const MOVE = 0.476,
          HOLD0 = 0.048; // fraction of a step: hold, move, hold
        const RADIUS = (num("borderRadius", 32) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;

        document.getElementById("loop-coverring-loop").dataset.duration = DUR;
        background(document.getElementById("loop-coverring-bg"));
        document.getElementById("loop-coverring-stage").style.perspective = f + "px";
        const ring = document.getElementById("loop-coverring-ring");
        ring.style.transform = `translate3d(0,0,${f - D}px) rotateX(${TILT}deg)`;
        document.getElementById("loop-coverring-stage").style.transform = `translateX(${OFFX}px)`; // screen-space shift (neighbours/slivers share it in the preview)

        // cubic-bezier(x1,y1,x2,y2) as a function of progress (Newton on x, then y)
        function bezier(u) {
          if (EASING === "linear") return u;
          const [x1, y1, x2, y2] =
            BEZ.length === 4 && BEZ.every((v) => !isNaN(v)) ? BEZ : [0.5, 0, 0.5, 1];
          const bx = (p) => 3 * (1 - p) * (1 - p) * p * x1 + 3 * (1 - p) * p * p * x2 + p * p * p;
          const by = (p) => 3 * (1 - p) * (1 - p) * p * y1 + 3 * (1 - p) * p * p * y2 + p * p * p;
          let p = u;
          for (let i = 0; i < 8; i++) {
            const d = (bx(p + 1e-4) - bx(p)) / 1e-4;
        ...
        function renderAt(t) {
          ...
          if (STEPS) {
            const k = Math.floor(u),
              g = u - k;
            const e = g < HOLD0 ? 0 : g < HOLD0 + MOVE ? bezier((g - HOLD0) / MOVE) : 1;
            spin = ((k + e) * 360) / N;
          } else spin = (u * 360) / N;
          for (const c of cards) {
            const th = c.theta + DIR * spin,
              rad = (th * Math.PI) / 180,
              cs = Math.cos(rad);
            c.el.style.transform = `translate3d(${R * Math.sin(rad)}px,0,${R * cs}px) rotateY(${th - YAWC * Math.sin(rad)}deg)`;
            // back half faded; short smoothstep around 90deg where cards are edge-on anyway
            const b = Math.max(0, Math.min(1, (0.15 - cs) / 0.3)),
              sm = b * b * (3 - 2 * b);
            c.el.style.opacity = 1 - FADE * sm;
          }
        }
        ...
        window.__timelines["loop-coverring"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `coverring` `3d`.
