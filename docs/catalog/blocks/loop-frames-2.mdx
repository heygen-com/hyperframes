...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-2.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-2.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-frames-2" item="loop-frames-2" />

That writes one file: `compositions/loop-frames-2.html`.
...
```html index.html
<div
  data-composition-id="loop-frames-2"
  data-composition-src="compositions/loop-frames-2.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `88` | 0 to 200, step 1 | Card size in scene units. |
...
| `rowsSkipped` | `0` | 0 to 4, step 0.1 | Rows Skipped (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-frames-2-bg"></div>
        <div id="loop-frames-2-sheet"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-frames-2-loop").dataset.duration = DUR;
        background(document.getElementById("loop-frames-2-bg"));
        const sheet = document.getElementById("loop-frames-2-sheet");
        ...
        window.__timelines["loop-frames-2"] = tl;
      })();
    </script>
  </body>
</html>
