...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wipe-2.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-wipe-2.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-wipe-2" item="loop-wipe-2" />

That writes one file: `compositions/loop-wipe-2.html`.
...
```html index.html
<div
  data-composition-id="loop-wipe-2"
  data-composition-src="compositions/loop-wipe-2.html"
  data-start="0"
  data-duration="13"
  data-track-index="1"
  data-width="1080"
...
| `imageFit` | `fill` | `fit`, `fill` | Image Fit (engine parameter; see the block header comment). |
| `planeSize` | `100` | 0 to 200, step 1 | Card size in scene units. |
| `scale` | `4` | 0 to 10, step 0.1 | Scale (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-wipe-2-bg"></div>
        <div id="loop-wipe-2-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-wipe-2-loop").dataset.duration = DUR;
        background(document.getElementById("loop-wipe-2-bg"));
        const stage = document.getElementById("loop-wipe-2-stage");
        ...
        window.__timelines["loop-wipe-2"] = tl;
      })();
    </script>
  </body>
</html>
