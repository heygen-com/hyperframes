...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-flip-6.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-flip-6.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-flip-6" item="loop-flip-6" />

That writes one file: `compositions/loop-flip-6.html`.
...
```html index.html
<div
  data-composition-id="loop-flip-6"
  data-composition-src="compositions/loop-flip-6.html"
  data-start="0"
  data-duration="16"
  data-track-index="1"
  data-width="1080"
...
| `visible` | `2` | 0 to 10, step 0.1 | Visible (engine parameter; see the block header comment). |
| `planeSize` | `470` | 0 to 1340, step 10 | Card size in scene units. |
| `gap` | `20` | 0px to 48px, step 1px | Spacing between cards. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-flip-6-bg"></div>
        <div id="loop-flip-6-stage"><div id="loop-flip-6-col"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-flip-6-loop").dataset.duration = DUR;
        background(document.getElementById("loop-flip-6-bg"));
        document.getElementById("loop-flip-6-stage").style.perspective = f + "px";
        const col = document.getElementById("loop-flip-6-col");
        ...
        window.__timelines["loop-flip-6"] = tl;
      })();
    </script>
  </body>
</html>
