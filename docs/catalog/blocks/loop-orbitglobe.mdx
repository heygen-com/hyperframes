---
title: "Loop Orbit Globe"
description: "Upright billboard image cards on the latitude rings of a globe that rotates. 8 image slots, 20s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbitglobe.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbitglobe.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-orbitglobe" item="loop-orbitglobe" />

That writes one file: `compositions/loop-orbitglobe.html`.
...
```html index.html
<div
  data-composition-id="loop-orbitglobe"
  data-composition-src="compositions/loop-orbitglobe.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `globeSize` | `50` | 0 to 100, step 1 | Globe Size (engine parameter; see the block header comment). |
| `backFade` | `55` | 0 to 110, step 1 | Back Fade (engine parameter; see the block header comment). |
| `tilt` | `27` | -180° to 180°, step 1° | Tilt (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-orbitglobe-bg"></div>
        <div id="loop-orbitglobe-stage"><div id="loop-orbitglobe-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // OrbitGlobe loop family — upright billboards on latitude rings of a
        // sphere whose axis is tilted in the screen plane, one full turn per loop, back side fading.
        // Measured on the 512 preview (2026-09-08):
        //   rings:   frame-0 similarity has minima every 75 frames (2.5 s) -> 8 cards per ring (45deg,
        //            360deg/20 s). Near cards fall on three lines tilted 27deg: rings at 0 and +-45deg
        //            (line offsets +-130px ~ R*sin45 under the fitted camera) -> 24 cards.
        //   tilt:    a tracked card moves right AND down with slope 0.505 = tan 27deg -> tilt is a
        //            rotateZ of the globe axis (CSS +z rotation = clockwise = down-right).
        //   size:    SCREEN-space: base 112px = 21.9% of H = 0.875 R, times (1 + scaleIntensity*zr),
        //            zr = z/R toward the camera (pole card 143px = 112*1.28; same card at zr 0.70 reads
        //            134 = 112*1.196; far pole 81 matches the 75-85px run-length mass).
        //   camera:  R = globeSize/200 * H (128px), D = 3.4R, f = 0.985*H: limb 154px (= bbox 210 -
        //            base/2) and centre speed 2.2px/frame at 18deg/s both fit.
        //   spin:    uniform, direction=left -> near side moves RIGHT (+6.5px/3 frames) = positive
        //            rotateY; 'right' mirrors; 'alternate' unknown (treated as left).
        //   backFade: colour x (1 - backFade/100) on the far side; ramp as in globe.html (unverified here).
        // Knobs: RINGS/COLS constants (not derivable from settings), half-pitch stagger on the +-45 rings
        // (guess), motion=waypoints not implemented (only the continuous preview exists), gap unused
        // (billboards overlap in the reference; cards are sized from base, not from a pitch).
        ...
        const GSIZE = num("globeSize", 50);
        const SI = num("scaleIntensity", 0.28);
        const BFADE = Math.min(100, Math.max(0, num("backFade", 55)));
        const TILT = (num("tilt", 27) * Math.PI) / 180;
        const DIR = str("direction", "left") === "right" ? -1 : 1;
        const RADIUS = (num("borderRadius", 11) * REF) / 1080;
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const RINGS = [0, 45, -45],
          COLS = 8; // ponytail: measured on the one preview

        document.getElementById("loop-orbitglobe-loop").dataset.duration = DUR;
        background(document.getElementById("loop-orbitglobe-bg"));

        const R = (GSIZE / 200) * REF,
          D = 3.4 * R,
          f = 0.985 * REF,
          BASE = 0.875 * R;
        const stage = document.getElementById("loop-orbitglobe-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-orbitglobe-world");
        const CZ = f - D; // camera at stage z = f, centre D in front

        const cards = [];
        RINGS.forEach((latDeg, ri) => {
          const lat = (latDeg * Math.PI) / 180,
            phase = ((ri % 2) * Math.PI) / COLS;
          for (let j = 0; j < COLS; j++) {
            const m = IMAGES[cards.length % N],
            ...
            cards.push({ el, shade, a, lat, lon: (j * 2 * Math.PI) / COLS + phase });
          }
        });
        const clamp01 = (x) => (x < 0 ? 0 : x > 1 ? 1 : x);
        function renderAt(t) {
          const ang = DIR * 2 * Math.PI * frac(t),
            ct = Math.cos(TILT),
            st = Math.sin(TILT);
          for (const c of cards) {
            // ring point, spun about the globe's y axis (CSS-handed), then the axis tilted by rotateZ
            const cl = Math.cos(c.lat),
              lo = c.lon + ang;
            const x0 = cl * Math.sin(lo),
              y0 = -Math.sin(c.lat),
              zr = cl * Math.cos(lo);
            const dx = x0 * ct - y0 * st,
              dy = x0 * st + y0 * ct;
            const depth = D - zr * R;
            const screen = BASE * (1 + SI * zr),
              k = depth / f; // undo the perspective division -> screen-space size
            const h = screen * k,
              w = h * c.a;
            ...
            c.el.style.transform = `translate3d(${dx * R}px,${dy * R}px,${CZ + zr * R}px) translate(-50%,-50%)`;
          }
        }
        ...
        window.__timelines["loop-orbitglobe"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `orbitglobe` `3d`.
