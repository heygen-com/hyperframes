---
title: "Loop Ticker Loop"
description: "A zoomed grid of marquee rows crawls across the frame at an angle. 8 image slots, 12s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-tickerloop.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-tickerloop.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-tickerloop" item="loop-tickerloop" />

That writes one file: `compositions/loop-tickerloop.html`.
...
```html index.html
<div
  data-composition-id="loop-tickerloop"
  data-composition-src="compositions/loop-tickerloop.html"
  data-start="0"
  data-duration="12"
  data-track-index="1"
  data-width="1080"
...
| `direction` | `opposed` | `opposed`, `same` | Direction of travel. |
| `zoom` | `45` | 0 to 90, step 1 | Zoom (engine parameter; see the block header comment). |
| `angle` | `-6` | -180° to 180°, step 1° | Angle (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-tickerloop-bg"></div>
        <div id="loop-tickerloop-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Crawl loop family — a zoomed square grid of marquee rows, the whole stage rotated by `angle`,
        // adjacent rows crawling in opposite directions at constant speed. Measured on the 512x512 preview (2026-09-08):
        //   row/column pitch 230px = 0.45 H (zoom 45 -> pitch = zoom% of H); gap 16px (rowGap 4 -> H/128 per unit);
        //   cards = pitch - gap = 214px squares; stage rotated -6deg (angle); 3 rows visible, centre row centred;
        //   centre row moves RIGHT, neighbours LEFT ("opposed"); speed 5.1 px/frame = 2 pitches per 3s (layout at f90
        //   repeats f0 with the next image) = 8 cards per 12s loop = one period (8 images per row, in order).
        // Guesses: row phase offset k*pitch/2 and image offset k*3 per row (reference offsets are irregular);
        //   movement=zoomPulse/zoomWaypoints and pulseCount/zoomPulseAmount/stops/waypointZoom/creepSpeed/pause not
        //   reproduced (continuous used); orientation=vertical rotates the stage by a further 90deg; direction=same -> all rows left.
        ...
        const PITCH = (num("zoom", 45) / 100) * REF;
        const GAP = (num("rowGap", 4) * REF) / 128;
        const ANGLE = num("angle", -6) + (str("orientation", "horizontal") === "vertical" ? 90 : 0);
        const OPPOSED = str("direction", "opposed") === "opposed";
        const RADIUS = num("borderRadius", 32) * 0.00009 * REF; // Bento calibration (unverified here)
        const IMAGES = pick(num("imageCount", 16));
        const N = IMAGES.length;
        const SIDE = PITCH - GAP;

        document.getElementById("loop-tickerloop-loop").dataset.duration = DUR;
        background(document.getElementById("loop-tickerloop-bg"));
        const stage = document.getElementById("loop-tickerloop-stage");
        stage.style.transform = `rotate(${ANGLE}deg)`;
        const DIAG = Math.hypot(W, H);
        const KR = Math.ceil(DIAG / 2 / PITCH) + 1; // rows either side of the centre row
        const rows = [];
        for (let k = -KR; k <= KR; k++) {
          const items = [];
          for (let i = 0; i < N; i++) {
            const m = IMAGES[(i + k * 3 + 3 * N) % N];
            const a = aspect(m, FIT, CROP_AR);
            items.push({ m, w: SIDE * a });
          }
          ...
          el.style.cssText = `left:${x0}px;top:${k * PITCH - SIDE / 2}px;height:${SIDE}px;width:${reps * P}px;`;
          let x = 0;
          for (let r = 0; r < reps; r++)
            for (const it of items) {
              const c = card(it.m, it.w, SIDE, RADIUS);
              c.style.left = x + "px";
              c.style.top = "0px";
              el.appendChild(c);
              x += it.w + GAP;
          ...
          // centre row (k=0) moves right when opposed; odd rows left. "same": everything left.
          rows.push({ el, P, dir: OPPOSED ? (k % 2 === 0 ? 1 : -1) : -1, phase: (k * PITCH) / 2 });
        }
        function renderAt(t) {
          for (const R of rows) {
        ...
        window.__timelines["loop-tickerloop"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `tickerloop` `carousel`.
