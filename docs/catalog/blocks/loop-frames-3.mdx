---
title: "Loop Frames 3"
description: "A sheet of large square image frames; each cycle the whole sheet shifts to the next frame. 8 image slots, 15s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-3.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-frames-3.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-frames-3" item="loop-frames-3" />

That writes one file: `compositions/loop-frames-3.html`.
...
```html index.html
<div
  data-composition-id="loop-frames-3"
  data-composition-src="compositions/loop-frames-3.html"
  data-start="0"
  data-duration="15"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `80` | 0 to 200, step 1 | Card size in scene units. |
| `gap` | `40` | 0px to 214px, step 10px | Spacing between cards. |
...
| `rowsSkipped` | `2` | 0 to 10, step 0.1 | Rows Skipped (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-frames-3-bg"></div>
        <div id="loop-frames-3-sheet"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-frames-3-loop").dataset.duration = DUR;
        background(document.getElementById("loop-frames-3-bg"));
        const sheet = document.getElementById("loop-frames-3-sheet");
        ...
        window.__timelines["loop-frames-3"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `frames`.
