---
title: "Loop Flip Grid"
description: "A padded grid of two-sided tiles; each tile flips to reveal its second image. 8 image slots, 8s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-flipgrid.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-flipgrid.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-flipgrid" item="loop-flipgrid" />

That writes one file: `compositions/loop-flipgrid.html`.
...
```html index.html
<div
  data-composition-id="loop-flipgrid"
  data-composition-src="compositions/loop-flipgrid.html"
  data-start="0"
  data-duration="8"
  data-track-index="1"
  data-width="1080"
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-flipgrid-bg"></div>
        <div id="loop-flipgrid-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // Flip loop family — a padded grid of tiles, each a two-sided card (image i / image i+tiles),
        // flipping in a ripple. Measured on the 512 preview (2026-09-08, per-slot coverage per frame):
        //   layout: padding 6% (31px), gap 3% (15px); 8 images -> 4 tiles in a 2x2 grid of 218px cells.
        //   flips:  tile i (row-major) is mid-flip at 0.5 + i*1.0 s and again at 4.5 + i*1.0 s: period DUR/2, stagger DUR/(2*tiles).
        //           projected width (|cos|): 11deg@0.2s, 23@0.3, 52@0.4, 79@0.5, 141@0.6, 164@0.7 -> a ~1s flip with
        //           cubic-bezier(.5,0,.5,1) over [i*stagger, (i+1)*stagger] fits within ~10deg (continuous ripple).
        //   axis=horizontal -> the sliver at mid-flip is vertical (rotateY); vertical -> rotateX.
        // Knobs: perspective (used 1000px*REF/1080, same convention as 3D), rotation sign (silhouettes are mirror-blind),
        // shadow units = 1% of H (guess). easing enum: smooth/linear/slowdown/accelerate/custom are real beziers, the rest fall back to smooth.
        ...
        const TILES = Math.max(1, Math.floor(N / 2));
        ...
        document.getElementById("loop-flipgrid-loop").dataset.duration = DUR;
        background(document.getElementById("loop-flipgrid-bg"));
        const stage = document.getElementById("loop-flipgrid-stage");
        const shadow = bool("shadowEnabled", false)
          ? `${(num("shadowX", 0) * REF) / 100}px ${(num("shadowY", 1) * REF) / 100}px ${(num("shadowBlur", 1.5) * REF) / 100}px ${(num("shadowSpread", -0.25) * REF) / 100}px ${str("shadowColor", "#000000")}${Math.round(
              ...
              .padStart(2, "0")}`
          : "";
        const ROT = AXIS === "vertical" ? "rotateX" : "rotateY";
        const flips = [];
        for (let i = 0; i < TILES; i++) {
          ...
          tile.className = "tile";
          tile.style.cssText = `left:${PAD + c * (CW + GAP)}px;top:${PAD + r * (CH + GAP)}px;width:${CW}px;height:${CH}px;perspective:${(1000 * REF) / 1080}px;`;
          ...
          for (const [k, back] of [
            ...
            if (back) el.style.transform = `${ROT}(180deg)`;
        ...
        function renderAt(t) {
          const T = frac(t) * DUR,
            j = Math.min(1, Math.floor(T / HALF)),
            local = T - j * HALF;
          flips.forEach((flip, i) => {
            const u = Math.max(0, Math.min(1, (local - i * ST) / ST));
            flip.style.transform = `${ROT}(${180 * (j + ease(u))}deg)`;
          });
        }
        ...
        window.__timelines["loop-flipgrid"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `flipgrid` `3d`.
