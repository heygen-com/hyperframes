---
title: "Loop Center Stage"
description: "One image card at a time exits left while fading, a beat of empty stage, then the next enters. 6 image slots, 7s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-centerstage.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-centerstage.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-centerstage" item="loop-centerstage" />

That writes one file: `compositions/loop-centerstage.html`.
...
```html index.html
<div
  data-composition-id="loop-centerstage"
  data-composition-src="compositions/loop-centerstage.html"
  data-start="0"
  data-duration="7"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.86` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `travel` | `60` | 0 to 120, step 1 | Travel (engine parameter; see the block header comment). |
| `ghosts` | `true` | `true`, `false` | Ghosts (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-centerstage-bg"></div>
        <div id="loop-centerstage-stage"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // CenterStage loop family (Stage) — one card at a time; it exits LEFT while fading, a beat of empty stage, the
        // next enters from the RIGHT fading in. `ghosts` = time-lagged translucent copies trailing the card.
        // Measured @512 (2026-09-08, 6 images, step = 7/6 s = 35 frames): card 388 = (1-2·padding)·H·scaleIntensity
        // (450·0.86 = 387); exit occupies 7 frames (0.2 step) starting at 0.77 of the step, entry 7 frames from 1.07
        // (= 0.07 of the next step) -> rest is the middle 50% of each step. Exit x = -travel·ease(u) with
        // travel = 60% of H (card is at +298px with 8% opacity on its first visible entry frame); opacity(out) =
        // 1 - ease^1.5, opacity(in) = ease (both fitted on the luma ramp). Ghost copies lag 1/2/3 frames at ~0.5/0.28/0.15
        // opacity (two visible in the preview at f30: 0.40 and 0.23 relative) — guess on the count.
        // `smooth` = easeInOutCubic (measured across the group). Knobs: shadow* units 1% of H.
        ...
        const __V = Object.assign(
          ...
          (window.__hyperframes && window.__hyperframes.getVariables
            ...
            : null) || {},
          { loopDuration: 7, loopRepeat: 2, imageCount: 6 },
        );
        const __PH = [
        ...
        const SI = num("scaleIntensity", 0.86);
        const TRAVEL = (num("travel", 60) / 100) * REF;
        const GHOSTS = bool("ghosts", true) ? 3 : 0;
        ...
        const OUT0 = 0.77,
          OUT1 = 0.97,
          IN0 = 0.07,
          IN1 = 0.27; // step phases (fractions of a step)
        const LAG = 1 / 30; // ghost lag, seconds per copy
        ...
        document.getElementById("loop-centerstage-loop").dataset.duration = DUR;
        background(document.getElementById("loop-centerstage-bg"));
        const shadow = bool("shadowEnabled", false)
          ? `${(num("shadowX", 0) * REF) / 100}px ${(num("shadowY", 1) * REF) / 100}px ${(num("shadowBlur", 1.5) * REF) / 100}px ${(num("shadowSpread", -0.25) * REF) / 100}px ${str("shadowColor", "#000000")}${Math.round(
              ...
              .padStart(2, "0")}`
          : "";
        const SIZE = (1 - 2 * PAD) * REF * SI;
        const stage = document.getElementById("loop-centerstage-stage");
        // one main + GHOSTS copies per image, ghosts behind
        const layers = [];
        for (let i = 0; i < N; i++) {
          const m = IMAGES[i],
            a = aspect(m, FIT, CROP_AR),
            w = Math.min(SIZE * a, (1 - 2 * PAD) * W),
            h = SIZE;
          const set = [];
          for (let g = GHOSTS; g >= 0; g--) {
            ...
            c.style.left = -w / 2 + "px";
            c.style.top = -h / 2 + "px";
            if (shadow && g === 0) c.style.boxShadow = shadow;
            stage.appendChild(c);
            set.push({ el: c, lag: g });
          }
          layers.push(set);
        }
        // state of image i at absolute loop time t: {x, alpha} or null when hidden
        function stateAt(i, t) {
          const step = DUR / N,
            ...
            ph = u - k;
          if (k === i) {
            if (ph < IN0) return null;
            if (ph < IN1) {
              const e = ease((ph - IN0) / (IN1 - IN0));
              return { x: TRAVEL * (1 - e), a: e };
            }
            if (ph < OUT0) return { x: 0, a: 1 };
            if (ph < OUT1) {
              const e = ease((ph - OUT0) / (OUT1 - OUT0));
              return { x: -TRAVEL * e, a: 1 - Math.pow(e, 1.5) };
            }
            return null;
          }
          return null;
        ...
        function renderAt(t) {
          for (let i = 0; i < N; i++)
            for (const L of layers[i]) {
              const s = stateAt(i, t - L.lag * LAG);
              if (!s) {
                L.el.style.display = "none";
                continue;
              }
              L.el.style.display = "";
              L.el.style.transform = `translate3d(${s.x}px,0,0)`;
              L.el.style.opacity = String(s.a * (L.lag ? [1, 0.5, 0.28, 0.15][L.lag] : 1));
            }
        }
        ...
        window.__timelines["loop-centerstage"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `centerstage`.
