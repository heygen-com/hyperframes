---
title: "Loop 3D Ring 10"
description: "Image cards ring a vertical axis facing outward while the ring spins under a 3D camera. 8 image slots, 20s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-3d-10.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-3d-10.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-3d-10" item="loop-3d-10" />

That writes one file: `compositions/loop-3d-10.html`.
...
```html index.html
<div
  data-composition-id="loop-3d-10"
  data-composition-src="compositions/loop-3d-10.html"
  data-start="0"
  data-duration="20"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `2220` | 0 to 8120, step 10 | Card size in scene units. |
| `orbitRadius` | `7073` | 0 to 25848, step 10 | Ring radius in scene units. |
| `rotationX` | `0` | -180° to 180°, step 1° | Formation tilt about the horizontal axis, degrees. |
| `rotationY` | `360` | -180° to 180°, step 1° | Formation turn about the vertical axis, degrees. |
| `rotationZ` | `0` | -180° to 180°, step 1° | Formation roll about the view axis, degrees. |
| `distance` | `12909` | 0 to 48424, step 10 | Camera distance from the formation. |
| `perspective` | `120` | 0 to 300, step 10 | Camera focal length: smaller = stronger perspective. |
...
| `surface` | `cylinder` | `flat`, `cylinder` | Surface (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-3d-10-bg"></div>
        <div id="loop-3d-10-stage">
          <div id="loop-3d-10-ring"><div id="loop-3d-10-spin"></div></div>
        </div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-3d-10-loop").dataset.duration = DUR;
        background(document.getElementById("loop-3d-10-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R, 1); // world -> px (ring centre or ring at z=0)
        const Rpx = R * s,
          Dpx = D * s;
        const stage = document.getElementById("loop-3d-10-stage");
        stage.style.perspective = f + "px";
        const ring = document.getElementById("loop-3d-10-ring");
        ...
        const spin = document.getElementById("loop-3d-10-spin");
        ...
        window.__timelines["loop-3d-10"] = tl;
      })();
    </script>
  </body>
</html>
