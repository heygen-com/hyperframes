---
title: "Loop Focus Slider"
description: "A row of image cards steps sideways; the focused card grows and the rest hold back. 8 image slots, 10s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-focusslider.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-focusslider.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-focusslider" item="loop-focusslider" />

That writes one file: `compositions/loop-focusslider.html`.
...
```html index.html
<div
  data-composition-id="loop-focusslider"
  data-composition-src="compositions/loop-focusslider.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `scaleIntensity` | `0.82` | 0 to 1, step 0.05 | Strength of the size change. |
...
| `centerScale` | `200` | 0 to 400, step 10 | Center Scale (engine parameter; see the block header comment). |
| `offset` | `55` | -100 to 100, step 1 | Offset (engine parameter; see the block header comment). |
| `weave` | `alternate` | `alternate`, `fixed` | Weave (engine parameter; see the block header comment). |
| `gap` | `6` | 0px to 12px, step 0.1px | Spacing between cards. |
| `glide` | `80` | 0 to 160, step 1 | Glide (engine parameter; see the block header comment). |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-focusslider-bg"></div>
        <div id="loop-focusslider-row"></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        // FocusSlider loop family (Sweep) — a row of cards steps sideways; the focused one is centerScale% bigger and the
        // others sit alternately above/below the axis (weave) so the path zigzags.
        // Measured @512 (2026-09-08): focused card 370 = (1-2·padding)·H·scaleIntensity (450·0.82 = 369); small cards
        // 185 = focused/(centerScale/100); gap 30px = gap% of H; small cards offset ±101px = offset% of the small size,
        // parity bound to the CARD (a card keeps its up/down side as it moves, so slot parity flips every step);
        // fixed weave = every side card on the same side. Vertical offset closes only in the outer half of a slot:
        // y = OFF·max(0, 2|d|-1)^1.2 (d = slot distance; at |d|=0.5 both cards sit exactly on the axis, measured).
        // Size and x interpolate linearly with one eased progress; `smooth` = easeInOutCubic (fits 8 samples to <1%).
        // Timing: step = DUR/N; transition = glide% of the step (0.8·37.5 = 30 frames, mid at frame 15 measured).
        // Knobs: shadow* units 1% of H (copied convention); elastic/bounce/overshoot/impulse/swing -> smooth.
        ...
        const SI = num("scaleIntensity", 0.82);
        const CSCALE = Math.max(1, num("centerScale", 200) / 100);
        const OFFP = num("offset", 55) / 100;
        const WEAVE = str("weave", "alternate");
        const GAP = (num("gap", 6) / 100) * REF;
        const GLIDE = Math.min(1, Math.max(0.05, num("glide", 80) / 100));
        ...
        document.getElementById("loop-focusslider-loop").dataset.duration = DUR;
        background(document.getElementById("loop-focusslider-bg"));
        const shadow = bool("shadowEnabled", false)
          ? `${(num("shadowX", 0) * REF) / 100}px ${(num("shadowY", 1) * REF) / 100}px ${(num("shadowBlur", 1.5) * REF) / 100}px ${(num("shadowSpread", -0.25) * REF) / 100}px ${str("shadowColor", "#000000")}${Math.round(
              ...
              .padStart(2, "0")}`
          : "";
        const BIG = (1 - 2 * PAD) * REF * SI,
          SMALL = BIG / CSCALE,
          OFF = OFFP * SMALL;
        const row = document.getElementById("loop-focusslider-row");
        const cards = [],
          asp = [];
        for (let i = 0; i < N; i++) {
          const m = IMAGES[i],
            a = aspect(m, FIT, CROP_AR);
          const c = card(m, BIG * a, BIG, RADIUS);
          c.style.left = "0px";
          c.style.top = "0px";
        ...
        // along-axis extent of card i at size s (height for horizontal rows, width for vertical)
        const along = (i, s) => (VERT ? s / asp[i] : s * asp[i]);
        const wrap = (r) => {
          r = ((r % N) + N) % N;
          return r > N / 2 ? r - N : r;
        };
        // along-axis centre of card i when card k is focused (small cards packed with GAP)
        function pos(i, k) {
          const r = wrap(i - k),
            ar = Math.abs(r),
            sg = Math.sign(r);
          if (r === 0) return 0;
          let x = along(k, BIG) / 2 + GAP;
          for (let j = 1; j < ar; j++) x += along((k + sg * j + N) % N, SMALL) + GAP;
          return sg * (x + along(i, SMALL) / 2);
        }
        const side = (i) => (WEAVE === "alternate" ? (i % 2 ? 1 : -1) : -1); // -1 = above / left of the axis
        function renderAt(t) {
          const u = frac(t) * N,
            k = Math.floor(u),
            g = u - k;
          const e = ease(Math.min(1, g / GLIDE));
          for (let i = 0; i < N; i++) {
            const c = cards[i],
              d = wrap(i - k) - e; // continuous slot distance
            const ad = Math.abs(d),
              s = BIG + (SMALL - BIG) * Math.min(1, ad);
            const p = pos(i, k) + (pos(i, (k + 1) % N) - pos(i, k)) * e;
            if (Math.abs(p) > Math.max(W, H)) {
              c.style.display = "none";
              continue;
            }
            c.style.display = "";
            const q = side(i) * OFF * Math.pow(Math.max(0, 2 * Math.min(1, ad) - 1), 1.2); // cross-axis weave
            const w = VERT ? s : s * asp[i],
              h = VERT ? s / asp[i] : s;
            c.style.width = w + "px";
            c.style.height = h + "px";
            c.style.transform = VERT
              ? `translate(${q - w / 2}px,${p - h / 2}px)`
              : `translate(${p - w / 2}px,${q - h / 2}px)`;
        ...
        window.__timelines["loop-focusslider"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `focusslider`.
