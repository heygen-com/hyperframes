...
<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-11.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-globe-11.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-globe-11" item="loop-globe-11" />

That writes one file: `compositions/loop-globe-11.html`.
...
```html index.html
<div
  data-composition-id="loop-globe-11"
  data-composition-src="compositions/loop-globe-11.html"
  data-start="0"
  data-duration="10"
  data-track-index="1"
  data-width="1080"
...
| `fade` | `23` | 0 to 100, step 1 | Fade (engine parameter; see the block header comment). |
...
| `minScale` | `1.19` | 0 to 28, step 1 | Min Scale (engine parameter; see the block header comment). |
| `maxScale` | `13.77` | 0 to 103, step 1 | Max Scale (engine parameter; see the block header comment). |
| `distance` | `750` | 0 to 8900, step 10 | Camera distance from the formation. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-globe-11-bg"></div>
        <div id="loop-globe-11-stage"><div id="loop-globe-11-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-globe-11-loop").dataset.duration = DUR;
        background(document.getElementById("loop-globe-11-bg"));

        const f = 1000 * (100 / PERSP) * (REF / 1080); // CSS focal length, px
        const s = f / Math.max(D, R); // world -> stage px (ratios are all that matter)
        const stage = document.getElementById("loop-globe-11-stage");
        stage.style.perspective = f + "px";
        const world = document.getElementById("loop-globe-11-world");
        ...
        window.__timelines["loop-globe-11"] = tl;
      })();
    </script>
  </body>
</html>
