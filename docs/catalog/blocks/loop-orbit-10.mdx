---
title: "Loop Orbit 10"
description: "Image cards on a 3D ring, always parallel to the screen, orbit under a perspective camera. 8 image slots, 14s seamless loop, 1:1."
---

import { InstallCommand } from "/snippets/install-command.jsx";

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbit-10.mp4" poster="https://static.heygen.ai/hyperframes-oss/docs/images/catalog/blocks/loop-orbit-10.png" autoPlay muted loop playsInline />

## Install

<InstallCommand command="npx hyperframes add loop-orbit-10" item="loop-orbit-10" />

That writes one file: `compositions/loop-orbit-10.html`.
...
```html index.html
<div
  data-composition-id="loop-orbit-10"
  data-composition-src="compositions/loop-orbit-10.html"
  data-start="0"
  data-duration="14"
  data-track-index="1"
  data-width="1080"
...
| `planeSize` | `372` | 0 to 1024, step 10 | Card size in scene units. |
| `orbitRadius` | `407` | 0 to 1036, step 10 | Ring radius in scene units. |
...
| `rotationZ` | `90` | -180° to 180°, step 1° | Formation roll about the view axis, degrees. |
| `perspective` | `100` | 0 to 280, step 10 | Camera focal length: smaller = stronger perspective. |
...
>
  ...
  <body>
    ...
    >
      ...
      >
        <div id="loop-orbit-10-bg"></div>
        <div id="loop-orbit-10-stage"><div id="loop-orbit-10-world"></div></div>
      </div>
    </div>
    <script>
    ...
    <script>
      (async () => {
        ...
        document.getElementById("loop-orbit-10-loop").dataset.duration = DUR;
        background(document.getElementById("loop-orbit-10-bg"));
        const f = 1200 * (100 / PERSP) * s;
        document.getElementById("loop-orbit-10-stage").style.perspective = f + "px";
        const world = document.getElementById("loop-orbit-10-world");
        ...
        window.__timelines["loop-orbit-10"] = tl;
      })();
    </script>
  </body>
</html>
...
Tagged `loop` `images` `showcase` `gallery` `orbit` `3d`.
