...
<Info>
**Quick version** - Fork the repo. Write one HTML file with a paused anime.js timeline, add `runtime: "animejs"` to `registry-item.json`, run `hyperframes lint` + `validate`, publish with `npx hyperframes publish`, and open a PR.
</Info>
...
`runtime` declares the animation engine used by the registry item. New contributions default to `"animejs"`. Existing or migrated GSAP-authored items declare `"runtime": "gsap"`, so installers and docs can route users to the correct adapter guidance. Omitted `runtime` is treated as legacy GSAP for backward compatibility, but new registry items should always set it explicitly.
...
<Steps>
  ...
  <Step title="Paused timeline">
    Use `anime.createTimeline({ autoplay: false })` or `anime.animate(..., { autoplay: false })`. The player controls playback.
  </Step>
  <Step title="Register timeline">
    `hyperframesAnime.register("id", tl, { labels })` must match `data-composition-id`.
  </Step>
  <Step title="No requestAnimationFrame">
    Drive Three.js/WebGL scenes from the registered timeline's current position or `hf-seek` events.
  </Step>
  <Step title="Hard kills on captions">
    Set caption opacity and visibility to hidden at the group end. No lingering text.
  </Step>
</Steps>

<Warning>
Break any of these and renders won't be reproducible. The renderer captures every frame by seeking the registered animation. If your animation depends on real time or random state, it breaks.
</Warning>
...
4. **"Animation registration not found"** - ID mismatch between HTML and JS
