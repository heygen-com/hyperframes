...
For Gemini narration, explicitly ask your agent to use Gemini 3.8 TTS and set
`GEMINI_API_KEY` (or `GOOGLE_API_KEY`) in your environment or project `.env`.
The shared audio workflow supports `gemini-3.8-flash-tts` and
`gemini-3.8-flash-lite-tts`. Setting a Google key alone does not change the
automatic voice-provider order above. See [Gemini voiceover](/prompting/media-and-audio#gemini-voiceover).

This adapter uses API-key authentication with the Gemini Developer API.
That API also supports appropriately scoped OAuth credentials, but this helper
does not yet accept OAuth tokens. Do not put service-account JSON in the API-key
variable. Cloud Text-to-Speech and Vertex AI have separate model availability.
