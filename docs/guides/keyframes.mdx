...
description: "Edit keyframes visually in Studio, including the supported GSAP adapter keyframe surface."
---

anime.js is the first-party default engine for new compositions. Studio also gives you visual tools to create and edit GSAP adapter keyframes without writing code, which is useful for existing or migrated GSAP compositions. You can adjust animation properties in the Design Panel, convert straight-line motion into curved arcs, and record gesture-based motion by dragging elements in the preview.

## Timeline Keyframe Diamonds

When you open a composition in Studio, the timeline shows **diamond markers** on clips that have editable animations. For GSAP adapter compositions, each diamond represents a keyframe, a point in time where a property value is set.
...
<Note>
  For GSAP adapter compositions, keyframe diamonds are synthesized from your tweens automatically. Every `.to()`, `.from()`, and `.fromTo()` call produces start and end markers on the timeline.
</Note>
...
- **Speed** - The adapter ease, for example `power2.inOut` or `back.out(3)` on GSAP tweens
...
<Steps>
  ...
  <Step title="Verify in Code tab">
    Switch to the Code tab to see the generated adapter code. For GSAP adapter compositions, every Design Panel edit writes valid GSAP that renders identically in preview and headless export.
  </Step>
</Steps>
...
Record motion by physically dragging an element in the preview while the timeline plays. The pointer path is simplified and converted into adapter keyframes automatically.
...
<Steps>
  ...
  <Step title="Stop recording">
    Press R again or wait for the timeline to reach the end. Recording stops, the motion is simplified (reducing ~180 raw samples to 5–15 clean keyframes), and the keyframes are written to the adapter script immediately.
  </Step>
