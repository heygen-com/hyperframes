...
For anime.js animation, register one paused timeline per composition:
...
<script>
  ...
  tl.add("#title", { opacity: [0, 1], translateY: [40, 0], duration: 600, ease: "outQuart" }, 0);
  tl.add({}, { duration: 5000 }, 0); // keeps the timeline at least 5 seconds long
...
<Warning>
  If a video cuts off early, check that the root `data-duration` is set or that the longest finite registered anime.js instance is at least as long as the intended edit.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Timeline Editing" icon="timeline" href="/guides/timeline-editing">
    Timing, tracks, and animation patterns
  </Card>
