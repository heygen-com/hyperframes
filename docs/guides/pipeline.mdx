...
<Tip>
  Not every project uses every step. A no-narration brand reel skips Step 5; a hand-authored
  composition skips Steps 1-2. But the order matters: scene durations come from narration, animation
  choices come from the storyboard, and the storyboard depends on the design reference. Skip a step
  only when you don't need its artifact downstream.
</Tip>
...
Hyperframes ships multiple TTS adapters (HeyGen, ElevenLabs, Cartesia, Kokoro); see [`/media-use`](/guides/prompting) for the skill that picks one. The local `npx hyperframes tts` command is Kokoro-only; cloud providers are selected by the `/media-use` audio engine. After generating audio, update `STORYBOARD.md` with the real beat boundaries from `transcript.json`.
