---
title: GSAP Adapter
description: "Use the supported GSAP adapter path for existing or migrated HyperFrames compositions."
---

anime.js (MIT licensed) is the first-party default animation engine for HyperFrames. This page documents the supported [GSAP](https://gsap.com/) adapter path for existing compositions, migrated compositions, and authors who prefer GSAP.

GSAP timelines are paused and controlled by the runtime. You define the animations, and HyperFrames handles playback by seeking the registered timeline. For new anime.js compositions, see [anime.js Animation](/guides/anime-animation). For background on how animation runtimes plug into HyperFrames, see [Frame Adapters](/concepts/frame-adapters).
...
<CardGroup cols={2}>
  ...
  <Card title="Frame Adapters" icon="plug" href="/concepts/frame-adapters">
    Learn how GSAP plugs into the render pipeline as a supported adapter
  </Card>
