...
Register GPU work so the renderer waits before screenshot capture:
...
Keep the DOM and GPU layers in the same composition tree and seek everything from the same `hf-seek` event. Do not run a free-running `requestAnimationFrame` loop for rendered output.
