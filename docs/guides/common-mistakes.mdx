...
<AccordionGroup>
  <Accordion title="Animating video element dimensions">
    ...
    **Cause:** Animating `width`, `height`, `top`, `left` directly on a `<video>` element can cause the browser to stop rendering frames.
  ...
  <Accordion title="Controlling media playback in scripts">
    ...
    **After (fixed):**
    ...
    // Use your animation runtime for visual animation only:
    tl.add("#el-video", { opacity: [0, 1], duration: 500 }, 0);
    ```
  ...
  <Accordion title="Composition duration shorter than video">
    ...
    **Cause:** Without a root `data-duration`, the composition duration is inferred from the longest finite registered animation, not `data-duration` on the video. If your last registered animation ends at 8 seconds, the composition is 8 seconds long, regardless of how long the video source is.

    **Before (broken):**

    ```javascript index.html
    // Timeline is only 7.8s long, video cuts off after 7.8 seconds
    tl.add("#lower-third", { translateX: -640, duration: 600 }, 7200);
    ```
    ...
    **After (fixed):**
    ...
    // Prefer explicit root data-duration for long media edits.
    // If you omit it, add a finite timeline entry that reaches the intended end.
    tl.add({}, { duration: 283000 }, 0);
    ```

    An explicit root `data-duration` is the clearest fix for long media edits. If you rely on anime.js duration inference instead, the longest finite registered instance must cover the intended render length.

    <Tip>
      A quick check: run `npx hyperframes compositions` to see the resolved duration of each composition. If it is shorter than expected, set root `data-duration` or extend the registered animation.
    </Tip>
  </Accordion>
  ...
  <Accordion title="Animation registration id doesn't match data-composition-id">
    **Symptom:** Animations don't play. The composition appears static.

    **Cause:** The id passed to `hyperframesAnime.register(...)` must exactly match the [`data-composition-id`](/concepts/data-attributes#composition-attributes) attribute on the composition root element.
    ...
    **After (fixed):**
    ...
    // <div data-composition-id="my-video" ...>
    hyperframesAnime.register("my-video", tl, { labels: {} });
    ```

    For the supported GSAP adapter path, the same id rule applies to `window.__timelines["my-video"]`.
  </Accordion>
</AccordionGroup>
...
2. **Animation registered?** Does `hyperframesAnime.register("<id>", instance, { labels })` use the same id as [`data-composition-id`](/concepts/data-attributes#composition-attributes)? For the GSAP adapter path, the equivalent check is `window.__timelines["<id>"]`.
3. **Visual-only animations?** Only animate visual properties (opacity, transform, color). See [anime.js Animation](/guides/anime-animation#contract) or [GSAP Adapter](/guides/gsap-animation#key-rules).
4. **Timeline long enough?** Set root `data-duration` or ensure the longest finite registered animation covers the intended edit length.
