...
| --- | --- |
| `class="clip"` | Marks a timed section. The framework owns its visibility — never animate a clip's own `display`, `visibility` or `autoAlpha`; fade it with `opacity`, or animate a child. |
