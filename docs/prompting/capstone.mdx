...
This is the full prompt, nothing paraphrased or trimmed. Two of its clauses — the chip's hard bind to the wire's path geometry, and the mural's pinned/iris-revealed/fade-out behavior — were added after frame review caught the builder drifting on exactly those points, and the film was patched to comply: the same catch-and-tighten loop [Iterating](/prompting/iterating) teaches, applied to the prompt itself so the next run can't regress.
