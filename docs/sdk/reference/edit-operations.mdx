...
| `E_NO_GSAP_TIMELINE` | Op requires a parsed GSAP adapter timeline. |
| `E_NO_GSAP_SCRIPT` | Op requires a GSAP `<script>` block; none found in the document. |

<Note>
  Several GSAP adapter ops (`addGsapTween`, `setGsapTween`, `removeGsapTween`, `addGsapKeyframe`, `setGsapKeyframe`, `removeGsapKeyframe`, etc.) return `{ ok: false, code: 'E_NO_GSAP_TIMELINE' }` from `can()` when the parser engine is unavailable. `dispatch()` still applies those ops structurally even when `can()` returns false.
</Note>
...
These ops add, edit, and remove GSAP tween entries in the composition's GSAP script block. anime.js is the first-party default runtime for new composition authoring; these operations are the supported SDK editing surface for existing or migrated GSAP adapter scripts. They operate by `animationId`, a stable string identifier minted when a tween is created. Use `comp.addGsapTween()` or `addWithKeyframes` to mint a new id; the typed wrapper returns it directly.

<Note>
  Several GSAP adapter ops require the parser engine to be active. When it is unavailable, `can()` returns `{ ok: false, code: 'E_NO_GSAP_TIMELINE' }` for these ops. `dispatch()` still applies the op structurally.
</Note>
...
GSAP adapter timeline labels mark named positions (in seconds) in the master timeline. Labels are referenced in `GsapTweenSpec.position` as strings like `"intro"` or `"intro+=0.5"`.
...
Arc path ops control the motion path of a GSAP adapter tween, the curve along which an element travels.
