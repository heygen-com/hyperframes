...
<ResponseField name="animationIds" type="readonly string[]">
  IDs of GSAP adapter tweens whose target is this element. Pass these to the GSAP tween ops in `dispatch()`.
</ResponseField>
...
<ResponseField name="gsapScript" type="string | null">
  Raw contents of the composition's GSAP adapter `<script>` block, if present.
</ResponseField>
...
Describes a single GSAP adapter tween entry to create or update. anime.js is the first-party default runtime for new composition authoring; this type supports existing or migrated GSAP adapter scripts.
