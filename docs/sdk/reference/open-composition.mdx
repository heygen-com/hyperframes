...
<ParamField path="persist" type="PersistAdapter">
  ...
  Built-in adapters: `createMemoryAdapter()` (tests/demos), `createFsAdapter()` (Node.js), `createHeadlessAdapter()` (no-op). Custom adapters implement the `PersistAdapter` interface from `@hyperframes/sdk/adapters`.
