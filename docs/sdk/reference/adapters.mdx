...
<ParamField path="addressableOnly" type="boolean" default="true">
  Consider only model-addressable elements (`[data-hf-id]`). Set `false` to widen the walk to every element, which catches nodes created after the document was stamped — split-text word spans, cloned nodes — at the cost of a larger candidate set.
</ParamField>
