...
description: "Set clip timing, elastic holds, GSAP adapter tweens, and keyframed animations on composition elements."
---
...
<ResponseField name="labels" type="string[]">
  GSAP adapter timeline label names whose numeric position falls within `[enterAt, exitAt]`. Parsed fresh from the GSAP script on every `getElementTimings()` call, never stale.
</ResponseField>
...
anime.js is the first-party default runtime for new composition authoring. The SDK methods in this section are the supported editing surface for existing or migrated GSAP adapter scripts. The SDK reads tween IDs from `element.animationIds` (returned by the query API) and writes through `addGsapTween`, `setGsapTween`, and `removeGsapTween`.
...
Some GSAP adapter operations (such as `setGsapKeyframe`, arc paths, and label ops) require the parser engine. Use `can()` to gate them before dispatching:
...
if (!check.ok) {
  if (check.code === "E_NO_GSAP_TIMELINE") {
    // Parser engine unavailable, show a disabled state or skip
    console.warn("GSAP adapter timeline not available:", check.message);
  }
  return;
}
...
| `E_NO_GSAP_TIMELINE` | Op requires the GSAP adapter parser engine. |
...
For keyframe-based animations in GSAP adapter scripts, use `addWithKeyframes` and `replaceWithKeyframes`. Both operate on the GSAP CSS-keyframes layer and return the minted animation ID.
...
// newAnimId !== oldAnimId because position-derived IDs renumber after the remove
```
...
<CardGroup cols={2}>
  ...
  <Card title="anime.js Animation" icon="wand-magic-sparkles" href="/guides/anime-animation">
    Default animation authoring contract for new composition HTML.
  </Card>
  <Card title="GSAP Adapter Guide" icon="plug" href="/guides/gsap-animation">
    Authoring GSAP adapter timelines in composition HTML.
  </Card>
