...
In embedded mode the host typically owns the undo stack. Pass `history: false` to skip attaching the SDK's built-in undo module — it keeps the session lightweight and avoids duplicating state:
