...
description: "Use anime.js by default, or bring any seekable animation runtime to Hyperframes."
---
...
anime.js (MIT licensed) is the first-party default animation engine for HyperFrames. GSAP remains a fully supported adapter for existing compositions and anyone who prefers it.
...
### anime.js, the first-party default

New compositions should create anime.js animations or timelines with `autoplay: false` and register them with `hyperframesAnime.register(id, instance, { labels })`:

```html
<script src="https://cdn.jsdelivr.net/npm/animejs@4.5.0/dist/bundles/anime.umd.min.js"></script>
<script>
  const tl = anime.createTimeline({ autoplay: false });
  tl.add(".title", { opacity: [0, 1], duration: 600, ease: "outQuart" }, 0);

  hyperframesAnime.register("main", tl, { labels: { intro: 0 } });
</script>
```

The registration id must match the composition root's `data-composition-id`. Labels are in seconds; anime.js positions and durations are in milliseconds.

HyperFrames primes each registered anime.js instance once at discovery so timeline children added after time zero render their initial state before the first real seek. If the root omits `data-duration`, HyperFrames infers the composition duration from the longest finite registered anime.js instance.

### GSAP, supported adapter

GSAP compositions remain fully supported through the GSAP adapter path. Build a paused `gsap.timeline()` or tween, then register it on `window.__timelines` with a key matching `data-composition-id`:
...
The GSAP adapter seeks in seconds with `timeline.totalTime(timeSeconds)` or `timeline.seek(timeSeconds)`.

| Runtime | Seek Method | Skill |
|---------|-------------|-------|
| [anime.js](/guides/anime-animation) | `instance.seek(timeMs)` for instances registered with `hyperframesAnime.register(...)` | `/hyperframes-animation` |
| [GSAP](/guides/gsap-animation) | `timeline.totalTime(timeSeconds)` or `timeline.seek(timeSeconds)` for timelines registered on `window.__timelines` | `/hyperframes-animation` |
...
<CardGroup cols={2}>
  ...
  <Card title="anime.js Animation" icon="wand-magic-sparkles" href="/guides/anime-animation">
    Build with the first-party default animation engine
  </Card>
  <Card title="GSAP Adapter" icon="plug" href="/guides/gsap-animation">
    Use the supported GSAP adapter path
  </Card>
