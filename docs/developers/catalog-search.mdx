...
- After a no, `--on-device` alone is skipped with a message. Only `--on-device --yes` in an interactive
  terminal (stdin and stdout both terminals, no `--json`, not CI) changes the answer; elsewhere a no stands.
