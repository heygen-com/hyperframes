...
It was chosen because its average score on the MTEB benchmark is close to that of the larger hosted
embedding model it replaces (62.17 against 62.3) with vectors a quarter of the size (384 numbers
instead of 1,536), and it is small enough to download on request. The CLI has no hosted search. It is pinned to one revision because the
...
out slightly different (cosine 0.9969). Rebuilding in batches of 16 reproduces the shipped rows
