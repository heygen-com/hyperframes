 ...
 * disagree about whether a given file would render something.
 *
 * `inlineSubCompositions.ts` intentionally stays tolerant (skip + continue)
 * for the preview/studio bundling path, where partial content while
 * iterating is expected. `lint` and the render pre-flight check
 * (`packages/producer/src/services/htmlCompiler.ts`) use this helper to
 * fail loudly and name the exact offending file, because a render that
 * silently drops a scene is strictly worse than a render that refuses to
 * start.
 */
 ...
 * so a file that passes this check is guaranteed to produce non-empty output
 * from the inliner, and a file that fails it is guaranteed to hit one of the
