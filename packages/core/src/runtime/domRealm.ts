 ...
 * `HTMLElement` of the realm this module was loaded into. A page can perfectly
 * legally hold, inside one document, elements created by a DIFFERENT realm: the
 * DOM spec's adoption step moves a node between documents but leaves its
 * prototype chain pointing at the realm that created it. The Studio preview does
 * exactly this on some loads — the composition `<body>` is built in the editor
 * window and adopted into the preview iframe's document, so `<html>`, `<head>`
 * and `<script>` belong to the iframe realm while every element under `<body>`
 * belongs to the editor realm.
 *
 * When that happens, `instanceof` does not throw and does not warn. It just
 * returns false, forever, for every element in the composition, and every guard
 * written as `if (!(node instanceof HTMLElement)) continue;` silently skips the
 * whole document. A whole pass goes dark with no error: the timed-element
 * visibility pass stops writing, so every scene paints on top of every other.
 *
 ...
 * nodes belong to the document's own realm and still fails when they were
 * adopted from a third one. A structural test has no such case to get wrong, so
 * these predicates are the single owner of the question for HTML and SVG, which
 * is everything a composition contains.
 */
