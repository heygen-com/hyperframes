...
describe("waapi adapter", () => {
  ...
  describe("cycle end", () => {
    const timing = (delay: number, duration: number, iterations = 1) => ({
      ...makeAnimation(),
      effect: {
        getComputedTiming: () => ({
          delay,
    ...
    it("counts one iteration of a repeating or infinite animation, delay included", () => {
      setAnimations([timing(1500, 1000, 40), timing(0, 2000, Infinity)]);

      const adapter = createWaapiAdapter();
      adapter.discover();

      expect(adapter.getAnimationCycleEndSeconds?.()).toBe(2.5);
      expect(adapter.getInferredDurationSeconds?.()).toBe(41.5);
    });

    it("still counts an animation that finished and left getAnimations()", () => {
      const done = timing(0, 4000);
      setAnimations([done]);
      const adapter = createWaapiAdapter();
      adapter.discover();

      const onFinish = done.addEventListener.mock.calls.find(([type]) => type === "finish")?.[1];
      onFinish?.();
      setAnimations([]);
      adapter.seek({ time: 5 });
    ...
    it("leaves CSS animations and transitions first seen at a seek to the CSS adapter", () => {
      class CSSAnimation {}
      class CSSTransition {}
      vi.stubGlobal("CSSAnimation", CSSAnimation);
      vi.stubGlobal("CSSTransition", CSSTransition);
      try {
        const cssOwned = [CSSAnimation, CSSTransition].map((type) =>
          Object.assign(Object.create(type.prototype), timing(500, 1000)),
        );
        const plain = timing(0, 1000);
        setAnimations([plain]);
        const adapter = createWaapiAdapter();
        adapter.discover();
        setAnimations([plain, ...cssOwned]);
        adapter.seek({ time: 9.9 });

        expect(adapter.getAnimationCycleEndSeconds?.()).toBe(1);
      } finally {
        vi.unstubAllGlobals();
      }
    });
