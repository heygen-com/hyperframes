...
describe("parseAudioElements identity origin", () => {
  it("stamps origin equal to start", () => {
    const [track] = parseAudioElements(
      `<audio id="a" src="tone.wav" data-start="2" data-end="5"></audio>`,
    );
    expect(track).toMatchObject({ start: 2, origin: 2 });
  });
});
