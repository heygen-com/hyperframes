...
export function buildZeroDurationDiagnostic(diag: HfDiagnostic): string {
  ...
  if (diag.declaredDuration <= 0) {
    hints.push(
      'Fix: add data-duration="<seconds>" to your root <div data-composition-id="..."> element.',
    );
  }
  if (diag.renderReady && diag.hasTimeline && diag.duration === 0) {
    hints.push(
      "A GSAP timeline is registered but has no positive duration; give it a timed animation or declare the root duration.",
    );
  }
