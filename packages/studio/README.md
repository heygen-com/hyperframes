...
A host with an agent panel of its own can also take the prompt that "Describe a change to the agent" builds for a selected element: pass `onAgentPrompt={(prompt) => …}` to `StudioApp` and the modal hands the prompt to it instead of copying it to the clipboard — and says "Send to agent" rather than "Copy prompt".
