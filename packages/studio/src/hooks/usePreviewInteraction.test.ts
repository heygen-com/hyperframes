...
interface HarnessArgs {
  ...
  resolveDomSelectionFromPreviewPoint: (
    clientX: number,
    clientY: number,
    options?: { activeGroupElement?: HTMLElement | null; skipSourceProbe?: boolean },
...
function renderHarness(args: HarnessArgs): {
  ...
  function Harness() {
    const interaction = usePreviewInteraction({
      ...
      buildDomSelectionFromTarget: args.buildDomSelectionFromTarget ?? vi.fn(async () => null),
...
function dispatchMouseDownNow(canvas: HTMLDivElement, init: MouseEventInit): void {
  act(() => {
    canvas.dispatchEvent(
      new MouseEvent("mousedown", {
        bubbles: true,
...
describe("usePreviewInteraction", () => {
  ...
  it("preserves group drill-in on double-click", async () => {
    ...
    const resolveAllDomSelectionsFromPreviewPoint = vi.fn(async () => [
      groupSelection,
      makeSelection("Stacked sibling", document.createElement("div")),
    ]);
    const { canvas, cleanup } = renderHarness({
      previewIframe: null,
      resolveDomSelectionFromPreviewPoint,
    ...
    expect(resolveDomSelectionFromPreviewPoint).toHaveBeenCalledWith(50, 60, {
  ...
  it("cycles stacked candidates on a rapid second click at the same spot", async () => {
    ...
    const buildDomSelectionFromTarget = vi.fn(async (target: HTMLElement) =>
      target === bottomElement ? bottomSelection : null,
    );
    const { canvas, cleanup } = renderHarness({
      previewIframe: null,
      resolveDomSelectionFromPreviewPoint,
    ...
    expect(buildDomSelectionFromTarget).toHaveBeenCalledTimes(1);
    expect(buildDomSelectionFromTarget).toHaveBeenCalledWith(bottomElement);
    expect(resolveDomSelectionFromPreviewPoint).toHaveBeenCalledTimes(2);
    expect(resolveDomSelectionFromPreviewPoint).toHaveBeenNthCalledWith(2, 50, 60, {
      skipSourceProbe: true,
    });
    cleanup();
  });

  it("commits source-probe affordances when cycling onto an SVG shape", async () => {
    const topElement = document.createElement("div");
    topElement.id = "top";
    const shapeElement = document.createElement("rect");
    shapeElement.id = "shape";
    const topSelection = makeSelection("Top", topElement);
    const transientShapeSelection = makeSelection("Shape", shapeElement);
    const committedShapeSelection: DomEditSelection = {
      ...transientShapeSelection,
      capabilities: {
        ...transientShapeSelection.capabilities,
        canEditStyles: false,
    ...
    const buildDomSelectionFromTarget = vi.fn(async () => committedShapeSelection);
    const { canvas, cleanup } = renderHarness({
      previewIframe: null,
      resolveDomSelectionFromPreviewPoint,
      resolveAllDomSelectionsFromPreviewPoint,
    ...
    expect(applyDomSelection).toHaveBeenNthCalledWith(2, committedShapeSelection);
    expect(applyDomSelection).not.toHaveBeenCalledWith(transientShapeSelection);
    expect(committedShapeSelection.capabilities).toMatchObject({
      canEditStyles: false,
      reasonIfDisabled: "This element is generated by a script and cannot be edited visually.",
    });
    expect(buildDomSelectionFromTarget).toHaveBeenCalledTimes(1);
  ...
  it("keeps the last clicked candidate when overlapping cycle probes resolve out of order", async () => {
    const topElement = document.createElement("div");
    topElement.id = "top";
    const middleElement = document.createElement("div");
    middleElement.id = "middle";
    const bottomElement = document.createElement("div");
    bottomElement.id = "bottom";
    const topSelection = makeSelection("Top", topElement);
    const middleSelection = makeSelection("Middle", middleElement);
    const bottomSelection = makeSelection("Bottom", bottomElement);
    let resolveBottom: ((selection: DomEditSelection) => void) | undefined;
    const bottomProbe = new Promise<DomEditSelection>((resolve) => {
      resolveBottom = resolve;
    });
    const applyDomSelection = vi.fn();
    const buildDomSelectionFromTarget = vi.fn((target: HTMLElement) => {
      if (target === middleElement) return Promise.resolve(middleSelection);
      if (target === bottomElement) return bottomProbe;
      return Promise.resolve(topSelection);
    });
    const { canvas, cleanup } = renderHarness({
      previewIframe: null,
      resolveDomSelectionFromPreviewPoint: vi.fn(async () => topSelection),
    ...
    await dispatchMouseDown(canvas, { detail: 1 });
    dispatchMouseDownNow(canvas, { detail: 1 });
    await Promise.resolve();
    await dispatchMouseDown(canvas, { detail: 1 });
    resolveBottom?.(bottomSelection);
    await act(async () => {
      await Promise.resolve();
      await Promise.resolve();
    });

    expect(applyDomSelection).toHaveBeenLastCalledWith(middleSelection);
    cleanup();
  });

  it("keeps the transient cycle candidate when its source probe cannot re-resolve it", async () => {
    const topElement = document.createElement("div");
    const bottomElement = document.createElement("div");
    const topSelection = makeSelection("Top", topElement);
    const bottomSelection = makeSelection("Bottom", bottomElement);
    ...
    const { canvas, cleanup } = renderHarness({
      previewIframe: null,
      resolveDomSelectionFromPreviewPoint: vi.fn(async () => topSelection),
      resolveAllDomSelectionsFromPreviewPoint: vi.fn(async () => [topSelection, bottomSelection]),
    ...
    expect(applyDomSelection).toHaveBeenLastCalledWith(bottomSelection);
    cleanup();
  });

  it("source-probes a committed hover fallback before exposing SVG style editing", async () => {
    const shapeElement = document.createElement("rect");
    shapeElement.id = "shape";
    const transientShapeSelection = makeSelection("Shape", shapeElement);
    const committedShapeSelection: DomEditSelection = {
    ...
    const { canvas, cleanup } = renderHarness({
      ...
      buildDomSelectionFromTarget,
      applyDomSelection,
      mouseDownOptions: { hoverSelection: transientShapeSelection },
    });

    await dispatchMouseDown(canvas, { detail: 1 });

    expect(buildDomSelectionFromTarget).toHaveBeenCalledWith(shapeElement);
    expect(applyDomSelection).toHaveBeenCalledWith(committedShapeSelection);
    expect(applyDomSelection).not.toHaveBeenCalledWith(transientShapeSelection);
    cleanup();
  });
