      ...
      {!collapsed && (
        <ul className="max-h-56 space-y-0.5 overflow-y-auto">
          {jobs.map((job) => (
            <li
              key={job.id}
              className="rounded-[10px] px-1.5 py-1.5 transition-colors duration-150 ease-out hover:bg-neutral-900/70"
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0 text-[10px] leading-none text-neutral-600 tabular-nums">
                  {elapsedLabel(job)}
                </span>
              </div>
