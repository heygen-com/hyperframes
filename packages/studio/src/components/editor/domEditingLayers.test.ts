...
describe("resolveDomEditSelection, source probe truth", () => {
  it("keeps source-backed HTML elements style-editable after probing", async () => {
  ...
  it("keeps transient SVG candidates unprobed, then disables styles on committed probe truth", async () => {
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    const shape = document.createElementNS("http://www.w3.org/2000/svg", "rect");
    shape.id = "shape";
    svg.appendChild(shape);
    document.body.appendChild(svg);
    const target = document.querySelector<HTMLElement>("#shape");
    if (!target) throw new Error("Expected SVG shape target");
    const fetchMock = vi.fn(
      async () =>
    ...
    const transient = await resolveDomEditSelection(target, {
      activeCompositionPath: "index.html",
      isMasterView: true,
      projectId: "project-1",
    ...
    const committed = await resolveDomEditSelection(target, {
      activeCompositionPath: "index.html",
      isMasterView: true,
      projectId: "project-1",
    });
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(transient?.capabilities.canEditStyles).toBe(true);
    expect(committed?.capabilities).toMatchObject({
      canEditStyles: false,
