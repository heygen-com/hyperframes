...
export const MotionPathOverlay = memo(function MotionPathOverlay({
  ...
  // Add/remove apply to non-cubic motionPath arcs only (cubic anchors carry
  // control points we don't synthesize; linear keyframe paths stay drag-only).
  ...
  const onPathDown = (e: React.PointerEvent) => {
    ...
    if (!np) return;
    e.stopPropagation();
    void commitAddWaypoint(
      animId,
      np.segIndex + 1,
      Math.round(np.x - home.x),
      Math.round(np.y - home.y),
      commitMutation,
    );
    setGhost(null);
  };
