 ...
 * lies on. Used to position the ghost "add" node and decide where a new
 * waypoint is inserted (between `segIndex` and `segIndex + 1`). Coordinates are
 * whatever space the caller passes (the overlay uses absolute composition px).
 */
export function nearestPointOnPath(
  px: number,
  py: number,
  nodes: Array<{ x: number; y: number }>,
): { x: number; y: number; segIndex: number; dist: number } | null {
  if (nodes.length < 2) return null;
  let best: { x: number; y: number; segIndex: number; dist: number } | null = null;
  for (let i = 0; i < nodes.length - 1; i++) {
    ...
    if (!best || dist < best.dist) best = { x: cx, y: cy, segIndex: i, dist };
  }
  return best;
}
