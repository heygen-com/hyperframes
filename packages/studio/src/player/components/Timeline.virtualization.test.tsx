...
describe("Timeline row virtualization", () => {
  it("mounts a bounded treegrid range over the full geometry height", async () => {
    ...
    expect(treegrid?.querySelectorAll('[role="rowheader"]').length).toBe(rows.length);
    expect(treegrid?.querySelectorAll('[role="gridcell"]').length).toBe(rows.length);
