...
describe("detectRotationPivotDrift", () => {
  ...
  // Two-axis scale/entrance: long side stays under 1.6× but height grows 3× without an axis-swap.
  ...
  // Scale pulse returns to the start size (firstArea===lastArea) but height still excursions 3× — not rigid.
  it("does not fire on a scale pulse that returns to the start AABB without an axis-swap", () => {
    const group = [
      sample({ time: 0, angle: 0, w: 400, h: 100, cx: 200, cy: 50 }),
