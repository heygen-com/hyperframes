...
**Always invoke the relevant skill before writing or modifying compositions.** Skills encode framework-specific patterns (e.g., `hyperframesAnime.register(...)`, `data-*` attribute semantics, shader-compatible CSS rules) that are NOT in generic web docs. Skipping them produces broken compositions.
...
3. anime.js timelines and animations are the default path. Build them paused (`autoplay: false`) and register them with `hyperframesAnime.register(id, instance, { labels })`:
   ```js
   const tl = anime.createTimeline({ autoplay: false });
   hyperframesAnime.register("composition-id", tl, { labels: {} });
   ```
   GSAP timelines remain supported via the paused-timeline-on-`window.__timelines` adapter path for existing compositions.
