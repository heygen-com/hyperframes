...
describe("vst routes", () => {
  ...
  describe("GET /vst/wav-path", () => {
    it("resolves a project-relative asset path to its absolute path on disk", async () => {
      const projectDir = createProjectDir();
      mkdirSync(join(projectDir, ".media", "audio", "bgm"), { recursive: true });
      writeFileSync(join(projectDir, ".media", "audio", "bgm", "bgm_001.wav"), "fake-wav");
      const api = makeApi({ resolveProject: async (id: string) => ({ id, dir: projectDir }) });

      const res = await api.request(
        "/vst/wav-path?projectId=demo&path=" + encodeURIComponent(".media/audio/bgm/bgm_001.wav"),
      );

      expect(res.status).toBe(200);
      expect(await res.json()).toEqual({
        path: join(projectDir, ".media", "audio", "bgm", "bgm_001.wav"),
      });
    });

    it("404s when the resolved file doesn't exist", async () => {
      const projectDir = createProjectDir();
      const api = makeApi({ resolveProject: async (id: string) => ({ id, dir: projectDir }) });

      const res = await api.request(
        "/vst/wav-path?projectId=demo&path=" + encodeURIComponent(".media/audio/bgm/missing.wav"),
      );

      expect(res.status).toBe(404);
    });

    it("404s when the project doesn't resolve", async () => {
      const api = makeApi({ resolveProject: async () => null });
    ...
    it("400s when the path query param is missing", async () => {
      ...
      const res = await api.request("/vst/wav-path?projectId=demo");

      expect(res.status).toBe(400);
    });

    it("400s when the projectId query param is missing", async () => {
      const projectDir = createProjectDir();
      const api = makeApi({ resolveProject: async (id: string) => ({ id, dir: projectDir }) });

      const res = await api.request(
        "/vst/wav-path?path=" + encodeURIComponent(".media/audio/bgm/bgm_001.wav"),
      );

      expect(res.status).toBe(400);
    });

    it("404s on a path-traversal attempt outside the project directory", async () => {
      const projectDir = createProjectDir();
      const api = makeApi({ resolveProject: async (id: string) => ({ id, dir: projectDir }) });

      const res = await api.request(
        "/vst/wav-path?projectId=demo&path=" + encodeURIComponent("../../etc/passwd"),
      );
