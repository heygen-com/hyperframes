 ...
 * seconds old. Change time, not mtime: copy tools (`cp -p`, rsync) set mtime back but cannot set
 * ctime, and a same-size rewrite within one tick keeps both, so only a settled file is tagged. */
