import { useRef, useMemo, useCallback, useState, type ReactNode } from "react";
...
/* SVGs live at /icons/timeline/*.svg — loaded as <img> with CSS invert for white-on-color */
...
export function Timeline({ onSeek }: TimelineProps = {}) {
  const { elements, duration, timelineReady, selectedElementId, setSelectedElementId } = usePlayerStore();
                ...
                {els.map((el, i) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${el.id || el.tag} \u2022 ${el.start.toFixed(1)}s \u2013 ${(el.start + el.duration).toFixed(1)}s`}
