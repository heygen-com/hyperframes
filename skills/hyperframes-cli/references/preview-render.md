...
**Read the summary's second line.** After the output path and size, `render` prints the capture path, GPU mode, and per-stage timings, e.g. `screenshot capture · software gpu · compile 0.7s · setup 1.1s · capture 22.9s · encode 5.8s`. `capture` is the frame loop and `setup` is calibration plus session init; the capture path reads `a|b` when parallel workers diverged, and `encode (during capture)` means the streaming encoder overlapped the loop, so do not add it to the wall time. If `capture` dominates and the line says `screenshot capture` with `software gpu` on Linux, a hint under it names the override (`PRODUCER_FORCE_SCREENSHOT=false`). It only appears when the GPU was auto-probed, never under `--docker` or `--no-browser-gpu`, where software GL is the point. Use the override only if the render is too slow to iterate on; heavy compositions can stall on software GL. Do not reach for `--debug` just to learn which path ran.
