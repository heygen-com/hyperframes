...
- **Gemini narration** is opt-in with `provider: "gemini"` and `GEMINI_API_KEY` (or `GOOGLE_API_KEY`). `tts_model` defaults to `gemini-3.8-flash-tts`; `gemini-3.8-flash-lite-tts` is also supported. `style` directs delivery without changing spoken text; a line's style overrides the request's style. CLI overrides: `--tts-model`, `--style`. See `audio/references/tts.md` for a complete request. The existing automatic provider order is unchanged.
