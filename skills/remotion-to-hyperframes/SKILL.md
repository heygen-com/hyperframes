...
- A single `<script>` tag at the bottom containing one paused `anime.createTimeline({ autoplay: false })`. Every Remotion `useCurrentFrame()` derivation becomes a tween on this timeline at the right millisecond offset.
- `hyperframesAnime.register("<composition-id>", tl);` registers the timeline with HF's runtime.
