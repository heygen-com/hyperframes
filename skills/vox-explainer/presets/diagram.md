# Style block — vox diagram (device/mechanism grammar; source: Vox "It's not you — phones", 2026-07 dissection)

For tech / product / how-it-works topics. Append to every clip prompt (generation routes), then the mute template.
...
Palette: paper white `#F4F1EA` / ink `#1a1a1a` / diagram blue `#2E5F8A` / callout red `#D2382B` / one mustard accent.

HF-native mapping (registry): `vox-thin-arrow` (blue; red for UI callouts) · `vox-caption-chip` · `vox-source-footnote` · boil optional and subtle (diagram pages are calmer than collage).
Text: labels are small-caps 2-5 words, always tied to the element they annotate — never free-floating.
