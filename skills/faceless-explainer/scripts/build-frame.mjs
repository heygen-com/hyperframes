...
const stagedFontFaces = new Set();
const fontFaces = [];
const fontSources = [];
if (brandFonts.length) {
  const fams = [...new Set(brandFonts)];
  const captureDir = join(hyperframesDir, "capture/assets/fonts");
  const srcDirs = [captureDir, fontOutDir].filter((d) => existsSync(d));
  const files = [];
  for (const d of srcDirs)
    for (const f of readdirSync(d).sort()) if (fontExtOf(f)) files.push({ d, f });
  // Capture files are loosely named (hash-named, or Google's "family-latin-400-normal"): a
  // single captured family owns ALL of them, several are told apart by the longest family
  // key the name contains. Files already in assets/fonts/ follow the staged-name contract
  // and get neither shortcut — a previous run may have staged a PRESET's faces there, and a
  // rerun after brand tokens arrive must not relabel EB Garamond's bytes as the brand font
  // (not even for a brand named "Garamond").
  const ranked = rankFontFamilies(fams);
  const famOf = (d, f) => {
    if (d !== captureDir) return familyOfFaceFile(f, ranked);
    if (fams.length === 1) return fams[0];
    return ranked.find((x) => normFontName(f).includes(x.key))?.family;
  };
  const brandFaces = [];
  for (const { d, f } of files) {
    ...
    const face = stageFontFile({
      ...
      srcPath: join(d, f),
      outDir: fontOutDir,
      stagedFaces: stagedFontFaces,
    });
    if (face) brandFaces.push(face.rule);
  }
  if (brandFaces.length) {
    fontFaces.push(...brandFaces);
    fontSources.push("the captured brand font");
    summary.push(`fonts: staged ${brandFaces.length} brand face(s) → assets/fonts/`);
  }
}
const presetFonts = stagePresetFonts({
  presetFontsDir: join(presetDir, presetName, "fonts"),
  outDir: fontOutDir,
  families: fontFamiliesNamed(md),
  stagedFaces: stagedFontFaces,
});
if (presetFonts.staged.length) {
  fontFaces.push(...presetFonts.faces);
  fontSources.push(`the ${presetName} preset`);
  summary.push(
    `fonts: staged ${presetFonts.staged.length} preset face(s) → assets/fonts/` +
      (presetFonts.licenses.length ? ` (+ ${presetFonts.licenses.join(", ")})` : ""),
  );
}
if (presetFonts.skipped.length) {
  summary.push(
    `fonts: skipped ${presetFonts.skipped.length} preset file(s) matching no family frame.md names: ${presetFonts.skipped.join(", ")}`,
  );
}
if (fontFaces.length) {
  md +=
