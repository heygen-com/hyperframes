...
- Propose one meaningful next step in the first conversational move, not a fixed end-to-end pipeline.
- Use `../hyperframes/references/capability-menu.md` as a trigger list. Read it before offering a capability, then offer one relevant capability when the user mentions its input or the build reaches its need. Do not dump the full catalog.
...
5. **Animate.** Use `/hyperframes-animation` to select the atomic rules or blueprint, runtime adapter, and scene transitions. Keep animation details in that owning skill rather than re-defining them here.
