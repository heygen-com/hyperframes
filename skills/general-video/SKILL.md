...
Position every element where it sits at its **most visible moment**, fully entered, correctly placed, not yet exiting. Write that as static HTML + CSS first. **No anime.js timeline yet.**
...
3. **Add entrances**, animate FROM offscreen/invisible TO the CSS position with `anime.createTimeline({ autoplay: false })` and explicit `tl.add(target, params, positionMs)` calls. In sub-compositions, set the start state explicitly in the first `tl.add(...)` event; see `hyperframes-core/references/sub-compositions.md`. The CSS position is ground truth; the tween is the journey to it.
...
| **Kinetic typography / text-forward**                                | `hyperframes-animation/techniques.md` (kinetic type) + anime.js easing/stagger guidance in the adapter references + `rules/kinetic-beat-slam.md`                                             |
...
| **Multi-scene / transitions**                                        | `hyperframes-animation/transitions/overview.md` **then** `transitions/catalog.md` (you are not done after the overview; the anime.js recipe is in the catalog)                               |
