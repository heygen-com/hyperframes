...
9. Each animatic still is a scene: its own file under `compositions/`, hosted in the root with `data-composition-src`, `data-start` and `data-duration`. The root's `main` timeline plays the between-scene moves from the table above on the hosts; drift inside a still lives in that scene's own timeline. Element chains (step 3) stay one composition, since their elements cross scene lines.
