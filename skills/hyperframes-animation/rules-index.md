...
<3d-text-true-extrude path="rules/3d-text-true-extrude.md">Arbitrary text as real extruded 3D geometry — font file → glyph outlines (opentype.js per-glyph, or typeface JSON single-mesh) → THREE.Shape → ExtrudeGeometry with proven bevel/material constants; camera can orbit it. Tags: text, 3d, extrude, bevel, three, typography, font, metal</3d-text-true-extrude>
...
<volume-squash-stretch path="rules/volume-squash-stretch.md">Volume-preserving squash & stretch — deformation ratio computed from impact velocity and implied mass, stretch axis along travel, scaleX×scaleY≈1 throughout, recovery on the spring registers. Type NEVER deforms geometrically — variable-font wdth or its container. Tags: squash, stretch, deformation, weight, impact, physics</volume-squash-stretch>
...
<gpu-particle-field path="rules/gpu-particle-field.md">Thousands-to-hundreds-of-thousands of deterministic GPU particles — THREE.Points with index-seeded attributes, position computed in the vertex shader as a pure function of uTime; includes the DOM→Canvas→GPU escalation ladder. Tags: particles, gpu, points, shader, field, confetti, dissolve, deterministic</gpu-particle-field>
