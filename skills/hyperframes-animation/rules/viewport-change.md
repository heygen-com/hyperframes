...
<script>
  ...
  tl.add(
    ".hero",
    { opacity: [0, 1], translateY: [HERO_Y, 0], duration: HERO_DUR * 1000, ease: "outQuart" },
    HERO_START * 1000,
  );
  tl.add(
    ".tagline",
    { opacity: [0, 1], translateY: [TAGLINE_Y, 0], duration: TAGLINE_DUR * 1000, ease: "outQuart" },
    TAGLINE_START * 1000,
  );
  ...
  tl.add(
    ...
    {
      opacity: [0, 1],
      scale: [CTA_REVEAL_SCALE, 1],
      duration: CTA_REVEAL_DUR * 1000,
      ease: `outBack(${BOUNCE_FACTOR})`,
    },
    CTA_REVEAL_START * 1000,
  );

  hyperframesAnime.register("viewport-scene", tl);
</script>
```
...
tl.add(
  scaleUp,
  { v: SCALE_UP_TARGET, duration: SCALE_UP_DUR * 1000, onUpdate: applyCompositeCamera },
  SCALE_UP_START * 1000,
);
tl.add(
  scaleDown,
  { v: SCALE_DOWN_TARGET, duration: SCALE_DOWN_DUR * 1000, onUpdate: applyCompositeCamera },
  SCALE_DOWN_START * 1000,
);
```
...
- **BOUNCE_FACTOR** - overshoot coefficient for `outBack(${BOUNCE_FACTOR})`.
  - Range: 1.2-2.5; lower = subtle settle, higher = pronounced overshoot. The ease family (`outBack`) is the choice; this number tunes its intensity.
  - Reference: ease family options: `outBack` (overshoot then settle), `outElastic` (oscillation), `outQuart` (clean decel, no overshoot)
...
- **No CSS `transition` on `.world`** - competes with anime.js
