...
Both run inside the anime.js timeline so HF seeks frame-by-frame deterministically.
...
<script>
  ...
  tl.add(
    ".hero",
    { opacity: [0, 1], translateY: [32, 0], scale: [0.96, 1], duration: 900, ease: "outQuart" },
    HERO_AT * 1000,
  );
  tl.add(
    ".tagline",
    { opacity: [0, 1], translateY: [16, 0], duration: 700, ease: "outQuart" },
    TAGLINE_AT * 1000,
  );
  tl.add(
    ".cta",
    { opacity: [0, 1], translateY: [8, 0], duration: 700, ease: "outQuart" },
    CTA_AT * 1000,
  );

  hyperframesAnime.register("cam-scene", tl);
</script>
```
...
- **PHASE_2_EASE / PHASE_3_EASE** - ease per transition
  - Discrete choice: `outCubic`, `outQuart`, `inOutCubic`
...
- **Phase springs softer than UI springs** - `inOutCubic` or `outQuart` for cinematic feel; spring/back easing on a camera feels uncomfortable
...
- **No CSS `transition` on `.camera`** - competes with the anime.js transform
