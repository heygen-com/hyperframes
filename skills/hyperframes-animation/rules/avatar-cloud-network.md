...
<script>
  ...
  tl.add(
    ".hub",
    {
      opacity: [0, 1],
      scale: [0.8, 1],
      duration: HUB_FADE_DUR * 1000,
      ease: `outBack(${HUB_BOUNCE})`,
    },
    HUB_FADE_START * 1000,
  );
  ...
  avatars.forEach((av, i) => {
    tl.add(
      av,
      {
        opacity: [0, 1],
        scale: [0, 1],
        duration: AVATAR_ENTRY_DUR * 1000,
        ease: `outBack(${AVATAR_BOUNCE})`,
      },
      (AVATAR_ENTRY_START + i * AVATAR_STAGGER) * 1000,
    );
  });
  ...
  lines.forEach((line, i) => {
    tl.add(
      ...
      {
        strokeDashoffset: 0,
        duration: LINES_DUR * 1000,
        ease: "outCubic",
      },
      (LINES_START + i * LINE_STAGGER) * 1000,
    );
  });
  ...
  hyperframesAnime.register("cloud-scene", tl);
</script>
```
...
- **HUB_FADE_DUR** - hub fade-in duration.
  - Range: 0.4-0.6s
- **HUB_BOUNCE** - `outBack(HUB_BOUNCE)` coefficient on the hub's scale entry.
...
- **AVATAR_BOUNCE** - `outBack(AVATAR_BOUNCE)` coefficient on each avatar's pop.
