...
<script>
  ...
  allLayers.forEach((el) => {
    ...
    tl.add(
      el,
      {
        opacity: [0, finalAlpha],
        duration: LAYER_FADE_DUR * 1000,
        ease: "outCubic",
      },
      (LAYER_CASCADE_START + (LAYER_COUNT - 1 - i) * LAYER_CASCADE_STEP) * 1000,
    );
  });
  ...
  tl.add(
    ...
    DEPTH_GROW_START * 1000,
  );

  hyperframesAnime.register("depth-scene", tl);
</script>
```
...
tl.add(
  ...
  {
    opacity: [0, 1],
    scale: [STATIC_ENTRY_SCALE, 1],
    duration: STATIC_ENTRY_DUR * 1000,
    ease: "outQuart",
  },
  0,
);
