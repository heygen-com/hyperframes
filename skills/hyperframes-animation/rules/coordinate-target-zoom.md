...
This `getBoundingClientRect` runs **once at setup**, before timeline registration, NOT per-frame (per-frame DOM reads desync under the renderer's parallel sampling; see SKILL universal constraints). Because the measurement is async (`fonts.ready`), build and register the timeline inside the same `async` setup so the baked offset is ready before `hyperframesAnime.register(id, tl)` runs.
...
<script>
  ...
  // async setup so the offset is ready before hyperframesAnime.register(id, tl) runs.
  ...
  tl.add(
    ...
    {
      opacity: [0, 1],
      translateY: [REVEAL_Y, 0],
      delay: anime.stagger(REVEAL_STAGGER * 1000),
...
Reverse the phases - start at zoomed-in, then `scale: 1` + `translateX: 0, translateY: 0` to pull back. The "reveal" beat is the panorama.
...
- **No CSS `transition` on `.zoom-outer` or `.zoom-inner`** - competes with anime.js
