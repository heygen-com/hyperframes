...
<script>
  ...
  tl.add(
    dot,
    {
      opacity: [0, 1],
      scale: [0, 1],
      duration: DOT_ENTRY_DUR * 1000,
      ease: `outBack(${BOUNCE_FACTOR})`,
    },
    DOT_ENTRY_START * 1000,
  );
  ...
  tl.add(
    ...
    IDLE_START_TIME * 1000,
  );

  hyperframesAnime.register("idle-scene", tl);
</script>
```
...
- **DOT_ENTRY_DUR**: dot back-out pop duration.
  - Range: 0.4-0.7s
- **BOUNCE_FACTOR**: `outBack(BOUNCE_FACTOR)` overshoot strength on the dot pop.
...
- **❗ Don't use CSS `@keyframes` for the idle loop**, CSS animation runs on the browser's render clock, which is independent of the HF seek clock. HF seeks frame-by-frame and a CSS-driven idle will flicker/desync. Drive idle inside the anime.js timeline.
