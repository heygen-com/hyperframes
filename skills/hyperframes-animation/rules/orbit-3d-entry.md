...
1. **Entry (per element)**: anime.js tween from hidden 3D orientation (`rotateX`, `rotateY`, negative `translateZ`) to flat (`rotateX: 0, rotateY: 0, translateZ: 0`). Spring-like ease (`outBack`) for the flip-in.
...
.orbit-item {
  position: absolute;
  /* Items live at stage center; anime.js translates them along the orbit. */
  top: 50%;
  left: 50%;
  width: 140px;
  height: 140px;
...
<script>
  ...
  items.forEach((el, i) => {
    ...
    tl.add(
      el,
      {
        ...
        opacity: 1,
        scale: 1,
        duration: ENTRY_DUR * 1000,
        ease: `outBack(${FLIP_BACK})`,
      },
      i * STAGGER * 1000,
    );
  ...
  tl.add(
    ".orbit-center",
    {
      ...
      scale: [0.6, 1],
      duration: ENTRY_DUR * 1000,
      ease: `outBack(${CENTER_BACK})`,
    },
    CENTER_FADE_AT * 1000,
  );

  hyperframesAnime.register("orbit-scene", tl);
</script>
```
...
- **FLIP_BACK** - `outBack(<n>)` overshoot for the flip-in
  - Range: 1.2–2.0
...
- **CENTER_BACK** - `outBack(<n>)` overshoot for the center label fade-in
  - Range: 1.2–1.8
...
- **Entry must flip IN PLACE at orbital position, NOT at center**: an entrance whose from and to values both have `translateX: 0, translateY: 0` keeps the item at the stage center during phase 1, so it collides with the center label during flip-in, then snaps to orbit on phase 2 start.

  The correct pattern (see Anime.js Timeline above) is to `anime.utils.set()` each item at `(cos(initialAngle)*RADIUS_X, sin(initialAngle)*RADIUS_Y)` with `opacity: 0` BEFORE adding tweens, then have phase 1 animate only rotation/opacity/scale - NOT translate. The item fades in IN PLACE at its orbital starting point, and phase 2 picks up the orbit smoothly from there.
