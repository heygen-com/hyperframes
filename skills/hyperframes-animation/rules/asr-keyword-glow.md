...
<script>
  ...
  // Single driver: 0 to composition duration in seconds.
  const driver = { t: 0 };
  tl.add(
    driver,
    {
  ...
  hyperframesAnime.register("asr-scene", tl);
</script>
```
...
- Single linear driver (`ease: "linear"`) so `t` maps 1:1 to scene time. Any other ease distorts the per-word envelope shape, do not change.
