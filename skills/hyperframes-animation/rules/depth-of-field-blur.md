...
<script>
  ...
  const ctx = [...document.querySelectorAll(".ctx")];
  ...
  hyperframesAnime.register("dof-scene", tl);
</script>
```
...
anime.utils.set("#planeA", { "--dof": "0px", opacity: 1 });
anime.utils.set("#planeB", { "--dof": `${MAX_BLUR}px`, opacity: DIM_LEVEL });

// Rack: A defocuses while B comes into focus, same position + duration.
tl.add(
  "#planeA",
  { "--dof": `${MAX_BLUR}px`, opacity: DIM_LEVEL, duration: RACK_DUR * 1000, ease: "inOutCubic" },
  RACK_START * 1000,
);
tl.add(
  "#planeB",
  { "--dof": "0px", opacity: 1, duration: RACK_DUR * 1000, ease: "inOutCubic" },
  RACK_START * 1000,
);
```
...
```js
[...document.querySelectorAll(".card:not(.hero)]").forEach((el) => {
  tl.add(
    el,
    { "--dof": `${GRID_BLUR}px`, opacity: DIM_LEVEL, duration: FOCUS_DUR * 1000, ease: "outCubic" },
    FOCUS_START * 1000,
  );
});
```
...
ctx.forEach((el) =>
  tl.add(
    el,
    { "--dof": "0px", opacity: 1, duration: REFOCUS_DUR * 1000, ease: "inOutCubic" },
    REFOCUS_START * 1000,
  ),
);
```
...
For a subtle "rack settling" feel, let the focal layer's blur breathe a hair around 0 during its hold: a _finite_ `ease:"linear"` driver writing `sin()` into `--dof` (never `loop: true`, never a CSS animation). Keep the amplitude well under 1px or it reads as "still focusing."
...
- **RACK_START / RACK_DUR** - rack-focus window (foreground ⇄ background).
  - Constraints: both planes' tweens share `RACK_START` and `RACK_DUR` so they cross at the midpoint; `anime.utils.set` the pre-blurred plane BEFORE `RACK_START`
...
- **`--dof` drives the blur; tween the variable, never a CSS `transition`.** Reading `filter: blur(var(--dof))` and animating `--dof` on the anime.js timeline keeps the blur on the HF seek clock. A CSS `transition` on `filter` interpolates on the browser's own clock and flickers/desyncs under frame-by-frame seek.
...
- **No `loop: true` / infinite tweens** - the focus pull is a finite tween; any breathing is a bounded `onUpdate` reading the driver phase (or a finite tween)
- **No `Math.random` / `Date.now`** - per-layer blur is derived from `data-depth` / element index so every seek is identical
- **Tween `filter` (blur) + `opacity` only here** - both paint-only and seek-safe. Use anime.js transform properties (`translateX`, `translateY`, `scale`) for any concurrent camera move; never tween `width` / `height` / `left` / `top`
