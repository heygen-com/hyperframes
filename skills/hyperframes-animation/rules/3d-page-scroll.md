...
.tilt-card {
  ...
  /* tilt + perspective set in CSS only if no other transform tween touches
     this element. If anime.js also tweens scale on .tilt-card, set the tilt
     via anime.utils.set() to avoid matrix overwrites. */
...
<script>
  ...
  tl.add(
    ".page-content",
    {
      translateY: -SCROLL_DISTANCE,
      duration: SCROLL_DUR * 1000,
      ease: "outQuart",
    },
    SCROLL_AT * 1000,
  );
  ...
  tl.add(
    ...
    {
      opacity: 1,
      duration: SPOTLIGHT_FADE_DUR * 1000,
      ease: "inOutQuad",
    },
    SPOTLIGHT_AT * 1000,
  );

  hyperframesAnime.register("page-scroll-scene", tl);
</script>
```
...
tl.add(
  ".page-content",
  { translateY: -SCROLL_DISTANCE_A, duration: SCROLL_DUR * 1000, ease: "outQuart" },
  SCROLL_AT_A * 1000,
);
tl.add(
  ".page-content",
  { translateY: -SCROLL_DISTANCE_B, duration: SCROLL_DUR * 1000, ease: "outQuart" },
  SCROLL_AT_B * 1000,
);
```

anime.js composes successive `translateY:` tweens on the same target as continuous property ownership: each tween starts from the value left by the previous tween.
