...
<script>
  ...
  tl.add(
    ".card-left",
    {
      translateX: [-ENTRY_SLIDE_DIST, 0],
      rotateY: [TILT + TILT_OVERSHOOT, TILT],
      opacity: [0, 1],
  ...
  tl.add(
    ".card-left",
    {
      translateY: -FLOAT_AMP,
      duration: (FLOAT_DURATION / 2) * 1000,
      ease: "inOutSine",
      alternate: true,
      loop: 2,
    },
  ...
  tl.add(
    ...
    {
      opacity: [0, 1],
      translateY: [COPY_RISE, 0],
      delay: anime.stagger(COPY_STAGGER * 1000),
      duration: COPY_DUR * 1000,
      ease: "outCubic",
    },
    COPY_REVEAL_AT * 1000,
  );

  hyperframesAnime.register("split-scene", tl);
</script>
```
...
- **FLOAT_DURATION**: full `alternate` round-trip duration (one breath).
...
- `/hyperframes-animation`, timeline + `alternate` for the idle bob
