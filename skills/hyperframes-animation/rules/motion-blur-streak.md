...
- **(A) Directional SVG blur** - an inline `<filter>` with `<feGaussianBlur stdDeviation="X 0">` (X on the axis of motion, 0 across it). anime.js tweens `X` from high → 0 through a proxy that calls `setAttribute` each frame. Cleanest, one element, true directional smear.
...
A fast move has a velocity profile: it accelerates off the start, peaks, then decelerates into the settle. An `out` ease (`outExpo`, `outQuint`) front-loads that - velocity is highest right at the start and bleeds to zero at the end. The fake works by mapping a **blur (or echo) envelope onto that same curve**:
...
<script>
  ...
  // anime.js can't tween an SVG attribute directly: tween a proxy and write it back each frame.
  ...
  tl.add(
    "#streak-el",
    { translateX: [ENTER_FROM_X, 0], opacity: [0, 1], duration: MOVE_DUR * 1000, ease: MOVE_EASE },
    MOVE_START * 1000,
  );
  ...
  tl.add(
    blurProxy,
    { v: 0, duration: MOVE_DUR * 1000, ease: MOVE_EASE, onUpdate: writeBlur },
    MOVE_START * 1000,
  );

  hyperframesAnime.register("streak-scene", tl);
</script>
```
...
<script>
  ...
  const ghosts = [...document.querySelectorAll(".streak-ghost")];
  ghosts.forEach((g) => {
    const i = Number(g.dataset.i); // 1..N-1, set in HTML (no Math.random)
    tl.add(
      g,
      {
        translateX: [ENTER_FROM_X - i * ECHO_STEP_PX, 0],
        opacity: [GHOST_BASE_OPACITY / i, 0],
...
Swap the motion axis: use `translateY` instead of `translateX` for the position tween, `stdDeviation="0 Y"` for Path A (blur on Y, 0 on X), and `ENTER_FROM_Y` / vertical echo offsets for Path B. A phrase that streaks _up_ into place pairs with `kinetic-type-beats`' rise-rotate beat.
...
tl.add(
  "#streak-el",
  { scale: [SCALE_FROM, 1], opacity: [0, 1], duration: MOVE_DUR * 1000, ease: MOVE_EASE },
  MOVE_START * 1000,
);
tl.add(
  blurProxy,
...
For a single kinetic phrase that "zooms past," keep the streak slightly visible a frame or two longer by easing the blur on a marginally _slower_ curve than the position (e.g. position `outExpo`, blur `outQuart`) - the element arrives, then the last wisp of smear resolves. Use sparingly; the default is locked envelopes.
...
- **MOVE_EASE** - shared ease for position and blur/echo.
  - Range: `outExpo` (hardest snap), `outQuint` (hard slam, default), `outQuart` (firm but softer)
...
- **`out`-family ease, always** - velocity must be front-loaded (fast off the start, decelerating in). `outExpo` / `outQuint` / `outQuart`. An `in` or `inOut` ease puts peak speed in the wrong place and the coupling falls apart.
...
- **Tween a proxy, write the attribute** (Path A) - anime.js tweens the plain `{ v }` object; `onUpdate` calls `setAttribute("stdDeviation", …)`. You cannot tween the SVG attribute directly, and you must **seed it once at setup** so a seek to `t=0` shows the streaked start.
...
- **No CSS `transition`** on the streaking element (or ghosts) - it interpolates independently of HF seek and causes flicker. Only anime.js drives the move and the blur.
- **No `loop: true` / infinite alternate motion** - a streak is a single finite arrival. Finite tweens only.
- **No `Math.random` / `Date.now`** - ghost offsets/opacities and any stagger derive from the element index; deterministic every seek.
- **anime.js transform properties only**: `translateX`, `translateY`, `scale`, `rotate`. Never tween `width` / `height` / `left` / `top`. Tweening `filter` (the proxy → `stdDeviation`) and `opacity` is seek-safe and fine.
...
- `/hyperframes-animation` - `out`-family easing, proxy-driven `onUpdate` attribute tweens, and locked-envelope coordination (`../../adapters/animejs.md`)
