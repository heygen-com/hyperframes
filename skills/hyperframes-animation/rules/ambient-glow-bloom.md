...
1. **Hero bloom**: once lit, the glow does a **bounded idle breathe** during the hold. Drive it with an `onUpdate` reading `tl.time()` (NOT an infinite `loop: true` alternate tween): a `Math.sin` of elapsed time nudges `opacity` and `scale` a hair around their peak. At `sin(0) = 0` the breathe starts exactly at the bloom's resting state, no jump.
...
<script>
  ...
  // the hero's settle. ease "outCubic" so it arrives soft, not snappy.
  tl.add(
    glow,
    {
      opacity: [0, GLOW_PEAK_OPACITY],
      scale: [GLOW_START_SCALE, 1],
      duration: BLOOM_DUR * 1000,
      ease: "outCubic",
    },
    BLOOM_START * 1000,
  );

  // Phase 2: bounded idle breathe during the hold. NOT an infinite alternate loop:
  ...
  tl.add(
    ...
    (BLOOM_START + BLOOM_DUR) * 1000, // breathe begins right as the bloom settles
  );

  // ── Form B: TRAVELING SWEEP ─────────────────────────────────────────
  // A single finite pass. opacity fades in, translateX travels from off one edge to
  // off the other, opacity fades out as it exits. One pass, no loop.
  const sweep = document.getElementById("sweep");
  tl.add(
    sweep,
    {
      translateX: [SWEEP_START_X, SWEEP_END_X],
      opacity: [0, SWEEP_PEAK_OPACITY],
      duration: SWEEP_DUR * 1000,
      ease: "linear", // constant glide reads as a raking light, not an ease-in object
    },
    SWEEP_START * 1000,
  );
  ...
  tl.add(
    sweep,
    { opacity: 0, duration: SWEEP_FADE_DUR * 1000, ease: "inQuad" },
    SWEEP_FADE_START * 1000,
  );

  hyperframesAnime.register("bloom-scene", tl);
</script>
```
...
For very short scenes (< 3s) or when the hero already has its own idle, skip Phase 2 entirely, bloom to peak and hold flat. The single `.add()` with explicit `[from, to]` values is the whole recipe; the glow is just lit presence.
...
tl.add(
  glow,
  {
    opacity: [0, GLOW_OVERSHOOT_OPACITY],
    scale: [GLOW_START_SCALE, 1.06],
    duration: BLOOM_DUR * 1000,
    ease: "outCubic",
  },
  BLOOM_START * 1000,
...
tl.add(
  glow,
  { opacity: GLOW_HOLD_OPACITY, scale: 1, duration: SETTLE_DUR * 1000, ease: "inOutCubic" },
  (BLOOM_START + BLOOM_DUR) * 1000,
);
```
...
- **Breathe is BOUNDED, never a loop**: the idle pulse is a finite `onUpdate` tween reading `tl.time()` (via the `phase` proxy), not `loop: true` / `alternate: true`. `sin(0) = 0` means it starts at the bloom's resting state with no jump. (Same reason as `sine-wave-loop`: an infinite/CSS loop desyncs from the HF seek clock.)
...
- **No `loop: true` / infinite alternate tween**, the breathe is a bounded finite tween; the sweep is one pass
- **No `Math.random` / `Date.now`**, the breathe phase is deterministic (`phase.p` over a fixed duration)
- **Anime transform properties only**: `translateX`, `translateY`, `scale`, `rotate`, plus `opacity` / `filter`. Never tween `width` / `height` / `left` / `top` (the halo swell is `scale`, the sweep travel is `translateX`).
