...
1. **Outgoing element**: scale `1.0 → EXIT_SCALE` + opacity `1 → 0` (fast `inCubic`)
2. **Incoming element**: scale `EXIT_SCALE → 1.0` + opacity `0 → 1` (bouncy `outBack(${BOUNCE_FACTOR})` with overshoot)
...
<script>
  ...
  tl.add(
    ...
    {
      opacity: [0, 1],
      translateY: [SUB_REVEAL_Y_PX, 0],
      duration: SUB_REVEAL_DUR * 1000,
      ease: "outQuart",
    },
    (TRIGGER + EXIT_DUR + SUB_REVEAL_DELAY) * 1000,
  );
  ...
  tl.add(
    ...
    {
      opacity: [0, 1],
      translateY: [BRAND_REVEAL_Y_PX, 0],
      duration: BRAND_REVEAL_DUR * 1000,
      ease: "outQuart",
    },
    BRAND_REVEAL_AT * 1000,
  );

  hyperframesAnime.register("swap-scene", tl);
</script>
```
...
```js
tl.add("#stateA", { scale: EXIT_SCALE, opacity: 0, duration: EXIT_DUR * 1000 }, TRIGGER_AB * 1000);
tl.add(
  "#stateB",
  { scale: 1.0, opacity: 1, duration: ENTER_DUR * 1000, ease: `outBack(${BOUNCE_FACTOR})` },
  (TRIGGER_AB + EXIT_DUR - OVERLAP) * 1000,
);
tl.add("#stateB", { scale: EXIT_SCALE, opacity: 0, duration: EXIT_DUR * 1000 }, TRIGGER_BC * 1000);
tl.add(
  "#stateC",
  { scale: 1.0, opacity: 1, duration: ENTER_DUR * 1000, ease: `outBack(${BOUNCE_FACTOR})` },
  (TRIGGER_BC + EXIT_DUR - OVERLAP) * 1000,
);
```
...
- **BOUNCE_FACTOR**: `outBack(${BOUNCE_FACTOR})` overshoot on the incoming.
...
- **Bouncy ease ONLY for the incoming**: outgoing uses `inCubic` (rushing away), incoming uses `outBack(${BOUNCE_FACTOR})` (arriving with weight). Reverse it and the swap feels mechanical
...
- **No CSS `transition`** on either swap element, competes with anime.js
