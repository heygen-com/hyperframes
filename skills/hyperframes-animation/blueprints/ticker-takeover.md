...
- hero shoves the text group aside on impact → `reactive-displacement` (the text is the displaced mass; express the hero's "heavy land" as a longer `outCubic` settle, not the rule's default `outBack`)
- hero's fast off-screen crash-in → `motion-blur-streak` (directional velocity blur resolving sharp as it lands)
- resting-hero aliveness → `sine-wave-loop` (low-amplitude dual-frequency register, scale + rotate jitter composing onto the hero's final landed scale; never an infinite alternate loop around 1)
