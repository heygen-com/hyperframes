...
- big→small scale-down on a word; springy scale-in/scale-out overshoot → `spring-pop-entrance` (spring pop/settle) backed by `techniques.md` for the plain scale tween
...
- particle/dot burst from behind text → `css-marker-patterns` (burst) backed by `techniques.md`
- `[accent]` selection-box frame around a word → `css-marker-patterns` (circle/box marker) + `techniques.md`
...
- letter-spacing collapse; bottom-up masked slide → `techniques.md` (tween letter-spacing / masked translate) + techniques: per-word kinetic typography / clip-path reveal
