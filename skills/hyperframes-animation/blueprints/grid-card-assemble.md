...
- randomized pop-in stagger → `techniques.md` (stagger recipe; randomized `from`/order)
- column slide-up into grid → `techniques.md` (per-column staggered slide-up)
- vertical-list step / slot-machine snap-and-hold → `vertical-spring-ticker` (STEPS = number of line advances)
- spring-overshoot marker pop → `spring-pop-entrance` (outBack spring), also `techniques.md` for the staggered pop chain
...
- dim-by-position de-emphasis → `techniques.md` (per-line opacity by slot position; no dedicated rule)
...
- book-end headline fade → `discrete-text-sequence` (or `techniques.md` fade)
...
- grid slide-up fly-out exit → `techniques.md` (plain staggered translate-off-frame; no dedicated rule needed, a basic exit tween, not a missing capability)
...
RULE GAPS: none, traveling-glow sweep → ambient-glow-bloom; motion-blur slide-out sweep (exit) → motion-blur-streak; grid slide-up fly-out (exit) → techniques.md (plain translate)
```

Merge tension: `center-outward-expansion` (the natural backing for stagger-assemble) caps cleanly at 3–8 items and explicitly warns 8+ causes mid-flight overlap chaos, but a Social_Proof logo wall is deliberately dense (12+ tiles), so for that variant the items must NOT burst from a shared center; they slide a short distance directly into their own slot (the rule's "starting partially-spread"/short-path form, or a `techniques.md` per-item stagger), which the consolidated Scene-1 verb already specifies as "short distance directly into its slot."
