...
- video entrance (smooth) and the weight-transfer slide → `techniques.md` (scale/opacity then x + scale on a long-tail `outQuart`); the video itself is a muted `<video class="clip">` direct child of the root
...
- pill `scaleX` snap + trailing glow halo → `techniques.md` (scaleX) + `ambient-glow-bloom` (the halo, resolving a beat behind)
