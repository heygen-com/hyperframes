...
- staggered spring-pop node entrances → `spring-pop-entrance` (elastic overshoot) + `techniques.md` (stagger recipe); 3D-flip-in flavor → `orbit-3d-entry`
...
- clockwise orbital revolve of upright badges → `orbit-3d-entry` (continuous elliptical orbit); technique `anime.svg.createMotionPath`
