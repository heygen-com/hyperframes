...
- background crossfade (light→dark gradient) → plain opacity/background tween via `techniques.md` (no dedicated rule needed)
...
- 3D logo assemble: part detach + spring dot → `spring-pop-entrance` (spring pop, `outBack` overshoot)
...
- wordmark cascade with overshoot (letters left→right) → recipe `techniques.md` (per-element staggered slide) + `spring-pop-entrance` (the `outBack` overshoot per letter)
...
- comet / spark accent on button → `asr-keyword-glow` (small glow accent); motion path via `techniques.md` anime.svg.createMotionPath (#9)
...
- letter-by-letter wordmark build → `discrete-text-sequence` (smooth-slice / per-state build); recipe `techniques.md` (typewriter / appending words)
...
- cursor arc-in + click on the wordmark → `cursor-click-ripple` (move → click → ripple); arc path via `techniques.md` anime.svg.createMotionPath (#9)
