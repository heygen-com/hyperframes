...
- The async callback must not be a **network request**. Registering inside `fetch(…).then(…)`, `d3.json(…).then(…)` or after `await fetch(…)` makes the render depend on the network at capture time and delays registration until the request resolves, so the engine's timeline poll waits on it (`lint`: `gsap_timeline_registered_behind_network_fetch`, error). Inline the data (bake it at authoring time) and register synchronously.
