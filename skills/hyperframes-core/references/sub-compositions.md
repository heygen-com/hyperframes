  ...
  <body>
    <template>
      ...
      <script>
        ...
        // ... build timeline ...
        hyperframesAnime.register("data-chart", tl, { labels: { intro: 0 } });
      </script>
    </template>
  </body>
</html>
...
<!-- RIGHT: both ids match, and the runtime registration key matches them too -->
...
<!-- registration: hyperframesAnime.register("data-chart", tl, { labels: { intro: 0 } }); -->
```

**Why this happens:** it feels natural to give the host slot a different name like `chart-mount` ("the mount point") vs `data-chart` ("the actual chart"). HyperFrames does not work that way. **The host's `data-composition-id` is the lookup key the framework uses to find the registered animation runtime state**. Lint passes because each file's ids are individually valid; the cross-file mismatch only blows up at render.

**Symptom:** the render logs missing sub-composition registrations for every mismatched slot, waits for the timeout per scene, then captures static initial-state frames (so the video is full-length but no animation plays).
...
#   2) host data-composition-id == internal data-composition-id == runtime registration key
grep "data-composition-id" index.html
grep "data-composition-id\|hyperframesAnime.register" compositions/<scene>.html
...
- Loads the file and registers its animation runtime state under its internal `data-composition-id`.
- Seeks the sub-composition's registered runtime state independently from the host's playhead.
- Plays the sub-composition's content from `data-start` of the host clip, for `data-duration` seconds.

**Do not** manually nest or sync a sub-composition timeline into the host timeline. HyperFrames already drives them independently, and manual nesting causes double-seeks.

### The host clip's `data-duration` is the slot's visible window

`data-duration` on the host clip defines **how long the slot is visible**, and it takes precedence over the sub-composition's internal runtime duration. Two consequences follow:

- **Internal timeline shorter than the slot -> the slot holds.** If the sub-composition's registered animation finishes before `data-duration` elapses, the slot keeps showing its final frame for the rest of the window. You do **not** need to pad the timeline with empty tweens.
...
Declare both endpoints for entrance tweens. In anime.js, use property arrays such as `tl.add(".title", { translateY: [60, 0], opacity: [0, 1], duration: 600 }, 200)` instead of relying on a runtime to snapshot the current state. The host re-seeks the sub-composition every time its clip becomes visible, so explicit start and end values replay cleanly.
