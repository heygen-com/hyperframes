...
| ---------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `data-composition-id`        | Yes           | Unique ID. Must match the animation runtime's registration key, for example `hyperframesAnime.register(id, ...)` for anime.js or the GSAP adapter's timeline key.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `data-width` / `data-height` | Yes           | Pixel frame size. Common values: `1920x1080`, `1080x1920`, `1080x1080`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `data-duration`              | Conditional\* | Render duration in seconds (total length / frame count), not an individual runtime timeline length. **Read once at compile time, like `data-width` / `data-height`**: a static root `data-duration` is locked before scripts run, so a script (`root.setAttribute("data-duration", ...)`) or a `--variables`-driven value cannot change the render length. To vary length per render, author the root `data-duration` directly. (A clip's `data-duration` is different: re-read from the live DOM, so scripts/variables can drive it.) Only when the root omits `data-duration` does the renderer derive total length from the live DOM or registered runtime instances after scripts run. |
...
\*`data-duration` is optional whenever the runtime can auto-infer duration: a finite registered anime.js instance, a registered GSAP timeline, a finite CSS animation, a finite WAAPI `element.animate()`, or a registered Lottie animation. It is **required** for Three.js (no auto-inference), for infinite/unbounded anime.js, CSS, or WAAPI animations, and for any composition with no duration-bearing animation signal at all. `npx hyperframes lint` enforces this (`root_composition_missing_duration_source`). See `determinism-rules.md` -> "Duration Contract" for the per-runtime breakdown.
