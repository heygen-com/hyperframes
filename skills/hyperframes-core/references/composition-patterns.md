...
| Runtime registration | One registered timeline keyed at the root's `data-composition-id` | Root timeline (often near-empty) + one registered timeline per sub-comp, each keyed by its `id` |
| Routing entry        | `references/minimal-composition.md`                               | `references/sub-compositions.md`                                                                |

Both architectures use the same runtime contract: `data-*` attributes plus a registered animation runtime key that matches `data-composition-id`. Anime.js uses `hyperframesAnime.register(id, instance, { labels })` by default. The choice is structural, not behavioral.
...
- The video has clear scene cuts, where each scene is its own segment of the timeline.
...
Conversion is mechanical and reversible. To lift a monolithic scene into a sub-comp: wrap the scene's markup + scoped CSS + its slice of the parent timeline into a `<template>`, save as `compositions/<scene>.html`, replace the inline content in `index.html` with a slot `<div data-composition-src="compositions/<scene>.html">`, and have the sub-comp register its own timeline with `hyperframesAnime.register("<scene>", tl, { labels })`. The parent timeline shrinks accordingly.
...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      hyperframesAnime.register("root", tl, { labels: {} });
    </script>
  </body>
</html>
```
...
- **Visual scenes on the same `data-track-index`** (e.g. `1`). Sequential scenes cannot overlap on the same track. For a cross-fade between two scenes, put one on a higher track and overlap their times by the fade duration.
...
<script>
  // MAIN timeline drives the host video. Global time: scene starts at 20s.
  const main = anime.createTimeline({ autoplay: false });
  main.add(
    "#final-video",
    { scale: [1.4, 1], opacity: [0, 1], duration: 900, ease: "outQuart" },
    20000,
  );
  hyperframesAnime.register("main", main, { labels: { final: 20 } });
</script>
...
<template>
  ...
  >
    <script>
      ...
      hyperframesAnime.register("final-anim", tl, { labels: { intro: 0 } });
    </script>
  </div>
</template>
```
...
- The host media must be a direct root child and exist in the DOM (static in `index.html`), which it always is.
...
When several beat-level scenes share continuous state, such as a chat thread that grows, a persistent headline word that carries across the cut, or a single canvas with internal phase changes, collapse them into one sub-comp and use **internal phase divs** rather than multiple sub-comp slots.
...
<template>
  <div data-composition-id="act2-merged" data-width="1920" data-height="1080" data-duration="9">
    ...
    <script>
      const tl = anime.createTimeline({ autoplay: false });
      tl.add("#phase-a", { opacity: [0, 1], duration: 1 }, 0);
      tl.add("#phase-a", { opacity: [1, 0], duration: 400 }, 3000);
      tl.add("#phase-b", { opacity: [0, 1], duration: 1 }, 3000);
      // ...
      hyperframesAnime.register("act2-merged", tl, { labels: { phaseA: 0, phaseB: 3 } });
    </script>
  </div>
</template>
```
...
| Runtime registration key            | matches `data-composition-id`               | `hyperframesAnime.register("act0-intro-bell", tl, ...)` |
