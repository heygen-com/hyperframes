...
**Remembered defaults.** Confirmed answers are memory. `media-use` keeps them in two tiers — the project's `.media/preferences.json` (committed, so the whole team inherits it) and the personal `~/.media/preferences.json` (a value earns the personal tier by being confirmed in two different projects). Before Round 2, read the merged view — `media-use` → `scripts/prefs.mjs get` — and let a remembered value become the recommended option, with a receipt that names its source: "1:1 — you confirmed this in world-cup-explained". Memory changes the default, never the question: every ask-marked field still gets asked, and what the request says this time always beats what the user picked last time. In autonomous mode a remembered value becomes the decided value, receipt included, in the heads-up.

Record only what the user actually confirmed. After the brief gate locks, `prefs.mjs record` each ask-marked answer; after the design step, the chosen preset (`--key style_preset --workflow <w>`); the voice only when the user explicitly picked one — a default nobody chose is not an answer. A "go" that accepts the recommended defaults counts as confirmation: the user saw them and let them stand. The first time a project records anything, say so in the gate's confirmation — one line, "I'll remember these as your defaults — next time they're the recommended answers" — and never repeat it; from then on the receipts carry the reminder. `mode` is remembered like any other field, but memory only reorders Round 1's options — Round 1 itself is never skipped by a remembered mode (§ 3).
