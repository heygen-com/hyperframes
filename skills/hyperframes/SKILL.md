...
| **Animate** - atomic motion, scene blueprints, transitions, runtime adapters (Anime.js first-party, GSAP non-default, Lottie, Three.js, CSS, WAAPI, TypeGPU)                        | `/hyperframes-animation` |
| **Author seek-safe keyframes** - anime.js timelines, CSS keyframes, GSAP non-default, WAAPI, FLIP, paths, masks, SVG morph/draw, 3D depth, plus `hyperframes keyframes` diagnostics | `/hyperframes-keyframes` |
...
- **Output:** a runnable HyperFrames composition + a **JSON island** the player's `SlideshowController` reads to turn the anime.js timeline into a navigable **deck**, with discrete slides, fragment reveals, branching sequences, hotspot navigation, presenter mode, and speaker notes. The deliverable is a deck, not an MP4.
