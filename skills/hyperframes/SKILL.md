...
| Workflow                   | Use it for                                                                                                                                                                                                                                                                                              |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/product-launch-video`    | **Any website input** — selling it (SaaS, app, company / product site → a **promo**) or showing it as-is (portfolio, blog, docs, personal, event → a **tour** built from its own screens). The default for any URL, even if the site is only named; promo-vs-tour is the intent layer's angle question. |
...
- **A URL or script** — any website input → `/product-launch-video`; selling it (promo) versus showing it as-is (tour) is that route's **angle** question at the intent layer, not a route split. A GitHub PR link → `/pr-to-video`; a concept with no product or site → `/faceless-explainer`.
...
- **Input:** Any website — **(a)** a URL (crawled with headless Chrome for assets + brand tokens), **(b)** a script / brief that names the site even without a link (resolved + crawled, unless the user opts out), or **(c)** a script with no derivable site / "don't scrape" (no-capture mode, promo angles only — a style preset supplies palette + design system). A supplied script can be the **verbatim** voice-over or **restructured** per scene — asked at the intent layer.
- **Output:** by angle. The **promo** angles: a product launch / SaaS promo → MP4 (sweet spot 30–90s) — the product's value is the subject. The **tour** angle: a site tour / showcase / social clip built from the site's own captured screens → MP4.
