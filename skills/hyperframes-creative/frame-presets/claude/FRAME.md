...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Fraunces at optical sizes for all
...
typography:
  ...
  # — display ramp (Fraunces 400, optical size, sentence case, negative tracking) —
  headline:{ fontFamily: "Fraunces", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.02em" }
  quote-pull:{ fontFamily: "Fraunces", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.018em", italic: true }
  display-italic:{ fontFamily: "Fraunces", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.018em", italic: true }
  display:{ fontFamily: "Fraunces", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.028em" }
  number-hero:{ fontFamily: "Fraunces", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.03em" }
  display-cover:{ fontFamily: "Fraunces", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.035em" }
  number-unit:{ fontFamily: "JetBrains Mono", cqw: 2.08, weight: 500, lineHeight: 1.0 }

spacing:
  slide-pad: "4.2cqw"   # ~80px @1920 — editorial, generous
  gap-md: "1.7cqw"
  hairline: "1px"
  radius-sm: "6px"
  radius-md: "8px"
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Fraunces figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Fraunces italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis — the one place a long line breathes. Fraunces italic, small Inter uppercase cite beneath."
  section-rule:
...
imprint or a considered research note, not a SaaS deck. The thesis is three colors held with
discipline: **cream is the floor, ink is the voice, coral is the voltage** — and a fourth (warm
navy) only where code shows itself. Every surface is **warm cream** (never pure white, never cool
gray); content gathers on a **tile** surface half a step darker — the demarcation is a half-step,
never a hard contrast. Elevation is a **1px hairline** ink border at low alpha plus, rarely, one
soft warm shadow. There are no heavy drops, no glows, no gradients on content. The restraint is the
material.

Three editorial voices, each in its own face: **Fraunces** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large optical sizes with tight negative tracking;
...
- **Cream / ink / coral trinity** + a warm-navy code surface; cream is the ~60% floor, ink the voice, coral the scarce voltage.
- **Fraunces 400** (optical size, sentence case, negative-tracked) for all display; **Inter** body/chrome; **JetBrains Mono** index + code.
...
- **Editorial density** — one clear focal per frame with generous cream around it; a crammed frame breaks the considered voice.
...
- **Squint** — one Fraunces display moment dominates at 3–6× its neighbor; the cream floor centers the eye.
- **Silence** — one subject per frame in generous cream; the **code surface and any card grid are the dense exception**.
- **Restraint** — cream floor / tile content step holds; ink-only text (cream on navy); coral appears **once**; hairline elevation only — no heavy shadow.
- **Reference** — aim at a **literary imprint page / considered research note / annual-report spread**; failure looks like a **cool-gray, shadowed, multi-accent SaaS dashboard**.
...
Tokens identical to the source. Default ground `{colors.cream}` (the 60% floor); content gathers on
...
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Fraunces `headline` 4.6cqw
→ `display-cover` 9.9cqw, weight 400, optical size, negative-tracked) carries every headline + stat.
...
- **Fraunces display is sentence case** (NOT title case, NOT uppercase), weight 400, optical-size, negative-tracked (−0.018..−0.035em); reach for **italic** when the line is a stance or a definition. **Inter body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.

## Depth & Surface

Hairline elevation — the restraint is the material:
...
- **number-lockup** — Fraunces figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Fraunces italic + cite (a commit message / reviewer line).
...
> One coral moment per frame; one focal in generous cream; open with a kicker-spike.

### 1 · Cover (identity · move: oversized Fraunces · cream · left)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Fraunces `display-cover` (sentence case, ink) left, under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Silence** right third holds air. **Fixed** Fraunces 400 sentence case, hairline, cream floor. **Free** title, the mono index. **Density** low.

### 2 · Statement (statement · move: single Fraunces line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity), generous pad. **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Fraunces `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Silence** ~55%. **Fixed** sentence-case serif, no second focal. **Free** the line, ground. **Density** low.

### 3 · Code Surface (code · move: warm-navy code window · the PR-critical frame)

**Ground** `{colors.cream}` framing a `{colors.navy}` **code-surface** (8px, cream@14% hairline, `navy-elev` title bar + filename in mono). **Composes** mono-label filename, the **`code-*` block** (code-diff / code-typing / code-snippet-_), optional `section-rule`. **Focal** the code panel — the diff / before→after / typed-on snippet. **Chrome** mono filename + status strip. **Accent** syntax coral/teal/amber inside the panel; one coral marker outside (e.g. a `+`/`−` gutter cue). **Silence** cream margin around the panel. **Fixed** warm-navy surface, mono code, hairline. **Free** which code-_ block, the code (from the diff). **Density** dense-exception.

### 4 · Number / Impact (data · move: oversized figure · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a Fraunces `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Silence** ~55%. **Fixed** serif figure + mono unit, hairline rule, flat. **Free** the figures (from the script), tag. **Density** low.

### 5 · Pull-quote (quote · move: Fraunces italic · cream)

**Ground** `{colors.cream}`, generous pad. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a Fraunces **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small Inter uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Silence** ~55%. **Fixed** Fraunces italic quote + uppercase cite. **Free** quote, attribution. **Density** low.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short Fraunces sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Silence** ~50%. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships. **Density** low–moderate.
...
- Set all display in **Fraunces 400, sentence case**, optical size, negative-tracked; **Inter 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Keep **one clear focal in generous cream**; open regions with a **kicker-spike**.
- Pair a Fraunces figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Fraunces display; no sans headline; no serif label; no serif-set numeric unit.
- No two coral moments in one frame; coral never sets a headline or body run.
- Don't cram edge-to-edge — the considered voice needs cream. Don't blow a headline past the measure — step the ramp down.
...
- **Squint** — one Fraunces display moment dominates; the cream floor centers the eye.
- **Silence** — one subject per frame in generous cream; only the code surface / card grid run dense.
...
- **Type** — Fraunces 400 sentence-case display (optical size, negative-tracked); Inter 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
...
- **Motion intentionally out of scope.** frame.md specifies composition only. Claude's motion register — quiet and considered, short cross-dissolves, no overshoot/bounce/elastic, coral the only "draw-on", numbers count up, code types on line by line — lives in the workflow's `motion-language.md` + `hyperframes-animation`, not here.
- **Fraunces (opsz) + Inter + JetBrains Mono via Google Fonts.** The optical-size axis on Fraunces is load-bearing; if it fails, fall to another high-contrast optical serif — never to a sans. CJK: Noto Serif SC (display) / Noto Sans SC (body) / Noto Sans Mono CJK (code); the sentence-case warmth carries when the serif drops.
