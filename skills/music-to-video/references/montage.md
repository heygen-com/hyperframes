...
**hard-kill** with a zero-duration `tl.add(el, { opacity: 0, duration: 0 }, anchorMs)`.
This equivalent hard-kill pattern is required because non-linear seeking otherwise bleeds
stale frames.
...
- **Crossfades animate `opacity`**, never `visibility`/`display` on a `.clip`
...
volume automation: `tl.add("#el-bgm", { volume: 0.15, duration: fadeMs }, clipStartMs)` ...
`tl.add("#el-bgm", { volume: 0.9, duration: fadeMs }, clipEndMs)`). The frame-worker mounts no audio. Keep clips muted in v1; wire clip-audio +
