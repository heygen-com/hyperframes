...
(Frame `type` values are PR-native — `diff` / `before_after` / `impact` / `credits` / … from story-design; the enrichment method below is the same regardless of type.)
...
- **`effects` choreograph the surrounding chrome, not the code motion.** The block owns the diff/typewriter/morph animation (it _is_ the development beat). Your `effects` / `blueprint` move the claude **Code Surface** around it — the navy window seating in, a `+N/−M` `count-up`, a coral underline drawing on. Cite 1–3 for that chrome; do not try to re-specify the code animation.
