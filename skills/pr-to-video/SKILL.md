...
**Pre-install the code blocks** named across `STORYBOARD.md` once, before dispatch, so parallel workers don't race on the registry:

`for b in <each code-* block named in the storyboard>; do npx hyperframes add "$b"; done`
