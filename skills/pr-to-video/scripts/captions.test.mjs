...
test("every build-frame.mjs copy stages the style axis it promises", () => {
  for (const skill of ["product-launch-video", "faceless-explainer", "pr-to-video"]) {
    ...
    assert.doesNotMatch(
      ...
      `${skill}/lib/font-faces.mjs must not assert font-style:normal over captured bytes`,
    );
    // captions.mjs parses the staged names back; it must read them with the same helpers
    // rather than a private copy that can drift from the staging side.
    ...
    assert.match(
      captions,
      /import \{[^}]*\bfontWeightInfo\b[^}]*\} from "\.\/lib\/font-faces\.mjs"/,
      `${skill}/captions.mjs must import its face helpers from lib/font-faces.mjs`,
    );
  }
});
