...
**The brief starts at the intent layer.** Opening rule, in order: **(1)** `BRIEF.md` exists → read it and ask nothing it answers — its `flow`/`storyboard` derive the mode (brief contract § 1). **(2)** No `BRIEF.md` but the project exists → resume from what's on disk; never re-interrogate. **(3)** A fresh creation request that arrived here directly → read `/hyperframes` and run the intent layer (`../hyperframes/references/intent.md`): it asks this route's must-haves (audience, destination/format, what to show — `route-briefs.md` § /website-to-video) and **announces the deferred asks** — message and narrative arc are confirmed at Step 2, where the captured site's real content grounds the recommendation. Write `BRIEF.md` at the project root as soon as Step 0 creates it, and record the preference-backed answers (`brief-format.md`). Edit requests skip all of this.

The workflow has 7 steps. Each produces an artifact that gates the next. By default it's collaborative — gates marked 💬 stop and ask the user. Mode semantics (signals, propagation, gate taxonomy) are canonical in `../hyperframes-core/references/brief-contract.md`; under an autonomous run (`storyboard: no`, or a "decide for me" / "surprise me" signal), 💬 user-preference gates are skipped — see step-2-brief.md for how that propagates through this workflow.
