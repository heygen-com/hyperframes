...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| **Fast** (billboard-per-beat) | Single `index.html`, stacked `<div class="beat">` elements, anime.js opacity sequencing. NO sub-compositions, NO HyperShader. Hard cuts via zero-duration `tl.add()`. See stacked-beats pattern below. | Sub-compositions add latency; hard cuts need instant swaps. One file = zero load delay. |
...
beats.forEach(function (b) {
  ...
  if (b.id !== "b01") tl.add(el, { opacity: 1, duration: 0 }, b.at * 1000);
  tl.add(el, { scale: 1.012, duration: 0 }, 0);
  tl.add(el, { scale: 1, duration: 250, ease: "outCubic" }, b.at * 1000);
  if (b !== beats[beats.length - 1]) {
    tl.add(el, { opacity: 0, duration: 0 }, (b.at + b.dur) * 1000);
  }
});
```
...
- Slow camera drift on the composed scene root: `tl.add(scene, { scale:[1.05,1], translateX:[20,-20], duration: BEAT * 1000, ease:"linear" }, 0)` (Ken-Burns style, but on your composed elements, not on a screenshot)
- Parallax text layers: `tl.add(text, { translateY:[30,-30], duration: BEAT * 1000, ease:"inOutQuad" }, 0)`
- 1–2s breathing room before text enters (don't animate everything at t=0)
- Soft easing: `outExpo` for entrances, `inOutQuad` for drifts
...
<div id="root" data-composition-id="main" data-start="0" data-duration="TOTAL"
  ...
  <div id="beat-1" class="scene"
    ...
    data-duration="4.5"   <!-- must match the BEAT constant in the composition -->
    data-track-index="1"
    data-width="1920" data-height="1080"
...
**Critical: beat host divs must have sequential `data-start` and matching `data-duration`.** Do NOT set `data-start="0"` on all beats, the render engine seeks each beat's animation timeline to `global_time - data_start`. At t=10s with `data-start=0`, a 5.5s timeline is seeked past its end and all content disappears.

`data-duration` must match the `BEAT` constant in the composition (the length of the sub-composition's internal timeline). If the two disagree, animations get cut off.
...
- `data-duration` = "Animation duration:" value from the storyboard
...
for actually referenced, is the headline ≥80px, does the animation timeline cover
...
The storyboard beat already contains everything, the concept, the visual choreography with exact timings, the CSS values, the SFX cues. The sub-agent's job is to translate that description into working HTML/CSS/anime.js, not to re-invent the creative direction. If you want, you can also paste any other relative and useful context to subagents if think it's good, why not.
...
**2. Build the static end-state first.** Position every element at its most visible moment. HTML+CSS only, no animation code yet. The CSS position is the ground truth.

**3. Add the animation sequence.** Follow the storyboard's choreography, it specifies what happens and when. Use `tl.add()` with explicit from-to arrays for entrances. Build the timeline in the order the storyboard describes.
...
- **No bare `anime.animate()`** - all tweens on `tl.add()`, never standalone
...
Beat N: compositions/beat-N-NAME.html
  ...
  First timeline event:   tl.add("...", {...}, <ms>)   beat-local t=<num>
  Last timeline event:    tl.add("...", {...}, <ms>)   beat-local t=<num>
  ...
  Technical gates:        data-composition-id matches hyperframesAnime.register key? yes/no
                          script INSIDE <template>? yes/no
                          no Math.random / no repeat:-1 / no bare anime.animate? yes/no
...
**Anything off, fix it inline (small CSS/anime.js correction) or re-dispatch the sub-agent with the specific problem quoted.** Do not move to Step 6 until every beat has its evidence block filled and PASS.
