...
- **Timeline coverage** - per-beat, timeline event positions must span ≥70% of the beat's `data-duration`. Catches "webpage not shot" failures where the beat has entrance tweens then goes static.
...
Beat N (Ns–Ns) - <name>
  ...
  Timeline coverage: events from <first t> to <last t>, beat duration <N>s (full coverage: yes/no)
...
- **Timeline tween overlaps** - elements fighting over the same property = visual glitches
