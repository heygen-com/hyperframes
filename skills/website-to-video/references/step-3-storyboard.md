...
```
Beat 3: composed kanban (4 cards-as-divs per column) + counter chip on In-Progress + outBack entrance stagger
  techniques: layered panels (capabilities §1), counter via zero-duration tl.add (techniques #15),
              anime.js stagger with outBack(1.7) (techniques #4)
...
When planning beats, decide which ones deserve an HTML-in-Canvas treatment vs. a standard anime.js animation. If you want it, name it in the storyboard, Step 5 will read [`../../hyperframes/references/html-in-canvas-patterns.md`](../../hyperframes/references/html-in-canvas-patterns.md) for implementation. You don't need to specify the API details here.
...
- **Animation timeline duration:** `[duration]s` (how long this beat's internal animations run, when does the last tween end?)

Example: `Transition in at: 4.2s · Animation duration: 5.5s` -> Step 5 sets `data-start="4.2" data-duration="5.5"`.
...
The two beats below are from the real Claude Design × HyperFrames production video. They show the expected level of specificity, exact timing, exact animation values, exact animation sequences.
