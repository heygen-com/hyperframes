...
- `/gsap` or `/gsap-core`, `/gsap-timeline`, `/gsap-plugins` - supported non-default GSAP adapter reference
...
<template>
  ...
  <script>
    (function () {
      ...
      hyperframesAnime.register("beat-N-name", tl);
    })();
  </script>
</template>
```
...
A beat is a SHOT in a film, not a webpage with entrance animations. Your anime.js timeline should have events spread across the ENTIRE beat duration, not just entrance tweens in the first 1-2 seconds followed by nothing. If an element is on screen, it should be doing something. After elements enter, add continuous hold motion: camera dolly, parallax layers moving at different speeds, secondary elements appearing mid-beat, real depth shifts.
...
- ANIME FROM TRAP: never rely on a one-sided opacity tween with CSS `opacity:0`. Use `tl.add()` with explicit `[from, to]` arrays.
- STYLE: use zero-duration hidden-state `tl.add()` calls for delayed elements, then reveal with explicit opacity/visibility values.
...
- CSS CENTERING: no `transform: translate(-50%, -50%)` with anime.js transform tweens. Use flexbox or a wrapper.
...
- COUNTERS: no `onUpdate` for numeric counters, use discrete `tl.add(el, { textContent: "42", duration: 0 }, 2500)` at timestamps. `onUpdate` callbacks are supported for canvas/WebGL rendering loops and character-by-character typing, see capabilities.md §10.
- TIMELINE: `hyperframesAnime.register("beat-N-name", tl)` synchronously. Key = `data-composition-id`.
- DETERMINISTIC: no `Math.random()`, `Date.now()`, `requestAnimationFrame`, `repeat:-1`.
- Always use explicit `[from, to]` arrays for entrances.
...
| Intent          | Anime.js Ease       | Use for                              |
| --------------- | ------------------- | ------------------------------------ |
| Snap (iOS feel) | `outQuint`          | Hero text, UI elements               |
| Whip overshoot  | `outBack(1.7)`      | Numbers, badges, impact              |
| Soft land       | `outExpo`           | Per-word reveals, gentle entrances   |
| Mechanical      | `outQuad`           | Terminal text, code typing           |
| Bounce settle   | `outElastic(1, .5)` | Counters, CTA buttons                |
| Dramatic        | `inOutExpo`         | Full-screen statements, hero reveals |
| Drift           | `linear`            | Parallax, Ken Burns, camera drift    |

Staggered items: `outQuint` with `delay: anime.stagger(80)` to `anime.stagger(150)`.
